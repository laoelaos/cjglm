...
| 2a | 准备向量辅助文件（可独立编译部分）：`compute_vector_relational.hpp`（仅依赖 `setup.hpp`）、`compute_vector_decl.hpp`（仅依赖 `setup.hpp`） | 确认每个辅助文件可独立编译通过 |
...
> **注**：步骤 2a 的辅助文件必须在任意 vec `.inl` 实现编译前就绪。`compute_vector_relational.hpp` 被全部 4 个 `type_vecN.inl` 无条件包含；`compute_vector_decl.hpp` 被 `type_vec3.inl` 和 `type_vec4.inl` 无条件包含。二者均仅依赖 `setup.hpp`，可在步骤 1 完成后安全独立准备。`_swizzle.hpp` 不属此类——其定义的结构体继承自 `vec<L, T, Q>`，语法可解析但依赖 vec 完整类型定义，编译随对应 `type_vecN.hpp` 一同进行，因此归入各 vec 类型的编译子步骤中。
