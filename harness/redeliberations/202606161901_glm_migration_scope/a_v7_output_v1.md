...
| 向量辅助文件（`compute_vector_relational.hpp`、`compute_vector_decl.hpp`、`_swizzle.hpp`） | 3 个辅助文件（随首轮 vec 类型同步迁移） |
...
首轮各向量类型的最小功能验收清单（运行时验证）：

| 类型 | 最小验收操作 | 通过标准 |
|------|-------------|---------|
| `vec<2, T, Q>` | 默认构造、分量构造 `vec2(1,2)`、`x/y` 成员读写、`+/-/*` 分量算术、`==`/`!=` 比较、`[]` 下标访问 | 编译通过且运行时结果与 GLM 等价的 C++ 参考值一致 |
| `vec<3, T, Q>` | 同上 + `z` 成员访问、叉积值（若 overload 了 `cross`） | 同上 |
| `vec<4, T, Q>` | 同上 + `w` 成员访问、与 `vec3` 的 swizzle 构造 | 同上 |
| `vec<1, T, Q>` | 默认构造、分量构造 `vec1(1)`、`x` 成员访问 | 同上 |
| 向量别名 | `vec2`, `vec3`, `vec4`, `ivec2`, `bvec2`, `dvec3` 等可通过别名实例化 | 编译通过 |
...
| 2a | 准备向量辅助文件：`compute_vector_relational.hpp`（仅依赖 `setup.hpp`）、`compute_vector_decl.hpp`（仅依赖 `setup.hpp`）、`_swizzle.hpp`（无额外 `#include` 依赖，依赖链闭合于首轮范围） | 确认每个辅助文件可独立编译通过 |
| 2b | 标量类型别名（仅依赖 `qualifier.hpp` 前向声明即可） | 确认标量类型别名编译通过 |
| 2c | `vec<1,T,Q>` + `vec1` 相关向量类型别名（依赖 `vec<1,T,Q>` 完整定义） | 创建向量、访问成员 |
| 3 | `vec<2,T,Q>` | 基本 2D 向量操作 |
| 4 | `vec<3,T,Q>` | 基本 3D 向量操作 |
| 5 | `vec<4,T,Q>` | 基本 4D 向量操作 |
| 6 | `vec2`-`vec4` 相关公共 type alias | 确认 `vec2`、`vec3`、`vec4` 等可用 |

> **注**：步骤 2a 的辅助文件必须在任意 vec `.inl` 实现编译前就绪。`compute_vector_relational.hpp` 被全部 4 个 `type_vecN.inl` 无条件包含；`compute_vector_decl.hpp` 被 `type_vec3.inl` 和 `type_vec4.inl` 无条件包含；`_swizzle.hpp` 在默认 `GLM_SWIZZLE_OPERATOR` 模式下被全部 4 个 `type_vecN.hpp` 条件包含。三者均仅依赖基础设施层，可在步骤 1 完成后安全独立准备。
...
>
> **`_swizzle.hpp` 依赖链分析**：经审查确认，`_swizzle.hpp` 自身不含额外 `#include` 语句（仅定义 swizzle 构造器、重载操作符等纯类型工具），其依赖链闭合于首轮基础设施层——该文件不引入任何向量/矩阵/函数库类型，可在首轮安全迁移。
>
