...
4. **跨类型逐分量构造**（工厂函数，降级为 `fromParts` 模式）
   - 降级为带类型转换闭包的具名工厂函数：`public static func fromParts<U>(conv: (U) -> T, x0: U, y0: U, ..., x3: U, z3: U, w3: U): Mat4x4<T,Q> where Q <: Qualifier`
...
5. **跨类型列向量构造**（工厂函数，降级为 `fromColumns` 模式）
   - 降级为带类型转换闭包的具名工厂函数：`public static func fromColumns<V1, Q2>(conv: (V1) -> T, v1: Vec4<V1, Q2>, v2: Vec4<V1, Q2>, v3: Vec4<V1, Q2>, v4: Vec4<V1, Q2>): Mat4x4<T,Q> where Q <: Qualifier, Q2 <: Qualifier`（以 Mat4x4 为例，各列向量分量类型 V1 到 T 的转换通过 conv 闭包）
...
6. **跨矩阵类型转换**（工厂函数，定义在带 `Number<T>` 约束的 extend 块中）
   ...
   **6a 和 6b 共享的设计属性**：
    ...
    **fromMat 目标→源矩阵类型映射表**（72 个转换对）：

    填充规则类别说明：
    - **A（主规则）**：遵循通用填充规则——复制重叠区域（`dst[j][i] = src[j][i]`，0 ≤ j < min(C_src,C_dst)、0 ≤ i < min(R_src,R_dst)），扩展列/行按单位矩阵模式填充。编码时可直接套用规则，无需逐场景核对 GLM .inl
    - **B（需核对 GLM .inl）**：该方向 C_src = C_dst 且 R_src < R_dst（同列数、源行数少于目标行数），已知 GLM 部分 .inl 实现在此模式下以零向量初始化后逐列设 (j,j)=one，完全忽略源矩阵对应列的扩展行数据。编码时必须逐一对照 GLM .inl 源文件确认实际行为

    | 目标矩阵 | 源矩阵（8 个）及填充规则 |
    |---------|------------------------|
    | Mat2x2 (2×2) | Mat2x3(A), Mat2x4(A), Mat3x2(A), Mat3x3(A), Mat3x4(A), Mat4x2(A), Mat4x3(A), Mat4x4(A) |
    | Mat2x3 (2×3) | Mat2x2(B: 同列扩展行), Mat2x4(A: 同列截断), Mat3x2(A), Mat3x3(A), Mat3x4(A), Mat4x2(A), Mat4x3(A), Mat4x4(A) |
    | Mat2x4 (2×4) | Mat2x2(B: 同列扩展行), Mat2x3(B: 同列扩展行), Mat3x2(A), Mat3x3(A), Mat3x4(A), Mat4x2(A), Mat4x3(A), Mat4x4(A) |
    | Mat3x2 (3×2) | Mat2x2(A), Mat2x3(A), Mat2x4(A), Mat3x3(A), Mat3x4(A), Mat4x2(A), Mat4x3(A), Mat4x4(A) |
    | Mat3x3 (3×3) | Mat2x2(A), Mat2x3(A), Mat2x4(A), Mat3x2(B: 同列扩展行), Mat3x4(A: 同列截断), Mat4x2(A), Mat4x3(A), Mat4x4(A) |
    | Mat3x4 (3×4) | Mat2x2(A), Mat2x3(A), Mat2x4(A), Mat3x2(B: 同列扩展行), Mat3x3(B: 同列扩展行), Mat4x2(A), Mat4x3(A), Mat4x4(A) |
    | Mat4x2 (4×2) | Mat2x2(A), Mat2x3(A), Mat2x4(A), Mat3x2(A), Mat3x3(A), Mat3x4(A), Mat4x3(A), Mat4x4(A) |
    | Mat4x3 (4×3) | Mat2x2(A), Mat2x3(A), Mat2x4(A), Mat3x2(A), Mat3x3(A), Mat3x4(A), Mat4x2(B: 同列扩展行), Mat4x4(A: 同列截断) |
    | Mat4x4 (4×4) | Mat2x2(A), Mat2x3(A), Mat2x4(A), Mat3x2(A), Mat3x3(A), Mat3x4(A), Mat4x2(B: 同列扩展行), Mat4x3(B: 同列扩展行) |

    编码阶段建议：先按 A 类主规则实现所有 72 个 fromMat 转换函数，再逐一对照 GLM .inl 修正 9 个 B 类方向的实现。此策略可确保大部分方向 (63/72) 一次性正确，仅对 B 类方向做针对性调整。

7. **跨类型同尺寸转换构造**（init<U, P>(m: Mat4x4<U, P>)）
    - 从相同矩阵尺寸但不同元素类型和/或不同 Qualifier 的实例转换
    - 当 U=T 时自然涵盖跨 Qualifier 复制（即原第 2 条功能被本条完全覆盖）
    - **可行性分析**：此构造函数在 struct 体内定义，U 和 T 均为无约束泛型参数。构造函数体内部直接将 `m.c0` 等赋值给 `this.c0`，这要求 Vec4<U, P> 可隐式转换为 Vec4<T, Q>——在无约束泛型上下文中不可行。因此本构造函数也需降级为工厂函数：`public static func fromMat<U, P>(conv: (U) -> T, m: Mat4x4<U, P>): Mat4x4<T,Q> where P <: Qualifier`，与第 4、5 条的 castVecN conv 闭包模式一致
    - **const init 不可用**：涉及类型转换闭包调用
...
| D08 | 排除 ++/-- 运算符的具名替代（increment/decrement）；仓颉复合赋值运算符不支持跨类型矩阵参数 | 仓颉不支持重载自增/自减运算符。C++ GLM 中全部 9 种矩阵类型（含 6 种非方阵）均提供 ++/-- 运算符，语义为逐元素加/减 1（非仅方阵）。仓颉版本中全部 9 种矩阵类型（含 6 种非方阵）的 ++/-- 均不可用，可通过逐元素加 1 等价替代（具体类型上下文中使用 `m += 1.0f`，泛型上下文中需通过 `one: T` 参数传入后使用 `m += one`）——此替代对所有 9 种类型均适用（因所有类型均支持 +=scalar 复合赋值，见第②类）。注意不能使用 `+= identity`（仅对角线加 1，语义不同）。C++ GLM 的 `operator+=(mat<C,R,U,Q>)` 中 U≠T 版本在仓颉中不可行——仓颉 extend 块中运算符右操作数类型固定为 `Mat{C}x{R}<T, Q>`，不支持跨类型矩阵复合赋值 |
...
| **无 ++/-- 运算符** | 仓颉不支持重载自增/自减运算符。C++ GLM 中全部 9 种矩阵类型（含 6 种非方阵）均提供 ++/-- 运算符，语义为逐元素加/减 1。仓颉版本中全部 9 种矩阵类型（含 6 种非方阵）均不可用 ++/--。可通过逐元素加 1 等价替代 C++ GLM 的 `++m` 语义（具体类型上下文中使用 `m += 1.0f`，泛型上下文中需通过 `one: T` 参数传入后使用 `m += one`）——此替代对所有 9 种类型均适用 |
| **无模板赋值运算符 `operator=(mat<C,R,U,Q>)`** | GLM 1.0.3 全部 9 个矩阵类型均声明了模板赋值运算符 `template<typename U> operator=(mat<C,R,U,Q> const& m)`，支持跨类型同尺寸矩阵直接赋值（如 `mat4_f32 = mat4_i32`）。仓颉不允许重载 `operator=`，所有矩阵的赋值运算符仅接受同类型同尺寸参数，跨类型同尺寸赋值不可用。若需跨类型转换赋值，推荐使用 `fromMat(conv, m)` 工厂函数（带显式类型转换闭包）或显式逐元素转换后构造 |
...
| 审查意见 | 修改措施 |
|---------|---------|
| **问题1（中等）**：D08 及 §9 中 ++/-- 替代方案 `m += T(1)` 与 §1 T(1) 约束矛盾——T(1) 在泛型上下文中不可编译 | **已采纳**。① D08 中将 `m += T(1)` 修正为"具体类型上下文中使用 `m += 1.0f`，泛型上下文中需通过 `one: T` 参数传入后使用 `m += one`"；② §9 差异声明中 ++/-- 条目同步修正 |
| **问题2（中等）**：§9 缺失模板赋值运算符 `operator=` 的差异条目——GLM 全部 9 个矩阵类型声明了 `template<typename U> operator=(mat<C,R,U,Q>)`，仓颉不支持 operator= 重载 | **已采纳**。§9 差异声明新增"无模板赋值运算符 `operator=(mat<C,R,U,Q>)`"条目，说明跨类型同尺寸矩阵赋值不可用，推荐通过 `fromMat(conv, m)` 或显式逐元素转换替代 |
| **问题3（中等）**：fromMat 工厂函数签名使用伪代码 `Mat{C_src}x{R_src}`，缺少具体的重载清单——编码人员需自行推导全部 72 个具体重载，存在遗漏风险 | **已采纳**。§3.3 跨矩阵类型转换数量推导后新增"fromMat 目标→源矩阵类型映射表"，完整列出 9 种目标矩阵各 → 8 种源矩阵的映射关系，标注每方向填充规则类别（A: 主规则 / B: 需核对 GLM .inl），覆盖全部 72 个转换对 |
| **问题4（轻微）**：fromParts/fromColumns/fromMat 第7条签名行未显式给出 `where` 约束 | **已采纳**。① fromParts 签名行补充 `where Q <: Qualifier`；② fromColumns 签名行补充 `where Q <: Qualifier`；③ fromMat 第7条签名行补充 `where P <: Qualifier` |
