# GLM 1.0.3 仓颉迁移阶段二 OOD 设计方案（v13）
...
7. **跨类型同尺寸转换构造**（init<U, P>(m: Mat4x4<U, P>)）
    ...
    **第 7 条与第 6a 条的功能重叠说明**：当源矩阵与目标矩阵元素类型相同（U=T）且尺寸相同时，第 7 条 `fromMat<U, P>(m, conv: (U) -> T)` 与第 6a 条 `fromMat<SrcQ>(m, one: T)` 功能完全重叠——此时第 7 条要求调用方提供不必要的 conv 闭包（如 `{x: T => x}`），而第 6a 条无需 conv 闭包即可完成同类型同尺寸复制（仅跨 Qualifier）。**推荐规则**：当源矩阵与目标矩阵元素类型相同且尺寸相同时，调用方应优先使用第 6a 条（更简洁，无需 conv 闭包）。第 7 条仅应在 U≠T 的跨类型转换场景中使用
...
| **问题 10（轻微）**：D19 排除 fmat/f32mat/f64mat 系列的数量佐证不足 | **已采纳**。① D19 补充具体数值：每族 9 个矩阵尺寸 × 3 种精度前缀 = 27 个别名/族，3 族共 81 个；加上 i8mat~u64mat 等 12 种整型前缀 × 9 种矩阵尺寸 = 108 个；合计约 189 个冗余别名，与纳入的约 60 个形成显著缩减对比 |
