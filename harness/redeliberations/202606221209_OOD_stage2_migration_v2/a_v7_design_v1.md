# GLM 1.0.3 仓颉迁移阶段二 OOD 设计方案（v18）
...
package glm.detail          — 核心实现层（新增/修改文件以 ★ 标记）
  ...
  │                          注意：此文件不包含 mod 函数（矩阵取模无数学意义，GLM 也未为矩阵类型定义 % 运算符）
   └── type_vecN.cj（修改）★  — Vec1~Vec4 各新增行向量×矩阵成员运算符 `Vec{R} * Mat{C}x{R}` extend 块（方案A），具体签名如下：
...
| D33 | Bool 矩阵不提供算术运算符和标量填充 | Bool 不实现 Number<T> 接口，无法参与算术运算和标量填充；但仍支持构造、下标访问、比较（==/!=/equalExact）和 @Derive[Hashable] |
...
| **Bool 矩阵不提供算术运算符和标量填充工厂函数** | Bool 不实现 Number<T> 接口，无法参与 ±*/ 等算术运算，也不提供 filled()/identity()（依赖 Number<T> 约束）。Bool 矩阵仍支持构造（逐分量/列向量）、下标访问、length()、比较（==/!=/equalExact）和 @Derive[Hashable] |
