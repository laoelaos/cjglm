...
> 注：#7 在 OOD 文档层面因 CL-04（仓颉语言限制）导致设计不可行，同时在编码层面浮点 mod 路径可独立实现但缺失，故双重分类。#20 的实质是 CL-01 API 变更后测试结构的固有限制（与 #23 同类），而非 OOD 文档本身的矛盾或错误。合计覆盖全部 30 项。
...
- **修复前提**：需先确认 `scalar_vec_ops.cj` 中当前 `mod` 的 4 个重载的约束条件，确认无法简单扩展为双路径（因 `Integer<T>` 与 `Number<T>` 约束不能并存于同一 extend 块）。可行的方案是新增 4 个浮点重载（`where T <: Number<T> & Equatable<T> & Comparable<T>`），函数体使用 `std.math.trunc` 实现 `x - y * trunc(x / y)` 恒等式，并在 `Vec1<T,Q>~Vec4<T,Q>` 上分别实现。需注意：`std.math.trunc` 的签名仅为 `trunc(x: Float64): Float64`，**不支持 `Float32` 或其他浮点类型**。对于 `Float64` 可直接使用 `trunc(x / y)`；对于 `Float32` 需通过 `Float32(trunc(Float64(x) / Float64(y)))` 转换实现。未在 `FloatingPoint<T>` 接口中发现泛型 `trunc` 方法。
- **副作用分析**：新增浮点 `mod` 重载后，(1) 浮点 Vec 可调用 `mod(s, v)`；与整数 `mod` 共存不产生歧义（编译器根据 T 的约束自动选择）；(2) `Float32` 的 trunc 实现经过 Float64 转换再截断，双精度中间计算可能导致 `Float32` 直接 trunc 的结果有微小差异（但 `mod` 的语义要求截断至零方向，双精度中间值足够精确）；(3) 若 trunc 返回值在极端值（Inf/NaN）下行为特殊，需额外保护
...
- **修复前提**：需先将 `assert` 调用替换为 const 兼容的等价实现（或内联检查），方可将 `componentAt` 标注为 `const`。此前置依赖影响优先级排序——在 assert 替换完成前无法直接修复 #12/#17。
...
- **判定**：**真实存在**（同 #12，CL-10）
- **理由**：同 #12 分析。Vec3 和 Vec4 的 `componentAt` 均缺少 `const` 修饰符，原因相同（`assert` 非 `const` 导致）。修复前提同为依赖 assert 替换前置。
...
- **判定**：**其他类型问题**
- **理由**：测试当前传入 `Int64(0)`/`Int64(1)` 作为 `zero`/`one` 参数验证函数返回值的正确性。这是 CL-01 的必然后果——因 `T(0)`/`T(1)` 构造在仓颉不可行，API 已变更为接收调用方传入的 `zero`/`one` 参数。测试只能验证从输入到输出的正确映射，无法验证"`true`→`T(1)`、`false`→`T(0)`"的原始语义——因为该语义在 API 层面已被委派给调用方。这不是测试代码的错误，也不是 OOD 文档本身的矛盾，而是 CL-01 API 变更后测试结构的固有限制——与 #23 的性质类似。测试逻辑本身是正确的，它验证了函数在当前 API 签名下的行为，只是无法触及已委派的原始语义层。
...
> 累加合计：8 + 7 + 1 + 2 + 7 + 2 + 2 + 1 = 30，覆盖全部 30 项。
>
...
| 项 | 缺陷描述 | 影响范围 | 副作用评估 |
|----|---------|---------|-----------|
| #7 | 浮点 mod 不可用 | `Float32`/`Float64` Vec 无法调用 `mod(s, v)` | 新增浮点重载后需确认函数重载解析无歧义；trunc 在 Inf/NaN 下的行为需专项验证；Float32 路径经 Float64 中转可能引入极微小精度差异 |
...
- #7 的浮点 mod 涉及新增重载和 math.trunc 使用，需额外验证 Float32/Float64 的 trunc 路径差异
- #11/#16 的 increment/decrement 实现相对简单
- **#12/#17 的 componentAt const 修复依赖 assert 替换前置**——需先将 assert 调用替换为 const 兼容的等价实现（如 `if (!predicate) { throw Exception("...") }`），然后方可将 componentAt 标注为 const。此前置依赖在修复前需独立安排 assert 替换任务，或将其作为 #12/#17 修复的第一个步骤同步执行。
...
| 修复项 | 潜在副作用 | 风险等级 | 缓解措施 |
|-------|-----------|---------|---------|
| #7 浮点 mod 重载 | 新增重载与现有整数重载形成函数重载组，编译器解析规则可能导致歧义 | 低 | 验证 `Float32`/`Float64` 实例化可正确匹配浮重重载而非整数重载；Float32 的 trunc 经 Float64 转换后验证边界值一致性 |
...
**#7 浮点 mod 重载 — 函数重载解析歧义验证**：当前 `mod` 的 4 个整数重载使用约束 `where T <: Integer<T>`（`scalar_vec_ops.cj:86,91,96,101`）。若新增 4 个浮重重载使用 `where T <: Number<T> & Equatable<T> & Comparable<T>`，则 `Float32`/`Float64` 满足浮重重载而**不满足**整数重载（因为 `FloatingPoint<T>` 不继承自 `Integer<T>`），编译器可无歧义地选择正确重载。验证通过，风险等级确认为低。
