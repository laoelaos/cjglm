...
- **修复前提**：需先确认 `scalar_vec_ops.cj` 中当前 `mod` 的 4 个重载的约束条件，确认无法简单扩展为双路径（因 `Integer<T>` 与 `Number<T>` 约束不能并存于同一函数重载组）。`std.math.trunc` 提供三种具体浮点类型重载（`trunc(Float16): Float16`、`trunc(Float32): Float32`、`trunc(Float64): Float64`），是重载自由函数而非泛型函数。因此**不能在泛型约束为 `where T <: Number<T>` 的函数体中使用 `trunc(x / y)`**——编译器无法将泛型参数 `T` 解析到具体的 `trunc` 重载。可行的方案是：对每种浮点类型（Float32、Float64、Float16）分别实现具体类型的 `mod` 重载，或通过 `FloatingPoint<T>` 接口提供的等效方法实现。需在修复前通过实际编译验证具体的实现路径。
- **副作用分析**：新增浮点 `mod` 重载后，(1) 浮点 Vec 可调用 `mod(s, v)`；与整数 `mod` 共存不产生歧义（编译器根据 T 的约束自动选择）；(2) 若采用具体浮点类型重载方案（对 Float32/Float64/Float16 分别实现），三种浮点类型均可直接调用对应类型的 `trunc` 重载，无需类型转换；(3) 若采用泛型约束方案，需通过 `FloatingPoint<T>` 接口方法或备选数学恒等式实现，需实际编译验证可行性；(4) trunc 返回值在极端值（Inf/NaN）下行为需专项验证
...
> - #17 与 #12 同为 componentAt const 缺失（依赖 assert 和 Exception("...") 构造两处 const 阻塞替换前置），归入「测试不完整」
...
| 项 | 缺陷描述 | 影响范围 | 副作用评估 |
|----|---------|---------|-----------|
| #7 | 浮点 mod 不可用 | `Float32`/`Float64` Vec 无法调用 `mod(s, v)` | 需采用具体浮点类型重载方案（`trunc` 是重载自由函数而非泛型函数，无法在 `where T <: Number<T>` 泛型函数中直接调用）；trunc 三种浮点类型均有直接重载可用，不存在精度损失问题；trunc 在 Inf/NaN 下的行为需专项验证 |
...
- #7 的浮点 mod 涉及新增重载，`trunc` 提供 Float16/Float32/Float64 三种具体类型重载，需在具体浮点类型级别实现而非泛型级别（因 `trunc` 是重载自由函数而非泛型函数）
...
| 修复项 | 潜在副作用 | 风险等级 | 缓解措施 |
|-------|-----------|---------|---------|
| #7 浮点 mod 重载 | 新增重载与现有整数重载形成函数重载组，编译器解析规则可能导致歧义；泛型约束方案中 `trunc(x / y)` 因 `trunc` 仅有具体类型重载而不可行 | 低 | 采用具体浮点类型级别重载（对 Float32/Float64/Float16 分别实现），避免在泛型函数体中调用 `trunc`；或通过 `FloatingPoint<T>` 接口方法实现；三种浮点类型均有直接 `trunc` 重载可用，无需类型转换 |
...
**#7 浮点 mod 重载 — 函数重载解析歧义验证**：当前 `mod` 的 4 个整数重载使用约束 `where T <: Integer<T>`（`scalar_vec_ops.cj:86,91,96,101`）。若新增 4 个浮重重载使用 `where T <: Number<T> & Equatable<T> & Comparable<T>`，则 `Float32`/`Float64` 满足浮重重载而**不满足**整数重载（因为 `FloatingPoint<T>` 不继承自 `Integer<T>`），编译器可无歧义地选择正确重载。**但函数体实现需注意**：`trunc` 是具体浮点类型重载自由函数（三组具体签名），在泛型约束 `where T <: Number<T>` 的函数体中无法直接调用 `trunc(x / y)`（`x: T`）。需采用具体浮点类型级别重载（对 Float32/Float64/Float16 分别实现），或通过 `FloatingPoint<T>` 接口提供的等效方法实现。此实现路线的可行性需实际编译验证。

**#7 浮点 mod — `std.math.trunc` 类型支持验证**：查阅标准库文档（`math_package_funcs.md`）确认，`trunc` 提供 `trunc(Float16): Float16`、`trunc(Float32): Float32`、`trunc(Float64): Float64` 三种具体浮点类型重载，是重载自由函数而非泛型函数。v3 报告中"仅支持 Float64"的断言为事实性错误，v4 已修正。但需注意：`trunc` 仅有具体类型重载，在泛型约束 `where T <: Number<T>` 的函数体中调用 `trunc(x / y)`（其中 `x: T`）**不可行**——编译器无法将泛型参数 `T` 解析到具体类型重载。修复者需采用具体浮点类型重载方案（对 Float32/Float64/Float16 分别实现），或通过 `FloatingPoint<T>` 接口提供的等效方法实现。
