...
- **修改方向**：为所有 25 个 shim 函数的 Float32 测试补充多组输入值，覆盖边界和常规值，使 Float32 测试深度与 Float64 版本对齐
...
- **真实性**：设计争议（零轴时的行为未明确，需 OOD 先完成决策）
...
- **修改方向**：在 `cjglm/tests/glm/gtc/packing_test.cj` 中补充上述 12 组 pack/unpack 函数的 round-trip 测试（包含边界值和常规值），使打包函数测试覆盖率达到 16/16
...
| **设计待决** | G24（testRotateZeroAxis 零轴行为待 OOD 决策） | 先完成 OOD 决策 |
...
| 质询意见 | 回应 |
|---------|------|
| **[一般] #1：G1 被错误归类为"测试正确性组"**（第474-477行） | 已修正。G1 从"测试正确性组"移至"库行为正确性组"第4项。理由：G1 修改方向是修改生产代码 `detail/common.cj:173-179` 的 roundEven 分支逻辑，非测试文件修改，归入"测试正确性组"会误导执行者。 |
| **[一般] #2：G24 定性为"真实问题"与正文描述矛盾**（第342行vs第343行） | 已修正。G24 真实性从"真实问题"改为"设计争议"。理由：正文描述明确指出零轴行为"未明确零轴是否应保留原 q 或返回单位四元数"，属于设计决策未定，不应标注为"真实问题"。修改方向已调整为"先完成 OOD 决策，再对齐测试和实现"。 |
| **[轻微] #3：G7 根因分类在正文与汇总表之间不一致**（第172行vs第446行） | 已修正。汇总表新增"实现编码遗漏"类别行，G7 从"实现与设计偏差"移入此新行。正文第172行根因标注为"实现编码遗漏"（OOD 未设计 NaN/Inf 检查，但同文件其他函数均有此保护，编码者应主动补充），与修正后的汇总表一致。 |
| **[轻微] #4：G16/G34 修改建议缺乏可操作细节**（第290-295行、第414行） | 已补充。G16：更新证据为"25 个 shim 函数均有 Float32 测试但覆盖深度不足"，列出 Float32 测试仅测试单组基本值而 Float64 版测试 2-3 组的对比情况。G34：补充 16 组 pack/unpack 函数对的具体清单，区分已覆盖 4/16（packUnorm4x8、packSnorm4x8、packHalf1x16、packDouble2x32）和未覆盖 12/16 的具体函数名。 |
| **[轻微] #5：S3 副作用评估"无"过于绝对**（第64行） | 已修正。增加连锁效应说明：当前测试期望值为数学推导错误，若生产代码 `rotate` 实现存在与错误期望值补偿的深层缺陷，测试修复后可能暴露该缺陷——修复者应在修正期望值后运行完整测试套件确认通过率，若失败需排查 `ext/quaternion_transform.cj:rotate` 与 GLM 1.0.3 的一致性。 |

## 修订说明（v6）

| 质询意见 | 回应 |
|---------|------|
| **[严重] #1：汇总表 G24 仍被归入"测试覆盖遗漏"组**（第468行） | 已修正。汇总表中将 G24 从"测试覆盖遗漏"行（G13-G37）排除，新增"设计待决"行，G24 独立归入此行，修改方向为"先完成 OOD 决策"。与 G3（OOD 文档修正）、G8（实现编码遗漏）的类似处理模式保持一致。 |
| **[一般] #2：G15 与 G24 仍错误引用 deviations.md 中不存在的内容**（第285行、第345行） | 已修正。G15：删除"根据 docs/deviations.md"前缀，保留 docs/05_ood_phase3.md 的正确引用。G24：删除"根据 docs/deviations.md"前缀（该条目描述的库行为边界问题非 Cangjie 特有偏差，不属于 deviations.md 的记录范围）。 |
| **[一般] #3：G14 标题仍声称"缺少 inversesqrtZero"与正文证据矛盾**（第274行） | 已修正。标题从"缺少 inversesqrtZero/sqrt(-1)/log(0)/log(-1)"改为"缺少 sqrt(-1)/log(0)/log(-1)"，删除了 inversesqrtZero（已有测试），与正文注脚一致。 |
| **[一般] #4：G10（slerp 修复）仍缺少副作用评估**（第220-232行） | 已补充。新增副作用评估覆盖：(1) 受影响测试文件：`ext/quaternion_common_test.cj` 中 slerp 测试期望值需重新计算；(2) 两参数和四参数 slerp 分开受影响范围——两参数仅补充 cosTheta<0 分支，四参数需修正公式 + 补充分支；(3) 向后兼容性说明——非兼容变更，修复后与 GLM 1.0.3 行为一致；(4) deviations.md 影响——当前 deviations.md 无 slerp/cosTheta 记录，无需更新。 |
| **[一般] #5：G8 删除 dquat 的引用清理不完整——缺少测试文件影响评估**（第196-199行） | 已修正。(1) 搜索命令扩展至 `cjglm/tests/`，确认 `cjglm/tests/glm/lib_test.cj:425-429`（testLibExtDquatAliasAccessible）使用 dquat；(2) 副作用评估新增此测试文件的影响分析：删除 ext.dquat 后该测试仍可通过 lib.cj 第 70 行的 gtc.dquat 自动获取，无需修改测试代码。 |
| **[轻微] #6：G34 修改方向缺少测试文件的具体定位信息**（第431行） | 已补充。修改方向中补充测试文件路径 `cjglm/tests/glm/gtc/packing_test.cj`。 |
