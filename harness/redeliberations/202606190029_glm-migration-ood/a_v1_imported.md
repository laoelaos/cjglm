...
- **`%` 运算的整数约束**：`%` 运算符在 Vec 上定义为逐分量取模。当 `T` 为整数类型时正常编译；当 `T` 为浮点类型时，因仓颉原生 `Float32`/`Float64` 不支持 `%`，在实例化时产生编译错误。此行为与 D5（宽松泛型约束）一致——错误触发于使用处而非定义处。对于需要浮点数取模的场景，使用具名函数 `mod`（属于首轮范围）。
...
- **浮点精确相等比较**（`equalExact`）：为浮点分量 Vec 提供 `const` 的 `equalExact(other: VecN<T,Q>): Bool` 扩展成员函数（定义在 `extend` 块中，声明为 `const`）。该函数跳过容差比较路径，直接使用仓颉原生 `==` 对每个分量做精确比较。适用于需要 IEEE 754 精确相等语义的场景（如 `Infinity == Infinity` 返回 `true`、`NaN == NaN` 返回 `false`）。`equalExact` 在各 `type_vecN.cj` 的 extend 块中定义为 `const` 成员函数，对整数/Bool 分量 Vec 其行为与 `==` 等价（整数/Bool 精确比较）。浮点类型在调用 `equalExact` 时不受 `==` 的 Epsilon 容差分支影响。`equalExact` 的 `@OverflowWrapping`/溢出策略：不涉及算术运算，无需标注。
...
**决策**：`ComputeEqual<T>.callConst` 中 `const operator ==` 的完整表达式链需系统性标注每项的 const 兼容性。同时明确定义回退方案的触发条件和验证优先级。
...
1. **验证优先级顺序**（编码阶段应按此顺序逐一验证，任一失败即触发回退）：
   - **优先级 1**：`T(0)` const 构造。验证 `Int32(0)`、`Float32(0.0f)`、`Bool(false)` 在 const 表达式中是否合法。**此项失败则直接无条件触发回退**（`const if (T(0) is Float64)` 分支前提不成立）。
   - **优先级 2**：`a - b`（浮点减法）const 兼容性。验证 `Float32`/`Float64` 的 `-` 在 const 函数体内的可用性。此项失败则浮点容差比较路径整体不可行。
   - **优先级 3**：`abs(x)` const 兼容性。若 `Float32.abs()` / `Float64.abs()` 非 const，尝试内联替代方案。
   - **优先级 4**：`epsilonOf<T>()` const 兼容性。若 `NumericLimits<T>.epsilon()` 非 const，尝试固定容差值替代。

2. **触发回退的阈值**：
   - **若优先级 1 失败**（`T(0)` const 构造对任一目标类型不可用）：**无条件触发回退**。所有类型使用精确比较 `a == b`，浮点容差比较不纳入 `const operator ==` 路径。
   - **若优先级 1 通过，但优先级 2~4 中任一失败**：触发回退。浮点容差比较从 `const operator ==` 路径中移除，通过独立的非 `const` 具名函数（如 `equalEpsilon`）提供给需要容差比较的场景。
   - **若所有优先级均通过**：确认完整路径在目标编译器上可行，维持当前设计。
...
| `Float16` | `false` | 非 IEC 60559 浮点类型。`isIec559Of<Float16>()` 隐式返回 `false`（`is` 类型检测不匹配），无需显式排除 |
