...
> **版本变更摘要**：v8 基于 v7（a_v7_copy_from_v6.md）迭代，系统性修复 const 扩展成员函数语法不可行问题（P1，影响 §3.2/§4.3/§4.4/§4.5/§7 D32/D33）、修正 `const if (T2 is Bool)` 为值级检测方案（P2，§9.4）、补充测试文件清单（P3，§8.1）、修正 `<<` 默认溢出策略为待验证假设（P4，§4.4）、增加构造函数跨 Q 重载解析风险分析（P5，§4.1）、统一 const if 编译抑制验证（P6，§10）、新增参考实现使用策略（P7，§8.4）。详见末尾修订说明。
