# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v21 — 审查迭代修订版）

> **版本变更摘要**：v21 基于 v20 迭代，根据 v21 审查反馈（发现 1~3）进行补充修订：在 §12.2 验收标准表中新增 `equalEpsilon`-`epsilonOf` 回退联动验收项（发现 1）、新增 `const init` 泛型构造可用性验收项（发现 2）、在 §4.3 补充 `mod` 扩展成员函数浮点路径不可用的显式 API 影响标注并在 §12.2 新增对应验收项（发现 3）。详见末尾修订说明。
...
| equalEpsilon-epsilonOf 回退联动验收 | 当 `epsilonOf<T>()` 的兼容性回退策略（移除 `Number<T>` 约束、内联固定容差）被触发时，`equalEpsilon` 实现同步将 `epsilonOf<T>()` 引用替换为内联固定容差 `T(1e-6)` | 编写原型测试模拟回退路径，验证 `equalEpsilon` 在回退路径下对 `Float32`/`Float64` Vec 的容差比较行为正确；确认 `equalEpsilon` 不再引用 `epsilonOf<T>()`（改为内联固定容差） |
