# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v20 — 审查迭代修订版）

> **版本变更摘要**：v20 基于 v19 迭代，根据 v20 审查反馈（发现 1~2）进行系统性清理：对全文档 25 处 `const if` 术语残留执行系统性搜索替换（发现 1，依据 D34 约定改写为"编译期 `if`"等表述）、修复 §12.3 中 `tests/glm.detail/` 路径混用为 `tests/glm/detail/`（发现 2），并采纳两项非阻塞改进项。详见末尾修订说明。
