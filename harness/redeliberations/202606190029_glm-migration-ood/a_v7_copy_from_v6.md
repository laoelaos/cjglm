...
> **版本变更摘要**：v7 基于 v6（a_v6_design_v2.md）迭代，系统性修复版本标识一致性、补充构建基础设施、完善 const 兼容性验证、修复 D32 验证缺口、拆分 compute_vector_decl.cj 双重角色、补充增量验证节点、重组 lib.cj 公共 API 面、扩展并发设计。详见末尾修订说明。
...
glm.detail（内部无包引用，同包直接可见）
  ...
  scalar_vec_ops → qualifier（无包内其他依赖，仅使用 Qualifier 约束）
...
- **具名函数的定义位置与形态**：
  ...
  - **`scalar-op-vec` 方向**（如 `add(s, v)`）：定义为 **包级独立函数**，集中定义在独立的 `scalar_vec_ops.cj` 文件中（与 ComputeVec* 运算策略 `compute_vector_decl.cj` 职责分离）。调用方通过 `import glm.detail.scalar_vec_ops.*` 或通过 `glm` 包统一引入。该方向无法定义为扩展成员函数（孤儿规则——标量类型如 `Int32`/`Float32` 不在本包中定义）。
  ...
  - **命名空间污染风险评估**：`scalar-op-vec` 方向具名函数（`add(s, v)`、`sub(s, v)` 等）定义为 `package glm.detail` 中的包级独立函数，通过 `lib.cj` 的 `public import` 暴露到 `package glm` 命名空间。这些函数的名称（`add`、`sub`、`mul`、`div`、`mod`）是常见英文单词，在不同上下文中可能与其他库或用户代码中的同名函数发生冲突。**风险等级**：低-中。在单一项目中此类冲突概率较低；但在大型项目中，`add`/`sub` 等名称可能被多个模块定义。**调用方最佳实践**：① 优先使用 `vec-op-scalar` 方向的扩展成员函数（`v.add(s)`）以避免导入包级函数；② 若需使用 `scalar-op-vec` 方向函数，通过限定导入（`import glm.detail.scalar_vec_ops.{ add, sub, mul, div, mod }`）明确来源；③ 若发生命名冲突，使用 `import glm` 的别名导入或通过局部函数包装消除歧义。
...
- `&&` 和 `||` 不在仓颉可重载运算符列表内，不可实现为运算符重载。改为提供具名函数。各 VecN 类型分别定义对应版本：
  - `logicalAnd(a: VecN<Bool, Q>, b: VecN<Bool, Q>): VecN<Bool, Q>` — 定义为 **扩展成员函数**（通过 `extend VecN<Bool, Q>` 块），逐分量逻辑与（各 VecN 分别定义）。调用方使用 `v1.logicalAnd(v2)` 语法
  - `logicalOr(a: VecN<Bool, Q>, b: VecN<Bool, Q>): VecN<Bool, Q>` — 定义为 **扩展成员函数**（通过 `extend VecN<Bool, Q>` 块），逐分量逻辑或（各 VecN 分别定义）。调用方使用 `v1.logicalOr(v2)` 语法
...
**const 兼容性验证要求**：40 个函数的 `const` 声明依赖目标类型 `T` 的算术运算符在 const 函数体内的可用性。若目标类型 `T`（如自定义数值类型或 `Float16`）的 `+`/`-`/`*`/`/` 等运算符在 const 上下文中不可用，对应实例化将在编码阶段报编译错误。验证计划如下：

| 验证项 | 验证方法 | 回退方案 |
|--------|---------|---------|
| `add`/`sub`/`mul`/`div` 对 `Float32`/`Float64`/`Int32` 等核心类型的 `const` 兼容性 | 编写原型测试实例化 `add`/`sub`/`mul`/`div` 各一种，确认 const 编译通过 | 若核心类型失败 → 降级为非 `const`（`mod` 因 `const if` 需求例外处理，见下） |
| `mod` 对核心类型的 `const` + `const if` 兼容性 | 编写原型测试分别以 `Int32` 和 `Float32` 实例化 `mod`，确认 const if 分支选择正确 | 若失败 → 将 `mod` 拆分为独立的整数 `modInt` 和浮点 `modFloat` 具名函数，均声明为非 `const` |
| 40 函数对非核心类型（如 `Int8`、`UInt64`、`Float16`）的 const 兼容性 | 按编码阶段目标类型覆盖逐一验证；若某类型不满足 const 约束，该类型的实例化在调用处报编译错误 | 此行为与 D5（宽松泛型约束，延迟检查）一致——错误发生在使用处而非定义处，不影响其他类型的编译通过性 |
...
| 8a | `scalar_vec_ops.cj`（新增） | `qualifier.cj` | scalar-op-vec 方向辅助函数（add(s,v)/sub(s,v)/mul(s,v)/div(s,v)/mod(s,v)，首轮即被消费） | 2a（随 Vec 运算） | 新增 |
...
| `Bool` → 数值 | 跨 Vec 转换构造中 `Bool` 不可直接转换为数值类型（仓颉中 `Int32(Bool)` 不被支持）。泛型构造函数 `public init<T2, Q2>(v: VecN<T2, Q2>)` 不受限于 T2 的具体类型——当 `T2 = Bool` 且 `T` 为数值类型时，构造函数体内的 `T(v.x)` 尝试将 `Bool` 转换为数值，将产生编译错误。**约束策略**：在 `init<T2, Q2>` 构造函数体内增加编译期约束或运行时分支——方案 A（编译期约束，优先）：在 `init` 体内使用 `const if (T2 is Bool && T is not Bool)` 阻止特定组合实例化，利用仓颉 `const if` 在编译期排除非法类型组合。方案 B（运行时兜底）：在 `init` 体内通过 `if (v.x is Bool)` 运行时检测并显式处理（`true → T(1)`，`false → T(0)`），此方案运行时开销可忽略。首轮采用方案 A，若编译期约束不可行（仓颉 `const if` 无法在构造函数体中直接判断类型参数），回退至方案 B。 |
...
| 依赖项 | 描述 | 风险等级 | 验证方法 |
|-------|------|---------|---------|
| ① `T(0)` const 构造 | `T(0)` 在 const 上下文中对所有目标类型（`Int32`、`Float32`、`Float16`、`Bool` 等）合法 | **高** — 若失败则 `const if` 分支前提不成立，整体方案不可行 | 编写原型测试：`const func testT0<T>(): Bool { const if (T(0) is Float64) { true } else { false } }`，分别对 `T = Int32, Float32, Float16, Bool` 验证编译通过 |
...
### `const if` 编译抑制行为验证（P16 关键项）

`mod` 函数的整数/浮点双路径通过 `const if (isIec559Of<T>())` 实现编译期分支选择。此设计依赖仓颉编译器对 `const if` 非选择分支的编译抑制行为——即当 `T` 为整数类型时，浮点分支（`x - y * trunc(x / y)`）中的 `trunc` 引用不应产生编译错误；当 `T` 为浮点类型时，整数分支（`x % y`）中的 `%` 运算符不应产生编译错误。

| 验证项 | 描述 | 风险等级 | 验证方法 |
|-------|------|---------|---------|
| ① `const if` 编译抑制 | `const if` 的非选择分支被编译器完全抑制（不执行语法/语义检查），而非仅做运行期优化跳过 | **严重** — 若失败则 `mod` 的双路径设计不可行，整数路径的 `%` 在浮点 `T` 上产生编译错误，浮点路径的 `trunc` 在整数 `T` 上产生约束冲突 | 编写原型测试：`const func testModConstIf<T>(): Vec2<T, PackedHighp> { const if (isIec559Of<T>()) { Vec2<T, PackedHighp>(T(0)) } else { Vec2<T, PackedHighp>(T(0)) } }` 分别以 `Int32` 和 `Float32` 实例化，验证两方向均编译通过 |
...
**验证优先级**：验证项①为**编码前原型验证**（与 D29/D30 同级），必须在编码启动前完成。若验证项①失败（`const if` 不抑制非选择分支），`mod` 函数回退方案为：拆分为独立具名函数 `modInt(v: VecN<T,Q>, s: T)` 和 `modFloat(v: VecN<T,Q>, s: T)`，分别使用 `%` 和 `trunc` 恒等式实现，调用方按类型自行选择。`add`/`sub`/`mul`/`div` 的实现不依赖 `const if`，不受此验证影响。
...
以下为历次迭代的修订记录（v2~v6），按时间倒序排列。历史内容已从主修订说明中移出以消除版本标识混乱，此处供追溯参考。
...
针对 `equalExact` const 声明验证、`Inf == Inf` 容差路径差异及迁移成本、`epsilonOf<T>()` 约束兼容性、`const if (T(0) is Float64)` 核心依赖验证、`@OverflowWrapping` 继承性验证、命名空间污染风险评估等 12 项审查意见的修改。
