# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v12 — 审查迭代修订版）

> **版本变更摘要**：v12 基于 v11 迭代，根据 v12 审查反馈（P1-P5）进行针对性修复：修正 §4.3 残留导入路径 `glm.detail.scalar_vec_ops.*` 为 `glm.detail.{ add, sub, mul, div, mod }` 并全文确认无其他残余（P1）、修正测试目录结构 `tests/glm.detail/` 为 `tests/glm/detail/` 并补充 `tests/glm/` 目录条目以匹配仓颉包路径解析规则（P2）、消除 §1 重复的 `### 设计目标` 标题（P3）、修复 §3.4/§3.6 两处 `const if` 术语残留为"编译期 `if` 分支"等表述（P4）、修正 §2 依赖表中 scalar_vec_ops 的依赖描述补充 Vec 类型引用说明（P5）。详见末尾修订说明。
