...
> **版本变更摘要**：v14 基于 v13 迭代，根据 v14 审查反馈（发现1~发现4）进行针对性修复：重构附录历史修订说明的三段式结构并添加版本体系说明（发现1）、修正 §10 `isIec559Of<Float16>` 说明为实现限制而非语义正确（发现2）、在 §12.1 层次二新增 `componentAt` const 访问验证项（发现3）、改进 §12.1 `equalExact` 回退路径等价性测试为手动修改 `==` 实现（发现4）。详见末尾修订说明。
