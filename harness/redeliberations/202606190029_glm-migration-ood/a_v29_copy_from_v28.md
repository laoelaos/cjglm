# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v29 — 基于 v28 审查基线迭代）

> **版本声明**：本文档版本号为 **v29**，基于 v28 审查意见迭代。文件名标注为 `a_v29_copy_from_v28.md`。此前版本 v28（基于 v27 审查基线）的标题未同步更新导致版本标识矛盾，本版本起统一以文件名为准、标题同步。

> **版本变更摘要**：v29 基于 v28 迭代，根据 v29 审查反馈进行修订：修正版本标识一致性（P1）、协同 equalExact const 化备选方案与 D32/D33 统一决策（P2）、在依赖表显式标注 scalar_vec_ops 的 std.math 外部依赖（P3）、在 §4.1 末尾新增 T2 跨类型约束矩阵表（P4）、在 §11 新增 equalEpsilon const 化备选方案成本评估（P5）。详见末尾修订说明。
...
glm.detail（内部无包引用，同包直接可见）
  ...
  scalar_vec_ops → qualifier + std.math（浮点 mod 路径依赖 std.math.trunc，详见 §10 浮点 mod 验证依赖②）
...
| ⑨ `internal` 类型测试访问策略验证 | §12.3 | 在 `tests/glm/detail/` 目录下创建测试文件（声明 `package glm.detail`），尝试实例化 `AlignedHighp` 类型 | 编译通过（同包 `internal` 可访问） | 按 §12.3 回退方案层级逐级尝试：① `protected` 可见性 → ② `public` 可见性 → ③ 测试 helper 文件 | 独立原型验证 |
...
- **浮点容差比较回退函数（`equalEpsilon`）**：为 `==` 回退路径提供浮点容差比较能力的独立非 `const` 具名函数。仅在 `==` 因 §7 D29 回退到精确比较时使用。各 VecN 类型分别在对应 `type_vecN.cj` 的 `extend` 块中定义：
  ...
  **⚠ const 真空警示**：当 D29 回退触发且 `==` 降级为精确比较后，`equalEpsilon` 作为唯一容差比较路径仅可在**非 const 上下文**中使用——它定义在 `extend` 块中，已被声明为非 `const` 函数。因此在 const 表达式（如 const 泛型函数、编译期常量初始化等场景）中，浮点 Vec 的容差比较能力完全不可用。若下游代码需要在 const 上下文中执行浮点容差比较，必须：
  1. 将 `equalEpsilon` 的定义从 `extend` 块移至 Vec 结构体体内（各 Vec 结构体自身定义该函数），利用 `const init` 的存在声明为 `const` 实例成员函数。
  2. 或者通过其他 const 兼容的容差比较方案（如调用方自行编写 const 内联比较表达式）。
...
| 5b | Bool→Numeric 转换 | 12b. type_fromBoolVec.cj | 编译通过；对 Vec1~Vec4 各验证一次 `fromBoolVec` 和 `fromBoolVecQ2` 的转换结果正确性 |
...
§4.5 的 ⚠ const 真空警示描述了 D29 回退触发后 `equalEpsilon`（定义在 `extend` 块中，声明为非 `const`）在 const 上下文中不可用的限制，并提出了两项缓解方案：① 将 `equalEpsilon` 定义从 `extend` 块移至 Vec 结构体体内声明为 `const` 实例成员函数；② 调用方自行编写 const 内联比较表达式。
...
**方案①成本评估**（将 `equalEpsilon` 移至 struct 体作为 `const` 实例成员函数）：

| 工作项 | 影响范围 | 工作量评估 |
|-------|---------|-----------|
| 将 `equalEpsilon` 定义从各 `type_vecN.cj` 的 `extend` 块移至 Vec 结构体体内部（移除 `extend` 块中的 `equalEpsilon`，在 struct 体内利用 `const init` 声明为 `const` 实例成员函数） | Vec1~Vec4 各一个函数定义，共 4 处移动 + 4 处移除，涉及 4 个文件 | 低（约 0.3 人天——4 处复制-移除+声明调整） |
| 验证 `abs()`/`epsilonOf<T>()` 在 `const` 实例成员函数中的可用性 | 原型测试验证（若 D29 已回退，此项验证应与 D29 回退验证合并执行） | 已在 §11.13 验证项②中覆盖（0.1 人天原型测试），无需额外成本 |
| 更新 `equalExact`（仍在 extend 块中）与 struct 体内 `equalEpsilon` 之间的扩展点隔离验证 | 确认移动后 `equalEpsilon` 与 `equalExact` 在不同定义位置上行为一致 | 低（约 0.1 人天——编译验证 + 回归测试） |
| 验证 const 上下文中 `equalEpsilon` 可用性 | 编写 const 泛型函数在 const 上下文中调用 `equalEpsilon` 原型测试 | 低（约 0.1 人天——已有原型测试框架可复用） |
| **合计** | | **约 0.5 人天** |

**方案②成本评估**（调用方自行编写 const 内联比较表达式）：
...
**推荐方案**：优先采用方案①（移至 struct 体作为 `const` 函数），理由：
- 方案①约 0.5 人天，方案②约 1.0-1.5 人天，方案①成本更低。
- 方案①实现集中（仅修改 4 个 Vec 结构体定义文件），方案②修改分散在各调用点——代码审查和维护成本更高。
- 方案①与 D33 备选方案（将 extend 块函数移至 struct 体）模式一致，可复用备选方案的验证框架和测试用例。
- 方案①实施后 `equalEpsilon` 的 API 签名不变（仅位置和 `const` 修饰符变化），下游调用代码无需修改。
...
| 审查意见 | 修改措施 |
|---------|---------|
| **P1（中等）** — 版本标识自相矛盾：标题标注 v27、文件路径含 v28、摘要描述 v26→v27。迭代6已识别同类问题，本次以新形式重现 | 统一标题标注为 `v29 — 基于 v28 审查基线迭代`；文件头新增版本声明块，明确版本号 `v29`、文件名 `a_v29_copy_from_v28.md`、版本状态；版本变更摘要更新为 v28→v29 |
| **P2（一般）** — §4.5 建议 `equalExact`"移至 struct 体利用 const init 声明为 const"，与 D32/D33 的"extend 块内函数统一非 const"约定不一致 | 在 D33 末尾补充 `equalExact` const 化备选方案说明，明确其与 `increment`/`decrement`/`bitwiseNot` 共用同一备选路径；同步修改 §4.5 `equalExact` const 声明说明段落，引用 D33 统一约定而非独立声明 |
| **P3（一般）** — `scalar_vec_ops` 的 `std.math` 外部依赖在 §2 自然段和 §8.1 说明列已标注，但模块依赖表本身未显式标注 | 在 §2 模块间依赖表 `scalar_vec_ops` 行中补充 `+ std.math（浮点 mod 路径依赖 std.math.trunc）`，并引用 §10 浮点 mod 验证依赖② |
| **P4（一般）** — T2 跨类型约束矩阵未集中归档，§4.1 以自然语言段落描述并引用 §9.4，缺少集中结构化矩阵表 | 在 §4.1 末尾新增 T2 跨类型约束矩阵表，行标题为 T 类型（Int8~Int64/UInt8~UInt64/Float32/Float64/Bool），列标题为 T2 类型，单元格填写编译/行为描述 |
| **P5（一般）** — §4.5 提出 `equalEpsilon` 迁至 struct 体的备选方案涉及架构变更，但 §11 未覆盖此方案的工作量和影响评估 | 在 §11.13 后新增 §11.14 `equalEpsilon const 化备选方案成本评估`，分别评估"移至 struct 体声明为 const"（方案①，约 0.5 人天）和"调用方自行编写内联比较"（方案②，约 1.0-1.5 人天），推荐方案①；声明此方案仅在 D29 回退触发的极端场景下考虑，不纳入 §11.12 成本合计 |

### 版本 v27 — 上一轮修订说明

| 审查意见 | 修改措施 |
|---------|---------|
| **F1（一般）** — `fromBoolVec` 定义为扩展成员函数，调用方须持有一个同类型 Vec 实例作为 receiver，与本设计其他工厂函数风格不一致 | 将 §4.8 中 `fromBoolVec` 的定义从 4 个 `extend` 块中的扩展成员函数改为 4 个包级独立函数（Vec1~Vec4 各一个）；同步更新 §4.1 Vec1 构造函数不对称性说明、§9.4 路径 A 描述、§12.1/§12.2 的交叉引用与验证覆盖 |
| **F2（一般）** — §4.1 Vec4 多重重载构造函数在重载解析中存在歧义风险，涉及约 30 个构造函数签名，重构成本较高但仅标注为[提示] | 在 §4.1 重载解析风险段落中将风险等级从[提示]升级为**中等**，新增具体回退方案（优先移除 4-6 个歧义分支改为具名工厂函数）；在 §10 新增"Vec4 构造函数重载解析验证"子节（3 项验证依赖 + 回退方案）；在 §2.1 检查清单中新增第⑬项（Vec4 重载解析验证） |
| **F3（中等）** — §4.5 D29 回退描述称"`equalEpsilon` 成为浮点 Vec 上唯一可用的运算符级容差比较路径"，但 `equalEpsilon` 为非 `const` 函数，在 const 表达式中不可用，形成功能真空 | 在 §4.5 `equalEpsilon` 重要限制段落中新增 **⚠ const 真空警示**子段落，显式说明 D29 回退后 const 上下文中容差比较不可用，并给出两项缓解方案；在 §12.2 验收标准表末新增"`equalEpsilon` D29 回退路径 const 真空验证"验收项 |
| **F4（提示）** — 设计依赖 12+ 个编译器行为假设，§11.13 列出各项独立回退成本，但未评估多项同时回退时的叠加效应和冲突 | 在 §11.13 末尾新增"多项回退叠加效应"子节，列出 7 组已知回退组合的叠加效应分析表，给出核心结论（无不可调和冲突、重叠成本不可叠加、最差路径实际成本约 3.5 人天） |
| **F5（提示）** — §2.1 检查清单第⑫项与第①③④⑤⑧项存在功能重叠，未说明是"取代"还是"补充" | 在 §2.1 清单表前新增"层级说明"注释行，明确标注第⑫项为 **统一集成验证**（取代独立验证结果），第①③④⑤⑧项为 **独立原型验证**（聚焦单一假设边界）；为每行新增"类型"列；第⑫项验证方法中新增 Vec4 重载解析原型测试 |
| **F6（提示）** — `fromBoolVec` 签名仅支持同 Q 转换，不支持跨 Q 转换 | 在 §4.8 中新增 4 个 `fromBoolVecQ2` 跨 Q 版本（Vec1~Vec4 各一个），签名追加 Q2 泛型参数以支持输入 Bool Vec 与输出数值 Vec 使用不同 Qualifier；在 §9.4 路径 A 末尾新增"跨 Q 转换说明"段落；在 §12.1/§12.2 中补充 `fromBoolVecQ2` 验证覆盖 |
