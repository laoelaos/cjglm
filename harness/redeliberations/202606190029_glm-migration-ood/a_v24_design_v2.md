# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v24 — 审查迭代修订版）

> **版本变更摘要**：v24 基于 v23 迭代，根据 v24 审查反馈（发现 1~4）进行补充修订：在 §3.8 脚本模板和 fwd.cj 示例中统一将 `type` 改为 `public type` 并新增"仓颉 `type` 别名可见性提醒"及"生成脚本生命周期警告"（发现 1/2）、在 §2.1 编码前验证检查清单新增验证项⑩⑪（发现 1/4）、在 §8.2 新增 §3G 范围确认结论追溯说明（发现 3）、在 §10 末尾新增"生成脚本验证依赖"子节（发现 4）、在 §12.2 新增"别名对外可见性"验收项（发现 1）。本版根据 v24 再审查反馈修正 `lib.cj` 中 `public import glm.fwd.*` 引用不存在的 `glm.fwd` 包的问题——`fwd.cj` 与 `lib.cj` 同属 `package glm`，其 `public` 声明已自动属于 `package glm`，下游 `import glm.*` 即可访问全部别名，无需 `lib.cj` 重导出。详见末尾修订说明。
...
| ⑪ 生成脚本映射表正确性验证 | §3.8 | 抽查 4 个家族的别名命名是否符合 §3.8 命名约定（如 B/BVec、I/IVec、U/UVec、Vec 各抽查一个精度变体）；确认脚本生成结果与人工审查一致 | 抽查项全部匹配 | 修正脚本映射表中的错误映射后重新生成并验证 |
