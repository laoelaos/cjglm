...
> **版本变更摘要**：v10 基于 v9（a_v9_copy_from_v8.md）迭代，根据 v10 审查反馈（P1-P6）进行针对性修复：修正 `public import` 路径使用包名而非文件名（P1，§2）、修正 `GLM_CONFIG_CLIP_CONTROL` 默认值来源说明（P2，§7 D6）、补充 `equalExact` 测试覆盖和行为差异说明（P3，§9.3/§12.1）、补充测试包可见性假设验证要求（P4，§12.3）、修正目录树形图结构（P5，§2）、修正 `countof` 偏离理由（P6，§8.2）。详见末尾修订说明。
...
- **`equalExact`**：验证浮点 Vec 上 `equalExact(Infinity)` 返回 `true`；`equalExact(NaN)` 返回 `false`；整数/Bool Vec 上 `equalExact` 行为与 `==` 等价（均走精确比较路径）；验证 `equalExact` 与 `==` 在容差路径下的行为差异——对有限值 `equalExact` 使用精确比较（可能因浮点舍入将微差视为不等）而 `==` 使用容差比较（在 epsilon 范围内视为相等）。**回退路径等价性测试**：若 `const operator ==` 因 D29 回退到精确比较路径，则 `==` 与 `equalExact` 对所有输入（含 Inf/NaN）的行为完全等价。编码阶段通过条件编译开关模拟回退路径，验证 `==` 与 `equalExact` 在等价性上的一致性。
