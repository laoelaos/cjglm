# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v9 — 审查迭代修订版）

> **版本变更摘要**：v9 基于 v8（a_v8_design_v2.md）迭代，根据组件 B 诊断报告（P1-P6）进行针对性修复：补充 vec-op-scalar 方向扩展成员函数的 `@OverflowWrapping` 委托策略说明（P1，§4.3/§4.6）、新增 Vec1 构造函数不对称性说明（P2，§4.1）、补充 `countof` 偏离追溯条目（P3，§8.2）、集中归档跨类型构造函数 T2 有效性范围并交叉引用 §9.4（P4，§4.1）、标注 `GLM_CONFIG_CLIP_CONTROL` 与 roadmap 差异说明（P5，§7 D6）、修正 D30 回退工作量评估（P6，§7 D30/§11.6）。详见末尾修订说明。
...
| `GLM_CONFIG_CLIP_CONTROL` | `const GLM_CONFIG_CLIP_CONTROL: Int64` | `0x0A`（十进制 10） | 裁剪空间约定（LH/RH, NO/ZO 位组合）。对应 C++ `RH_BIT(8) \| NO_BIT(2)` = 0x0A，首轮使用默认 RH_NO。**与 roadmap 差异说明**：roadmap §3A.1 记录该配置默认值为 `0x02`，但经与 GLM 1.0.3 参考实现 `setup.hpp` 中 `#define GLM_CONFIG_CLIP_CONTROL GLM_CLIP_CONTROL_DISABLE` 的定义链验证，`GLM_CLIP_CONTROL_DISABLE` 等价于 `RH_BIT(8) | NO_BIT(2) = 0x0A`。文档值 `0x0A` 已验证正确，roadmap 值 `0x02` 有误。此差异已在文档中标注，编码阶段以本设计文档为准 |
...
| S3 | S | `<cstddef>` shim（不含 `countof`） | §3.6 Shim 层 | `shim_cstddef.cj` | **偏离**：GLM `countof` 宏基于 `sizeof` 实现数组元素计数。因仓颉 `VArray<T, $N>` 的 `$N` 必须为固定数值字面量不可声明为值泛型参数，首轮决定不映射 `countof` 函数。调用方使用 `VArray` 的 `size` 属性或 `LengthT` 作为替代。§3.6 已声明此决策，但上一轮 §8.2 追溯表遗漏此偏离的记录，现补充标注 |
