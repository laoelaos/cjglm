# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v18 — 审查迭代修订版）

> **版本变更摘要**：v18 基于 v17 迭代，根据 v18 审查反馈（发现 1~3）进行针对性修复：在 §10/§12 归档 `equalEpsilon` 与 `epsilonOf<T>()` 回退路径的隐式依赖链（发现 1）、在 §9.4/§4.1 补充 Vec1 `fromBoolVec` 工厂函数的不对称性说明和实现模式（发现 2）、在 §3.2/§12.1 补充 `Display`/`ToString`/`Format` 未实现策略的选择理由和编译验证（发现 3）。详见末尾修订说明。
...
**备选路径**（若 `const init` 或 `const if` 在特定上下文不可用）：所有类型使用精确比较（`a == b`）。此路径不损失正确性——浮点数直接比较在大多数使用场景下足够。浮点容差比较通过独立的非 `const` 具名函数 `equalEpsilon`（定义见 §4.5）提供给需要容差比较的场景。**回退路径一致性**：此备选路径与 §7 D29 定义的二值决策中的"回退路径"一致——若 `const` 表达式链验证失败，`==` 整体回退到精确比较路径，浮点容差比较由非 `const` 具名函数 `equalEpsilon` 提供。
...
| ④ `epsilonOf<T>()` 回退路径与 `equalEpsilon` 的隐式依赖链 | §3.5 定义了 `epsilonOf<T>()` 的约束兼容性回退方案（移除 `Number<T>` 约束、在 `ComputeEqual.callConst` 中内联固定容差 `T(1e-6)`）。但 `equalEpsilon` 函数体继续引用 `epsilonOf<T>()`——当回退被触发后，`equalEpsilon` 体内的 `epsilonOf<T>()` 引用因 `Number<T>` 约束在非浮点 T 上的不满足而失效。此依赖链未被文档显式归档，属于隐式耦合 | **中等** — 依赖链跨两处设计决策（§3.5 兼容性策略和 §4.5 equalEpsilon 实现），单独修改任一处的实施者可能遗漏另一处的同步 | 编码阶段验证路径：① 若 `epsilonOf<T>()` 的 **`Number<T>` 约束保留** → `equalEpsilon` 体内继续引用 `epsilonOf<T>()`，验证整数 T 上 `Number<T>` 约束满足（与依赖②合并）；② 若 `epsilonOf<T>()` 的 **`Number<T>` 约束被移除**（回退路径触发）→ `equalEpsilon` 体内 `epsilonOf<T>()` 的约束变为无约束版本，此时需将 `equalEpsilon` 的实现从引用 `epsilonOf<T>()` 改为内联固定容差 `T(1e-6)`，与 `ComputeEqual.callConst` 的回退路径保持一致。回退路径的联动修改作为 §12.1 层次三 `equalEpsilon` 验证项的交叉引用注解 |
...
| 审查意见 | 修改措施 |
|---------|---------|
| **发现 1（中等）** — §3.5 定义了 `epsilonOf<T>()` 的约束兼容性回退方案（移除 `Number<T>` 约束、在 `ComputeEqual.callConst` 中内联固定容差 `T(1e-6)`）。但 §4.5 的 `equalEpsilon` 函数体继续引用 `epsilonOf<T>()`。当 `epsilonOf<T>()` 的回退被触发后，`equalEpsilon` 体内的引用失效 | ① §12.1 层次三 `equalEpsilon` 验证项末尾新增交叉引用注解：当 `epsilonOf<T>()` 回退路径被触发后，`equalEpsilon` 的实现需同步将引用替换为内联固定容差 `T(1e-6)`；② §10 `equalEpsilon` 设计阶段验证表新增依赖④，归档此隐式依赖链的回退联动要求 |
| **发现 2（中等）** — §9.4 推荐路径 A（`fromBoolVec` 工厂函数）定义为各 VecN 类型 `extend` 块中的扩展成员函数。Vec1 存在已知的构造函数不对称性（§4.1：仅 `const init(x: T)`，无 `public init(scalar: T)`），但未说明此不对称性对 `fromBoolVec` 的实际影响 | ① §9.4 路径 A 描述末尾新增"Vec1 特殊说明"段落，阐明 `fromBoolVec` 为具名函数（非 `init`）故不受 Vec1 构造函数不对称性影响，并给出 Vec1 版本的完整定义模式；② §4.1 Vec1 不对称性说明末尾新增交叉引用，链接至 §9.4 路径 A 的 Vec1 特殊说明 |
| **发现 3（一般）** — §3.2 字符串表示策略声明 Vec 类型不实现自定义 `toString()` 或 `Format`/`Display`，但 §12 的测试策略中未覆盖此决定的验证 | ① §3.2 字符串表示策略补充三点选择理由（编译器默认表示足够调试、无实际消费方、策略待后续统一设计）；② §12.1 层次一新增"`Display`/`ToString`/`Format` 未实现验证"编译验证项 |
