# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v27 — 审查迭代修订版）

> **版本变更摘要**：v27 基于 v26 迭代，根据 v27 审查反馈进行修订：将 `fromBoolVec` 从扩展成员函数改为包级独立函数并新增跨 Q 重载版本（F1/F6）、在 §4.1/§10/§2.1 强化构造函数重载解析风险标注与回退方案（F2）、在 §4.5/§12.2 新增 `equalEpsilon` const 真空警示与验证项（F3）、在 §11.13 新增多项回退叠加效应子节（F4）、在 §2.1 标注检查清单层次关系（F5）。详见末尾修订说明。
