# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v23 — 审查迭代修订版）

> **版本变更摘要**：v24 基于 v23 迭代，根据 v24 审查反馈（发现 1~4）进行补充修订：在 §3.8 脚本模板和 fwd.cj 示例中统一将 `type` 改为 `public type` 并新增"仓颉 `type` 别名可见性提醒"及"生成脚本生命周期警告"（发现 1/2）、在 §2.1 编码前验证检查清单新增验证项⑩⑪（发现 1/4）、在 §8.2 新增 §3G 范围确认结论追溯说明（发现 3）、在 §10 末尾新增"生成脚本验证依赖"子节（发现 4）、在 §12.2 新增"别名对外可见性"验收项（发现 1）。详见末尾修订说明。
...
| ⑩ 别名对外可见性验证 | §3.8/§12.2 | 从独立测试包中通过 `import glm.BVec2` 访问别名并实例化，验证编译通过 | 编译通过 | 检查 `fwd.cj` 中所有 `type` 别名是否添加 `public` 修饰符；确认 `lib.cj` 中 `public import glm.fwd.*` 正确导出 |
...
**仓颉 `type` 别名可见性提醒**：仓颉中 `type` 别名的默认可见性为 `internal`（包及子包可见）。若要对外暴露给下游消费者，必须在 `type` 前添加 `public` 修饰符。以上所有 `fwd.cj` 中的别名定义均使用 `public type` 形式，确保通过 `lib.cj` 的 `public import glm.fwd.*` 重新导出后，下游消费者可正常访问。
