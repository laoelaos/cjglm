# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v11 — 审查迭代修订版）

> **版本变更摘要**：v11 基于 v10（a_v10_copy_from_v9.md）迭代，根据 v11 审查反馈（P1-P4）进行针对性修复：全文将 `const if (...)` 改写为 `if (...)` 避免误导实施者编写不存在的关键字（P1，§3.5/§3.6/§4.3/§10）、补充 `glm.detail` 包内命名空间占用声明约定 `scalar_vec_ops.cj` 对 `add`/`sub`/`mul`/`div`/`mod` 的独占权（P2，§2/§7 D35）、新增 `equalExact` 迁移模式系统覆盖（P3，§11.9）、补充参考实现前置验证步骤和兜底条款（P4，§1/§8.4）。详见末尾修订说明。
...
| 依赖项 | 描述 | 风险等级 | 验证方法 |
|-------|------|---------|---------|
| ① `T(0)` const 构造 | `T(0)` 在 const 上下文中对所有目标类型（`Int32`、`Float32`、`Float16`、`Bool` 等）合法 | **高** — 若失败则 `const if` 分支前提不成立，整体方案不可行 | 编写原型测试：`const func testT0<T>(): Bool { if (T(0) is Float64) { true } else { false } }`，分别对 `T = Int32, Float32, Float16, Bool` 验证编译通过 |
...
|-------|---------|---------------|--------------|---------|------------------------|
| A. `ComputeEqual.callConst` | §3.5 | `if (isIec559Of<T>())` | 整数路径：`a == b`；浮点路径：`abs(a - b) <= epsilonOf<T>()` | **严重** — 失败则 `==` 整体回退到精确比较 | `callConst` 声明为 `const`；若 `const if` 抑制不成立，`==` 降级为非 `const` 精确比较 |
...
| D. `ComputeEqual.callConst` + `isIec559Of` 组合 | §3.5+§3.6 | `if (T(0) is Float64)` 嵌套于 `callConst` 内 | 整数分支 `a == b` | **严重** — 两层 const if 嵌套的抑制行为 | 同上 A，`callConst` 为 `const`；若失败 → 整体回退 |
...
| ④ 整数 Vec `mod` 与浮点 `mod` 分支选择的 `const if` 正确性 | `if (isIec559Of<T>())` 正确区分整数/浮点路径 | 低 — 与 `ComputeEqual` 分支逻辑一致 | 确认 `isIec559Of<T>()` 对整数类型返回 `false`，对浮点类型返回 `true` |
