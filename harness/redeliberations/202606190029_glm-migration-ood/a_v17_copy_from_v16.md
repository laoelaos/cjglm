# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v17 — 审查迭代修订版）

> **版本变更摘要**：v17 基于 v16 迭代，根据 v17 审查反馈（P1~P4）进行针对性修复：统一 `epsilonOf<T>()` 为固定容差语义并修正 §9.3 描述（P1）、修正 §4.5 `equalEpsilon` 对 Bool T 编译行为的描述并澄清 `Comp` 变量未使用问题（P2/P3）、在 §10 新增 `equalEpsilon` 验证依赖项并在 §12 补充验证覆盖（P4）。详见末尾修订说明。
...
- **`equalEpsilon`**：验证 `equalEpsilon` 在 `Float32`/`Float64` Vec 上编译通过且容差比较行为正确（构造微差分量，验证 epsilon 范围内相等）；验证 `equalEpsilon` 在 `Int32`/`Int64` Vec 上编译通过且行为等价于精确比较（`-`/`abs` 对整数类型合法）；验证 `equalEpsilon` 在 `Bool` Vec 上实例化时产生编译错误（`-` 对 `Bool` 不可用）。验证 `abs()` 在 `extend` 块中的可用性（与 §4.5 设计假设一致）。
