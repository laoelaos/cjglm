...
- **方案 A（优先）**：移除 `epsilonOf<T>()` 的 `where T <: Number<T>` 约束，将 epsilon 逻辑内联到 `ComputeEqual.callConst` 的浮点分支中。内联形式为 `const if (isIec559Of<T>()) { abs(a - b) <= T(1e-6) } else { a == b }`，使用固定容差值 `T(1e-6)` 替代 `NumericLimits<T>.epsilon()`。
- **方案 B**：在 `shim_limits.cj` 中为 `NumericLimits<Bool>` 添加空壳特化（仅声明结构体，不实现 `epsilon()` 方法），使 `Bool` 类型满足 `Number<T>` 约束的类型检查但不实际调用。此方案引入无意义的特化，不推荐。
