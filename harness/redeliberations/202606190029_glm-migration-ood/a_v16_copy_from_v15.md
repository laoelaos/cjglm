# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v16 — 审查迭代修订版）

> **版本变更摘要**：v16 基于 v15 迭代，根据 v16 审查反馈（P1~P4）进行针对性修复：在 §4.5 新增 `equalEpsilon` 容差比较回退函数完整定义（P1）、修正 §9.4 Bool→数值转换方案 B 的运行时 `if` 分支抑制假设并明确标注为待验证（P2）、重构附录版本历史组织方式并将当前修订说明前置（P3）、在 §9.4 扩展 Vec2~Vec4 的 Bool 检测模式说明（P4）。详见末尾修订说明。
...
- **浮点容差比较回退函数（`equalEpsilon`）**：为 `==` 回退路径提供浮点容差比较能力的独立非 `const` 具名函数。仅在 `==` 因 §7 D29 回退到精确比较时使用。各 VecN 类型分别在对应 `type_vecN.cj` 的 `extend` 块中定义：
  ...
  extend<T, Q> Vec2<T, Q> where Q <: Qualifier {
      public func equalEpsilon(other: Vec2<T, Q>): Bool {
          ...
          // 注意：仅当 T 为浮点类型时有意义，整数/Bool T 上行为等价于 exact
...
| 审查意见 | 修改措施 |
|---------|---------|
| **P1（中等）** — `equalEpsilon` 回退函数在行为契约中缺失正式定义。§3.5 和 §7 D29 引用 `equalEpsilon` 但 §4 中未定义 | §4.5 新增完整 `equalEpsilon` 定义（含非 `const` 扩展成员函数签名、行为约定、`Inf`/`NaN` 边界行为、与 `equalExact` 的关系）；§3.5 和 §7 D29 的引用指向更新为 `equalEpsilon`（定义见 §4.5） |
| **P2（中等）** — §9.4 Bool→数值转换方案 B 的非 `const` `if` 分支抑制假设未经论证。仅 `const` 函数中的 `if` 具有编译期分支抑制行为，非 `const` 函数中编译器会检查所有分支 | §9.4 `Bool→数值` 行重写：① 移除"方案 B 为首选推荐路径"确定性表述；② 将方案 B 标注为"待验证"路径，增加 `as` 运算符方案的可行性论证（含 3 项编译器行为假设）；③ 新增路径 A（具名工厂函数 `fromBoolVec`，零编译依赖）为首选推荐；④ §10 新增 `Bool→Numeric` 转换路径 B 可行性验证子节，覆盖 3 项依赖验证 |
| **P3（一般）** — 附录历史修订版本序列混乱：混用两套版本编号体系，排列方向不一致，版本跳跃，当前 v15 修订说明位于附录末尾 | ① 附录结构重构：新增版本关系时间线图，清晰展示两个审议周期的衔接关系；② 当前版本（v16）修订说明已移至附录章节顶部；③ 后续版本的修订说明按倒序排列 |
| **P4（一般）** — §9.4 中 `v.x is Bool` 运行时类型检测对 Vec2~Vec4 的适用性未讨论。仅以 Vec1 示例描述 | §9.4 `Bool→数值` 行新增"Vec2~Vec4 Bool 检测模式"子节，明确给出 Vec2 的逐分量 `as` 检测示例和 `fromBoolVec` 工厂函数在 Vec2~Vec4 上的逐分量构造模式 |
