# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v5 — 审查迭代修订版）
...
首轮中 `compute_vector_decl.cj` 和 `vectorize.cj` 中的 `compute_vec_*` 策略结构体（如 `compute_vec_add`、`compute_vec_sub` 等）定义类型但不被 Vec 运算符实现消费，为后续轮次预留。但 `compute_vector_decl.cj` 中 §4.3 新增的 `scalar-op-vec` 方向辅助函数（如 `add(s, v)`、`sub(s, v)` 等）属于首轮即被消费的例外——Vec 使用者需在 `scalar + vec` 等场景中调用它们。
...
- **`const init` 构造函数**：每个 Vec 定义一个 `const init(x: T, y: T, ...)` 构造函数，允许 `const` 实例成员函数的定义，同时其本身在非 const 上下文中也可用于运行时构造（const README §3.2 规则 5），因此不再需要同参数列表的普通 `init`。`const init` 的存在使得 `==` 等比较运算符可以声明为 `const` 函数，从而在函数体内使用 `const if` 实现编译期分支选择。`extend` 块中也可定义 `const` 实例成员函数（规则与类型体内一致——struct 定义了 `const init` 即可），语法示例见 §4.5 `equalExact` 的 `const` 声明。
...
- **`++`/`--` 运算符**：仓颉可重载运算符列表中不包含 `++`/`--`，因此无法为 Vec 类型重载。改为提供具名函数 `increment()`（返回逐分量加 1 的新向量）和 `decrement()`（返回逐分量减 1 的新向量）。这些函数标注 `@OverflowWrapping` 并声明为 `const` 函数，语义与 C++ GLM 的 `operator++`/`operator--` 返回新副本的行为一致（C++ 前缀形式返回引用，后缀返回副本；仓颉值类型统一返回副本）。C++ 的后缀 `v++` 在迁移时需替换为 `v = v.increment()`，前缀 `++v` 替换为 `v.increment()` 后重新赋值。
...
**`epsilonOf<T>()` 定义**：`ComputeEqual.callConst` 中引用的 `epsilonOf<T>()` 是定义于 `shim_limits.cj` 的 `const` 泛型函数。其签名为 `const func epsilonOf<T>(): T where T <: Number<T>`，内部实现委托给 `NumericLimits<T>.epsilon()` ——`Number<T>` 约束在此处合法，因为 `const if (isIec559Of<T>())` 分支仅在浮点 `T` 上进入，而浮点类型（`Float32`/`Float64`）满足 `Number<T>` 约束。定义形式如下：
...
- 移除 `epsilonOf<T>()` 的 `where T <: Number<T>` 约束，将 epsilon 逻辑内联到 `ComputeEqual.callConst` 的浮点分支中。内联形式为 `const if (isIec559Of<T>()) { abs(a - b) <= T(1e-6) } else { a == b }`，使用固定容差值 `T(1e-6)` 替代 `NumericLimits<T>.epsilon()`。
...
- **const 上下文中的分量访问**：仓颉的 `const` 修饰符不构成函数重载的区分依据（const README §3.2 规则 7），因此无法同时定义 `const` 和非 `const` 两个版本的 `operator[]` 取值形式。对于需要在 `const` 实例成员函数中通过索引访问分量的场景，提供具名 const 函数 `public const func componentAt(i: Int64): T`。该函数与 `operator[]` 取值形式的行为完全一致（含边界断言），但标注为 `const`，可在 `const` 实例成员函数体内使用。`componentAt` 在非 const 上下文中同样可用。
...
- **标量在左的运算替代方案**：`scalar + vec`、`scalar - vec`、`scalar / vec` 等左标量右向量的运算符重载不可实现。提供具名函数替代，包括双向重载覆盖两种操作数顺序：
  - `add(v: VecN<T,Q>, s: T): VecN<T,Q>` 和 `add(s: T, v: VecN<T,Q>): VecN<T,Q>`（均声明为 `const`）
  - `sub(v: VecN<T,Q>, s: T): VecN<T,Q>` 和 `sub(s: T, v: VecN<T,Q>): VecN<T,Q>`（均声明为 `const`）
  - `mul(v: VecN<T,Q>, s: T): VecN<T,Q>` 和 `mul(s: T, v: VecN<T,Q>): VecN<T,Q>`（均声明为 `const`）
  - `div(v: VecN<T,Q>, s: T): VecN<T,Q>` 和 `div(s: T, v: VecN<T,Q>): VecN<T,Q>`（均声明为 `const`）
  - `mod(v: VecN<T,Q>, s: T): VecN<T,Q>` 和 `mod(s: T, v: VecN<T,Q>): VecN<T,Q>`（均声明为 `const`）
- **具名函数的定义位置与形态**：
  ...
  - **`scalar-op-vec` 方向**（如 `add(s, v)`）：定义为 **包级独立函数**，集中定义在 `compute_vector_decl.cj` 文件中（与 ComputeVec* 运算策略同文件）。调用方通过 `import glm.detail.compute_vector_decl.*` 或通过 `glm` 包统一引入。该方向无法定义为扩展成员函数（孤儿规则——标量类型如 `Int32`/`Float32` 不在本包中定义）。
  ...
  - **命名空间污染风险评估**：`scalar-op-vec` 方向具名函数（`add(s, v)`、`sub(s, v)` 等）定义为 `package glm.detail` 中的包级独立函数，通过 `lib.cj` 的 `public import` 暴露到 `package glm` 命名空间。这些函数的名称（`add`、`sub`、`mul`、`div`、`mod`）是常见英文单词，在不同上下文中可能与其他库或用户代码中的同名函数发生冲突。**风险等级**：低-中。在单一项目中此类冲突概率较低；但在大型项目中，`add`/`sub` 等名称可能被多个模块定义。**调用方最佳实践**：① 优先使用 `vec-op-scalar` 方向的扩展成员函数（`v.add(s)`）以避免导入包级函数；② 若需使用 `scalar-op-vec` 方向函数，通过限定导入（`import glm.detail.compute_vector_decl.{ add, sub, mul, div, mod }`）明确来源；③ 若发生命名冲突，使用 `import glm` 的别名导入或通过局部函数包装消除歧义。
...
- **`%` 运算的整数约束**：`%` 运算符在 Vec 上定义为逐分量取模。当 `T` 为整数类型时正常编译；当 `T` 为浮点类型时，因仓颉原生 `Float32`/`Float64` 不支持 `%`，在实例化时产生编译错误。此行为与 D5（宽松泛型约束）一致——错误触发于使用处而非定义处。对于需要浮点数取模的场景，使用具名函数 `mod`（属于首轮范围）。`mod` 对浮点类型 Vec 的实现策略：整数 T 场景直接使用 `%` 运算符（与 Vec 二元 `%` 运算符行为一致）；浮点 T 场景因仓颉原生 `Float32`/`Float64` 不支持 `%`，改用算术恒等式 `x - y * trunc(x / y)` 实现浮点取模——其中 `trunc` 取自 `std.math`。对于 `Float32` 分量，通过先提升至 `Float64` 计算截断再转换回 `Float32` 的方式实现（`std.math.trunc` 仅提供 `Float64` 版本，但此精度提升无损取模语义的正确性）。此双路径在函数体内通过 `const if (isIec559Of<T>())` 实现编译期分支选择。仓颉中 `const if` 要求所在函数必须为 `const` 函数，因此 `mod` 在两种方向（`vec-op-scalar` 扩展成员函数和 `scalar-op-vec` 包级独立函数）上均声明为 `const`。对于 `add`/`sub`/`mul`/`div`，虽其实现不依赖 `const if`，但为保持 API 一致性、支持 const 上下文调用及为后续轮次预留 const 路径扩展点，同样声明为 `const`。
- **`++`/`--` 替代方案**：`++`/`--` 不在仓颉可重载运算符列表中，不可实现为运算符重载。改为提供具名函数 `increment(): VecN<T,Q>` 和 `decrement(): VecN<T,Q>`（标注 `@OverflowWrapping`，声明为 `const` 函数），返回逐分量加/减 1 的新向量。C++ 的 `v++`（后缀）迁移为 `v = v.increment()`，`++v`（前缀）迁移为 `v = v.increment()`。对于 `VecN<Bool, Q>`，`increment()`/`decrement()` 在实例化时因 `Bool` 不支持 `+`/`-` 产生编译错误，与 `%` 的整数约束行为一致。
...
extend<T, Q> Vec2<T, Q> where Q <: Qualifier {
    ...
    public const func bitwiseNot(): Vec2<T, Q> {
        Vec2(!this.x, !this.y)
    }
}
```
`extend` 块的类型参数声明语法与 struct 定义一致——`extend<T, Q> Vec2<T, Q> where Q <: Qualifier`。扩展成员函数默认可见性与被扩展类型一致（`internal`），可通过 `public` 提升为对外可见。`this` 关键字在扩展成员函数中指向被扩展的 Vec 实例。
...
- **浮点精确相等比较**（`equalExact`）：为浮点分量 Vec 提供 `equalExact(other: VecN<T,Q>): Bool` 扩展成员函数（定义在 `extend` 块中，声明为 `const`）。该函数跳过容差比较路径，直接使用仓颉原生 `==` 对每个分量做精确比较。适用于需要 IEEE 754 精确相等语义的场景（如 `Infinity == Infinity` 返回 `true`、`NaN == NaN` 返回 `false`）。`equalExact` 在各 `type_vecN.cj` 的 extend 块中定义为 `const` 成员函数，对整数/Bool 分量 Vec 其行为与 `==` 等价（整数/Bool 精确比较）。浮点类型在调用 `equalExact` 时不受 `==` 的 Epsilon 容差分支影响。`equalExact` 的 `@OverflowWrapping`/溢出策略：不涉及算术运算，无需标注。
- **`equalExact` 的 `const` 声明验证**：`const` 扩展成员函数要求被扩展的 struct 已定义 `const init`（该条件满足，见 §3.2）。`extend` 块中声明 `const` 实例成员函数的语法示例如下：
  ```cangjie
  // 假设 Vec2<T, Q> 已定义 const init(x: T, y: T)
  extend<T, Q> Vec2<T, Q> where Q <: Qualifier {
      public const func equalExact(other: Vec2<T, Q>): Bool {
          this.x == other.x && this.y == other.y
      }
  }
  ```
  **验证状态**：`extend` 块中 `const` 实例成员函数的语法支持性已在仓颉 const 文档中确认（struct 定义了 `const init` 即可定义 `const` 实例成员函数，extend 块中的成员函数与被扩展类型体中的成员函数在 `const` 修饰符规则上一致）。编码阶段需编写原型测试确认。若验证失败，降级为非 `const` 函数——`equalExact` 的功能正确性不受影响（浮点 `==` 在非 `const` 上下文中同样可用），仅失去在 `const` 表达式中调用的能力。
...
**`@OverflowWrapping` 标注继承性验证要求**：当前策略断言编译器自动生成的复合赋值版本继承二元运算符上的 `@OverflowWrapping` 标注（见 §7 D30 验证计划）。**此行为依赖编译器的未文档化行为**——仓颉注解文档（§1.1）声明溢出注解"只能标记于函数声明上"，但未明确说明自动生成的复合赋值函数是否继承被委托二元运算符的注解。此继承性在仓颉语言层面既未确认也未否认，属于对编译器实现行为的推测。编码阶段需优先编写原型测试代码进行验证。若验证失败，整体切换到备选方案——所有 Vec 算术运算通过 extend 块中的具名函数实现，在这些具名函数上直接标注 `@OverflowWrapping`，二元运算符委托给这些具名函数。备选方案的额外工作量和成本已纳入 §11.6 迁移成本评估。
...
### D16. Functor/ComputeVec 首轮仅定义不消费（含方向辅助函数例外）

**决策**：`vectorize.cj` 和 `compute_vector_decl.cj` 中的类型在首轮的行为分为两类：

1. **`compute_vec_*` 策略结构体**（如 `compute_vec_add`、`compute_vec_sub` 等）和 **Functor 类型**：首轮仅定义结构体，不被 Vec 运算符实现调用，为后续 SIMD 轮次预留。
2. **`scalar-op-vec` 方向辅助函数**（如 `add(s, v)`、`sub(s, v)` 等，定义于 `compute_vector_decl.cj`）：首轮即被消费——Vec 使用者在 `scalar + vec` 等场景中需调用它们。此为该文件中的例外项。

**理由**：首轮 Vec 运算符实现直接使用泛型算术运算符逐分量运算，无需通过 ComputeVec* 或 Functor 委托。方向辅助函数（`add(s, v)` 等）是 `scalar + vec` 运算符重载不可实现的替代方案，属于首轮公共 API 的必要部分。Functor 和 ComputeVec 为后续 SIMD 轮次、swizzle 操作轮次或性能优化轮次预留。首轮定义它们可以尽早验证其在仓颉类型系统中的可行性，但运算符代码不引用它们。后续轮次引入 SIMD 路径时，可将 Vec 运算符实现重构为通过 ComputeVec* 委托，利用 Functor 实现函数映射。
...
2. **备选方案（若继承性不成立）**：所有 Vec 算术运算通过 extend 块中的具名函数（如 `add`、`sub`、`mul`、`div`）实现，在这些具名函数体内标注 `@OverflowWrapping`。此方案虽失去运算符语法糖，但确保溢出语义精确可控。二元运算符可委托给这些具名函数实现，但需接受运算符本身无法直接标注 `@OverflowWrapping`（复合赋值也无法自动标注）的约束。备选方案的额外工作量已纳入 §11.6 迁移成本评估。
...
| 任意 | ❌ 回退 | 算术运算通过具名函数实现，标注 `@OverflowWrapping`；`==` 依 D29 状态选择路径 | 具名函数中的算术运算不受 D29 影响（比较与算术独立）；若 D29 同时回退，`==` 使用精确比较，`equalExact` 与 `==` 行为等价 |

**交互分析**：两项回退独立正交，无冲突。D29 控制比较语义（容差 vs 精确），D30 控制算术溢出策略（运算符标注 vs 具名函数标注），影响的运算类别不重叠。当两者同时触发时，`==` 使用精确比较（非容差路径），算术运算使用具名函数实现（标注 `@OverflowWrapping`）。此组合在运行时语义上等价于"所有类型使用精确比较 + 算术运算通过具名函数 wrapping"——虽损失容差比较优势和运算符语法糖，但功能正确性不受损。**复杂度成本**：编码阶段需同时实现 D29 的 `equalEpsilon` 独立函数和 D30 的 20 个算术具名函数，合计约 2-3 人天额外工作量（已在 §11.6 评估中覆盖）。**建议**：编码阶段按 D29 → D30 顺序逐项验证，一项失败后再进入下一项的组合验证，降低并行失败的信息熵。
...
### D32. 具名函数（add/sub/mul/div/mod）统一声明为 const 函数

**决策**：`add`、`sub`、`mul`、`div`、`mod` 的两种方向（`vec-op-scalar` 扩展成员函数和 `scalar-op-vec` 包级独立函数）均声明为 `const` 函数。

**理由**：`mod` 的实现使用 `const if (isIec559Of<T>())` 实现整数/浮点双路径编译期分支选择，仓颉要求 `const if` 所在函数必须标注 `const` 修饰符。`add`/`sub`/`mul`/`div` 虽不直接使用 `const if`，但声明为 `const` 有以下益处：① 与 `mod` 的 API 签名一致，降低使用者的认知负担；② 允许在 const 表达式和 const 变量初始化等编译期上下文中调用这些运算函数；③ 为后续轮次引入 const 路径优化预留扩展点。所有五种运算的两种方向共计 40 个函数在所有 `T` 分量类型和 `Q` 精度参数上均合法——所有运算函数均为纯函数（不修改参数），`const` 修饰符不影响运行时行为。

### D33. increment()/decrement()/bitwiseNot() 统一声明为 const 函数

**决策**：`increment()`、`decrement()`、`bitwiseNot()` 均声明为 `const` 函数，与 D32 中 `add`/`sub`/`mul`/`div`/`mod` 的 const 声明保持一致。

**理由**：此三函数与 D32 中的算术具名函数同为纯函数——仅读取 Vec 分量值并返回新 Vec，不修改实例成员。声明为 `const` 有以下考量：① 与 D32 中 40 个算术具名函数的 API 签名一致，消除对称性断裂；② 允许在 const 表达式和 const 变量初始化等编译期上下文中调用这些运算函数；③ `increment()`/`decrement()` 虽标注 `@OverflowWrapping`，但与 `const` 修饰符无冲突——`@OverflowWrapping` 为溢出行为注解，不影响函数的 const 属性；`bitwiseNot()` 不涉及算术运算，无需溢出注解，与 const 修饰符无交互约束。所有三函数在所有 `T` 分量类型和 `Q` 精度参数上，对纯读取+构造新实例的函数体模式均合法，`const` 修饰符不影响运行时行为。
...
|------|--------------------|------|------|---------------|-----------------|
| 13 | `lib.cj`（新增） | `glm.detail.*` | 公共 API 重导出——通过 `public import` 暴露 Vec 类型、Qualifier 类型及 `scalar-op-vec` 方向辅助函数（`add`/`sub`/`mul`/`div`/`mod`） | 8（随向量别名） | 新增 |
...
| `Float16` | `false` | `isIec559Of<Float16>()` 因 `Float16` 不匹配 `Float64`/`Float32` 而隐式返回 `false`，无需显式排除 |
...
**设计阶段验证要求**：`isIec559Of<T>()` 的 `const if (T(0) is Float64 || T(0) is Float32)` 方案依赖三项核心假设，均须在设计阶段（而非编码阶段）完成编译器验证：

| 依赖项 | 描述 | 风险等级 | 验证方法 |
|-------|------|---------|---------|
| ① `T(0)` const 构造 | `T(0)` 在 const 上下文中对所有目标类型（`Int32`、`Float32`、`Bool` 等）合法 | **高** — 若失败则 `const if` 分支前提不成立，整体方案不可行 | 编写原型测试：`const func testT0<T>(): Bool { const if (T(0) is Float64) { true } else { false } }`，分别对 `T = Int32, Float32, Bool` 验证编译通过 |
...
**验证时序**：三项依赖应在设计阶段完成原型验证。若依赖①或②失败 → `const if (T(0) is Float64)` 方案不可行，整体回退到备选路径（所有类型精确比较 `a == b`，§3.5 备选路径）。若依赖①和②通过但依赖③失败 → 浮点容差比较从 `const operator ==` 路径中移除，通过独立的非 `const` 具名函数（如 `equalEpsilon`）提供。**风险标注**：本方案的可行性最终取决于目标编译器对 `const` 泛型上下文中 `is` 运算符和 `T(0)` 构造的支持程度。编码阶段启动前必须完成上述原型验证，验证结果决定本设计的 `const` 路径是否可行。
...
| ④ 整数 Vec `mod` 与浮点 `mod` 分支选择的 `const if` 正确性 | `const if (isIec559Of<T>())` 正确区分整数/浮点路径 | 低 — 与 `ComputeEqual` 分支逻辑一致 | 确认 `isIec559Of<T>()` 对整数类型返回 `false`，对浮点类型返回 `true` |
| ⑤ `mod` 函数的 `const` 声明 | `mod` 在两种方向上均须声明为 `const` 函数（仓颉中 `const if` 要求所在函数为 `const`） | 低 — 语法语义明确 | 编码阶段确认 `const` 修饰符出现在所有 `mod` 函数的签名中；同步确认 `add`/`sub`/`mul`/`div` 也统一声明为 `const`（理由见 §7 D32） |
...
| ② 语义兼容性 | `@OverflowWrapping` 在 `const` 函数体内对算术运算的 wrapping 行为与非 `const` 函数一致 | **中** — 若 `const` 函数路径下编译器对溢出注解的处理策略与非 `const` 路径不同，可能导致 wrapping 行为不一致 | 编写原型测试：在 `const` 函数中执行 `UInt8(255) + UInt8(1)`（标注 `@OverflowWrapping`），期望结果 `0`，与同非 `const` 函数行为对照 |
| ③ `add(v,s)` 扩展成员函数的 `const`+`@OverflowWrapping` 共存 | `vec-op-scalar` 方向扩展成员函数同时标注 `@OverflowWrapping` 和声明为 `const`，在 extend 块中合法 | 低 — 语法结构与依赖①一致 | 与依赖①合并验证 |

**验证建议**：将依赖①和②合并为单一原型测试函数，在 `const` 和 `non-const` 两种上下文中对比 `@OverflowWrapping` 标注的算术运算行为。若验证通过，则 `increment()`/`decrement()` 的 `const`+`@OverflowWrapping` 方案、以及 §4.3 中所有算术具名函数的 `const` 声明均不受此约束影响。若验证不通过（`@OverflowWrapping` 在 `const` 路径下 behavior 异常），则 `increment()`/`decrement()`/算术具名函数降级为非 `const` 版本，同时保持 `@OverflowWrapping` 标注——功能正确性不受损，仅失去编译期调用能力。
...
当前设计假设编译器自动生成的复合赋值版本继承二元运算符上的 `@OverflowWrapping` 标注（§4.6、§7 D30）。此行为依赖编译器的未文档化行为。若验证不通过，需切换到备选方案——通过 extend 块中的具名函数实现 Vec 算术运算。

#### 备选方案变更成本

| 变更项 | 影响范围 | 工作量评估 |
|-------|---------|-----------|
| 新增 Vec 算术具名函数（`add_ow`/`sub_ow`/`mul_ow`/`div_ow`/`mod_ow`，标注 `@OverflowWrapping`） | 各 Vec 的 extend 块，共 5 函数 × 4 Vec = 20 个具名函数 | 中等（20 个函数定义，手动编写约 1-2 人天） |
| 二元运算符委托给具名函数 | 各 Vec 结构体内的 5 个运算符实现，修改委托目标 | 低（5 处修改，机械替换） |
| 复合赋值运算符消除 `@OverflowWrapping` 语义 | 由编译器自动生成，无需修改源码 | 无 |
...
- **若标注继承性不成立**：额外成本约 1-2 人天（编写 20 个具名函数 + 修改 5 个运算符委托实现）。
...
- **`equalExact`**：验证浮点 Vec 上 `equalExact(Infinity)` 返回 `true`；`equalExact(NaN)` 返回 `false`；整数/Bool Vec 上行为与 `==` 等价。
...
- **对 `internal` 类型的访问策略**：需要测试 `glm.detail` 中 `internal` 可见性类型（如 AlignedHighp/AlignedMediump/AlignedLowp）的测试代码，放置在 `tests/glm.detail/` 包中，并显式 `import glm.detail.*`。由于测试代码属于同名包（`glm.detail`），可访问同一包中的 `internal` 成员。Cangjie 的包可见性规则允许同一包内的跨文件访问 `internal` 声明，因此无需将测试目标提升为 `public`。
...
| 审查意见 | 修改措施 |
|---------|---------|
| **严重问题**：Hashable 实现策略不可行。设计声称"整数/Bool 分量 Vec 使用 `@Derive[Hashable]`，浮点分量 Vec 不实现 `Hashable`"，但 `@Derive[Hashable]` 在泛型结构体上对字段类型已实现 `Hashable` 的所有实例化均生效——`Float32`/`Float64` 是仓颉核心类型且已实现 `Hashable`，因此浮点 Vec 也会自动获得 `Hashable` 实现，无法产生所期望的编译错误。仓颉的类型系统不提供"类型参数不为某类型时派生"的条件派生能力 | §3.2 重写 Hashable 实现策略：所有 Vec 类型统一通过 `@Derive[Hashable]` 自动派生，浮点 Vec 的 HashMap 一致性问题记录为已知限制而非设计排除。§4.7 同步更新：移除"浮点 Vec 不实现 Hashable"的声明，改为统一实现 + 已知限制说明。§12.2 验收标准：移除"浮点 Vec 在哈希上下文中产生编译错误"的要求，改为所有 Vec 均可正常用作哈希键 |

## 修订说明（v13）

| 审查意见 | 修改措施 |
|---------|---------|
| **P1（严重）**：256 个别名的命名约定未明确定义。§3.8 列举的别名家族使用全小写无分隔符的 GLM 原生风格，而示例格式使用大写驼峰风格，两套命名风格冲突 | §3.8 新增「命名约定」子节，统一采用大写驼峰（PascalCase）风格，明确四条强制规则：家族名首字母大写、精度前缀使用驼峰拼接（无下划线）、格式模板 `[Precision]FamilyName[Dimension]`、16 个家族的完整映射规则。新增 4 精度变体示例表覆盖完整模式 |
| **P2（高）**：`scalar op vec` 具名函数的定义位置和形态未指定 | §4.3 新增「具名函数的定义位置与形态」子节，明确：① `vec-op-scalar` 方向定义为 Vec 的扩展成员函数（`type_vecN.cj` extend 块）；② `scalar-op-vec` 方向定义为包级独立函数（`compute_vector_decl.cj`）；③ 各函数为 Vec1~Vec4 各定义四个版本；④ 后续轮次可通过宏生成减少重复代码 |
| **P3（高）**：Aligned* Qualifier 类型的可见性策略未明确 | §3.1 新增「Aligned 系列的可见性策略」段落，明确首轮 Aligned 结构体声明为 `internal`（包级可见），并在后续轮次切换 `GLM_CONFIG_ALIGNED_GENTYPES` 为 `true` 时给出可见性变更步骤和 `lib.cj` 导出策略 |
| **P4（中）**：`compute_vector_relational.cj` 的依赖声明包含不必要的 `shim_cstddef.cj` | §2 模块依赖图和 §8.1 文件清单中 `compute_vector_relational` 的依赖从 `shim_cstddef + shim_limits` 修正为仅 `shim_limits` |
| **P5（中）**：`LengthT` 类型别名在 Vec 类型中未被使用 | §3.6 新增「`LengthT` 的使用边界」段落，说明 `LengthT` 是供下游消费者使用的通用长度别名，Vec 内部保持直接使用 `Int64`（编译期常量更清晰、索引范围固定、别名间接引用不必要） |
| **P6（中）**：浮点 Vec 的 IEEE 754 精确相等比较无可用入口 | §4.5 新增 `equalExact(other: VecN<T,Q>): Bool` 具名函数，定义为扩展成员函数（非 `const`），使用仓颉原生 `==` 做精确分量比较。适用于 `Infinity == Infinity` 等需要 IEEE 754 精确语义的场景。`equalExact` 对整数/Bool 分量 Vec 行为与 `==` 等价 |
| **P7（低）**：默认字符串表示的行为约定过于模糊 | §3.2 字符串表示策略修正为精确声明：首轮依赖仓颉编译器为 struct 生成的默认字符串表示，格式和内容由编译器确定，调用方不应依赖特定格式 |
...
| 审查意见 | 修改措施 |
|---------|---------|
| **问题 1（一般）**：D16 与 §4.3 存在隐式矛盾——D16 声明 `compute_vec_*` 策略结构体"首轮仅定义不消费"，但 §4.3 将 `scalar-op-vec` 方向具名函数置于 `compute_vector_decl.cj` 中且被消费。D16 的全文件笼统陈述会使编码人员误判 | §2 模块划分中明确区分"`compute_vec_*` 策略结构体首轮不消费"和"方向辅助函数首轮被消费"两个子句。§7 D16 重写为两类分开陈述，新增例外说明 |
| **问题 2（一般）**：`equalExact` 缺少验收标准 | §12.2 验收标准表新增"equalExact 正确性"验收项 |
| **问题 3（轻微）**：文档版本标识与实际内容不一致（标题为"v10"，修订说明包含至 v13） | 文档标题从"v10"更新为"v13" |
| **问题 4（轻微）**：`const operator ==` 回退方案的触发条件未明确定义 | §7 D29 新增"回退触发条件与验证优先级"子节，明确验证优先级顺序（优先级 1：`T(0)` const 构造，此项失败则无条件回退）和触发回退的阈值 |
...
| 审查意见 | 修改措施 |
|---------|---------|
| **问题 1（中等）**：`equalExact` 函数声明为非 `const` 但无技术理由，且与 `==` 的 const 设计策略不一致。该函数仅执行只读分量比较，不修改实例成员 | §4.5 将 `equalExact` 的声明从"非 `const` 的扩展成员函数"改为"`const` 扩展成员函数" |
| **问题 2（一般）**：`epsilonOf<T>()` 函数在 §3.5 ComputeEqual 代码示例中被引用（`abs(a - b) <= epsilonOf<T>()`），但该函数的签名、所属文件、返回值类型、约束条件在文档任何位置均未显式定义 | §3.5 在 `ComputeEqual` 代码示例之后新增「`epsilonOf<T>()` 定义」子节，给出完整签名（`const func epsilonOf<T>(): T where T <: Number<T>`），说明其定义位置（`shim_limits.cj`）和实现方式（委托 `NumericLimits<T>.epsilon()`） |
| **问题 3（一般）**：`equalExact` 验收标准在 §12.2 验收标准表中已包含，但 §12.1 的四个验证层次中均未显式提及 `equalExact` 的测试用例 | §12.1 层次三新增 `equalExact` 验证子项，覆盖浮点 Vec 的 Infinity/NaN 精确比较和整数/Bool Vec 的等价性验证 |
| **问题 4（轻微）**：`Float16` 的 `isIec559Of` 对照表将 `Float16` 标注为 `false`，附注说明"简化策略，首轮不纳入 `isIec559Of` 判断"，但实际 `isIec559Of<T>()` 实现即使不添加 `Float16` 排除逻辑也会返回 `false` | §10 对照表中 `Float16` 行的附注从"半精度浮点，类型身份检测已排除（简化策略，首轮不纳入 `isIec559Of` 判断）"修改为"非 IEC 60559 浮点类型。`isIec559Of<Float16>()` 隐式返回 `false`（`is` 类型检测不匹配），无需显式排除" |
| **问题 5（轻微）**：文档版本标识与实际增量内容不匹配。文档标题标注为"v13"，但末尾包含完整的 v14 修订说明（4 项修订） | 文档标题从"v13"更新为"v14"；原"修订说明（v14）"修正为"修订说明（v13）" |
...
| 审查意见 | 修改措施 |
|---------|---------|
| **问题 1（严重，上轮未修复）**：§4.1 Vec1 缺失从 Vec2/Vec3/Vec4 的截断构造函数（共3个），Vec2 缺失从 Vec3/Vec4 的截断构造函数（共2个），Vec3 缺失从 Vec4 的截断构造函数（1个）。共6个构造函数缺失。文档声明覆盖 GLSL 5.4.1 规范但实际清单不完整 | §4.1 Vec1 构造函数清单新增 3 个跨分量数截断构造（`init<T2,Q2>(v: Vec2<...>)`、`init<T2,Q2>(v: Vec3<...>)`、`init<T2,Q2>(v: Vec4<...>)`，均取 x 分量）；Vec2 新增 2 个（从 Vec3/Vec4 截断，取 x,y）；Vec3 新增 1 个（从 Vec4 截断，取 x,y,z）。合计补全 6 个截断构造函数 |
| **问题 2（严重，上轮未修复）**：Vec3 缺少 Vec2+Vec1 和 Vec1+Vec2 组合构造 | §4.1 Vec3 构造函数清单在 `Vec1+Vec1+Vec1` 行之后新增 `Vec2+Vec1` 和 `Vec1+Vec2` 两个组合构造函数 |
| **问题 3（一般，上轮未修复）**：`mod` 具名函数首轮可用性状态自相矛盾——§4.3 列于首轮具名函数清单中，却又声明"不属于首轮范围" | §4.3 `%` 运算的整数约束段落修正：将"不属于首轮范围"改为"属于首轮范围"，消除矛盾。`mod` 作为 5 运算之一（add/sub/mul/div/mod）属于首轮 40 个具名函数的必要组成部分 |
| **问题 4（一般，新发现）**：`scalar-op-vec` 方向辅助函数（`add(s,v)`、`sub(s,v)` 等 20 个）未列入 `lib.cj` 公共 API 导出清单，违反封装原则 | §2 `lib.cj` 的 `public import` 列表新增 `public import glm.detail.compute_vector_decl.{ add, sub, mul, div, mod }` 行。§8.1 文件清单中 `lib.cj` 描述同步更新。§8.2 范围可追溯性对照表 `lib.cj` 偏离说明同步更新 |
| **问题 5（轻微，上轮未修复）**：§7 设计决策编号顺序混乱——D28~D31 出现在 D20~D24 之前 | §7 重新排序：将 D20~D24 移至 D19 之后、D28 之前，调整为严格的 D1→D19→D20→D24→D28→D31 编号顺序 |
...
| 审查意见 | 修改措施 |
|---------|---------|
| **问题 1（严重）**：`equalExact` 的 `const` 声明与 extend 块中 `const` 函数的编译器支持性未经验证（§4.5，§3.2） | §4.5 新增 `extend` 块中定义 `const` 函数的完整语法示例（Vec2 位运算符 extend 块 + `equalExact` 的 `const` 声明语法）。标注验证状态——已在仓颉 const 文档中确认（struct 定义了 `const init` 即可定义 `const` 实例成员函数，extend 块规则一致）。若验证失败，降级为非 `const` 函数——功能正确性不受影响。§3.2 `const init` 描述增加对 extend 块中 `const` 函数支持的引用 |
| **问题 2（严重）**：`Inf == Inf` 在容差路径下返 false，与 C++ GLM 行为显著偏离，且迁移成本评估未覆盖（§9.3，§11） | §9.3 Infinity 比较行重写：移除推测性"改进方案"描述，将行为约束合并到行为表格（标明为"已知局限性"），指明补救措施为使用 `equalExact()`。§11 新增 11.5 子节，系统评估行为差异影响、受影响表达式模式、搜索规则和替换模式、迁移工作量（低-中） |
| **问题 3（一般）**：`epsilonOf<T>()` 的 where 约束与 D5 宽松约束在非执行路径上的编译器行为不确定性（§3.5，§7 D5） | §3.5 `epsilonOf<T>()` 定义扩展为兼容性验证段落：增加非执行路径上编译器行为的分析和两种兼容性策略（方案 A：移除 where 约束并内联固定容差；方案 B：添加 `NumericLimits<Bool>` 空壳特化） |
| **问题 4（严重）**：`const if (T(0) is Float64)` 方案的三项核心依赖均未经过编译器验证（§3.5，§10，§7 D29） | §10 的"编码前验证要求"升级为重写的"设计阶段验证要求"，以表格形式列出三项核心依赖（`T(0)` const 构造、`is` 在 const 泛型中的可用性、`abs()`/`epsilon()` const 兼容性），标注每项的风险等级、验证方法和验证时序 |
| **问题 5（严重）**：`@OverflowWrapping` 自动继承假设未经验证（§4.6，§7 D30） | §4.6 和 §7 D30 均增加"此行为依赖编译器的未文档化行为"声明，引用仓颉注解文档的声明范围。备选方案工作量纳入 §11.6 新增的迁移成本评估子节 |
| **问题 6（一般）**：`scalar-op-vec` 方向具名函数的命名空间污染风险未评估（§4.3，§2） | §4.3 具名函数定义位置段落后新增"命名空间污染风险评估"段落：评估风险等级（低-中）、给出三条调用方最佳实践（优先使用扩展成员函数、限定导入、别名导入消除歧义） |
| **问题 7（一般）**：§9.3 "改进方案"段落混入设计文档主体，造成编码人员混淆（§9.3） | §9.3 Infinity 比较行重写：移除"改进方案"的推测性方案描述，将实现约束直接合并到行为表格中，仅保留确定性约束声明 |
| **问题 8（一般）**：D29 验证树过于复杂，将设计不确定性实质性地推至编码阶段（§7 D29） | D29 完全重写：将四级优先级 + 三级回退阈值简化为**二值决策**——编写单一原型测试验证完整表达式链，编译通过则维持当前设计，否则整体回退到精确比较路径。移除细分的逐级回退方案和验证优先级子节 |
| **问题 9（一般）**：256 个别名的首轮生成策略未明确（§3.8） | §3.8 首轮实现策略新增"外部脚本生成策略"段落：推荐使用 Python/Node.js 脚本按命名约定模板枚举 256 种组合，自动生成 `fwd.cj` 的别名代码。给出脚本策略的具体步骤（枚举映射、模板生成、分组输出） |
| **问题 10（一般）**：`add(v,s)` 扩展成员函数与 `v + s` 运算符的功能重叠未明确（§4.3） | §4.3 `%` 运算的整数约束前新增"二元运算符与具名函数的关系及选择指南"段落：说明三种关系（运算符是首选语法糖、扩展成员函数等价但可替换为优化版本、独立函数覆盖运算符无法表达的操作数顺序），给出编码阶段建议 |
| **问题 11（一般）**：§7 D29 的回退路径在文档其他章节中未体现（§7 D29 vs §3.5、§4.5、§9.3） | §3.5 备选路径段落末尾新增"回退路径一致性"标注。§4.5 新增 `==` 回退路径标注子段落，说明回退路径下 `equalExact` 与 `==` 的关系。§9.3 Infinity 比较行新增回退路径影响说明（回退路径下 `Inf == Inf` 返回 `true`，差异自动消除） |
| **问题 12（轻微）**：`extend` 块中定义二元运算符的语法细节不足（§3.2，§4.4） | §4.4 位运算章节开头新增完整的 `extend` 块语法示例（Vec2 的位运算符 extend 块，包含 `<<`/`>>`/`&`/`|`/`^`/`bitwiseNot` 的完整定义），并附注语法扩展规则说明（类型参数声明、可见性、this 访问） |
...
| 审查意见 | 修改措施 |
|---------|---------|
| **问题 1（严重）**：§8.1 与 §8.2 范围追溯编号不一致——§8.1 文件清单中 `setup.cj` 的 "Roadmap §3E 序号" 列为 "S1"，但 §8.2 范围可追溯性对照表中 `setup.hpp` 对应的 "Roadmap §3E 序号" 为 "1" | §8.1 文件清单中 `setup.cj` 的 §3E 序号从 "S1" 修正为 "1"，与 §8.2 对照表中的一致。shim 文件（`shim_assert.cj`/`shim_limits.cj`/`shim_cstddef.cj`）的 S1/S2/S3 编号在两张表中已一致，无需修改 |
| **问题 2（严重）**：D25-D27 设计决策仅存于修订说明，未纳入 §7 主设计决策节 | D25（`VecN<Bool,Q>.bitwiseNot()` 行为差异认定）、D26（跨类型构造函数标量参数使用目标类型）、D27（Qualifier 跨 Q 赋值/转换行为约定）从「新增设计决策（v8）」修订说明中移入 §7，插入 D24 之后、D28 之前。§7 设计决策编号顺序更新为 D1→D31 连续 |
| **问题 3（中等）**：`mod` 具名函数在浮点 Vec 上的实现方案未定义——§4.3 声明 `mod` 可用于浮点取模，但仓颉 `Float32`/`Float64` 不支持 `%` 运算符 | §4.3 补充浮点 `mod` 实现策略（方案 A）：整数 T 使用 `%` 运算符；浮点 T 使用算术恒等式 `x - y * trunc(x / y)`（`trunc` 取自 `std.math`）。`Float32` 分量通过先提升至 `Float64` 计算截断再转回的方式实现。双路径通过 `const if (isIec559Of<T>())` 编译期分支选择 |
| **问题 4（中等）**：Vec1 的 `const init(x: T)` 角色未显式声明为标量填充构造的等效入口 | §4.1 Vec1 构造函数清单中为 `const init(x: T)` 增加注释说明："同时承担标量填充构造角色（等效于其他 VecN 的 `init(scalar: T)`）" |
| **问题 5（一般）**：`isIec559Of<Float16>()` 的说明文字语义模糊——"非 IEC 60559 浮点类型"使人误以为 Float16 本身不符合标准规范 | §10 对照表 `Float16` 行的附注从"非 IEC 60559 浮点类型。`isIec559Of<Float16>()` 隐式返回 `false`（`is` 类型检测不匹配），无需显式排除"改写为"`isIec559Of<Float16>()` 因 `Float16` 不匹配 `Float64`/`Float32` 而隐式返回 `false`，无需显式排除" |
| **问题 6（一般）**：测试组织结构未与包结构对齐——§12.3 未指定测试包与源码包的映射关系，未说明测试代码对 `internal` 类型的访问策略 | §12.3 补充测试包组织结构约定：测试包命名空间与源码包一一对应（`tests/glm.detail/` ↔ `glm.detail`，`tests/glm/` ↔ `glm`）；`internal` 类型通过将测试代码置于同名包中直接访问；公共 API 测试通过 `import glm.*` 遵循下游消费者路径；文件级映射（`test_type_vecN.cj` 对应 `type_vecN.cj` 等） |
...
| 审查意见 | 修改措施 |
|---------|---------|
| **问题 1（严重 / 事实错误）**：`bitwiseNot()` 对 `VecN<Bool, Q>` 的排除策略在仓颉中不可实现——设计声明"若编码阶段需要与 C++ GLM 行为严格一致，应为 `VecN<Bool, Q>` 排除 `bitwiseNot()`"，但仓颉不支持偏特化、不支持负向约束、不存在可用于条件性排除的接口 | §3.2 `VecN<Bool, Q>` 行为约定中移除"若编码阶段需要严格一致，应为 `VecN<Bool, Q>` 排除 `bitwiseNot()`"的不可行建议，改为明确声明此为**不可消除的已知行为差异**，并补充说明不可消除的技术原因（仓颉不支持偏特化或负向约束）。§7 D25 对应更新，理由中增加不可消除性论证。§11.4 迁移规则补充说明：`~bvec` 迁移需要人工审查 |
| **问题 2（中等 / 需求响应充分度）**：PascalCase 别名（`BVec2`、`IVec3` 等）与 C++ GLM 小写名（`bvec2`、`ivec3`）不一致，全局重命名迁移成本在 §11 中未被覆盖 | §11 新增「11.7 别名命名约定偏离的迁移成本」子节，包含受影响别名映射表、机械替换可行性分析（搜索替换不可行，需上下文感知替换）、迁移工具策略（字典映射脚本 + 编译验证）和工作量评估（约 0.5-1 人天） |
| **问题 3（中等 / 关键遗漏）**：仓颉泛型不支持类型参数默认值，所有 Vec 类型须显式写出 Q 参数，此迁移负担在 §11 中未被评估 | §11 新增「11.8 跨 Q 赋值迁移模式」子节，包含需要补全 Q 的场景对照表、可通过别名避免的场景（变量声明、函数返回值、容器类型）、无法通过别名避免的场景（泛型函数、跨精度运算）、搜索替换规则、自动化工具可行性分级和工作量评估（约 2-3 人天） |
| **问题 4（一般 / 事实错误）**：为 `NumericLimits<Bool>` 添加空壳特化的方案（Option B）与 D18 约束冲突（`Bool` 不实现 `Number<T>`），且在仓颉中不支持偏特化 | §3.5 完全移除 Option B（`NumericLimits<Bool>` 空壳特化方案），仅保留 Option A（内联固定容差 `T(1e-6)`）作为唯一的兼容性策略。相关文字从"方案 A（优先）/ 方案 B（备选）"改为"方案 A（唯一策略）" |
| **问题 5（一般 / 关键遗漏）**：未提供 const 版本的 `operator[]` 取值形式，导致 `const` 实例成员函数中无法通过 `[]` 访问分量 | §4.2 新增「const 上下文中的分量访问」段落，说明仓颉不支持通过 `const` 修饰符区分运算符重载版本（const README §3.2 规则 7），提供具名 const 函数 `componentAt(i: Int64): T` 满足 const 上下文中的分量访问。§7 D28 从"`operator[]` 赋值形式须为 `mut` 函数"扩展为"`operator[]` const 取值形式与赋值形式"，同时覆盖 const 取值形式的设计决策 |
| **问题 6（一般 / 深度不足）**：浮点 `mod` 采用算术恒等式 `x - y * trunc(x / y)` 但 `Float32`→`Float64` 提升→截断→转回的精度策略未经验证，边界值行为未覆盖 | §10 末尾新增「浮点 `mod` 实现正确性验证」子节，以表格形式列出 4 项验证依赖——`std.math.trunc` 可用性（低风险）、`Float32` 提升→截断→降级无语义损失（中风险，覆盖 `Float32.Max`、`Float32.MinPositive`、`0.0`、`±Inf`、`NaN` 边界值）、负操作数符号正确性（中风险）、整数/浮点 `const if` 分支正确性（低风险） |
...
| 审查意见 | 修改措施 |
|---------|---------|
| **问题 1（中等）**：`mod` 具名函数在 §4.3 中声明使用 `const if` 实现编译期分支选择，但未标注 `const` 修饰符。仓颉中 `const if` 要求所在函数为 `const` 函数 | §4.3 将 `mod` 的 `vec-op-scalar` 扩展成员函数和 `scalar-op-vec` 包级独立函数均标注为 `const` 函数。同步评估 `add`/`sub`/`mul`/`div` ——虽不直接使用 `const if`，但为 API 一致性、const 上下文可用性和后续扩展，同样声明为 `const`。§4.3 函数清单列表更新为所有五种运算均标注 `（均声明为 `const`）`。§7 新增 D32 设计决策记录此选择理由。§10 浮点 `mod` 验证表新增第 ⑤ 项验证 `const` 声明 |
| **问题 2（一般）**：§3.2 中 `public static func length(): Int64` 返回编译期常量但缺少 `const` 修饰符，无法在编译期上下文中使用 | §3.2 签名修正为 `const public static func length(): Int64` |
...
| **P5（中等）**：`increment()`/`decrement()`/`bitwiseNot()`未被声明为`const`函数，但与已声明`const`的`add()`/`sub()`等同样为纯函数，缺少对称性理由 | §7 新增 D33 设计决策，统一声明三者为 `const` 函数并给出理由（纯函数、API 一致性、const 上下文可用性）。同步更新：§3.2 运算符节、§4.3 `++`/`--` 替代方案节、§4.4 `bitwiseNot()` 描述及其 extend 块示例代码 |
| **P6（中等）**：§3.8定义的大写驼峰别名未评估对新代码开发的长期认知负担——下游使用者在C++ GLM文档（小写名）和仓颉代码（大写驼峰）之间的mental mapping成本 | §3.8 末尾新增"命名差异的长期影响"段落，评估过渡期映射成本（前1-3个月显著后趋于自动化）、三条缓解措施（映射表文档、文件注释、快速参考卡片），结论为首轮可接受 |
| **P7（一般）**：§3.5关于`epsilonOf<T>()`的兼容性策略仍使用"采取以下方案之一"的表述，但仅列出方案A（唯一策略） | §3.5 兼容性策略修正：将"采取以下方案之一"改为"采用以下唯一策略"，移除"方案 A"编号 |
| **P8（一般）**：文档分别验证了@OverflowWrapping的继承性和const路径兼容性，但未建立原型测试验证两个修饰符共存于同一函数（`const func add(...)`上同时标注`@OverflowWrapping`）的场景 | §10 新增"@OverflowWrapping 与 const 函数共存验证"子节，以表格列出三项验证依赖（语法共存性、语义兼容性、extend 块共存性）及验证建议（合并为单一原型测试）。若验证不通过，相关函数降级为非 const 版本 |
| **P9（一般）**：D32设计决策未纳入§7正文 | 同 P2，已在原文中确认 D32 纳入 |
