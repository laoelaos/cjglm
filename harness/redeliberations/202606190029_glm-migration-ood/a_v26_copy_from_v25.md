# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v26 — 审查迭代修订版）

> **版本变更摘要**：v26 基于 v25 迭代，根据 v26 审查反馈进行修订：修复 §11.12 §11.5 合计行添加脚注标注与 §11.9 非累加关系（F1）、在 §4 新增 §4.8 `fromBoolVec` 完整签名清单并更新 §9.4/§12.1/§12.2 的交叉引用与验证覆盖（F2）、在 §2 初始化步骤中新增 `scripts/` 目录创建指令（F3）、在 §7 D6 中补充 roadmap 值 `0x02` 错误的根因推测（F4）、在 §2.1 清单顶部新增 D29/D30 验证时序建议注释（F5）。详见末尾修订说明。
...
|---------|---------|---------|---------|---------|
| ① 首次编码前原型验证 | §2 步骤 7 | 编写原型测试验证 `const` 表达式链、`T(0)` const 构造、`is` 在 const 泛型中的可用性、`@OverflowWrapping` 继承性 | 所有原型测试编译通过 | 根据失败项调整设计（详见对应章节的回退方案） |
...
**签名清单**（Vec1~Vec4，定义于各 `type_vecN.cj` 的 extend 块中）：

```
// Vec1<T, Q> 扩展成员函数
extend<T, Q> Vec1<T, Q> where Q <: Qualifier {
    public func fromBoolVec(v: Vec1<Bool, Q>): Vec1<T, Q> {
...
// Vec2<T, Q> 扩展成员函数
extend<T, Q> Vec2<T, Q> where Q <: Qualifier {
    public func fromBoolVec(v: Vec2<Bool, Q>): Vec2<T, Q> {
        Vec2(if (v.x) { T(1) } else { T(0) }, if (v.y) { T(1) } else { T(0) })
    }
}

// Vec3<T, Q> 扩展成员函数
extend<T, Q> Vec3<T, Q> where Q <: Qualifier {
    public func fromBoolVec(v: Vec3<Bool, Q>): Vec3<T, Q> {
        Vec3(if (v.x) { T(1) } else { T(0) }, if (v.y) { T(1) } else { T(0) }, if (v.z) { T(1) } else { T(0) })
    }
}

// Vec4<T, Q> 扩展成员函数
extend<T, Q> Vec4<T, Q> where Q <: Qualifier {
    public func fromBoolVec(v: Vec4<Bool, Q>): Vec4<T, Q> {
...
- 此函数定义在 `extend` 块中，受仓颉扩展语法限制声明为非 `const` 函数。
- `fromBoolVec` 为所有 VecN 类型（Vec1~Vec4）均适用，Vec1 的构造函数不对称性不影响其定义（`fromBoolVec` 为具名函数，参数列表与任何现有构造函数无重载冲突）。
- 调用方使用 `v.fromBoolVec()` 语法，其中 `v` 为 `VecN<Bool, Q>` 类型。
...
- **`fromBoolVec` 构造验证**：对 Vec1~Vec4 各调用一次 `fromBoolVec`，验证 `VecN<Bool, PackedHighp>` 到 `VecN<Int32, PackedHighp>` 的转换结果正确性（`true`→`T(1)`，`false`→`T(0)`）。
...
- **`fromBoolVec` 正确性验证**：验证 `fromBoolVec` 在 Vec1~Vec4 各分量数上的转换正确性；验证多精度变体（`PackedHighp`/`PackedMediump`/`PackedLowp`）均编译通过；验证 `fromBoolVec` 返回值参与算术运算的正确性。
...
| `fromBoolVec` 正确性 | Vec1~Vec4 的 `fromBoolVec` 工厂函数对所有数值类型（`Int32`/`Float32` 等）正确转换：布尔 `true`→`T(1)`、`false`→`T(0)`；多精度变体均编译通过 | 层次二构造验证 + 层次三运算验证；对各分量数 Vec 至少验证一个数值目标类型 |
...
| 审查意见 | 修改措施 |
|---------|---------|
| **F1（中等）** — §11.12 合计评估表中 §11.5 与 §11.9 存在重复计算风险：§11.5 声明"与 §11.9 非累加"，但合计表中仍作为独立成本项分别计入 | 在 §11.12 合计表 §11.5 行末尾添加脚注标记 `[^1]`，在表后"说明"段落首行新增脚注内容："§11.5 与 §11.9 非累加关系，实际以 §11.9 为准" |
| **F2（一般）** — `fromBoolVec` 工厂函数仅在 §9.4 以行内代码片段呈现，未在 §4 关键行为契约中正式定义，也未在 §12 获得验证/验收覆盖 | ① 在 §4.7 末尾新增 §4.8 子节，给出 `fromBoolVec` 的 Vec1~Vec4 完整签名清单和行为约定；② 将 §9.4 路径 A 中行内详述改为交叉引用指向 §4.8；③ 在 §12.1 层次二新增"`fromBoolVec` 构造验证"项、层次三新增"`fromBoolVec` 正确性验证"项；④ 在 §12.2 验收标准表末新增"`fromBoolVec` 正确性"验收项 |
| **F3（提示）** — §2 初始化步骤无任何一步提及创建 `scripts/` 目录 | 在 §2 步骤 4（初始编译验证）前插入新的步骤 4："创建辅助脚本目录"，说明创建 `scripts/` 目录用于存放别名生成脚本；后续步骤编号顺延（原 4→5、5→6、6→7） |
| **F4（提示）** — §7 D6 中文档指出 roadmap 值 `0x02` 有误，但未分析根因 | 在 §7 D6 `GLM_CONFIG_CLIP_CONTROL` 行末的"roadmap 值 `0x02` 有误"之后补充根因推测："Roadmap 值 `0x02` 可能遗漏了 `RH_BIT(8)` 组合位，仅截取了 `NO_BIT(2)` 的低位部分" |
| **F5（提示）** — §2.1 编码前检查清单中 D29（第①项）与 D30（第⑤项）分散排列，可能被按编号顺序执行而弱化回退顺序关联性 | 在 §2.1 清单表格上方新增"验证时序建议"注释行："建议优先执行第①项（对应 §7 D29 const 表达式链验证）和第⑤项（对应 §7 D30 @OverflowWrapping 继承性验证），并参考 §7 D29/D30 组合回退场景分析评估两者的交互影响" |
