...
> **版本变更摘要**：v13 基于 v12 迭代，根据 v13 审查反馈（P1-P6）进行针对性修复：修复 §10 全文 `const if` 术语残留（P1）、修正 §12.3 测试目录路径 `tests/glm.detail/` 为 `tests/glm/detail/`（P2）、新增 §2.1 编码前验证检查清单（P3）、移除非首轮验证类型 Float16（P4）、软化 §4.3 mod 浮点精度假设为需验证断言（P5）、为所有 `const init` 构造函数补充 `public` 修饰符并在 §7 D11 声明公共性（P6）。详见末尾修订说明。
