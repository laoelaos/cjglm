# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v30 — 基于 v29 审查基线迭代）

> **版本声明**：本文档版本号为 **v30**，基于 v29 审查意见迭代。文件名标注为 `a_v30_copy_from_v29.md`。

> **版本变更摘要**：v30 基于 v29 迭代，根据 v30 审查反馈进行修订：修正 const 真空缓解方案逻辑矛盾（发现 1）、修复修订说明版本序列 v28 缺失（发现 2）、调整 `protected` 回退方案层级顺序（发现 3）、补充完整 `cjpm.toml` 测试配置（发现 4）、强化 `equalEpsilon`-`epsilonOf` 交叉引用（发现 5）、细化 `std.math` 依赖标注（发现 6）、补充别名编译时间考量（发现 7）、补充 §8.4 验证失败回退步骤（发现 8）、补充 `fromBoolVec` T 参数范围说明与别名快速选择指引（发现 9）、补充 `mod` 运算说明及 `@OverflowWrapping` 迁移注意事项（发现 10）、拆分阶段 5 为 5a/5b 并补充 Vec2 运算符语法模板（发现 11）。详见末尾修订说明。
...
| 审查意见 | 修改措施 |
|---------|---------|
| **发现 1（中等）** — const 真空缓解方案存在逻辑矛盾：§4.5 方案①（将 `equalEpsilon` 从 extend 块移至 struct 体内）与 D29 回退触发条件根本矛盾——函数体不变（`abs(a-b) <= epsilonOf<T>()`），D29 已验证此表达式链在 const 上下文中编译失败，因此方案①必然同样失败。§11.14 成本评估（0.5 人天）基于不可行前提，需重新估计 | 在 §4.5 const 真空警示中将方案①从缓解方案修正为"理论路径"并标注前提条件（仅当 D29 回退由 extend 语法限制单独导致而非表达式链不可用时可行）；可行性评级降级为"极低"；补充路径②（非 const 上下文中使用 `equalEpsilon`，const 上下文中使用精确比较 `==`）作为实际可行推荐；同步修正 §11.14，将方案①成本标注为理论值并声明前提条件，路径②标注为 0 人天 |
| **发现 2（中等）** — 修订说明版本序列存在跳变：v29 → v27 缺少 v28 的修订说明。新读者无法追溯 v27→v28 间的完整变更 | 在本附录中新增"版本 v28"条目标注为确认性审查——v27→v28 为内部审查确认轮次，无实质设计内容变更，修订说明为空 |
| **发现 3（一般）** — `protected` 回退方案可能无法跨模块工作：文档将 `protected` 作为 `internal` 测试访问回退的第 1 优先级方案，但仓颉规范中 `protected` 可见域为当前模块，在 `cjpm test` 独立编译场景下 `protected` 不足以跨模块访问 | 在 §12.3 回退方案层级表中标注 `protected` 方案的前提条件（需验证测试包与生产包是否同模块）；调整回退层级顺序，将同包测试直接访问（即当前首选方案）与 `protected` 方案之间新增判断条件说明 |
| **发现 4（一般）** — `cjpm.toml` 缺少测试运行器配置：文档提供最小 `cjpm.toml` 骨架但未提供完整、可直接拷贝使用的配置 | 在 §2 项目初始化与构建基础设施中补充完整 `cjpm.toml` 配置示例（含测试目录 `src-dir` 和 `tests` 目录设置）；在 §10 新增验证项确认 `cjpm test` 可正确发现 `@Test` 标注用例 |
| **发现 5（一般）** — `equalEpsilon` 在整数 Vec 上的编译通过性断言可能不成立：`equalEpsilon` 函数体包含 `epsilonOf<T>()` 调用，其签名为 `const func epsilonOf<T>(): T where T <: Number<T>`。`Int32` 满足 `Number<Int32>` 约束，但 `epsilonOf<T>()` 的约束与 D5 宽松策略间的兼容性需验证 | 在 §4.5 `equalEpsilon` 代码示例注释中显式指向 §3.5 兼容性策略路径 A/B；在 §10 补充验证步骤明确 `equalEpsilon` 验证与 §3.5 双路径策略的联动关系 |
| **发现 6（提示）** — 依赖拓扑中 `scalar_vec_ops.cj` 对 `std.math` 的依赖声明不完整：依赖表将 `std.math` 标注为整个 `scalar_vec_ops` 模块的依赖，实际上仅 `mod` 的浮点路径依赖它 | 将 §2 模块依赖表中 `scalar_vec_ops` 行的依赖标注修改为 `+ std.math（仅 mod 浮点路径）`；同步更新 §8.1 文件清单中 `scalar_vec_ops.cj` 的依赖说明 |
| **发现 7（提示）** — 256 个别名的编译时间影响未在架构评估中考虑：别名定义阶段影响低，但别名使用阶段导致 256 种 Vec 具现化组合，在泛型函数实例化时可能影响编译时间 | 在 §3.8 末尾新增编译时间考量说明子节，区分别名定义阶段和别名使用阶段的影响 |
| **发现 8（一般）** — 需求响应充分度维度评估确认：设计全量覆盖 roadmap 范围，偏离说明完备，参考实现策略详尽。建议在 §8.4 策略 0 中补充验证失败的具体回退操作步骤 | 在 §8.4 策略 0 末尾补充验证失败回退步骤——三项验证项各自的失败回退路径及文档更新流程 |
| **发现 9（中等/新增）** — 接口定义充分性评估：公共 API 完备性良好；别名缺少快速选择指引；`fromBoolVec` 的 `T` 参数范围未在接口层面说明；API 迁移对标完整 | 在 §4.8 补充 `fromBoolVec` 的 `T` 参数类型范围说明（所有数值类型 + `Bool`）；在 §3.8 别名实用性分级表后新增消费者快速选择指引段落 |
| **发现 10（一般/新增）** — 异常场景与边界条件系统性评估：现有覆盖完善（8 场景组 20+ 约定），§9→§12 映射完整。缺口 A：`mod` 的 NaN/Infinity 边界未在 §9.3 显式声明；缺口 B：有符号整数 `@OverflowWrapping` 与 C++ UB 的迁移注意事项缺少说明 | 在 §9.3 新增 `mod` 运算的 NaN/Infinity 边界说明行；在 §5.1 补充有符号整数 `@OverflowWrapping` 与 C++ UB 迁移注意事项 |
| **发现 11（一般/新增）** — 编码直接指导性评估：实施顺序阶段 5 粒度较粗；运算符签名缺少完整语法模板；`equalEpsilon` 跨章节依赖缺少集中参考框；§10→§12 交叉引用应补充 | 拆分 §8.3 迁移顺序阶段 5 为阶段 5（Vec 类型）+ 阶段 5b（Bool→Numeric 转换）；在 §4.3 补充 Vec2 算术运算符完整语法模板；在 §4.5 `equalEpsilon` 定义前新增集中参考框；在 §10 `equalEpsilon` 依赖④中补充 §12.2 验收项引用 |

### 版本 v28 — 确认性审查轮次

> **确认说明**：v28 为内部确认性审查轮次，基于 v27 审查基线进行一致性确认，无实质设计内容变更。v27→v28 间的变更已在 v27 修订说明中完整覆盖，本条目仅用于保持修订说明版本序列的连续性。

| 审查意见 | 修改措施 |
|---------|---------|
| **确认性审查** — 版本标识、术语清理、cross-reference 一致性检查 | 无实质设计内容变更。所有确认项均通过，无需修改 |
