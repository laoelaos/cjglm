# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v25 — 审查迭代修订版）

> **版本变更摘要**：v25 基于 v24 迭代，根据 v25 审查反馈进行修订：修复 §3.8 脚本模板中 `Vec`/`DVec`/`FVec` 三家族的命名错误（C1）、新增 §1 范围可追溯性中 roadmap §3E+§3G 对应说明（A5）、在 §3.2 末尾新增 const 上下文 API 可用性速查表（B2）及 `VecN<Bool, Q>` 模运算不可用性声明行（C2）、在 §2.1 新增编译器依赖统一验证测试（B3）、在 §8.4 策略 0 中补充参考实现文件路径/行号范围（A2）及被排除 `#define`/`#if` 块抽查（B4）、在 §11 末尾新增合计评估（A1）、编译器依赖验证回退成本（B3）及 const/extend 审查工作量（B2）、在 §附录首段新增跨文件依赖声明（A3）。详见末尾修订说明。
...
| ⑧ Bool→Numeric 转换路径 B `as` 方案可行性验证 | §9.4/§10 | 按 §10 `Bool→Numeric` 转换验证子节的方法，编写 3 项原型测试验证 `as` 运算符、`if-let` 模式匹配、`T(1)`/`T(0)` 构造器的可用性 | 三项均编译通过 | 路径 B 标记为"不可行"，回退至路径 A（`fromBoolVec` 具名工厂函数） |
| ⑨ `internal` 类型测试访问策略验证 | §12.3 | 在 `tests/glm/detail/` 目录下创建测试文件（声明 `package glm.detail`），尝试实例化 `AlignedHighp` 类型 | 编译通过（同包 `internal` 可访问） | 按 §12.3 回退方案层级逐级尝试：① `protected` 可见性 → ② `public` 可见性 → ③ 测试 helper 文件 |
| ⑩ 别名对外可见性验证 | §3.8/§12.2 | 从独立测试包中通过 `import glm.BVec2` 访问别名并实例化，验证编译通过 | 编译通过 | 检查 `fwd.cj` 中所有 `type` 别名是否添加 `public` 修饰符；确认 `fwd.cj` 声明 `package glm` 且其 `public type` 别名通过 `import glm.*` 直接可达 |
| ⑪ 生成脚本映射表正确性验证 | §3.8 | 抽查 4 个家族的别名命名是否符合 §3.8 命名约定（如 B/BVec、I/IVec、U/UVec、Vec 各抽查一个精度变体）；**重点验证 `Vec`/`DVec`/`FVec` 三家族不产生 `VecVec`/`DVecVec`/`FVecVec` 等错误名称**；确认脚本生成结果与人工审查一致 | 抽查项全部匹配，`Vec`/`DVec`/`FVec` 三家族命名正确 | 修正脚本映射表中的错误映射后重新生成并验证；若 `Vec`/`DVec`/`FVec` 命名错误，检查脚本条件格式 `'' if fam.endswith('Vec') else 'Vec'` 是否正确实现 |
| ⑫ 编译器依赖统一验证测试 | §10 依赖点 A~E、§7 D29/D30 | 编写统一原型程序 `testConstIfSuppression<T>()`，以 `Float32`/`Int32`/`Bool` 分别实例化，验证编译期 `if` 非选择分支抑制机制对所有 5 个依赖点成立；同时编写 `@OverflowWrapping` 继承性原型测试 `testOverflowWrappingInheritance()` 验证自动生成复合赋值是否继承标注语义；编写 `@OverflowWrapping`+`const` 语法共存性原型测试 | 所有原型测试编译通过，运行时溢出行为符合 wrapping 预期 | 各依赖点按独立回退方案处理（详见 §10 回退表及 §11.13 回退成本评估）；若编译器行为假设不成立，对应设计章节按回退方案修改 |
...
```python
#!/usr/bin/env python3
"""gen_fwd_aliases.py — Generate 256 type aliases for fwd.cj"""
...
def gen_aliases():
    ...
    for fam, typ in FAMILIES.items():
        ...
        for dim in DIMS:
            for prec, qual in PRECISIONS:
                # 注意：Vec/DVec/FVec 家族名已包含 "Vec"，不追加后缀；其他家族追加 "Vec"
...
if __name__ == '__main__':
    with open('src/glm/fwd.cj', 'w') as f:
        f.write(gen_aliases())
    print('Generated src/glm/fwd.cj with 256 aliases')
```

**脚本定制指南**：
- 若需要为每个家族添加精度前缀别名（`highp_`/`mediump_`/`lowp_`），将 `PRECISIONS` 中的 `Highp`/`Mediump`/`Lowp` 替换为 `highp_`/`mediump_`/`lowp_`（注意需同时调整命名约定以符合 §3.8 的 PascalCase 风格要求）。
- 若需要按"必备→常用→可选"分级输出，可在 `FAMILIES` 中先声明必备家族（B/I/U/Vec），再声明常用家族（DVec/I8/U8），最后声明可选家族（其余）。
...
断言行为：Debug/Release 模式下均保留断言检查（首轮简化策略）。后续轮次可通过 `const` 配置 + 编译期 `if` 在 Release 模式下跳过。
...
0. **前置验证**（编码启动前必须完成）：确认参考实现目录结构完整，关键路径的实现逻辑与设计假设一致。具体验证项包括：① 确认 `references/glm-1.0.3/glm/glm/detail/setup.hpp` 中 `GLM_CONFIG_CLIP_CONTROL` 的条件编译逻辑与 §7 D6 描述的决策分支一致（第 450~570 行的三层条件编译：`GLM_FORCE_DEPTH_ZERO_TO_ONE` 设置 `ZO_BIT`/`NO_BIT`，`GLM_FORCE_LEFT_HANDED` 设置 `LH_BIT`/`RH_BIT`，无额外定义时最终值为 `0x0A`）；② 确认 `references/glm-1.0.3/glm/glm/detail/qualifier.hpp` 第 50~80 行中 `storage` 模板的非 SIMD 路径仅使用 `T data[L]` 数组（与 D23 省略理由一致）；③ 确认 `references/glm-1.0.3/glm/glm/detail/type_vec2.inl` 第 10~40 行及 `type_vec4.inl` 第 10~50 行中构造函数宏展开后的参数列表与 §4.1 清单匹配（抽查 Vec2/Vec4 各一个）；④ 抽查 3~5 处首轮未迁移的 `#define`/`#if` 条件编译块（如 `setup.hpp` 中 `GLM_ARCH_DETECTED` 检测宏第 80~160 行、`GLM_COMPILER` 检测宏第 200~320 行、`GLM_PLATFORM` 检测宏第 350~440 行），确认首轮其他文件对这些被排除块无间接引用。若参考实现中的条件分支逻辑与当前设计描述不符，**以参考实现为准并更新 §7 D6 及对应设计章节**，同时记录于设计变更日志。
...
| `Bool` → 数值 | 跨 Vec 转换构造中 `Bool` 不可直接转换为数值类型（仓颉中 `Int32(Bool)` 不被支持）。泛型构造函数 `public init<T2, Q2>(v: VecN<T2, Q2>)` 不受限于 T2 的具体类型——当 `T2 = Bool` 且 `T` 为数值类型时，构造函数体内的 `T(v.x)` 尝试将 `Bool` 转换为数值，将产生编译错误。**解决方案路径**（按可行性优先级排序）：<br><br>**路径 A（推荐，无编译依赖）**：不尝试在单个泛型构造函数中处理所有 T2→T 组合。接受 Bool→Numeric 在通用 `init<T2, Q2>` 中产生编译错误的固有限制（D5 延迟检查语义）。为 `VecN<Bool, Q2>`→`VecN<数值类型, Q>` 转换场景，提供**具名工厂函数**（如 `fromBoolVec(v: VecN<Bool, Q2>): VecN<T, Q>`），在函数体内显式处理：对 Vec1 使用 `if (v.x) { T(1) } else { T(0) }`（Cangjie `if` 为表达式），对 Vec2~Vec4 逐分量 `(if (x) { T(1) } else { T(0) }, if (y) { T(1) } else { T(0) }, ...)`。此路径完全规避编译期分支抑制依赖，对所有分量数（Vec1~Vec4）适用模式一致。<br><br>**Vec1 特殊说明**：Vec1 的 `fromBoolVec` 扩展成员函数定义为 `extend<T, Q> Vec1<T, Q> where Q <: Qualifier { public func fromBoolVec(v: Vec1<Bool, Q>): Vec1<T, Q> { Vec1(if (v.x) { T(1) } else { T(0) }) } }`。Vec1 的构造函数不对称性（§4.1：仅 `const init(x: T)` 无 `public init(scalar: T)`）**不影响** `fromBoolVec` 工厂函数——`fromBoolVec` 为具名函数（非 `init`），参数列表 `(v: Vec1<Bool, Q>)` 与任何现有构造函数均无重载冲突。Vec1 版本在 extend 块中的定义模式与其他 VecN（Vec2~Vec4）一致，仅分量数量不同。<br><br>**路径 B（待验证，若编译器支持）**：在非 `const` `init` 体内通过 `as` 运算符做运行时类型检测并分派——`let bOpt = v.x as Bool; if (let Some(b) <- bOpt) { this.x = if (b) { T(1) } else { T(0) } } else { this.x = T(v.x) }`。此方案的可行性依赖以下假设（均须 §10 编码前验证）：① `v.x as Bool` 对任意泛型 `T2` 编译通过（`as` 运算符始终可用）；② `if (let Some(b) <- bOpt)` 模式匹配在非 `const` 函数中编译通过；③ `T(1)`/`T(0)` 作为常量构造器对目标类型 `T` 合法。若三项验证均通过，路径 B 可无缝嵌入现有 `init<T2, Q2>` 签名，无需新增工厂函数。<br><br>**路径 C（备选）**：在 `const init` 体内利用编译期 `if` 的分支抑制行为——`if (isIec559Of<T2>()) { ... } else if (/* 检测 Bool ... */) { ... } else { ... }`。但此路径需要在 `const` 上下文中对 `T2` 做类型检测，而编译期 `if` 在 `const` 函数体中对泛型参数的检测能力受限于编译期 `is` 表达式的可用性。可行性较低，作为理论备选方案记录。<br><br>**首轮推荐**：采用路径 A（具名工厂函数），理由：零编译器行为依赖，所有 VecN 分量数（Vec1~Vec4）适用模式一致。若编码阶段路径 B 验证通过，可作为可选优化路径（将运行时检测逻辑内联到 `init<T2, Q2>` 体内，减少工厂函数调用开销）。两路径的具体模式及 Vec2~Vec4 的逐分量检测方式见下文"Vec2~Vec4 Bool 检测模式"。<br><br>**Vec2~Vec4 Bool 检测模式**：路径 B 的 `as` 检测模式对 Vec2~Vec4 同样适用，但需逐分量处理——每个分量独立执行 `as Bool` 检测。Vec2 示例：`let b0 = v.x as Bool; let b1 = v.y as Bool; if (let Some(b0v) <- b0, let Some(b1v) <- b1) { /* Bool 路径 */ } else { /* 数值路径 */ }`。路径 A 的 `fromBoolVec` 工厂函数同理逐分量处理：`Vec2(if (v.x) { T(1) } else { T(0) }, if (v.y) { T(1) } else { T(0) })`。注意：若采用路径 A，`fromBoolVec` 的命名应避免与 `init` 重载冲突——工厂函数名为 `fromBoolVec`（如 `Vec2<Int32, PackedHighp>.fromBoolVec(src: Vec2<Bool, PackedHighp>)`），在各 VecN 的 `extend` 块中定义。<br><br>**§10 验证依赖**：路径 B 的三项假设均纳入 §10 设计阶段验证清单——验证项编号见 §10 `Bool→Numeric` 转换验证子节。若验证失败，路径 B 标记为"不可行"，首轮编码仅采用路径 A。同时从 §9.4 中移除"方案 B 为首选推荐路径"的确定性表述，转为中性优先级描述。 |
...
| ④ `epsilonOf<T>()` 回退路径与 `equalEpsilon` 的隐式依赖链 | §3.5 的兼容性回退策略已明确 `epsilonOf<T>()` 的两种生命周期路径：**路径 A（保留 `epsilonOf<T>`）**——移除 `Number<T>` 约束，实现返回固定容差 `T(1e-6)`，`equalEpsilon` 无需联动修改；**路径 B（完全移除 `epsilonOf<T>`）**——`equalEpsilon` 的 `epsilonOf<T>()` 引用须同步替换为内联固定容差 `T(1e-6)`。此依赖链已在 §3.5 中显式归档为两条独立路径，联动修改要求已标注 | **中等** — 依赖链跨两处设计决策（§3.5 兼容性策略和 §4.5 equalEpsilon 实现），单独修改任一处的实施者可能遗漏另一处的同步 | 编码阶段验证路径：① **路径 A**——若 `epsilonOf<T>()` 保留（无 `Number<T>` 约束，返回 `T(1e-6)`），`equalEpsilon` 体内继续引用 `epsilonOf<T>()`，验证 `epsilonOf<T>()` 对所有目标 `T`（含 `Bool`）编译通过；② **路径 B**——若 `epsilonOf<T>()` 完全移除，`equalEpsilon` 体内的 `epsilonOf<T>()` 引用须替换为内联固定容差 `T(1e-6)`，与 `ComputeEqual.callConst` 的回退路径保持一致。路径 B 的联动修改在 §12.1 层次三 `equalEpsilon` 验证项中已标注交叉引用注解 |
...
| §11.5 `Inf == Inf` 容差路径差异 | 0（回退路径下为 0） | 0.5 | 1.0 |
...
| **合计验证成本** | **1.2 人天** | **3.5 人天（若全部回退）** |

**最优路径**（~1.2 人天）：所有验证通过，回退成本为 0。
**最可能路径**（~2.5 人天）：1~2 项回退触发（通常是 D30 继承性或依赖③的分支抑制验证），其余通过。
**最差路径**（~4.7 人天）：多项验证同时回退（多项编译器行为假设不成立）。
...
- **`equalEpsilon`**：验证 `equalEpsilon` 在 `Float32`/`Float64` Vec 上编译通过且容差比较行为正确（构造微差分量，验证 epsilon 范围内相等）；验证 `equalEpsilon` 在 `Int32`/`Int64` Vec 上编译通过且行为等价于精确比较（`-`/`abs` 对整数类型合法）；验证 `equalEpsilon` 在 `Bool` Vec 上实例化时产生编译错误（`-` 对 `Bool` 不可用）。验证 `abs()` 在 `extend` 块中的可用性（与 §4.5 设计假设一致）。**交叉引用注解**：当 `epsilonOf<T>()` 的回退路径（§3.5 兼容性策略：移除 `Number<T>` 约束、内联固定容差 `T(1e-6)`）被触发后，`equalEpsilon` 的实现需同步调整——将 `epsilonOf<T>()` 引用替换为内联固定容差 `T(1e-6)`，与 `ComputeEqual.callConst` 的回退路径保持一致。此联动修改已在 §10 `equalEpsilon` 设计阶段验证依赖④中归档。
...
| 优先级 | 方案 | 变更范围 | 影响评估 |
|-------|------|---------|---------|
| **1st** | 将测试目标类型的可见性从 `internal` 提升为 **`protected`** | 仅修改 `qualifier.cj` 中 AlignedHighp/AlignedMediump/AlignedLowp 的可见性声明（`internal` → `protected`）；无需更新 `lib.cj` 导出列表。`protected` 在当前仓颉版本中仅允许同包和子包访问，比 `public` 更严格 | 不影响公共 API 面（`lib.cj` 不导出 `protected` 类型）；仅包内和子包可访问；对下游消费者无影响。**注意**：此方案的可行性依赖仓颉编译器对 `protected` 可见性跨包访问的支持程度，编码阶段需验证 |
| **2nd** | 将测试目标类型的可见性提升为 **`public`** | 修改 `qualifier.cj` 中 AlignedHighp/AlignedMediump/AlignedLowp 的可见性声明（`internal` → `public`）；需同步更新 `lib.cj` 的 `public import` 列表，新增 Aligned 类型导出行；需在现有 §12.2 验收标准中新增"Aligned 类型不意外暴露"的反向验证项 | **影响最大**——将三组本不应在首轮暴露的对齐类型变为 `public`，可能被下游错误使用（虽然从类型系统层面 Qualifier 接口限制可减少误用，但不能完全消除） |
| **3rd** | 创建测试 helper 文件，在 `glm.detail` 包内提供 `internal` 类型的测试访问包装函数 | 在 `glm.detail` 包中新增一个 `internal` 可见性的 helper 文件（如 `detail_test_helpers.cj`），定义返回 `AlignedHighp` 等类型的工厂函数（`package glm.detail`，`internal` 可见性）；测试文件同包调用这些工厂函数 | 不修改现有类型可见性；额外引入一个仅供测试使用的 helper 文件，需在 `cjpm.toml` 或构建脚本中确保 Release 构建排除该文件（避免生产二进制中包含测试辅助代码） |

**回退方案选择条件**：
1. 优先尝试 `protected` 方案——在编码前验证阶段确认仓颉编译器对 `protected` 的支持后采用。若编译通过，这是侵入性最低的方案。
2. 若 `protected` 不兼容（编译器不支持跨包 `protected` 访问），评估 `public` 方案——接受 Aligned 类型在首轮暴露，通过文档标注"首轮仅供内部测试使用，正常消费者应使用 Packed 系列和 Defaultp"。
3. 若 `public` 方案因范围评审拒绝（不可接受 `internal` 类型泄露），采用 helper 文件方案，需额外处理 Release 构建排除逻辑。
...
> **跨文件依赖声明**：本附录引用了外部文件 `a_v18_revision_history.md`（历次修订记录 v2~v17）。获取本设计文档时须同时获取该文件以保持修订历史的完整性。依赖路径：相对于本文件所在目录的 `./a_v18_revision_history.md`。

### 版本 v25 — 当前修订说明

| 审查意见 | 修改措施 |
|---------|---------|
| **C1（严重）** — 脚本模板命名错误：`gen_fwd_aliases.py` 中 `f'{prec}{fam}Vec{dim}'` 格式对 `Vec`/`DVec`/`FVec` 三家族（键名已包含 "Vec"）生成错误名称 | 修复 §3.8 脚本模板：将无条件 `f'{prec}{fam}Vec{dim}'` 改为条件格式——`fam` 以 "Vec" 结尾时使用 `f'{prec}{fam}{dim}'`，其他家族保持 `f'{prec}{fam}Vec{dim}'`；在脚本注释中标注条件逻辑原因；更新 §2.1 验证项⑪的校验正则以覆盖所有 16 家族 |
| **A1（中等）** — §11 迁移成本评估包含 11 个子节但缺少汇总合计 | 在 §11 末尾新增 §11.12 合计评估子节，按乐观（4.9 人天）/最可能（10.55 人天）/悲观（19.3 人天）三档汇总，并附各子节成本明细表 |
| **B2（中等）** — const/extend 不对称的消费者体验影响未充分评估（11 个日常操作在 const 上下文中不可用） | 在 §3.2 末尾新增"const 上下文中的 API 可用性速查表"（覆盖 add/sub/mul/div/mod/equalExact/logicalAnd/logicalOr/increment/decrement/bitwiseNot 共 11 个 API）；在 §11.12 合计评估表中新增 B2 审查工作量行（0.3~1.0 人天） |
| **A2（一般）** — §7 D6 等处声明 C++ GLM 参考实现行为但未给出具体文件行号 | 在 §8.4 策略 0 前置验证项中补充参考实现文件路径和行号范围：`setup.hpp` 第 450~570 行（GLM_CONFIG_CLIP_CONTROL 条件编译）、`qualifier.hpp` 第 50~80 行（storage 模板非 SIMD 路径）、`type_vec2.inl` 第 10~40 行及 `type_vec4.inl` 第 10~50 行（构造函数宏展开） |
| **A3（一般）** — 附录引用外部修订历史文件 `a_v18_revision_history.md` 但未声明跨文件依赖 | 在 §附录首段新增跨文件依赖声明，明确依赖路径 `./a_v18_revision_history.md` |
| **B3（一般）** — 9 项独立编译器行为假设未做系统性多回退组合分析 | 在 §2.1 编码前验证检查清单中新增验证项⑫——"编译器依赖统一验证测试"原型程序覆盖所有 5 个编译期 `if` 依赖点及 `@OverflowWrapping` 继承性等共 9 项验证；在 §11 末尾新增 §11.13 "编译器依赖验证回退成本"子节，按验证项逐一评估独立验证成本和回退触发时的额外修复成本 |
| **A5（提示，非缺陷）** — §1 范围可追溯性缺少"首轮对应 roadmap §3E+§3G"显式说明 | 在 §1 范围可追溯性段落末尾补充一句说明，明确用户需求中的"首轮"对应 roadmap §3E + §3G 范围确认结论中的基础设施层（子范围 1）、类型系统（子范围 2a/4~8）及 shim 层（子范围 S1~S3） |
| **B4（提示）** — 首轮 C++ 代码裁剪的完整性验证策略不够明确 | 在 §8.4 策略 0 前置验证项中新增第④验证项：抽查 3~5 处被排除的 `#define`/`#if` 块（`setup.hpp` 中 `GLM_ARCH_DETECTED` 第 80~160 行、`GLM_COMPILER` 第 200~320 行、`GLM_PLATFORM` 第 350~440 行），确认首轮其他文件中无间接引用 |
| **C2（提示）** — `VecN<Bool, Q>` 的模运算不可用性分散在三处描述 | 在 §3.2 `VecN<Bool, Q>` 行为约定表中新增"模运算（`%` / `mod`）"行，标注"所有形式均不可用"，与算术运算符的不可用声明并列 |

### 版本 v24 — 上一轮修订说明

| 审查意见 | 修改措施 |
|---------|---------|
| **发现 1（严重）** — `fwd.cj` 中 `type` 别名缺少 `public` 可见性修饰符，公共 API 面不可达 | 在 §3.8 脚本模板的生成逻辑中将 `type {name}` 改为 `public type {name}`（line 506）；同步更新 §3.8 中预期的 `fwd.cj` 文件结构示例（lines 458-468）中所有 `type` 为 `public type`；在 §12.2 验收标准中新增"别名对外可见性"验收项；在 §2.1 编码前验证检查清单中新增验证项⑩ |
| **发现 2（中等）** — `type` 别名可见性相关章节的表述存在自我矛盾 | 统一全文中所有 `type` 别名的示例为 `public type` 形式（§3.1 `Defaultp`、§3.2 `Vec2f`、§3.7 `uint`、§3.8 所有别名示例）；在 §3.8 末尾新增"仓颉 `type` 别名可见性提醒"段落 |
| **发现 3（中等）** — Roadmap §3G 范围确认项缺失显式追溯 | 在 §8.2 范围可追溯性对照表前新增一段说明，显式追溯 §3G 的依赖闭合性、独立可编译性、最小可行范围定义三项结论与 §2 模块依赖分析、§8.3 迁移顺序和 §3.8 别名范围的对应关系 |
| **发现 4（一般）** — 验证计划缺少对生成脚本的 CI 集成验证 | 在 §2.1 编码前验证检查清单中新增验证项⑪（生成脚本映射表正确性验证）；在 §10 末尾新增"生成脚本验证依赖"子节（覆盖脚本产物一致性、持续一致性、映射表正确性三项依赖）；在 §3.8 末尾新增"生成脚本生命周期警告"说明手动修改 `fwd.cj` 后须同步更新脚本映射表 |
| **v24 再审查问题** — `lib.cj` 中 `public import glm.fwd.*` 引用了不存在的包 `glm.fwd`（`fwd.cj` 声明 `package glm`，与 `lib.cj` 同包，不存在 `glm.fwd` 子包） | 移除 `lib.cj` 代码示例中的 `public import glm.fwd.*` 行（§2 公共 API 面设计），改为注释说明 `fwd.cj` 声明与 `lib.cj` 同属 `package glm`、`public type` 别名已自动对外可见；同步更新 §2 对应的描述文本：将"`public import glm.fwd.*` 将 256 个别名一并暴露"改写为"`fwd.cj` 与 `lib.cj` 同属 `package glm`，`public type` 别名已自动对外可见"；同步更新 §3.8 仓颉 `type` 别名可见性提醒中引用 `public import glm.fwd.*` 的表述；同步更新 §8.1 文件清单中 `lib.cj` 的依赖列（移除 `glm.fwd`） |

### 版本 v23 — 上一轮修订说明

| 审查意见 | 修改措施 |
|---------|---------|
| **发现 1（一般）** — `fwd.cj` 256 个别名缺乏可执行的生成脚手架，§3.8 推荐外部脚本但未提供脚本模板 | 在 §3.8 "外部脚本生成策略"末尾新增"最小生成脚本示例"子节（约 40 行 Python 脚本模板），覆盖 256 个别名的全量生成逻辑并附定制指南；在 §2 初始目录结构中新增 `scripts/` 目录（含 `gen_fwd_aliases.py` 占位）；在 §11 末尾新增 §11.11 评估脚本开发工作量（0.75 人天） |
| **发现 2（一般）** — §4.5 `equalEpsilon` 代码示例仅展示路径 A（调用 `epsilonOf<T>()`），未体现 §3.5 双路径备选，缺少显式交叉引用 | 在 §4.5 `equalEpsilon` 代码示例下方新增双路径交叉引用标注，说明路径 B 的联动修改要求，并指向 §3.5 和 §12.2 的对应描述 |
| **发现 3（一般）** — §12.3 中 `internal` 类型测试访问的备选方案仅简化为"提升为 `public`"，缺少影响评估和替代方案 | 在 §12.3 末尾新增"`internal` 类型测试访问回退方案"子节，明确首选方案和三级回退层级（`protected` → `public` → helper 文件），每级附变更范围和影响评估；在 §2.1 编码前验证检查清单中新增补充验证项⑨记录此验证 |
| **发现 4（提示）** — §11 迁移成本评估缺失 `fwd.cj` 脚本开发工作量，§11.7~§11.10 均未覆盖 | 在 §11 末尾新增 §11.11 "`fwd.cj` 脚本开发与验证工作量"，对比手动编码（1-2 人天）和脚本生成（0.75 人天）两条路径，推荐脚本路径并附工作量明细和脚本生命周期管理说明 |

### 版本 v22 — 上一轮修订说明

| 审查意见 | 修改措施 |
|---------|---------|
| **发现 1（中等）** — `@OverflowWrapping` 标注策略未覆盖 scalar-op-vec 方向包级独立函数，20 个函数缺漏溢出标注，导致 `add(s, v)` 与 `v.add(s)` 溢出行为不一致 | 在 §4.6 `@OverflowWrapping` 标注分布表中新增条目：明确 scalar-op-vec 方向包级独立函数（`add(s,v)`/`sub(s,v)`/`mul(s,v)`/`div(s,v)`/`mod(s,v)`，5 运算 × 4 Vec = 20 个）须直接标注 `@OverflowWrapping`；在 §4.3 具名函数定义的 `@OverflowWrapping` 标注策略段落末尾补充 scalar-op-vec 方向的说明及指向 §4.6 的引用；更新 §4.6 标注统计总数为 52 处 |
| **发现 2（一般）** — §8.2 范围可追溯性表缺少 `scalar_vec_ops.cj` 的显式映射行 | 在 §8.2 范围可追溯性表中新增一行（位于序号 5 与序号 6 之间），将 `scalar_vec_ops.cj` 的 roadmap 映射关系（Roadmap 子范围 2a、§3E 序号"新增"）显式记录，并在偏离说明中标注新增理由 |
| **发现 3（已接受风险）** — Vec4 多重重载构造函数的重载解析期望未显式说明 | 在 §4.1"跨 Q 构造函数重载解析风险与约束验证"段落末尾新增"重载解析期望注解"：说明"具体类型优先于泛型类型"的预期解析规则，并标注需编码阶段原型验证确认 |
| **发现 4（提示）** — `equalEpsilon` 中"内联固定容差"回退策略存在语义漂移：内联位置歧义（callConst 还是 equalEpsilon？）和 `epsilonOf<T>()` 函数生命周期歧义 | 在 §3.5 兼容性策略中，将原有的单一路径改为两条显式路径——**路径 A（保留 `epsilonOf<T>()`）**：移除 `Number<T>` 约束，实现返回 `T(1e-6)`，`equalEpsilon` 无需联动修改；**路径 B（完全移除 `epsilonOf<T>()`）**：`equalEpsilon` 和 `ComputeEqual.callConst` 均须同步内联固定容差 `T(1e-6)`。同步更新 §10 依赖④描述和 §12.2 `equalEpsilon-epsilonOf` 回退联动验收项的验收标准描述 |

### 版本 v21 — 上一轮修订说明

| 审查意见 | 修改措施 |
|---------|---------|
| **发现 1（中等）** — `equalEpsilon` 与 `epsilonOf<T>()` 回退路径联动修改未在 §12.2 验收标准中覆盖 | 在 §12.2 验收标准表末尾新增 equalEpsilon-epsilonOf 回退联动验收项：要求回退路径触发后 `equalEpsilon` 将 `epsilonOf<T>()` 引用同步替换为内联固定容差 `T(1e-6)` |
| **发现 2（一般）** — `public const init` 在 Vec2~Vec4 上的 const 可达性依赖未在 §12.2 验收标准中独立列项 | 在 §12.2 验收标准表末尾新增 const init 泛型构造可用性验收项：验证 `const init` 在非 const 上下文中可作为逐分量运行时构造函数使用 |
| **发现 3（一般）** — `mod` 扩展成员函数浮点路径不可用的显式 API 影响说明缺失 | 在 §4.3 `%` 运算的整数约束段落末尾新增 **API 影响标注**：显式声明 `v.mod(s)` 仅支持整数类型，浮点类型须使用包级独立函数 `mod(s, v)`；同时新增 §12.2 验收项验证此约束 |

> 历次修订记录（v2~v17）已归档至 [`a_v18_revision_history.md`](./a_v18_revision_history.md)，供历史追溯查阅。

### 版本 v20 — 上一轮修订说明

| 审查意见 | 修改措施 |
|---------|---------|
| **发现 1（中等）** — D34 "const if" 术语清理不彻底，全文 25 处残留，实施者面临误导风险 | 对全文档执行系统性搜索替换：所有"const if"术语依上下文改写为"编译期 `if`"（独立术语场景）、"`const` 函数体内的 `if` 表达式"或"编译期 `if` 分支"（D34 推荐表述）。重点修复 D32 第 1087 行——设计决策章节自身使用"const if"。代码注释中的"const if"一并改写。总计修复 25+ 处 |
| **发现 2（一般）** — §12.3 同一行内混用 `tests/glm/detail/` 和 `tests/glm.detail/` 两种目录路径表示法 | 将 `tests/glm.detail/` 改写为 `tests/glm/detail/`。全文 grep `glm\.detail/` 模式一次性清理，确认仅此一处 |
| **非阻塞改进项 1** — §8.2 范围可追溯性表的 GLM 源文件列使用裸文件名，建议标注实际路径 | 在 §8.2 表头新增注释行标注 GLM 参考实现目录结构为 `glm/glm/detail/` 双层嵌套 |
| **非阻塞改进项 2** — 缺少通用范围变更管理流程 | 在 §7 D35 后新增 D36，定义通用范围变更日志条目模板 |
...
| 审查意见 | 修改措施 |
|---------|---------|
| **发现 1（中等）** — §11.5 与 §11.9 存在结构重叠，迁移工作量存在重复计算风险 | §11.5 开头新增覆盖声明："本节定义的 `Inf == Inf` 差异为 §11.9 所述行为差异的子集，§11.9 迁移模式已覆盖本节场景，无需重复审查"；§11.5 迁移工作量评估末尾补充非累加关系说明 |
| **发现 2（一般）** — §4.3 残留 "const if" 术语，与 D34 约定不一致 | §4.3 第 646 行"`const if`"改为"编译期分支 `if`"，与 D34 统一 |
| **发现 3（一般）** — §11.4 引用了未在 §4 中定义的 `logicalNot` 函数 | §11.4 迁移影响表（第 1529 行）"按需使用 `logicalNot`"改为"直接使用 `!` 运算符（已重载）" |
| **发现 4（一般）** — D31 中 Vec1 跨类型 fill-from-Vec1 的豁免理由未明确 | D31 末尾追加说明：Vec1 的跨类型 fill-from-Vec1（`init<T2, Q2>(v: Vec1<T2, Q2>)`）不受跨类型限制——Vec1 仅单个分量，跨类型版本实质是分量级类型转换而非广播填充，与 `const init(x: T)` 的参数列表类型结构不同，不构成重载冲突 |
| **发现 5（轻微）** — §8.2 S3 偏离理由中 VArray 引用与 countof 功能无关 | §8.2 S3 行偏离理由简化，移除 VArray 引用，替换为准确描述：Cangjie 无 C 数组类型——countof 宏基于 sizeof(array)/sizeof(array[0]) 计算 C 数组元素个数，此模式在 Cangjie 中无对应映射，所有原使用场景在迁移时删除 |
| **发现 6（提示）** — 历史修订记录占文档总篇幅约 55%，影响可导航性 | 将 v2~v17 的所有历史修订记录移出主文档，归档至独立文件 `a_v18_revision_history.md`，本附录仅保留当前版本（v19）的修订说明，并附指向归档文件的链接 |
