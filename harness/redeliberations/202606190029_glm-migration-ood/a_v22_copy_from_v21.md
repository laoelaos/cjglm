# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v22 — 审查迭代修订版）

> **版本变更摘要**：v22 基于 v21 迭代，根据 v22 审查反馈（发现 1~4）进行补充修订：在 §4.6 新增 scalar-op-vec 方向包级独立函数的 `@OverflowWrapping` 标注策略并在 §4.3 同步补充说明（发现 1）、在 §8.2 范围可追溯性表中新增 `scalar_vec_ops.cj` 显式映射行（发现 2）、在 §4.1 末尾补充重载解析期望注释（发现 3）、在 §3.5 兼容性策略中明确 `epsilonOf<T>()` 回退生命周期并同步更新 §10 依赖描述与 §12.2 验收标准（发现 4）。详见末尾修订说明。
