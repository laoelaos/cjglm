# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v8 — 审查迭代修订版）

> **版本变更摘要**：v8 基于 v7（a_v7_copy_from_v6.md）迭代，系统性修复 const 扩展成员函数语法不可行问题（P1，影响 §3.2/§4.3/§4.4/§4.5/§7 D32/D33）、修正 `const if (T2 is Bool)` 为值级检测方案（P2，§9.4）、补充测试文件清单（P3，§8.1）、修正 `<<` 默认溢出策略为待验证假设（P4，§4.4）、增加构造函数跨 Q 重载解析风险分析（P5，§4.1）、统一 const if 编译抑制验证（P6，§10）、新增参考实现使用策略（P7，§8.4）。v2 修正 §4.4 bitwiseNot() 残留的 `const` 扩展成员函数声明（审查确认 extend 块不允许 `const`）、补充 D30 交互分析第三维度（const-in-extend × @OverflowWrapping 继承性）。详见末尾修订说明。
...
| 任意 | ❌ 回退 | 不可用（已确认） | 算术运算通过 extend 块中非 `const` 具名函数实现，标注 `@OverflowWrapping`；`==` 依 D29 状态选择路径 | const-in-extend 不影响具名函数的功能正确性——`@OverflowWrapping` 和非 `const` 函数兼容 |

结论：`const` 在 extend 块中不可用的约束已由本设计的全局策略（所有扩展成员函数声明为非 `const`）统一处理，D30 回退路径中的算术具名函数属于此策略覆盖范围，无需额外回退。
...
**理由**：仓颉 `extend` 块不支持 `const` 修饰符（仓颉扩展 §4.2），因此 `vec-op-scalar` 方向扩展成员函数无法声明为 `const`。`scalar-op-vec` 方向包级独立函数无此限制。`mod` 的包级独立函数方向使用 `const if (isIec559Of<T>())` 实现整数/浮点双路径编译期分支选择，仓颉要求 `const if` 所在函数必须标注 `const` 修饰符，因此仅包级独立函数方向可以承载 `const if` 分支。扩展成员函数方向 `mod(v, s)` 仅提供整数路径（使用 `%`），浮点路径由包级独立函数 `mod(s, v)` 覆盖。
...
4. **配置宏确认**：`setup.cj` 中 18 个 `GLM_CONFIG_*` 常量的默认值以参考实现中 `setup.hpp` 的 `#define` 定义为准。当配置值与参考实现不一致时，以参考实现为准并更新设计文档中对应表格。
...
`const if` 编译期分支的非选择分支抑制行为是本设计多处依赖的核心编译器行为。以下统一列出所有依赖点及其验证要求。

#### 依赖点清单

| 依赖点 | 所在章节 | const if 表达式 | 非选择分支内容 | 风险等级 | 对非 `const` 函数的适用性 |
|-------|---------|---------------|--------------|---------|------------------------|
| A. `ComputeEqual.callConst` | §3.5 | `const if (isIec559Of<T>())` | 整数路径：`a == b`；浮点路径：`abs(a - b) <= epsilonOf<T>()` | **严重** — 失败则 `==` 整体回退到精确比较 | `callConst` 声明为 `const`；若 `const if` 抑制不成立，`==` 降级为非 `const` 精确比较 |
| B. `isIec559Of<T>()` | §3.6 | `const if (T(0) is Float64 \|\| T(0) is Float32)` | 非浮点分支：`else { false }` | **严重** — 失败则编译期类型检测方案整体不可行 | 函数声明为 `const`；若失败 → 所有类型回退到精确比较 |
| C. `mod` 包级独立函数 | §4.3 | `const if (isIec559Of<T>())` | 整数路径：`x % y`；浮点路径：`x - y * trunc(x / y)` | **严重** — 失败则双路径不可行 | 包级独立函数声明为 `const`；若失败 → 拆分为 `modInt`/`modFloat` 非 `const` 具名函数 |
| D. `ComputeEqual.callConst` + `isIec559Of` 组合 | §3.5+§3.6 | `const if (T(0) is Float64)` 嵌套于 `callConst` 内 | 整数分支 `a == b` | **严重** — 两层 const if 嵌套的抑制行为 | 同上 A，`callConst` 为 `const`；若失败 → 整体回退 |
| E. 构造函数 Bool→数值检测 | §9.4 | `const if (v.x is Bool)`（备选方案 A） | 数值分支：`T(1)/T(0)` | **低** — 已采用方案 B 运行时兜底 | `const init` 中可用（备选）；主方案为运行时分支 |

#### 统一验证方法

所有依赖点（A~E）的核心验证逻辑相同——验证 `const if` 在 `const` 上下文中的非选择分支抑制行为。建议采用以下统一原型测试验证：

```cangjie
// 统一验证：const if 在 const 泛型函数中对非选择分支的编译抑制
const func testConstIfSuppression<T>(): Bool {
    const if (T(0) is Float64 || T(0) is Float32) {
        // 浮点分支：使用仅浮点支持的运算
        true
    } else {
...
若以上三项均编译通过 → `const if` 非选择分支抑制机制对所有依赖点 A~E 均成立。

若 `const if` 抑制不成立，各依赖点的独立回退方案：
...
| D. 嵌套 const if | 与 A 回退路径一致（整体 `==` 退至精确比较） |
| E. Bool 检测 | 采用方案 B 运行时分支（已为首选方案，无需变更） |

#### mod 函数边界值验证（独立于 const if 抑制）

若 `const if` 抑制验证通过，`mod` 的浮点路径还需额外验证恒等式的数值正确性：
...
| ⑤ `mod` 函数的 `const` 声明 | `mod` 的包级独立函数方向须声明为 `const`（仓颉中 `const if` 要求所在函数为 `const`）；扩展成员函数方向因 `extend` 块限制（仓颉扩展 §4.2）声明为非 `const`，且不承载 `const if` 分支 | 低 — 语法语义明确 | 编码阶段确认 `const` 修饰符出现在 `mod` 包级独立函数的签名中；同步确认 `add`/`sub`/`mul`/`div` 的包级独立函数也声明为 `const`（理由见 §7 D32） |
...
| **P2（中等）** — §9.4 Bool→数值转换的 `const if (T2 is Bool)` 语法可行性未验证——`is` 运算符是值级类型检测而非类型参数级检测 | §9.4 重写：明确说明 `T2 is Bool` 语法不可行（`is` 需要值表达式而非类型参数）；原方案 A 改为分量值检测方案（`const if (v.x is Bool)`），但运行时分支（方案 B）为首选推荐路径；优先采用方案 B（运行时兜底），理由：避免 `const if` 依赖、非热路径、可行性有保障 |
