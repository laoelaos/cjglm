# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v13）

> **产出内部版本号**：v13
> **外部再审议迭代轮次**：第 13 次（第 13 轮设计迭代）
> **此轮审查来源**：a_v13_iteration_requirement.md
> **上一版本**：a_v12_copy_from_v11.md（v12）
> **修订日期**：2026-06-19
> **版本谱系说明**：本文件源自 v12（a_v12_copy_from_v11.md），在其基础上经过 v12 修订后进一步迭代为本版本 v13。版本追溯路径：v4 → v5 → v6 → v7 → v8 → v9 → v10 → v11 → v12 → v13
...
> 
> **当前评估**：本设计在 25 项验证全部通过时可直接编码；1~3 项部分回退时可基于 §2.1 回退方案和 §11.13 叠加效应分析指导编码；仅当 3 项以上核心假设同时失败时才需停工重评估。编码阶段建议优先执行第①④⑤⑫项以尽早暴露最高风险项。
...
| §11.6 `@OverflowWrapping` 回退成本`[^overflow]` | 0（继承性成立） | 0.5（补充标注） | 1.0（补充标注+修改委托） |
