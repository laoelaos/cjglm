# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v6）

> **产出内部版本号**：v6
> **外部再审议迭代轮次**：第 3 次（第 5 轮设计迭代）
> **上一版本**：a_v5_copy_from_v4.md（v5）
> **修订日期**：2026-06-19
...
| P7 | §7 D6 CLIP_CONTROL 默认值"已验证正确"措辞超前 | 第 2 轮 | ✅ 已解决（v3）— 改为"推导结论，待编码前验证项②确认"，补充位定义值，新增 roadmap 冲突跟踪项 |
...
| `GLM_CONFIG_CLIP_CONTROL` | `const GLM_CONFIG_CLIP_CONTROL: Int64` | `0x0A`（十进制 10） | 裁剪空间约定（LH/RH, NO/ZO 位组合）。对应 C++ `RH_BIT \| NO_BIT` = 0x0A，首轮使用默认 RH_NO。**默认值来源推导说明**：GLM 1.0.3 `setup.hpp` 中的 `GLM_CONFIG_CLIP_CONTROL` 默认值由两层条件编译决定——首先判断 `GLM_FORCE_DEPTH_ZERO_TO_ONE` 设置 `ZO_BIT`/`NO_BIT`，再判断 `GLM_FORCE_LEFT_HANDED` 设置 `LH_BIT`/`RH_BIT`。在无额外定义时，两层条件编译均走 `#else` 分支，最终等效于 `RH_NO = 0x0A`。**位定义值说明**：参考实现中 `RH_BIT = 1 << 3 = 8`（第 3 位，位值为 8），`NO_BIT = 1 << 1 = 2`（第 1 位，位值为 2），两者按位或得 `8 \| 2 = 0x0A`。编码前验证项②须读取 `setup.hpp:563-584` 的原始条件编译块逐行确认分支走向与 `0x0A` 推导一致。**与 roadmap 差异说明**：roadmap §3A.1 记录该配置默认值为 `0x02`，与本文档值 `0x0A` 冲突。经参考实现条件编译逻辑推导，文档值 `0x0A` 为推导结论（待编码前验证项②确认），roadmap 值 `0x02` 可能遗漏了 `RH_BIT = 8` 组合位，仅截取了 `NO_BIT = 2` 的低位部分。此差异构成需显式协调的跟踪项——编码阶段以 `setup.hpp` 原始条件编译结果为准 |
