...
> **`[^compiler-dep]` 编译器推断依赖**：`fromBoolVecQ2` 函数体中的 `Vec1(...)` 调用未显式标注类型参数 `<T, Q>`，依赖仓颉编译器从函数返回值类型 `Vec1<T, Q>` 推断 `Vec1` 构造函数的泛型实参。此推断行为在当前设计阶段**未被验证覆盖**——编码前须编写原型验证确认编译器是否支持从返回值类型推断 struct 构造函数的泛型参数。若验证失败（编译器不支持此推断），须在所有 `fromBoolVecQ2`（Vec1~Vec4）的 `VecN(...)` 调用处显式标注类型参数，如 `Vec1<T, Q>(if (v.x) { T(1) } else { T(0) })`。
...
| §11.5 浮点容差比较与 C++ GLM 精确比较的全量行为偏离[^1] | 0（已纳入 §11.9） | 0（已纳入 §11.9） | 0（已纳入 §11.9） |
...
| epsilonOf 兼容性策略（§3.5 路径 A vs 路径 B） | 项⑲⑫ | **依赖项⑲+分支抑制验证**：若 `Int32(1e-6)` 编译错误但分支抑制有效→路径 A 安全；若分支抑制对泛型约束无效→路径 B | 路径 A：移除 epsilonOf `Number<T>` 约束，返回 `T(1e-6)`；路径 B：完全移除 epsilonOf，内联固定容差到 equalEpsilon 和 callConst | §3.5 双路径策略 | **路径 A 触发时**：修改 `shim_limits.cj` 中 `epsilonOf<T>()` 定义（移除 `Number<T>` 约束，改为 `T(1e-6)`）→ 验证编译期 `if` 分支抑制保护有效（§2.1 项④⑫）→ 确认整数 `T` 不进入 `epsilonOf<T>()` 调用路径 → 若抑制保护无效，回退至路径 B。`equalEpsilon` 无需联动修改；`ComputeEqual.callConst` 浮点分支保持 `abs(a-b) <= epsilonOf<T>()` 不变。**路径 B 触发时**：修改 `shim_limits.cj` 移除 `epsilonOf<T>()` 函数定义 → 修改 Vec1~Vec4 的 `equalEpsilon` 定义（将 `epsilonOf<T>()` 引用替换为 `T(1e-6)` 内联固定容差，共 4 处） → 修改 `ComputeEqual.callConst` 浮点分支（将 `epsilonOf<T>()` 引用替换为 `T(1e-6)`，共 1 处）。路径 B 合计联动修改 6 处（1 移除 + 4 替换 + 1 替换） |
