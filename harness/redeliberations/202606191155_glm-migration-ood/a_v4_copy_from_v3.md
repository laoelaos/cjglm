# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v5）

> **产出内部版本号**：v5
> **外部再审议迭代轮次**：第 3 次（第 4 轮设计迭代）
> **上一版本**：a_v4_copy_from_v3.md（v4）
> **修订日期**：2026-06-19
...
- **Vec1 广播运算模式**：C++ GLM 的 `type_vecN.inl` 中为 Vec2~Vec4 均提供了 `vecN op vec1` 和 `vec1 op vecN` 两种广播运算符重载（覆盖 `+`、`-`、`*`、`/`、`%` 共 5 种运算符），以及 `vecN op= vec1` 形式的广播复合赋值。仓颉中此广播模式通过 `extend` 块定义——在 `extend<T, Q> VecN<T, Q> where Q <: Qualifier` 块中新增 `operator func +(rhs: Vec1<T, Q>): VecN<T, Q>` 等广播运算符，逐分量将 `rhs.x` 广播到左操作数的每个分量进行运算。Vec1 左操作数的广播（`vec1 op vecN`）因运算符重载定义在左操作数类型作用域内的规则，可在 Vec1 的 `extend` 块中定义——`operator func +(rhs: Vec2<T, Q>): Vec2<T, Q>` 等，将 Vec1 的单一分量广播到右操作数的每个分量。**定义位置**：广播运算符在对应 `type_vecN.cj` 的 `extend` 块中与位运算符一同定义；Vec1 左操作数方向的广播运算符在 `type_vec1.cj` 的 `extend` 块中定义。**复合赋值**：Vec1 广播二元运算符的返回类型与左操作数类型匹配（如 `Vec2 + Vec1` 返回 `Vec2`），仓颉编译器自动生成对应的广播复合赋值版本（如 `Vec2 += Vec1`）。Vec1 左操作数方向的广播运算符返回类型为右操作数类型（如 `Vec1 + Vec2` 返回 `Vec2`），但复合赋值要求左操作数与返回类型匹配——`Vec1 += Vec2` 中左操作数为 Vec1、返回类型为 Vec2，不匹配，因此编译器**不自动生成**此方向的复合赋值。调用方需使用 `v1 = v1 + v2` 两步模式替代。
...
**总结**：`@OverflowWrapping` 在二元算术运算符（5 个：`+`、`-`、`*`、`/`、`%`）、`<<`（1 个）、`increment()`/`decrement()`（2 个）、**20 个 scalar-op-vec 方向包级独立函数**和**30 个 Vec1 广播算术运算符 + 30 个 Vec1 广播位运算符**（其中 `<<` 相关广播已标注 `@OverflowWrapping`，`&`/`|`/`^`/`>>` 无溢出风险无需标注，实际需标注约 15 个算术广播 + 3 个 `<<` 广播 = 18 个）上直接标注；20 个 vec-op-scalar 方向扩展成员函数通过委托间接继承 wrapping 语义，无需直接标注；复合赋值通过编译器自动生成继承标注语义。`@OverflowThrowing` 默认策略对 `>>`、`&`、`|`、`^` 保持原样（这些运算不产生溢出）。无内部矛盾的唯一决策。
...
- 最可能合计（~12 人天）：D30 标注继承性回退触发 + 别名命名需适度人工审查 + 跨 Q 部分场景需人工确认。
- 悲观合计（~22 人天）：D29 const 表达式链回退 + D30 标注继承性回退 + 编译器依赖验证多项回退 + 大面积别名人工审查 + 跨类型复合赋值人工审查。
