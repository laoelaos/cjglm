# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v17）

> **产出内部版本号**：v17
> **外部再审议迭代轮次**：第 17 次（第 17 轮设计迭代）
> **此轮审查来源**：a_v17_iteration_requirement.md
> **上一版本**：a_v16_copy_from_v15.md（v16）
> **修订日期**：2026-06-19
> **版本谱系说明**：本文件源自 v16（a_v16_copy_from_v15.md），在其基础上经过 v16 修订后进一步迭代为本版本 v17。版本追溯路径从 v4 起始（v1-v3 已被 v4 重构合并，不再追溯）：v4 → v5 → v6 → v7 → v8 → v9 → v10 → v11 → v12 → v13 → v14 → v15 → v16 → v17
...
| 阶段 | 步骤 | 文件 | 增量验证 | 预期代码量 |
|------|------|------|---------|-----------|
| 0 | 非 Vec 文件 API 快速扫描（编码启动前置） | 对 7 个非 Vec 核心文件（setup.hpp/qualifier.hpp/compute_vector_relational.hpp/compute_vector_decl.hpp/_vectorize.hpp/fwd.hpp/shim 文件）按 §1 下方「7 文件快速扫描依赖项核查清单」逐项确认无遗漏移植依赖项 | 扫描执行人：设计审议方提供核查清单，编码实施者按清单逐项确认并签字；通过标准：7 项核查清单全部确认通过，每项核查要点逐条应答；逐 API 级对照延迟至第 2 轮编码阶段以附录 A 为模板补充；超时升级路径：发现未覆盖依赖项且 24 小时内无法确认处理方案 → 上报主 Agent + 设计审议；发现≠解决闭环原则：扫描标识的差异须以编码阶段实际编译验证为最终闭环依据 | ~0 行（扫描审查，无代码编写） |
