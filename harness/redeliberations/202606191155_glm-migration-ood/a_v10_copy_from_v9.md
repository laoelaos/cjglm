# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v10）

> **产出内部版本号**：v10
> **外部再审议迭代轮次**：第 10 次（第 10 轮设计迭代）
> **此轮审查来源**：a_v10_iteration_requirement.md
> **上一版本**：a_v9_copy_from_v8.md（v9）
> **修订日期**：2026-06-19
> **版本谱系说明**：本文件源自 v9（a_v9_copy_from_v8.md），在其基础上经过 v9 修订后进一步迭代为本版本 v10。版本追溯路径：v4 → v5 → v6 → v7 → v8 → v9 → v10
...
| P7 | §7 D6 CLIP_CONTROL 默认值协调跨文件未闭环 | 第 2 轮 | 🔴 跟踪项——需外部协调——参考实现已验证 `0x0A` 正确（`setup.hpp:563-584` 条件编译逻辑确认），但 roadmap §3A.1 仍记录为 `0x02`，两文件未同步。本设计文档以 `0x0A` 为准（见 §8.2 正式设计决策注释）。**协调责任方**：roadmap 维护者。**完成时限**：编码启动前（首个 `cjpm build` 前）。**完成标准**：roadmap §3A.1 默认值更新为 `0x0A`，且两文件值一致。**失败升级路径**：若编码启动时仍未同步，由主 Agent 升级至项目负责人评审。此条在 roadmap 侧同步完成前标记为外部跟踪项，不阻塞本设计决策 |
...
| P37 | C++ GLM API 映射完整性与 type_vecN.inl 逐文件交叉验证 | 第 5 轮 | 🔄 持续问题（本轮 v10）— 附录 A 验证范围限定为 type_vec{1,2,3,4}.hpp/.inl 四个 Vec 类型文件（即 4/11 个核心文件已完成逐 API 级对照）。其余 7 个核心文件（setup.hpp、qualifier.hpp、compute_vector_relational.hpp、compute_vector_decl.hpp、_vectorize.hpp、fwd.hpp、shim 文件）以子范围粒度映射至 §8.2 范围可追溯性对照表和 §8.5 排除清单，未做逐 API 级对照。其余 7 个文件的逐 API 级对照规划在第 2 轮编码阶段以本附录为模板补充完成。未覆盖的泛型多元构造函数等已纳入 §8.5 排除清单 |
...
> **§2.1 维护原则**：本清单是验证任务的**唯一权威出处**。§10 仅保留设计特有行为描述和回退方案依赖关系（如编译期 `if` 分支抑制各依赖点的回退方案、`equalEpsilon` 双路径触发条件等），所有原型测试方法和验证过程描述均以 §2.1 为准。§12.3 仅保留验证→设计路径选择映射关系。凡在此三章节间发现内容重叠，**以 §2.1 为裁决依据**，其余两处应同步消重。维护者在后续轮次中新增验证项时，应优先追加至本清单。

以下清单集中列出所有必须在编码前完成的设计阶段验证任务，确保编译器对关键语言特性的支持符合设计假设。每项验证通过后方可进入正式编码。

> **验证时序建议**：建议优先执行第①项（对应 §7 D29 const 表达式链验证）、第⑤项（对应 §7 D30 `@OverflowWrapping` 继承性验证）**和第④项（编译期 if 分支抑制验证）**，并参考 §7 D29/D30 组合回退场景分析评估三者的交互影响。第④项的验证结果决定多项编译器依赖（依赖点 A~E）的走向——若编译期 if 分支抑制不成立，不仅 `==` 容差路径不可用，`mod` 双路径方案也需回退，影响面覆盖比较语义、取模实现和泛型约束策略等多个维度。建议在第①项确认 `T(0)` const 构造可用后立即执行第④项，尽早暴露最高风险项。三项通过后再按编号顺序依次验证其余各项。
...
**与 roadmap 的冲突及协调声明**：roadmap §3A.1 记录该配置默认值为 `0x02`，与本文档值 `0x0A` 冲突。经参考实现条件编译逻辑验证，本文档值 `0x0A` 为已验证结论，roadmap 值 `0x02` 遗漏了 `RH_BIT = 8` 组合位。**以本文档为准**，编码阶段实施者须同步更新 roadmap §3A.1 默认值为 `0x0A`。此值在 roadmap 侧同步完成前标记为外部跟踪项（详见 §1 P7 状态 "🔴 跟踪项——需外部协调"），不阻塞本设计决策。**协调责任方**：roadmap 维护者。**完成时限**：编码启动前（首个 `cjpm build` 前）。**完成标准**：roadmap §3A.1 默认值更新为 `0x0A`，且两文件值一致。**失败升级路径**：若编码启动时仍未同步，由主 Agent 升级至项目负责人评审。
...
| `GLM_CONFIG_CLIP_CONTROL` 编码前一致性确认 | 编码启动前确认 roadmap §3A.1 默认值与本文档 §7 D38 一致（均为 `0x0A`），两文件值已同步 | 人工比对 roadmap §3A.1 值与 §7 D38 记录值；若不一致，按 P7 升级路径走失败升级流程 |
...
| 验证依赖 | §2.1 检查项 | 验证通过→选择路径 | 验证失败→回退路径 | 回退决策章节 |
|---------|-----------|-----------------|----------------|------------|
...
| epsilonOf 兼容性策略（§3.5 路径 A vs 路径 B） | 项⑲⑫（联动修改注意事项已在 §2.1 项⑲ 回退方案列中标注） | **依赖项⑲+分支抑制验证**：若 `Int32(1e-6)` 编译错误但分支抑制有效→路径 A 安全；若分支抑制对泛型约束无效→路径 B | 路径 A：移除 epsilonOf `Number<T>` 约束，返回 `T(1e-6)`；路径 B：完全移除 epsilonOf，内联固定容差到 equalEpsilon 和 callConst | §3.5 双路径策略 |
