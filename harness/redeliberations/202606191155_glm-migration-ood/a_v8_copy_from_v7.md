# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v8）

> **产出内部版本号**：v8
> **外部再审议迭代轮次**：第 7 次（第 8 轮设计迭代）
> **此轮审查来源**：a_v7_iteration_requirement.md
> **上一版本**：a_v7_copy_from_v6.md（v7）
> **修订日期**：2026-06-19
> **版本谱系说明**：本文件源自 v7（a_v7_copy_from_v6.md），在其基础上经过 v7 修订后进一步迭代为本版本 v8。版本追溯路径：v4 → v5 → v6 → v7 → v8
...
| P7 | §7 D6 CLIP_CONTROL 默认值协调跨文件未闭环 | 第 2 轮 | ⚠ 依赖项——需外部协调——参考实现已验证 `0x0A` 正确（`setup.hpp:563-584` 条件编译逻辑确认），但 roadmap §3A.1 仍记录为 `0x02`，两文件未同步。本设计文档以 `0x0A` 为准（见 §8.2 正式设计决策注释），roadmap 侧需由其维护者同步更新。此条在 roadmap 侧同步完成前标记为外部依赖，不阻塞本设计决策 |
...
| P37 | C++ GLM API 映射完整性与 type_vecN.inl 逐文件交叉验证 | 第 5 轮 | 🔄 持续问题（本轮 v8）— 附录 A 验证范围限定为 type_vec1.inl~type_vec4.inl 四个 Vec 类型文件。首轮 11 个核心文件其余 7 个（setup.hpp、qualifier.hpp、compute_vector_relational.hpp、compute_vector_decl.hpp、_vectorize.hpp、fwd.hpp、shim 文件）的 API 逐项覆盖对照已在 §8.2 范围可追溯性对照表和 §8.5 排除清单中完成。未覆盖的泛型多元构造函数等已纳入 §8.5 排除清单 |
...
| ⑰ `%` 运算符编译器定义处/实例化处报错验证 | §4.3/§7 D13 | 编写原型测试：定义包含 `%` 运算符的泛型 struct（`operator func %(rhs: VecN<T, Q>)`），验证编译器是否在泛型定义处报错（类似 Rust well-formedness 检查），还是仅在实例化处（如 `Float32`）报错 | 编译器在实例化处报错（延迟检查语义），定义处编译通过 | 若编译器在定义处报错，`%` 运算符需改为 `extend` 块中定义或从 struct 体中完全移除（改为仅 `extend` 块中的 `mod` 具名函数） | 独立原型验证 |
| ⑱ `public import` 重导出验证 | §2 公共 API 面 | 编写原型测试：在 `package a` 中定义 `public struct S`，在 `package b` 中通过 `public import a.S` 重导出，验证从 `package c` 通过 `import b.S` 是否可正常访问 `S` 类型，验证重导出后类型的可见性和名义路径 | 重导出类型从 `package b` 可正常访问，名义路径为 `b.S` | 若重导出行为与预期不符，调整 `lib.cj` 策略——改为在 `package glm` 中直接定义 `public type Vec2 = glm.detail.Vec2` 类型别名替代 `public import` 重导出 | 独立原型验证 |
| ⑲ `T(1e-6)` 对整数类型可用性验证 | §3.5 路径 A | 编写原型测试：定义泛型函数 `func epsilonOf<T>(): T { T(1e-6) }`，分别以 `Int32`、`Float32`、`Bool` 实例化，验证编译行为 | 对 `Float32`/`Float64` 编译通过；对 `Int32`/`Bool` 明确编译错误 | 若 `Int32(1e-6)` 编译错误，路径 A 需编译期分支或完全切换至路径 B（内联固定容差 `T(1e-6)` 仅限浮点分支），重新评估路径排序 | 独立原型验证 |
...
| ㉔ `<<` 运算符默认溢出策略验证 | §4.4/§7 D37 A19 | 编写原型测试：分别对 `UInt8`、`Int32`、`UInt64` 执行超出位宽的左移（如 `UInt8(1) << 8`），观察编译器默认行为：若抛 `ArithmeticException` → 默认 `@OverflowThrowing`，需标注 `@OverflowWrapping` 对齐 C++ 高位丢弃语义；若自动 wrapping → 默认 `@OverflowWrapping`，无需额外标注 | 确认默认策略后 `@OverflowWrapping` 标注策略与编译器实际行为一致 | 若默认 `@OverflowWrapping`，移除 `<<` 运算符上的 `@OverflowWrapping` 标注；若默认 `@OverflowThrowing`，保留现有标注 | 独立原型验证 |
...
**`T` 参数类型范围**：`T` 可为任意数值类型（`Int8`~`Int64`、`UInt8`~`UInt64`、`Float32`/`Float64`）及 `Bool` 自身。当 `T = Bool` 时，`fromBoolVec` 生成 `VecN<Bool, Q>`，语义为"Bool 值保持不变"（`true→T(1)=Bool(1)=true`，`false→T(0)=Bool(0)=false`），即此函数等价于恒等映射。注意：`T(1)`/`T(0)` 构造器对所有数值类型和 `Bool` 均合法，因此该函数对所有目标 `T` 均编译通过，不受 §4.1 跨类型构造函数中 Bool→Numeric 编译错误的限制。
...
**与 roadmap 的冲突及协调声明**：roadmap §3A.1 记录该配置默认值为 `0x02`，与本文档值 `0x0A` 冲突。经参考实现条件编译逻辑验证，本文档值 `0x0A` 为已验证结论，roadmap 值 `0x02` 遗漏了 `RH_BIT = 8` 组合位。**以本文档为准**，编码阶段实施者须同步更新 roadmap §3A.1 默认值为 `0x0A`。此值在 roadmap 侧同步完成前标记为外部依赖项（详见 §1 P7 状态 "⚠ 依赖项——需外部协调"），不阻塞本设计决策。
...
| 3 | 辅助工具（可并行） | 6. compute_vector_relational.cj + 7. vectorize.cj | 编译通过；`ComputeEqual` 对 `Int32`/`Float32`/`Bool` 各实例化一次确认。注：`compute_vector_relational.cj` 与 `vectorize.cj` 无相互编译依赖，可并行编译；`compute_vector_decl.cj` 依赖 `vectorize.cj`，须在阶段 4 编译 |
...
| 完全泛型多元构造函数 `template<A,B,...> vecN(A, B, ...)` — 各分量类型可独立推导 | `type_vec1.inl`~`type_vec4.inl` | 仓颉构造函数不能为泛型（运算符重载限制），每个分量的类型须与 `VecN<T,Q>` 的 `T` 一致。此模式在 C++ GLM 中通过模板 `template<typename A, typename B> vec(A _x, B _y)` 实现，允许 `vec2(Int32(1), Float32(2.0))` 各分量异构。仓颉中仅可通过 `init(scalar: T)` 同类型模式部分覆盖。替代方案：调用方先将异构参数统一为目标类型 `T` 后调用同类型构造函数 | 永久排除（仓颉语言限制） | 附录 A |
...
**设计阶段验证要求**：`isIec559Of<T>()` 的 `if (T(0) is Float64 || T(0) is Float32)` 方案依赖三项核心假设，均须在设计阶段（而非编码阶段）完成编译器验证。具体验证方法详见 §2.1 编码前验证检查清单项①③及项⑫（统一集成验证）：

| 依赖项 | 描述 | 风险等级 | 对应 §2.1 检查项 |
|-------|------|---------|----------------|
| ① `T(0)` const 构造 | `T(0)` 在 const 上下文中对所有目标类型（`Int32`、`Float32`、`Bool` 等）合法 | **高** — 若失败则编译期 `if` 分支前提不成立，整体方案不可行 | 项①③ |
| ② `is` 在 const 泛型中的可用性 | `is` 运算符在 const 泛型函数中可参与编译期求值，对泛型类型参数 `T` 的实例化值执行类型检测 | **中** — 虽已在 const 表达式列表中确认可用，但泛型 `T` 的特殊性需单独确认 | 项①③（合并验证） |
| ③ `abs()` / `epsilon()` const 兼容性 | `Float32.abs()`、`NumericLimits<T>.epsilon()` 在 const 函数体内可调用 | **中** — 失败则容差路径不可用 | 项③㉒（`std.math.abs` 验证） |
...
| 依赖点 | 所在章节 | 编译期 `if` 表达式 | 非选择分支内容 | 风险等级 | 对非 `const` 函数的适用性 | 对应 §2.1 检查项 |
|-------|---------|---------------|--------------|---------|------------------------|----------------|
| A. `ComputeEqual.callConst` | §3.5 | `if (isIec559Of<T>())` | 整数路径：`a == b`；浮点路径：`abs(a - b) <= epsilonOf<T>()` | **严重** — 失败则 `==` 整体回退到精确比较 | `callConst` 声明为 `const`；若编译期 `if` 抑制不成立，`==` 降级为非 `const` 精确比较 | 项④⑫ |
| B. `isIec559Of<T>()` | §3.6 | `if (T(0) is Float64 \|\| T(0) is Float32)` | 非浮点分支：`else { false }` | **严重** — 失败则编译期类型检测方案整体不可行 | 函数声明为 `const`；若失败 → 所有类型回退到精确比较 | 项①③ |
| C. `mod` 包级独立函数 | §4.3 | `if (isIec559Of<T>())` | 整数路径：`x % y`；浮点路径：`x - y * trunc(x / y)` | **严重** — 失败则双路径不可行 | 包级独立函数声明为 `const`；若失败 → 拆分为 `modInt`/`modFloat` 非 `const` 具名函数 | 项⑥⑫ |
| D. `ComputeEqual.callConst` + `isIec559Of` 组合 | §3.5+§3.6 | `if (T(0) is Float64)` 嵌套于 `callConst` 内 | 整数分支 `a == b` | **严重** — 两层编译期 `if` 嵌套的抑制行为 | 同上 A，`callConst` 为 `const`；若失败 → 整体回退 | 项④⑫ |
| E. 构造函数 Bool→数值检测 | §9.4 | `if (v.x is Bool)`（备选方案 A） | 数值分支：`T(1)/T(0)` | **低** — 已采用方案 B 运行时兜底 | `const init` 中可用（备选）；主方案为运行时分支 | 项⑧ |
...
`mod` 具名函数对浮点类型 Vec 的实现采用算术恒等式 `x - y * trunc(x / y)`，其中 `trunc` 取自 `std.math.trunc`（仅提供 `Float64` 版本）。对 `Float32` 分量，采用先提升至 `Float64` 计算截断再转换回 `Float32` 的精度策略。此实现策略涉及以下关键假设，均须在设计阶段验证（具体验证方法详见 §2.1 编码前验证检查清单项⑥）：

| 依赖项 | 描述 | 风险等级 | 对应 §2.1 检查项 |
|-------|------|---------|----------------|
| ① `std.math.trunc(Float64) → Float64` 可用性 | `trunc` 函数签名和行为的正确性 | 低 — 标准库函数 | 无需独立验证（标准库文档已确认） |
| ② `Float32` → `Float64` 提升 → `trunc` → `Float32` 降级无语义损失 | 恒等式 `x - y * trunc(x / y)` 在提升-截断-降级过程中保持正确性 | **中** — 边界值行为需验证 | 项⑥（边界值验证） |
| ③ 恒等式对负操作数的正确性 | `trunc` 向零取整，`x - y * trunc(x / y)` 在负操作数时符号行为与 C++ GLM `std::fmod` 一致 | 中 | 项⑥（负操作数验证） |
| ④ 整数 Vec `mod` 与浮点 `mod` 分支选择的编译期 `if` 正确性 | `if (isIec559Of<T>())` 正确区分整数/浮点路径 | 低 — 与 `ComputeEqual` 分支逻辑一致 | 项⑫（统一集成验证） |
| ⑤ `mod` 函数的 `const` 声明 | 包级独立函数方向须声明为 `const`；扩展成员函数方向声明为非 `const` | 低 | 编码阶段直接确认，无需原型验证 |

### `equalEpsilon` 设计阶段验证

`equalEpsilon` 具名函数（§4.5）提供非 `const` 浮点容差比较能力，作为 `==` 回退路径的补充或显式容差比较 API。其设计阶段验证依赖以下前提（验证方法详见 §2.1 项⑫统一集成验证及 §12.1 层次三测试）：

| 依赖项 | 描述 | 风险等级 | 对应 §2.1 检查项 |
|-------|------|---------|----------------|
| ① `abs()` 在非 `const` 扩展成员函数中的可用性 | `abs()` 函数可在非 `const` 扩展成员函数体内调用 | 低 | 项㉒（`std.math.abs` const 兼容性验证，非 const 上下文无需独立验证） |
| ② `epsilonOf<T>()` 对浮点/整数 T 的编译通过性 | 非浮点 T 上 `epsilonOf<T>()` 的编译器行为 | **中** | 项⑲（`T(1e-6)` 对整数类型可用性验证） |
| ③ `Inf`/`NaN` 边界行为 | `equalEpsilon` 在 `Inf`/`NaN` 分量上的行为与 §9.3 约定一致 | 低 | 无需独立原型验证（浮点算术语义已确定） |
| ④ `epsilonOf<T>()` 回退路径与 `equalEpsilon` 的隐式依赖链 | 路径 A/B 选择对 `equalEpsilon` 实现的联动修改要求 | **中等** | 详见 §12.2 `equalEpsilon-epsilonOf` 回退联动验收项及 §12.3 决策映射表 |
...
`equalExact` 具名函数（§4.5）跳过容差比较路径，直接使用仓颉原生 `==` 对每个分量做精确比较。其设计阶段验证依赖以下前提（验证方法详见 §12.1 层次三测试）：

| 依赖项 | 描述 | 风险等级 | 验证方式 |
|-------|------|---------|---------|
| ① 组件级 `==` 在任意 Vec 实例化上可用 | 仓颉原生 `==` 对所有数值类型和 `Bool` 均支持 | 低 | 随 §12.1 层次二构造验证一并覆盖 |
| ② 容差路径与 `equalExact` 行为差异可观测 | 有限值微差场景中 `==` 与 `equalExact` 行为差异 | 低 | 随 §12.1 层次三比较运算验证覆盖 |
| ③ `equalExact(Inf)` → `true` | IEEE 754 精确相等语义 | 低 | 语言定义行为，无需额外验证 |
| ④ `equalExact(NaN)` → `false` | IEEE 754 精确相等语义 | 低 | 语言定义行为，无需额外验证 |
| ⑤ 回退路径下 `==` 与 `equalExact` 等价性 | D29 回退触发后的行为等价性 | 低 | 仅在 D29 回退触发后执行回归验证 |

**验证时机**：验证项①和②在 Vec 结构体编码完成后立即执行（作为构造-访问-运算验证的一部分）；验证项③④为语言定义行为，可作为编译确认项；验证项⑤仅在 D29 回退触发后执行（作为回退路径的回归测试）。

### `@OverflowWrapping` 与 `const` 函数共存验证

`@OverflowWrapping` 溢出注解与 `const` 函数修饰符可在同一函数上共存。此共存性涉及以下验证依赖项（验证方法详见 §2.1 项⑫统一集成验证）：

| 依赖项 | 描述 | 风险等级 | 对应 §2.1 检查项 |
|-------|------|---------|----------------|
| ① 语法共存性 | 同一函数同时标注 `@OverflowWrapping` 和 `const` 修饰符语法合法 | 低 | 项⑫（`@OverflowWrapping`+`const` 语法共存性原型测试） |
| ② 语义兼容性 | `@OverflowWrapping` 在 `const` 函数体内的 wrapping 行为与非 `const` 函数一致 | **中** | 项⑫（统一集成验证中的溢出行为对照） |
| ③ 包级独立函数的 `const`+`@OverflowWrapping` 共存 | 包级独立函数上两者共存语法合法 | 低 | 与依赖①合并验证 |

**验证建议**：将依赖①和②合并为单一原型测试函数，在 `const` 和 `non-const` 两种上下文中对比 `@OverflowWrapping` 标注的算术运算行为。若验证通过，则包级独立函数的 `const`+`@OverflowWrapping` 方案（§4.3）可行。若验证不通过，则所有 `const` 算术包级函数降级为非 `const` 版本，同时保持 `@OverflowWrapping` 标注——功能正确性不受损，仅失去编译期调用能力。扩展成员函数（`add(v,s)` 等）因声明为非 `const`，不受此验证约束。

### `@Derive[Hashable]` 在泛型 struct 上的约束传递性验证

`@Derive[Hashable]` 在泛型 struct `VecN<T, Q>` 上的自动派生依赖类型参数 `T` 的 `Hashable` 约束传递性（§3.2）。验证方法详见 §2.1 项㉑。

| 依赖项 | 描述 | 风险等级 | 对应 §2.1 检查项 |
|-------|------|---------|----------------|
| ① `@Derive[Hashable]` 在 `VecN<T, Q>` 上的编译通过性 | 对已实现 `Hashable` 的类型自动派生通过 | **低** | 项㉑ |
| ② `HashSet<VecN<T, Q>>` 的编译通过性 | 编译通过 | 低 | 项㉑（HashSet 编译测试） |
| ③ 非 `Hashable` 类型参数的编译错误预期 | 非 `Hashable` `T` 产生编译错误 | **低** | 项㉑（负向测试） |

### `Bool→Numeric` 转换路径 B 可行性验证

`as` 运算符运行时类型检测方案（§9.4 路径 B）依赖以下编译器行为的兼容性，须在编码阶段完成验证（验证方法详见 §2.1 项⑧）：

| 依赖项 | 描述 | 风险等级 | 对应 §2.1 检查项 |
|-------|------|---------|----------------|
| ① `v.x as Bool` 对泛型 `T2` 的编译通过性 | `as` 运算符在泛型函数体中编译通过 | **中** | 项⑧（`as` 方案可行性验证） |
| ② `if-let` 模式匹配在非 `const` `init` 中的可用性 | 模式匹配语法合法 | 低 | 项⑧（if-let 可用性验证） |
| ③ `T(1)`/`T(0)` 常量构造器对泛型 `T` 的合法性 | 数值类型和 `Bool` 均支持 `T(1)`/`T(0)` | **低** | 项⑧（`T(1)`/`T(0)` 构造验证） |

**验证时序**：三项依赖按①→②→③顺序串行验证。若①或②失败 → 路径 B 标记为"不可行"。若③失败 → `T(1)`/`T(0)` 无法用于泛型构造，路径 B 降级为使用具体类型的直接字面量赋值（仅在 `T` 可推断时可行）或回退至路径 A（具名工厂函数）。

**验证通过标准**：三项依赖均编译通过，且运行时行为正确（`Bool(true)` → `T(1)`，`Bool(false)` → `T(0)`，非 `Bool` 类型原样转换）。

### Vec4 构造函数重载解析验证

Vec4 的多重重载构造函数（约 30 个签名）在泛型参数实例化时的重载解析行为需在设计阶段通过原型测试验证。验证方法详见 §2.1 项⑬。

| 依赖项 | 描述 | 风险等级 | 对应 §2.1 检查项 |
|-------|------|---------|----------------|
| ① `Vec3 + T` vs `Vec3 + Vec1` 歧义验证 | 对偶重载分支的歧义检测 | **高** — 涉及 6 组重载对 | 项⑬ |
| ② 多元 Vec1 参数组合内部歧义验证 | 同参数数量不同参数位置的组合内部歧义 | **中** | 项⑬（全扫描验证） |
| ③ "具体类型优先于泛型类型"规则验证 | 非泛型版本优先匹配期望 | **中** | 项⑬ |

**回退方案**：若依赖①或②发现歧义，按 §4.1 定义的回退方案执行——移除 4-6 个歧义分支（保留更常用的一侧），将移除的分支改为具名工厂函数 `fromVec3AndVec1` 等。若依赖③发现编译器行为与预期不符（泛型版本优先于非泛型版本），则所有 fill-from-Vec1 非泛型构造函数均需改为具名工厂函数形式。

**验证时序**：三项依赖在 §2.1 检查清单第⑬项中统一验证。验证通过后，构造函数体系按当前设计编码；验证失败则按回退方案修改 Vec4 构造函数清单后重新编译验证。**系统性扫描策略**：除了 §2.1 检查清单第⑬项的原型验证外，编码阶段完成 Vec4 实现后，须新增 Vec4 构造函数全量编译验证项——对每个构造函数签名各构造一个实例验证编译通过，确保无遗漏歧义。

### 生成脚本验证依赖

`fwd.cj` 采用外部脚本自动生成 256 个别名，脚本本身的正确性和同步性需要单独验证。验证方法详见 §2.1 项⑪。

| 依赖项 | 描述 | 风险等级 | 对应 §2.1 检查项 |
|-------|------|---------|----------------|
| ① 脚本生成产物与命名约定一致性 | 256 个别名必须严格遵循 §3.8 命名约定 | **中** | 项⑪（抽查 4 个家族各一个精度变体） |
| ② 脚本与 `fwd.cj` 持续一致性 | 手动修改后脚本未同步导致覆盖 | **低** | 项⑪（CI 一致性检查） |
| ③ 脚本映射表错误 | 类型名映射错误 | **低** | 项⑪（重点验证 `Vec`/`DVec`/`FVec` 命名正确性） |
...
| §11.5 浮点容差比较与 C++ GLM 精确比较的全量行为偏离[^1] | 0（回退路径下为 0） | 0.5 | 1.0 |
| §11.6 `@OverflowWrapping` 回退成本 | 0（继承性成立） | 0.5（补充标注） | 1.0（补充标注+修改委托） |
| §11.7 别名命名约定偏离 | 0.5 | 1.0 | 2.0 |
| §11.7a Vec1 广播运算迁移 | 0（语法一致） | 0.1 | 0.5（跨类型广播复合赋值人工审查） |
| §11.7b 跨类型复合赋值迁移模式 | 0.3 | 0.75 | 1.5 |
| §11.7c Bool→Numeric 构造降级迁移模式 | 0.3 | 0.5 | 0.8 |
| §11.8 跨 Q 赋值迁移模式 | 1.0 | 2.0 | 3.0 |
| §11.9 `equalExact` 迁移模式 | 0.5 | 1.0 | 2.0 |
| §11.10 标量在左运算替换 | 1.0 | 1.5 | 2.5 |
| §11.11 `fwd.cj` 脚本开发与验证 | 0.5 | 0.75 | 1.0 |
| §11.13（新增）编译器依赖验证回退成本 | 0（验证通过） | 1.0 | 3.0 |
| §11.15 Bool Vec 相关 API 迁移模式 | 0.1 | 0.15 | 0.2 |
| 测试运行器配置验证 | 0.1 | 0.1 | 0.2 |
| B2 const/extend 不对称审查工作量 | 0.3 | 0.5 | 1.0 |
| **合计** | **6.0** | **12.65** | **23.5** |

**说明**：
- `[^1]`：§11.5 与 §11.9 非累加关系，实际以 §11.9 为准。§11.5 已从"Inf==Inf"单一差异扩展为"浮点容差比较与 C++ GLM 精确比较的全量行为偏离"，覆盖所有浮点 Vec `==` 使用场景，与 §11.9 的迁移范围一致。两项评估不可简单求和，实施者按 §11.9 执行一次即可覆盖两份工作。
- 乐观合计（~6 人天）：所有编译器行为假设均通过验证，无回退路径触发。
- 最可能合计（~13 人天）：D30 标注继承性回退触发 + 别名命名需适度人工审查 + 跨 Q 部分场景需人工确认 + 浮点 `==` 全量审查（§11.9 扩大范围）。
- 悲观合计（~24 人天）：D29 const 表达式链回退 + D30 标注继承性回退 + 编译器依赖验证多项回退 + 大面积别名人工审查 + 跨类型复合赋值人工审查 + 浮点 `==` 全量替换。
...
| 验证依赖 | §2.1 检查项 | 验证通过→选择路径 | 验证失败→回退路径 | 回退决策章节 | 修改触发联动检查清单 |
|---------|-----------|-----------------|----------------|------------|---------------------|
| `isIec559Of<T>()` — `T(0)` const 构造 + `is` 运算符 | 项①③ | **路径 A**：`ComputeEqual.callConst` 使用编译期 `if` 容差比较路径 | **整体回退精确比较**：`==` 退至全类型精确 `a == b`，浮点容差由 `equalEpsilon` 提供 | D29、§3.5 备选路径 | — |
| `abs()`/`epsilon()` const 兼容性 | 项③⑫ | **保留容差路径**：`abs(a-b) <= epsilonOf<T>()` 在 const 函数体中可用 | **局部回退**：移出容差比较，由非 const 的 `equalEpsilon` 承载；若 `epsilonOf<T>()` 不可用，走 §3.5 路径 B（固定容差内联） | D29、§3.5 | — |
| 编译期 `if` 分支抑制（依赖点 A~E） | 项④⑫ | **维持双路径**：浮点/整数路径通过分支抑制安全分离 | **单路径回退**：各依赖点按独立方案（详见 §2.1 项④⑫ 回退说明），`mod` 拆分 `modInt`/`modFloat` | §10 回退表 | — |
| `@OverflowWrapping` 标注继承性 | 项⑤⑫ | **当前设计**：二元运算符标注，复合赋值自动继承 | **备选方案**：20 个已有扩展成员函数上补充标注，运算符委托给标注函数 | D30、§4.6 | — |
| 浮点 `mod` 恒等式正确性 | 项⑥ | **双路径 mod**：整数 `%` + 浮点 `trunc` 恒等式 | **拆分方案**：`modInt`/`modFloat` 非 const 具名函数 | §10 mod 回退 | — |
| Vec1 跨类型 fill-from-Vec1 重载冲突 | 项⑦⑳ | **维持当前**：仅同类型 fill-from-Vec1 版本 | **移除跨类型**：仅保留同类型版本，跨类型通过其他构造组合覆盖 | D31 | — |
| Bool→Numeric 转换路径 B — `as` 方案可行性 | 项⑧ | **路径 B（可选优化）**：在 `init<T2,Q2>` 中内联运行时类型检测 | **回退路径 A**：使用 `fromBoolVec`/`fromBoolVecQ2` 包级独立工厂函数 | §9.4 路径选择、§4.8 | — |
| `@Derive[Hashable]` 约束检查策略 | 项㉑ | **延迟检查**：定义处不要求 `T <: Hashable` | **定义处检查**：添加 `where T <: Hashable` 约束或移除 `@Derive[Hashable]` 改手动实现 | §3.2 Hashable 策略 | — |
| Vec4 构造函数重载解析 | 项⑬ | **当前 30 个签名**：编译器正确区分所有对偶分支 | **移除 4-6 个歧义分支**：改为具名工厂函数（`fromVec3AndVec1` 等） | §4.1 回退方案 | — |
| `const`+`@OverflowWrapping` 语法共存 | 项⑫ | **维持 `const` 声明**：包级独立函数保留 `const` 修饰符 | **降级非 const**：包级独立函数改为非 `const`，保持 `@OverflowWrapping` 标注 | §10 @OverflowWrapping 与 const 共存验证 | — |
| epsilonOf 兼容性策略（§3.5 路径 A vs 路径 B） | 项⑲⑫ | **依赖项⑲+分支抑制验证**：若 `Int32(1e-6)` 编译错误但分支抑制有效→路径 A 安全；若分支抑制对泛型约束无效→路径 B | 路径 A：移除 epsilonOf `Number<T>` 约束，返回 `T(1e-6)`；路径 B：完全移除 epsilonOf，内联固定容差到 equalEpsilon 和 callConst | §3.5 双路径策略 | **路径 A 触发时**：修改 `shim_limits.cj` 中 `epsilonOf<T>()` 定义（移除 `Number<T>` 约束，改为 `T(1e-6)`）→ `equalEpsilon` 无需联动修改；`ComputeEqual.callConst` 浮点分支保持 `abs(a-b) <= epsilonOf<T>()` 不变。**路径 B 触发时**：修改 `shim_limits.cj` 移除 `epsilonOf<T>()` 函数定义 → 修改 Vec1~Vec4 的 `equalEpsilon` 定义（将 `epsilonOf<T>()` 引用替换为 `T(1e-6)` 内联固定容差，共 4 处） → 修改 `ComputeEqual.callConst` 浮点分支（将 `epsilonOf<T>()` 引用替换为 `T(1e-6)`，共 1 处）。路径 B 合计联动修改 6 处（1 移除 + 4 替换 + 1 替换） |

**使用说明**：
1. 编码前按 §2.1 检查清单顺序逐项验证（建议优先验证项①③④⑤⑫）。
2. 每项验证完成后查阅本表确定路径选择。
3. 若验证通过，按"验证通过→选择路径"列实施；若失败，按"回退路径"列修改对应设计章节。
4. 回退修改完成后在 §12.2 验收标准中补充对应回退路径的验收项。
...
> **验证范围声明**：本附录的 API 逐项覆盖对照范围限定为 **type_vec{1,2,3,4}.hpp/.inl 四个 Vec 类型文件**。首轮 11 个核心文件中其余 7 个文件（`setup.hpp`、`qualifier.hpp`、`compute_vector_relational.hpp`、`compute_vector_decl.hpp`、`_vectorize.hpp`、`fwd.hpp` 及其 shim 依赖）的 API 覆盖状态已在 §8.2 范围可追溯性对照表中以 roadmap 子范围粒度完成映射，并在 §8.5 排除清单中逐项记录未覆盖设施的排除理由。本附录不做其余 7 个文件的逐 API 级对照——如需逐项审查，可在编码阶段以本附录为模板补充。
...
| 完全泛型双参数构造 | `template<A,B> vec1(A _x, B _y)` | ✗ | 无对应版本 | C++ 中 Vec1 接受 2 参数属广义构造扩展，仓颉 Vec1 仅 1 分量，此构造语义上应构造 Vec2；此类跨维度泛型构造在仓颉中因运算符重载不能泛型而不可行。已纳入 §8.5 |
...
| 跨类型双参数构造 | `template<A,B> vec2(A _x, B _y)` | ✗ | 无对应版本 | C++ 中 A/B 可独立推导为不同类型，仓颉构造函数不能为泛型（运算符重载限制），此模式仅可通过 `init(scalar: T)` 同类型双参数部分覆盖。已纳入 §8.5 |
...
| C++ 成员/运算符 | 覆盖状态 | 备注 |
|---------------|---------|------|
| 跨类型完全泛型多元构造 `template<A,B,C> vec3(A,B,C)` | ✗ | 原因同 Vec2 泛型参数限制，已纳入 §8.5 |
...
| C++ 成员/运算符 | 覆盖状态 | 备注 |
|---------------|---------|------|
| 跨类型完全泛型多元构造 `template<A,B,C,D> vec4(A,B,C,D)` | ✗ | 原因同 Vec2 泛型参数限制，已纳入 §8.5 |
...
| C++ API | 对应 .inl 文件 | 未覆盖原因 | 处理策略 |
|---------|--------------|-----------|---------|
| `template<A,B> vec1(A,B)` | type_vec1.inl | 泛型多元构造在仓颉中不可行 | 纳入 §8.5 排除清单 |
| `template<A,B> vec2(A,B)` | type_vec2.inl | 同上 | 纳入 §8.5 排除清单 |
| `template<A,B,C> vec3(A,B,C)` | type_vec3.inl | 同上 | 纳入 §8.5 排除清单 |
| `template<A,B,C,D> vec4(A,B,C,D)` | type_vec4.inl | 同上 | 纳入 §8.5 排除清单 |

**验证结论**：除以上 4 项完全泛型多元构造函数（各分量类型可独立推导）因仓颉泛型构造函数限制不可实现外，type_vec1.inl~type_vec4.inl 所有公开 API 均已覆盖。未覆盖 API 的处理策略为"永久排除（仓颉语言限制）"，已纳入 §8.5 排除清单。
