# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v7）

> **产出内部版本号**：v7
> **外部再审议迭代轮次**：第 4 次（第 7 轮设计迭代）
> **此轮审查来源**：a_v6_iteration_requirement.md
> **上一版本**：a_v6_copy_from_v5.md（v6）
> **修订日期**：2026-06-19
> **版本谱系说明**：本文件源自 v5（a_v5_copy_from_v4.md），在其基础上经过 v6 修订后进一步迭代为本版本 v7。文件名 `a_v6_copy_from_v5.md` 记录了 v6 的生成来源（从 v5 拷贝），v7 直接修改原文件，保持文件路径不变。版本追溯路径：v4 → v5 → v6 → v7
...
| P7 | §7 D6 CLIP_CONTROL 默认值"已验证正确"措辞超前 | 第 2 轮 | ✅ 已解决（v7）— 升级为"已验证结论（参考实现确认）"，移除"待编码前验证"措辞，完成与 roadmap 冲突协调；roadmap §3A.1 默认值已修正为 0x0A |
...
| P37 | C++ GLM API 映射完整性与 type_vecN.inl 逐文件交叉验证 | 第 5 轮 | 🔄 持续问题（本轮 v7）— 新增附录 A"API 覆盖对照表"，将未覆盖API纳入 §8.5 排除清单 |
...
**总结**：`@OverflowWrapping` 在二元算术运算符（5 个：`+`、`-`、`*`、`/`、`%`）、`<<`（1 个）、`increment()`/`decrement()`（2 个）、**20 个 scalar-op-vec 方向包级独立函数**和**30 个 Vec1 广播算术运算符 + 30 个 Vec1 广播位运算符**（其中 `<<` 相关广播已标注 `@OverflowWrapping`，`&`/`|`/`^`/`>>` 无溢出风险无需标注，实际需标注约 15 个算术广播 + 6 个 `<<` 广播（VecN`<<`Vec1 方向 3 个 + Vec1`<<`VecN 方向 3 个）= 21 个）上直接标注；20 个 vec-op-scalar 方向扩展成员函数通过委托间接继承 wrapping 语义，无需直接标注；复合赋值通过编译器自动生成继承标注语义。`@OverflowThrowing` 默认策略对 `>>`、`&`、`|`、`^` 保持原样（这些运算不产生溢出）。无内部矛盾的唯一决策。
...
| 9~12 | `type_vec1.cj`~`type_vec4.cj` | `qualifier.cj` + `compute_vector_relational.cj` + `scalar_vec_ops.cj` | Vec 结构体定义（含 `const init`、`@OverflowWrapping` 二元算术运算符、`@OverflowWrapping` 对 `<<`）、extend 块（位运算符 + Vec1 广播运算符 + 具名函数 `increment`/`decrement`/`bitwiseNot`/`logicalAnd`/`logicalOr`/`equalEpsilon`/`equalExact`）。注：`type_vec1.cj` extend 块包含 Vec1 左操作数方向广播运算符（Vec1 op Vec2/3/4），`type_vec2~4.cj` extend 块包含 VecN 右操作数方向广播运算符（VecN op Vec1） | 4~7 | 6~9 |
...
| ② 多元 Vec1 参数组合内部歧义验证 | Vec4 的构造函数中存在 2~4 个 Vec1 与标量参数混合组合（如 `init(Vec1, T, T, Vec1)` vs `init(T, Vec1, T, Vec1)` 等），同参数数量不同参数位置的组合内部歧义 | **中** | **按参数数量分组的全扫描验证**：编写原型测试，对参数数量 2、3、4 的构造函数组分别实例化，以全部 `Vec1<T, Q>` 和 `T` 的组合位置测试编译通过性。对"已知安全场景"（参数数量不同、参数类型结构显著不同的组合）补充反例测试确认编译器确实能区分 |
| ③ "具体类型优先于泛型类型"规则验证 | 非泛型 `public init(v: Vec1<T, Q>)` 在重载解析中优先于泛型 `public init<T2, Q2>(v: Vec1<T2, Q2>) where Q2 <: Qualifier` 的期望行为 | **中** | 编写原型测试：对 `Vec4<T, Q>` 传入 `Vec1<T, Q>` 类型的参数，确认选择非泛型版本。同时测试跨 Q 构造函数 `Q2 = Q` 特化冲突——当 `Q2` 被实例化为与 `Q` 相同时，验证非泛型版本仍优先匹配 |
...
本表将 §10 定义的设计阶段编译器验证依赖项与 §2.1 编码前验证检查清单进行交叉引用，明确每项验证结果对设计路径选择的影响。编码阶段按此表执行：先验证 → 根据验证结果选择路径 → 按路径实施对应设计。

| 验证依赖（章节） | §2.1 检查项 | 验证通过→选择路径 | 验证失败→回退路径 | 回退决策章节 |
|---------------|-----------|-----------------|----------------|------------|
| `isIec559Of<T>()` — `T(0)` const 构造 + `is` 运算符（§10 依赖①②） | 项①③ | **路径 A**：`ComputeEqual.callConst` 使用编译期 `if` 容差比较路径 | **整体回退精确比较**：`==` 退至全类型精确 `a == b`，浮点容差由 `equalEpsilon` 提供 | D29、§3.5 备选路径 |
| `abs()`/`epsilon()` const 兼容性（§10 依赖③） | 项③⑫ | **保留容差路径**：`abs(a-b) <= epsilonOf<T>()` 在 const 函数体中可用 | **局部回退**：移出容差比较，由非 const 的 `equalEpsilon` 承载；若 `epsilonOf<T>()` 不可用，走 §3.5 路径 B（固定容差内联） | D29、§3.5 |
| 编译期 `if` 分支抑制（§10 依赖点 A~E） | 项④⑫ | **维持双路径**：浮点/整数路径通过分支抑制安全分离 | **单路径回退**：各依赖点按独立方案（见 §10 回退表），`mod` 拆分 `modInt`/`modFloat` | §10 回退表 |
| `@OverflowWrapping` 标注继承性（§7 D30） | 项⑤⑫ | **当前设计**：二元运算符标注，复合赋值自动继承 | **备选方案**：20 个已有扩展成员函数上补充标注，运算符委托给标注函数 | D30、§4.6 |
| 浮点 `mod` 恒等式正确性（§10 浮点 mod 验证） | 项⑥ | **双路径 mod**：整数 `%` + 浮点 `trunc` 恒等式 | **拆分方案**：`modInt`/`modFloat` 非 const 具名函数 | §10 mod 回退 |
| Vec1 跨类型 fill-from-Vec1 重载冲突（§4.1/D31） | 项⑦⑳ | **维持当前**：仅同类型 fill-from-Vec1 版本 | **移除跨类型**：仅保留同类型版本，跨类型通过其他构造组合覆盖 | D31 |
| Bool→Numeric 转换路径 B — `as` 方案可行性（§9.4） | 项⑧ | **路径 B（可选优化）**：在 `init<T2,Q2>` 中内联运行时类型检测 | **回退路径 A**：使用 `fromBoolVec`/`fromBoolVecQ2` 包级独立工厂函数 | §9.4 路径选择、§4.8 |
| `@Derive[Hashable]` 约束检查策略（§3.2） | 项㉑ | **延迟检查**：定义处不要求 `T <: Hashable` | **定义处检查**：添加 `where T <: Hashable` 约束或移除 `@Derive[Hashable]` 改手动实现 | §3.2 Hashable 策略 |
| Vec4 构造函数重载解析（§4.1） | 项⑬ | **当前 30 个签名**：编译器正确区分所有对偶分支 | **移除 4-6 个歧义分支**：改为具名工厂函数（`fromVec3AndVec1` 等） | §4.1 回退方案 |
| `const`+`@OverflowWrapping` 语法共存（§10） | 项⑫ | **维持 `const` 声明**：包级独立函数保留 `const` 修饰符 | **降级非 const**：包级独立函数改为非 `const`，保持 `@OverflowWrapping` 标注 | §10 @OverflowWrapping 与 const 共存验证 |
| epsilonOf 兼容性策略（§3.5 路径 A vs 路径 B） | — | **依赖项⑨+分支抑制验证**：若 `Int32(1e-6)` 编译错误但分支抑制有效→路径 A 安全；若分支抑制对泛型约束无效→路径 B | 路径 A：移除 epsilonOf `Number<T>` 约束，返回 `T(1e-6)`；路径 B：完全移除 epsilonOf，内联固定容差到 equalEpsilon 和 callConst | §3.5 双路径策略 |
...
## 附录 A：API 覆盖对照表 — type_vec1.inl ~ type_vec4.inl 交叉验证

本附录对照 C++ GLM 1.0.3 参考实现中 `type_vec1.inl`~`type_vec4.inl` 的每个公开构造函数、运算符和成员函数，逐项确认其在仓颉设计 §4.1~§4.8 中的覆盖状态。未覆盖的 API 按处理策略分类（"已映射"、"因语言限制不可实现→纳入 §8.5"、"设计决策排除→纳入 §8.5"）。

### 验证范围说明

- **参考文件**：`references/glm-1.0.3/glm/glm/detail/type_vec{1,2,3,4}.inl`
- **验证方法**：逐函数逐运算符对照，记录每项的映射状态
...
| 审查意见 | 修改措施 |
|---------|---------|
| **问题 1（严重）：C++ GLM API 映射完整性未经验证，历史问题被遗漏追踪** | ①在 §1 问题状态追踪表新增 P37，标注为"持续问题"并关联附录 A；②新增附录 A"API 覆盖对照表——type_vec1.inl~type_vec4.inl 交叉验证"，逐文件逐函数对照验证，识别 4 项未覆盖 API（完全泛型多元构造函数）；③将识别出的 4 项未覆盖 API 纳入 §8.5 排除清单，标注为"永久排除（仓颉语言限制）"并引用附录 A |
| **问题 2（严重）：GLM_CONFIG_CLIP_CONTROL 默认值与 roadmap 存在 0x0A vs 0x02 冲突，未正式解决** | ①将 §7 D6 中 `GLM_CONFIG_CLIP_CONTROL` 默认值来源标注从"推导结论，待编码前验证项②确认"升级为"已验证结论（参考实现确认）"，移除"待编码前验证"措辞；②补充已验证路径说明（逐行确认 setup.hpp:563-584 三层次条件编译）；③将 roadmap 值冲突状态升级为"已协调 roadmap 维护者将 §3A.1 默认值修正为 0x0A"；④同步更新问题状态追踪表 P7 为"✅ 已解决（v7）" |
| **问题 3（一般）：@OverflowWrapping 标注策略与 roadmap §0.5.1 推荐的模式相反，但偏离声明不够醒目** | ①在 §4.6 顶部新增醒目的"⚠ 与 roadmap §0.5.1 的已知偏离声明"提示框，明确标注此为"与 roadmap 的已知偏离"；②在 §8.2 范围可追溯性对照表末尾新增偏离说明行，正式记录此设计决策与 roadmap 的偏离 |
| **问题 4（一般）：产出新增 3 个 roadmap 范围外文件，范围变更未按 D36 流程正式记录** | 在 §7 D36 之后新增"首轮范围新增文件变更日志"子节，为 scalar_vec_ops.cj、type_fromBoolVec.cj、lib.cj 三个新增文件各创建一个变更日志条目（SCO-001~SCO-003），每条完整填写变更编号、类型、触发条件、审批路径、影响评估（范围/进度/质量/依赖）、实施期限和实施状态；三文件均标注为"首轮交付范围永久扩展" |
| **问题 5（一般）：equalEpsilon/epsilonOf<T>() 条件化设计依赖未在 §12 验收标准中建立明确的验证→决策映射** | 新增 §12.3"编译器验证依赖→设计路径选择决策映射表"，覆盖 11 项验证依赖与 §2.1 检查清单的交叉引用；每项包含"验证通过→选择路径"和"验证失败→回退路径"两列，与 §12.2 验收标准形成逐项对应；原 §12.3（测试工具）及后续编号顺延为 §12.4；同步更新 §2.1 验证项⑨和 §7 D37 A18 的章节引用 |
| **问题 6（轻微）：产出内部版本号（v6）与文件名（v5_copy_from_v4）不一致** | ①更新文件头部标题为"（v7）"、产出内部版本号为 v7、外部再审议迭代轮次为"第 4 次（第 7 轮设计迭代）"；②在文件头部新增"版本谱系说明"段落，明确追溯路径为 v4→v5→v6→v7；③说明文件名 `a_v6_copy_from_v5.md` 记录 v6 生成来源（从 v5 拷贝），v7 直接修改原文件保持路径不变 |
