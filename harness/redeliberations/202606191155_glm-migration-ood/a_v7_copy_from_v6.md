...
| P7 | §7 D6 CLIP_CONTROL 默认值"已验证正确"措辞超前 | 第 2 轮 | 🔄 持续问题（本轮 v7）— 已验证 0x0A 正确（`setup.hpp:563-584` 条件编译逻辑确认），但 roadmap §3A.1 仍记录为 0x02，两文件未同步。须在本轮落地：同步更新 roadmap §3A.1 默认值为 0x0A，然后在 §1 状态追踪表中将此条标记为"✅ 已解决" |
...
| 验证依赖（章节） | §2.1 检查项 | 验证通过→选择路径 | 验证失败→回退路径 | 回退决策章节 | 修改触发联动检查清单 |
|---------------|-----------|-----------------|----------------|------------|---------------------|
...
| epsilonOf 兼容性策略（§3.5 路径 A vs 路径 B） | — | **依赖项⑨+分支抑制验证**：若 `Int32(1e-6)` 编译错误但分支抑制有效→路径 A 安全；若分支抑制对泛型约束无效→路径 B | 路径 A：移除 epsilonOf `Number<T>` 约束，返回 `T(1e-6)`；路径 B：完全移除 epsilonOf，内联固定容差到 equalEpsilon 和 callConst | §3.5 双路径策略 | **路径 A 触发时**：修改 `shim_limits.cj` 中 `epsilonOf<T>()` 定义（移除 `Number<T>` 约束，改为 `T(1e-6)`）→ `equalEpsilon` 无需联动修改；`ComputeEqual.callConst` 浮点分支保持 `abs(a-b) <= epsilonOf<T>()` 不变。**路径 B 触发时**：修改 `shim_limits.cj` 移除 `epsilonOf<T>()` 函数定义 → 修改 Vec1~Vec4 的 `equalEpsilon` 定义（将 `epsilonOf<T>()` 引用替换为 `T(1e-6)` 内联固定容差，共 4 处） → 修改 `ComputeEqual.callConst` 浮点分支（将 `epsilonOf<T>()` 引用替换为 `T(1e-6)`，共 1 处）。路径 B 合计联动修改 6 处（1 移除 + 4 替换 + 1 替换） |
