# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v11）

> **产出内部版本号**：v11
> **外部再审议迭代轮次**：第 11 次（第 11 轮设计迭代）
> **此轮审查来源**：a_v11_iteration_requirement.md
> **上一版本**：a_v10_copy_from_v9.md（v10）
> **修订日期**：2026-06-19
> **版本谱系说明**：本文件源自 v10（a_v10_copy_from_v9.md），在其基础上经过 v10 修订后进一步迭代为本版本 v11。版本追溯路径：v4 → v5 → v6 → v7 → v8 → v9 → v10 → v11
...
> | 目录结构模板 | §2 初始目录结构 |
> | 编码前必须验证的编译器假设清单 | §2.1 编码前验证检查清单（24 项，含验证方法/通过标准/回退方案） |
...
> | 验收标准 | §12.2 验收标准 |
>
> 编码阶段建议：先在 §2.1 完成全部 24 项编码前验证，再按 §8.3 迁移顺序分阶段编码。每完成一个阶段后运行 `cjpm build` 增量验证。
...
| P37 | C++ GLM API 映射完整性与 type_vecN.inl 逐文件交叉验证 | 第 5 轮 | 🟡 已确认覆盖风险（本轮 v11）— 附录 A 验证范围限定为 type_vec{1,2,3,4}.hpp/.inl 四个 Vec 类型文件（即 4/11 个核心文件已完成逐 API 级对照，覆盖状态：✅ 全部已映射或已排除）。其余 7 个核心文件的覆盖风险等级如下：① **setup.hpp** — 中（1000+ 行平台/编译器检测宏首轮不迁移，18 个 GLM_CONFIG_* 常量已显式映射，依赖已知可替代）；② **qualifier.hpp** — 中（storage/gentypeTrait 等已排除至后续轮次，Qualifier 接口及实现类型已映射，未发现未覆盖依赖）；③ **compute_vector_relational.hpp** — 低（简化版 ComputeEqual<T> 已映射，无未覆盖 API）；④ **compute_vector_decl.hpp** — 低（compute_vec_* 策略结构体已映射，首轮仅定义不消费）；⑤ **_vectorize.hpp** — 低（Functor 体系已按分量数拆分映射）；⑥ **fwd.hpp** — 低（标量别名 + 256 向量别名已全量映射，映射表已确认）；⑦ **shim 文件** — 低（assert/limits/cstddef 替代已全量映射）。整体评估：无高等级风险文件，编码启动前对 7 个文件做一次快速扫描确认无遗漏移植依赖项即可进入编码。其余 7 个文件的逐 API 级对照规划在第 2 轮编码阶段以本附录为模板补充完成 |
...
> |------|---------|--------------------|--------------------|-------------|
> | **X — 可直接编码** | 24 项验证项全部通过 | ✓ 按 §8.3 迁移顺序直接编码，无需设计修订 | — | — |
> | **Y — 可基于回退方案编码** | 1~3 项回退触发（较大概率） | — | ✓ 按各回退方案（§2.1 回退方案列 + §11.13 叠加效应分析）局部修订后编码，无需停工 | 回退方案间无不可调和冲突（§11.13 已分析全部已知组合） |
> | **Z — 需停工重评估** | 3 项以上核心假设同时回退 | — | — | ①②③⑫⑮ 五项中任意三项同时失败 → 触发停工评估流程（主 Agent + 设计审议 + 项目负责人），预计 1-2 周重新评估路径 |
> 
> **当前评估**：本设计在 24 项验证全部通过时可直接编码；1~3 项部分回退时可基于 §2.1 回退方案和 §11.13 叠加效应分析指导编码；仅当 3 项以上核心假设同时失败时才需停工重评估。编码阶段建议优先执行第①④⑤⑫项以尽早暴露最高风险项。
...
> **验证时序建议**：建议优先按以下顺序执行验证：（1）**第⑮项（编译器版本确认）**——确认 cjc 版本 ≥ 1.0.5，为后续所有验证提供编译器基础；（2）**第⑫项（统一集成验证）**——在版本确认后立即执行，一次性覆盖编译期 `if` 分支抑制（依赖点 A~E）、`@OverflowWrapping` 继承性和 `const`+`@OverflowWrapping` 语法共存性三项核心编译器行为假设的集成验证，尽早暴露最高风险项。第⑫项的验证结果决定 D29/D30 和多项编译器依赖的走向——若统一集成验证通过，后续验证项可顺序推进；若失败，按 §2.1 回退方案逐项评估。第⑫项通过后，再按编号顺序（①→②→③→④→⑤→⑥→⑦→⑧→⑨→⑩→⑪→⑬→⑭→⑯→⑰→⑱→⑲→⑳→㉑→㉒→㉓→㉔）依次验证其余各项。
...
以上 24 项验证项按其失败时影响的设计章节数量分为三类：

| 影响范围 | 数量 | 验证项编号 | 失败时影响的设计章节数 |
|---------|------|-----------|-------------------|
| **高** — 影响 5+ 个章节，含核心语义 | 4 项 | ①③④⑫ | 5-8 章（§3.5/§4.3/§4.5/§4.6/§7 D29-D30/§10/§11.6/§11.12 等） |
| **中** — 影响 2-4 个章节，含局部修订 | 9 项 | ②⑤⑥⑧⑯⑰⑲⑳㉑ | 2-4 章（局部回退方案） |
| **低** — 影响 ≤1 个章节，独立局部 | 11 项 | ⑦⑨⑩⑪⑬⑭⑮⑱㉒㉓㉔ | 0-1 章（或无影响，仅验证确认） |

> **量化说明**：24 项验证全部通过时可直接编码；1-4 项部分回退时（以中/低影响项目为主）可基于 §2.1 回退方案局部修订后继续编码，无需停工；仅当高影响项（①②④⑫ 等）中 2+ 项同时失败时才需触发停工重评估流程。当前设计确定性声明见 §1 "设计确定性声明"框。
...
| 验证项 | 描述 | 风险等级 | 对应 §2.1 检查项 |
|-------|------|---------|----------------|
| ① 浮点 `mod` 边界值 | `Float32.Max % Float32(1.0)`、`Float32.MinPositive % Float32(2.0)`、`0.0 % Float32(1.0)`、`Inf % Float32(1.0)`、`NaN % Float32(1.0)` | **中** | 项⑥ — 验证方法详见 §2.1 边界值验证描述 |
| ② 负操作数符号 | `(-3.5f) % 2.0f`、`3.5f % (-2.0f)`、`(-3.5f) % (-2.0f)` | 中 | 项⑥ — 验证方法详见 §2.1 负操作数验证描述 |
...
- 乐观合计（~5.9 人天）：所有编译器行为假设均通过验证，无回退路径触发。
