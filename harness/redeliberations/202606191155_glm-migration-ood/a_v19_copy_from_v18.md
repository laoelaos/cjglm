# GLM 1.0.3 仓颉迁移首轮 OOD 设计方案（v19）

> **产出内部版本号**：v19
> **外部再审议迭代轮次**：第 19 次（第 19 轮设计迭代）
> **此轮审查来源**：a_v19_iteration_requirement.md
> **上一版本**：a_v18_copy_from_v17.md（v18）
> **修订日期**：2026-06-19
> **版本谱系说明**：本文件源自 v18（a_v18_copy_from_v17.md），在其基础上经过 v18 修订后进一步迭代为本版本 v19。版本追溯路径从 v4 起始（v1-v3 已被 v4 重构合并，不再追溯）：v4 → v5 → v6 → v7 → v8 → v9 → v10 → v11 → v12 → v13 → v14 → v15 → v16 → v17 → v18 → v19
...
> | 目录结构模板 | §2 初始目录结构 |
> | 编码前必须验证的编译器假设清单 | §2.1 编码前验证检查清单（26 项，含验证方法/通过标准/回退方案） |
...
> | 验收标准 | §12.2 验收标准 |
>
> 编码阶段建议：先在 §2.1 完成全部 26 项编码前验证，再按 §8.3 迁移顺序分阶段编码。每完成一个阶段后运行 `cjpm build` 增量验证。
...
| P37 | C++ GLM  API 映射完整性与 type_vecN.inl 逐文件交叉验证 | 第 5 轮 | ✅ 部分解决——4/11 文件（type_vec1~4）已完成逐 API 级对照；7 个非 Vec 文件已提供核查清单，逐 API 级对照推迟至编码阶段。§1 P37 下方新增完整核查清单表格（每文件 3~5 个核查要点），§8.3 阶段 0 同步更新执行人描述 |
...
| `v.decrement()` | 算术：`--` 替代 | 改用 `v - T(1)`（若二元运算符 `-` 可声明为 `const`）或改用 `v + (-T(1))`（通过加法实现递减效果） |
...
> **`[^compiler-dep]` 编译器推断依赖**：`fromBoolVecQ2` 函数体中的 `Vec1(...)` 调用未显式标注类型参数 `<T, Q>`，依赖仓颉编译器从函数返回值类型 `Vec1<T, Q>` 推断 `Vec1` 构造函数的泛型实参。此推断行为在当前设计阶段**未被验证覆盖**——编码前须编写原型验证确认编译器是否支持从返回值类型推断 struct 构造函数的泛型参数。若验证失败（编译器不支持此推断），须在所有 `fromBoolVecQ2`（Vec1~Vec4）的 `VecN(...)` 调用处显式标注类型参数，修改后的完整函数体如下（覆盖 Vec1~Vec4 全部 4 个跨 Q 版本）：
> ```
> // Vec1 — 跨 Q 版本（显式标注类型参数 <T, Q> 后的完整函数体）
> public func fromBoolVecQ2<T, Q, Q2>(v: Vec1<Bool, Q2>): Vec1<T, Q> where Q <: Qualifier, Q2 <: Qualifier {
>     Vec1<T, Q>(if (v.x) { T(1) } else { T(0) })
> }
> 
...
> }
> 
> // Vec3 — 跨 Q 版本（显式标注类型参数 <T, Q> 后的完整函数体）
> public func fromBoolVecQ2<T, Q, Q2>(v: Vec3<Bool, Q2>): Vec3<T, Q> where Q <: Qualifier, Q2 <: Qualifier {
>     Vec3<T, Q>(if (v.x) { T(1) } else { T(0) }, if (v.y) { T(1) } else { T(0) }, if (v.z) { T(1) } else { T(0) })
> }
> 
...
> }
> ```
> 以上 4 个版本覆盖 `fromBoolVecQ2` 的所有重载版本（Vec1~Vec4），同 Q 版本 `fromBoolVec` 不受影响（无类型推断需求）。
...
| 审查意见 | 修改措施 |
|---------|---------|
| **问题 1（严重）：P37 "已解决（v16）"状态声明与实际解决程度存在实质性差距** | 将 §1 P37 状态从"✅ 已解决（v16）"修改为"✅ 部分解决——4/11 文件（type_vec1~4）已完成逐 API 级对照；7 个非 Vec 文件已提供核查清单，逐 API 级对照推迟至编码阶段"。附录 A 验证范围声明保持不变，P37 状态描述与附录 A 粒度一致 |
| **问题 2（一般）：§3.2 const 上下文 API 可用性速查表中 `decrement()` 的 const 替代方案存在语义错误** | 将 §3.2 const 速查表中 `v.decrement()` 行的 const 替代方案从"改用 `sub(T(1), v)`"修正为"改用 `v - T(1)`（若二元运算符 `-` 可声明为 `const`）或改用 `v + (-T(1))`（通过加法实现递减效果）"，消除 `sub(1, v)` 语义为 `1 - v_i` 的错误 |
| **问题 3（一般）：`@OverflowWrapping` 标注对浮点运算符代码生成的潜在影响缺乏验证覆盖** | (1) 在 §2.1 编码前验证清单新增第㉖项"`@OverflowWrapping` 浮点标注编译器行为影响验证"，编写原型测试对比标注与不标注含 NaN/Infinity 分量的浮点运算结果差异；回退方案为条件性标注（仅整数 T 标注）；(2) 同步更新 §2.1 维护原则声明（25→26 项）、验证项影响范围分类表（低影响项 11→12 项）、验证时序建议的编号序列（追加→㉖）；(3) 在 §4.6 浮点 `@OverflowWrapping` 说明段中引用项㉖作为验证方法指引 |
| **问题 4（轻微）：§7 D6 `GLM_CONFIG_CLIP_CONTROL` 常量的"已验证结论"措辞与 §8.4 验证项①的依赖关系未显式关联** | 在 §7 D6 配置常量表中 `GLM_CONFIG_CLIP_CONTROL` 行的"默认值来源已验证"和"已验证结论"两处措辞后分别添加指向 §8.4 前置验证项①的交叉引用："（详见 §8.4 前置验证项①）"和"（验证方法详见 §8.4 前置验证项①）" |
| **问题 5（轻微）：§8.3 阶段 0 "非 Vec 文件 API 快速扫描" 与 P37 的延迟对照存在执行时序矛盾** | 将 §8.3 阶段 0 的"通过标准"列从"7 项全部确认通过，每项核查要点逐条应答"修改为"7 项核查清单全部确认通过，每项核查要点逐条应答；逐 API 级对照延迟至第 2 轮编码阶段以附录 A 为模板补充"，消除退出标准与 P37 范围声明之间的措辞矛盾 |

## 修订说明（v18）

| 审查意见 | 修改措施 |
|---------|---------|
| **Q1（严重）：D30 回退方案中循环委托遗漏未被修复（自 v1 审查以来持续未被修复）** | ① §4.3 `@OverflowWrapping` 标注策略"委托实现模式示例"段落新增 ⚠ D30 回退场景反转警告脚注，说明扩展成员函数在 D30 回退时须从"委托给二元运算符"改为"直接逐分量运算"；② §4.6 vec-op-scalar 标注段新增 ⚠ D30 回退场景反转指示，注明该模式下"无需直接标注"失效，扩展成员函数须直接标注并改为直接逐分量运算；③ §7 D30 决策正文新增"循环委托防治改造步骤"子节，将扩展成员函数改造（弃委托、改直接逐分量运算）明确列为 D30 回退的必备前置步骤；④ §11.6 备选方案变更成本表新增"扩展成员函数实现改造"行项（20 处改造，约 0.3-0.5 人天），同步修正合计评估（从 1 人天提升至 1.0-1.5 人天）；⑤ §11.13 三重回退场景设计快照中"算术溢出策略"修订列和编码步骤第 2 步均更新为包含扩展成员函数改造步骤 |
| **Q2（一般）：扩展成员函数可见性依赖未纳入验收前置条件** | ① §12.2 验收标准新增"扩展成员函数 `public import` 可见性验证"作为非阻塞验收前置项，注明与 §2.1 项⑱(b) 验证结果联动；② §8.3 阶段 0 退出标准扩展为同时包含"扩展成员函数可见性验证"要求，注明失败时须在阶段 0 内采用回退方案后方可进入阶段 1；③ §8.5 排除清单末尾新增"扩展成员函数 `public import` 可见性风险记录"风险项段落，详细记录风险描述、影响评估、缓解措施和风险等级（待验证） |
| **Q3（一般）：`fromBoolVecQ2` 显式标注模板不完整** | §4.8 `[^compiler-dep]` 脚注中补充 Vec1 和 Vec3 的显式标注 `<T, Q>` 后完整函数体模板（此前仅提供 Vec2/Vec4），覆盖全部 4 个跨 Q 版本（Vec1~Vec4），同步调整提示说明文字从"Vec1/Vec3 的修改模式与 Vec2/Vec4 一致"改为列出 4 个完整模板 |

## 修订说明（v19）

| 审查意见 | 修改措施 |
|---------|---------|
| **问题 1（严重）：§1 设计确定性声明中验证项数量自相矛盾（25 vs 26）** | 将 §1 设计确定性声明表中"25 项验证项全部通过"修正为"26 项验证项全部通过"；同步更新 §1 当前评估段及 §1 编码实施快速参考表的对应描述；更新 §11.12 合计评估表中对 §2.1 验证项数量的引用 |
| **问题 2（一般）：§4.6 标注计数中 `<<` 广播版本的标注未计入** | 在 §4.6 `<<` 运算符标注点中补充 Vec1 广播方向 `<<` 的标注计数说明——VecN `<<` Vec1 方向 3 个 + Vec1 `<<` VecN 方向 3 个 = 6 处，与原有 4 处合并为共 10 处标注 |
| **问题 3（一般）：§9.4 Bool→Numeric 路径分析段尾残留编辑注释** | 移除 §9.4 类型转换边界表"Bool→数值"行路径分析段末尾的自指描述文本"首选推荐路径'的确定性表述，转为中性优先级描述。" |
| **问题 4（一般）：验证时序中独立验证项 ①③④⑤⑧ 的选择依据缺失** | 在 §2.1 验证时序建议段中补充 ①③④⑤⑧ 的选择依据说明，引用 §1 快速预筛步骤的价值分析逻辑，说明①③④覆盖最高风险项、⑤是⑫的前置依赖、⑧影响构造函数方向 |
| **问题 5（轻微）：§3.2 `const init` 与非 `const init` 共存分析中对 Vec1 的特殊情况未作说明** | 在 §3.2 共存性分析中"参数个数与参数类型均不同"后添加脚注 `[^vec1-param-count]`，说明 Vec1 的 `const init(x: T)` 与跨类型构造函数参数个数相同（均为单参数），但参数类型不同不构成歧义 |
