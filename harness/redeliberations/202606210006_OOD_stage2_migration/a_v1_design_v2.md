...
**二元运算符（标量-矩阵）**：
- `+`、`-`、`*`、`/`（左操作数为 `T`，右操作数为矩阵）：通过矩阵的 `extend` 块中接收 `T` 参数的运算符处理
...
| D06 | 标量在矩阵左侧的运算通过矩阵结构体上的非成员风格运算符处理 | 仓颉不支持非成员 `operator func`，改为在矩阵的 `extend` 块中定义接收左侧标量参数版本的运算符，与阶段一标量-向量运算的约束一致 |
...
### 3 个 stub 文件（`src/detail/`）
- `common.cj`、`matrix.cj`、`geometric.cj`
...
- `src/lib.cj`：新增 9 个矩阵类型的 `public import`
...
| 审查指出 D-08 中"标量在矩阵左侧的运算符通过非成员运算符函数实现"在仓颉中不可行——`operator func` 只能在 class/interface/struct/enum/extend 中定义 | 已采纳。将 D06 决策理由更新为：在矩阵的 `extend` 块中定义接收左侧标量参数的运算符，与阶段一标量-向量运算（`scalar_vec_ops.cj` 中的具名函数方案）的约束一致 |
