# GLM 1.0.3 仓颉迁移阶段二 OOD 设计方案（v11）
...
5. **跨类型列向量构造**（工厂函数，降级为 `fromColumns` 模式）
   - 降级为带类型转换闭包的具名工厂函数：`public static func fromColumns<V1, Q2>(conv: (V1) -> T, v1: Vec4<V1, Q2>, v2: Vec4<V1, Q2>, v3: Vec4<V1, Q2>, v4: Vec4<V1, Q2>): Mat4x4<T,Q>`（以 Mat4x4 为例，各列向量分量类型 V1 到 T 的转换通过 conv 闭包）
   - 实际编码时按矩阵列数调整参数：2 列矩阵仅需 v1, v2；3 列矩阵需 v1, v2, v3
   - **fromColumns 单一 V1 类型参数限制**：所有列向量共享同一分量类型 V1，与 C++ GLM 的 4 列独立类型参数（如 `mat4x4(vec4_a, vec4_b, ivec4_c, ivec4_d)` 中各列可独立指定类型）不同。仓颉版本中各列分量类型必须相同——这是泛型工厂函数签名的必然约束：每个类型参数须在签名中声明，而声明 4 个独立列分量类型参数将使签名膨胀且调用时类型推断困难。此差异属于仓颉与 C++ GLM 的容量差异，记录在 §9 差异声明中
...
6. **跨矩阵类型转换**（工厂函数，定义在带 `Number<T>` 约束的 extend 块中）
   ...
   **6a. 同类型跨尺寸转换**：`public static func fromMat<SrcQ>(m: Mat{C_src}x{R_src}<T, SrcQ>, one: T): Mat{C}x{R}<T,Q>`
   ...
   - 完整泛型参数展开（以 Mat4x4 ← Mat2x2 为例）：`extend<T, Q> Mat4x4<T, Q> where T <: Number<T>, Q <: Qualifier` 中的 `public static func fromMat<SrcQ>(m: Mat2x2<T, SrcQ>, one: T): Mat4x4<T,Q>`——Q/SrcQ 从实例上下文推断或显式指定，T 从 `one` 参数推断
   
   **6b. 跨类型跨尺寸转换**：`public static func fromMat<SrcT, SrcQ>(conv: (SrcT) -> T, m: Mat{C_src}x{R_src}<SrcT, SrcQ>, one: T): Mat{C}x{R}<T,Q>`
   ...
   - **填充规则（跨类型版本）**：源矩阵第 j 列第 i 行的元素 `src[j][i]`，当 0 ≤ j < min(C_src, C_dst) 且 0 ≤ i < min(R_src, R_dst) 时，经 conv 转换写入目标矩阵：`dst[j][i] = conv(src[j][i])`；当 C_dst > C_src 时，额外列从单位矩阵取值——即 (j,j) 填 `one`（= T(1)），其余填 T(0)；当 R_dst > R_src 时，额外行从单位矩阵取值——即 (i,i) 填 `one`，其余填 T(0)。T(0) 通过 `one - one` 在 `Number<T>` 上下文中演算获得
   ...
   **6a 和 6b 共享的设计属性**：
   - **精确填充规则（同类型 6a 版本）**：源矩阵第 j 列第 i 行的元素 M_src[j][i] 复制到目标矩阵的相同坐标位置（即 dst[j][i] = src[j][i]），其中 0 ≤ j < min(C_src, C_dst)、0 ≤ i < min(R_src, R_dst)。当 C_dst > C_src 时，额外列（j ≥ C_src）从单位矩阵取值——即 (j,j) 填 `one`，其余填 T(0)；当 R_dst > R_src 时，额外行（i ≥ R_src）从单位矩阵取值——即 (i,i) 填 `one`，其余填 T(0)
...
- **完整函数签名清单**：
  - `func transpose<C, R, T, Q>(m: Mat{C}x{R}<T, Q>): Mat{R}x{C}<T, Q> where T <: Number<T>, Q <: Qualifier`（每个矩阵尺寸各一版重载，共 9 个）
  ...
  - `func matrixCompMult<C, R, T, Q>(x: Mat{C}x{R}<T, Q>, y: Mat{C}x{R}<T, Q>): Mat{C}x{R}<T, Q> where T <: Number<T>, Q <: Qualifier`（每个尺寸各一版重载，共 9 个）
...
| D15 | **scalar_mat_ops.cj 与 scalar_vec_ops.cj 共同定义 add/sub/mul/div 包级函数，通过参数类型重载解析区分；scalar_mat_ops.cj 不包含 mod 函数** | 阶段一 OOD 中约定"仅 scalar_vec_ops.cj 定义 add/sub/mul/div 包级函数"。本阶段新增 scalar_mat_ops.cj 在同一包中定义同名函数，两者参数类型（Vec 型 vs 矩阵型）完全不同，仓颉重载解析可自然区分。矩阵无取模运算，因此 scalar_mat_ops.cj 不提供 mod 函数，与 scalar_vec_ops.cj 提供 mod 形成不对称——此不对称由数学语义决定（向量逐分量取模有意义而矩阵无意义） |
...
| D20 | **单位矩阵构造改为 `identity(one: T)` 静态工厂函数，不提供 `init()` 无参默认构造** | C++ GLM 的 `mat4()` 默认构造为单位矩阵，但仓颉无约束泛型不支持 `T(0)/T(1)` 构造调用（deviations.md DV-01 记录的同类限制：无约束时只能传递/返回泛型类型的值，不能进行其他操作）。即使添加 `Number<T>` 约束，`Number<T>` 接口也不提供 `T(n)` 构造声明。此外构造函数必须在 struct 定义中声明，不能在 extend 块中添加。单位矩阵构造只能在 `Number<T>` 约束的 extend 块中作为静态工厂函数实现。代价：`Mat4x4<Float32, PackedHighp>()` 不可用，改为 `Mat4x4<Float32, PackedHighp>.identity(1.0f)`。参数 `one` 的必要性：`Number<T>` 约束下无法演算 T(1)——`!` 运算符属于 `Integer<T>` 接口而非 `Number<T>` 接口，对 Float32/Float64 不可用。因此 T(1) 必须由调用方显式传入。T(0) 通过 `one - one` 在 `Number<T>` 上下文中演算获得 |
...
### 4 个辅助文件（src/detail/）
- common.cj（stub，完整函数清单见 §3.7）
- matrix.cj（stub，完整函数清单见 §3.7）
- geometric.cj（stub，完整函数清单见 §3.7）
- scalar_mat_ops.cj（标量-矩阵运算全局函数，36 个重载 [add/sub/mul/div × 9 矩阵类型]，均标注 @OverflowWrapping；不含 mod 函数）
...
| **跨矩阵类型转换拆分为同类型 fromMat(m, one) 和跨类型 fromMat(conv, m, one)** | C++ GLM 的 `mat4(mat2)` 等转换构造函数在仓颉不可行——struct 体内的无约束泛型 T 无法获得 T(0)/T(1)。降级为 extend 块中的工厂函数。同类型版本 `fromMat<SrcQ>(m, one: T)` 处理 SrcT=T 的跨尺寸/跨 Qualifier 转换；跨类型版本 `fromMat<SrcT, SrcQ>(conv, m, one: T)` 处理 SrcT≠T 的跨类型转换，conv 闭包逐元素转换。调用方式：`Mat4x4<Float32, PackedHighp>.fromMat(m2x2, one: 1.0f)` 或 `Mat4x4<Float32, PackedHighp>.fromMat({ x => Float32(x) }, m2x2_int, one: 1.0f)` |
...
| **问题 2（一般）**：`init(scalar: T)` 标量填充构造中 T(0) 获取方式不可行——struct 体内 T 无约束，不能执行 `scalar - scalar`；且矩阵的 init(scalar) 语义是对角矩阵而非 Vec 的全分量赋同一值 | **已采纳**。① 将标量填充构造从 `init(scalar: T)` 降级为 extend 块中的 `filled(scalar: T)` 静态工厂函数；② 在 `Number<T>` 约束的 extend 块中通过 `scalar - scalar` 获得 T(0)；③ 新增 D22 记录此决策；④ 更新 §9 差异声明新增"标量填充构造为 filled 工厂函数"条目 |
| **问题 3（一般）**：72 个跨矩阵类型转换构造函数需 T(0)/T(1) 但 struct 体内无约束泛型无法获得；与问题 2 根因相同但改进策略不一致 | **已采纳**。① 将 72 个跨尺寸转换构造统一降级为 `Number<T>` 约束 extend 块中的 `fromMat(m, one: T)` 静态工厂函数；② T(0) 通过 `someValue - someValue` 在 `Number<T>` 上下文中演算，T(1) 通过参数 `one` 传入；③ 新增 D23 记录此决策；④ 更新 §9 差异声明 |
| **问题 4（轻微）**：精度变体命名是否覆盖方阵短别名未明确 | **已采纳**。在 §8 fwd.cj 命名规则中补充"精度变体覆盖方阵短别名"行。更新 D16 |
| **问题 5（一般）**：`[]` 运算符赋值版本签名缺少 `mut` 修饰符 | **已采纳**。在 §3.4 中将赋值版本签名补全为 `mut operator func [](i: Int64, value!: VecR<T,Q>): Unit` |
| **问题 6（一般）**：跨类型逐分量构造和跨类型列向量构造在 struct 体内的实现可行性未分析；未参考阶段一 castVecN 模式 | **已采纳**。① 分析可行性并统一降级为 conv 闭包模式；② 新增 D24 |
| **问题 7（轻微）**：`col()` 与 `[]` 并存但未说明编码级使用场景差异指导 | **已采纳**。在 §3.4 中补充编码指导 |
| **问题 8（一般）**：lib.cj 导出 add/sub/mul/div 时下游 import 歧义评估 | **已采纳**。在 §8 lib.cj 条目中补充歧义评估 |
| **问题 9（一般）**：`const init` 条件性描述未为各构造函数分类标注 | **已采纳**。新增"构造函数/工厂函数 const init 可用性汇总"表 |
| **问题 10（轻微）**：跨尺寸矩阵乘法运算符的 import 策略描述冗余/误导 | **已采纳**。明确说明同包内无需 import |
| **问题 11（轻微）**：测试文件中缺少别名类型的构造函数可用性验证 | **已采纳**。补充验证要求 |
| **问题 12（一般）**：`identity()` 未分析 const func 可行性，未在 §9 差异声明中记录 | **已采纳**。新增 D26 + §9 差异声明 |
...
| 审查意见 | 修改措施 |
|---------|---------|
| **问题 4（严重）**：`fromMat` 签名在 `SrcT ≠ T` 时缺少 `conv: (SrcT) -> T` 闭包参数，跨类型矩阵转换无法完成源元素类型转换，编码时将编译阻断 | **已采纳**。将第 6 条"跨矩阵类型转换"拆分为两个独立工厂函数：6a 同类型跨尺寸转换 `fromMat<SrcQ>(m, one: T)`（SrcT=T，直接赋值可行，无需 conv）和 6b 跨类型跨尺寸转换 `fromMat<SrcT, SrcQ>(conv, m, one: T)`（SrcT≠T，需 conv 闭包逐元素转换）。补充 6b 版本的填充规则：`dst[j][i] = conv(src[j][i])`。新增 D27 记录此决策 |
| **问题 13（严重）**：`fromMat` 填充规则描述 `dst[j][i] = src[j][i]` 在 `SrcT ≠ T` 时赋值不可行，类型不匹配 | **已采纳**。与问题 4 统一解决——6b 版本的填充规则改为 `dst[j][i] = conv(src[j][i])`，通过 conv 闭包完成跨类型元素转换。6a 版本保持 `dst[j][i] = src[j][i]`（同类型直接赋值） |
| **问题 1（一般）**：`fromColumns` 签名仅使用单一 `V1` 类型参数，与 C++ GLM 四列独立类型参数的能力不符，且未在差异声明中标注 | **已采纳**。① 在 §3.3 第 5 条 fromColumns 描述中补充"单一 V1 类型参数限制"说明；② 新增 D28 记录此设计决策；③ 在 §9 差异声明中新增"fromColumns 仅支持单一分量类型 V1"条目 |
| **问题 2（一般）**：`fromMat` 中 `SrcQ` 类型参数声明但未使用，导致不必要的重载膨胀 | **已采纳**。① 在 §3.3 第 6a 条中补充 SrcQ 的作用说明：SrcQ 仅出现在源矩阵类型参数中，使 fromMat 可接受不同 Qualifier 的源矩阵，不参与运算逻辑；② 说明 SrcQ 的影响：SrcQ 使 fromMat 对每个 Qualifier 组合形成重载，但由于仓颉泛型实例化按需进行且编译器合并相同 Qualifier 的实例，运行时代码量不显著膨胀 |
| **问题 3（轻微）**：72 个跨尺寸转换构造的数量计算缺乏精确推导 | **已采纳**。在 §3.3 第 6 条末尾补充"跨矩阵类型转换工厂函数的数量推导"：9 种目标矩阵 × 8 种非自身源矩阵 = 72 个；其中每个 fromMat 针对同类型（6a）或跨类型（6b）各一版；SrcQ 是类型参数不影响编译产生的静态函数数量 |
| **问题 5（轻微）**：`filled` 示例使用行主序显示可能混淆编码者 | **已采纳**。在 §3.3 第 3 条 filled 示例和 §4.1 行为契约中，将 filled 示例改为列主序展示格式，并明确标注"列主序展示，与矩阵内部数据组织方式一致" |
| **问题 6（轻微）**：`fromParts` 仅支持单一源类型 U，未在 §9 差异声明中记录 | **已采纳**。① 在 §3.3 第 4 条 fromParts 描述中补充"单一 U 类型限制"说明；② 新增 D29 记录此设计决策；③ 在 §9 差异声明中 fromParts 条目补充"仅支持单一源类型 U"说明 |
| **问题 7（轻微）**：精度变体方阵短别名命名风格与阶段一向量 PascalCase 约定不一致 | **已采纳**。在 D16 中补充命名风格说明：方阵短别名精度前缀采用 camelCase 首段（如 `Highpmat4`），因为短别名本身是 camelCase（`mat4`），精度前缀仅在其前拼接。命名结构为 `{精度前缀}{短别名}` 而非 `{精度前缀}{类型名}`，与向量精度别名（如 `HighpVec4`）不构成直接类比——此差异属于 GLM C++ 源码的命名惯例历史传承 |
| **问题 8（轻微）**：extend 块中 `identity(one: T)` 和 `filled(scalar: T)` 工厂函数的 Q 类型参数来源及推断行为未分析 | **已采纳**。在 §3.3 第 3 条（filled）和第 8 条（identity）中分别补充完整泛型参数展开示例：`extend<T, Q> Mat4x4<T, Q> where T <: Number<T>, Q <: Qualifier` 中的 `public static func filled/identity(...)`——Q 从实例上下文推断或显式指定，T 从参数推断。在 §4.1 行为契约中补充带完整泛型参数的调用示例 |
| **问题 9（轻微）**：scalar_mat_ops.cj 不包含 mod 函数的情况未明确说明 | **已采纳**。① 在 §2 包组织中 scalar_mat_ops.cj 条目补充"此文件不包含 mod 函数"；② 在 §3.5 标量-矩阵运算描述中补充"scalar_mat_ops.cj 不包含 mod 函数"说明；③ 在 D15 中补充 mod 缺失说明及理由 |
| **问题 10（一般）**：分量级修改三步操作未在关键行为契约中示例，且缺少 `componentAt(row, col)` 赋值版本的设计决策记录 | **已采纳**。① 在 §4 中新增 §4.6 分量级修改操作行为契约示例；② 在 §3.4 行列访问中补充"未提供 componentAt(row, col) 赋值版本的设计决策"说明；③ 在 §9 差异声明中更新 `[]` 返回副本条目，补充"不提供 componentAt(row, col) 赋值版本"说明及理由 |
| **问题 11（一般）**：三个 stub 文件的函数签名清单使用"等"字省略，编码者无法确定完整函数列表 | **已采纳**。在 §3.7 中为三个 stub 文件各提供完整函数签名清单：common.cj（12 个函数）、matrix.cj（27 个函数含重载）、geometric.cj（19 个函数含重载） |
| **问题 12（轻微）**：ext/ 向量别名文件中 `vec1`/`dvec1` 非 GLSL 标准但作为 GLM 扩展提供，其必要性未说明 | **已采纳**。在 §3.8 文件范围说明后补充 vec1/dvec1 必要性说明：虽然 GLSL 不定义 vec1，但 GLM 通过 ext/vector_float1.hpp 扩展了 vec1/dvec1 别名，服务于标量-向量混合操作和 C++ GLM 完整迁移场景 |
| **问题 14（一般）**：cjpm 子包发现机制未验证，`src/ext/` + `package glm.ext` 是否可编译通过未确认 | **已采纳**。① 在 §2 末尾新增"cjpm 子包构建预验证"段落，描述编码首日须执行的验证步骤；② 在 §8 产出物清单中新增"编码启动前验证项"条目 |
