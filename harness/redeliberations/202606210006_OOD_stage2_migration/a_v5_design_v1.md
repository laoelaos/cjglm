...
5. **列向量构造**（init(c0: VecR<T,Q>, c1: VecR<T,Q>, ...)）
   - 直接以 C 个列向量构建矩阵

6. **跨类型逐分量构造**（泛型，各分量可为不同类型）
   - 接收与逐分量构造相同数量的参数，但各参数可为不同类型，内部执行隐式类型转换

7. **跨类型列向量构造**（init(v1: VecR<V1, Q>, v2: VecR<V2, Q>, ...)）
...
DESIGN_WRITTEN:C:\Develop\Software\cjglm_wp\harness\redeliberations\202606210006_OOD_stage2_migration\a_v5_design_v1.md
