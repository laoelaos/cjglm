...
**产出物**：
- **基础函数库（core）**：
  - `common.cj`、`exponential.cj`：完整实现
  - `matrix.cj`、`geometric.cj`、`trigonometric.cj`：完整实现（替换阶段 2/3 的 stub）
- **ext/ 扩展函数库**：
...
- 基线文档首轮迁移范围中排除的文件重新评估（如 `_swizzle.hpp` 的 swizzle 功能替代机制设计，详见 `docs/01_roadmap.md` 第 4.5 节）
