...
- **第一批（`[新建]` `common.cj`、`exponential.cj`、`ext/scalar_common.cj`）**：
...
- **降级路径（SIMD 不可用）**：
  ...
  - 性能基线数据采集完成并输出至指定文件：
    - **最少覆盖函数范围**：`Vec3`/`Vec4` 各 5 个核心运算（构造、加法、点积、叉积、归一化），`Mat4` 2 个核心运算（矩阵乘、行列式），`geometric.cj` `dot`/`normalize`/`reflect` 共 3 个函数——合计约 15 个关键函数
