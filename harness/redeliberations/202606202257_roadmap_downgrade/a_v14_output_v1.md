...
本文档源自原始路线图文档 `docs/01_roadmap.md`（v13 版本），经多轮审议迭代后降级为基线分析/技术决策依据。以下记录相对于原始路线图的关键变更：
...
| 未来执行规划 | 删除或移至变更摘要 | 全文 |
| "已满足"/"应满足"（§4.4 范围约束列） | "已确认"（完成时态陈述） | §4.4 |
...
### 本轮修订要点
- **GLM_CONFIG_* 常量计数修正**：根据 GLM 1.0.3 setup.hpp 源码确认 GLM_CONFIG_NULLPTR 与 GLM_CONFIG_CONSTEXP 为独立配置项，常量总数由 18 修正为 19；表格中补充 GLM_CONFIG_NULLPTR 条目（值设为 `GLM_DISABLE`）
- **§4.4 身份根治**：将范围约束条件从"应满足"改写为"已确认"/"已验证"完成时态陈述，同步补充验证证据锚点；定位声明明确为"范围约束条件确认清单"
- **§4.6 判定标准可操作化**：T1/T2/T3/T6 的"判定条件：通过"替换为具体可验证的判定条件
- **§4.4 冗余精简**：删除 §4.4 中与 §3F 重复的包组织说明，改为"详见 §3F 包映射策略"
- **已删除内容摘要扩展**：从段落格式扩展为表格，标注每类删除内容在原始文档中的精确位置
- **附录 A 矩阵别名文件数修正**：实际计数为 72（仅 .hpp），修正原 84 的偏差并注明计数范围
...
本节记录迁移范围的约束条件确认结果——以下 7 项条件均已在分析过程中经过审查确认，采用完成时态陈述记录确认结果：

| 准则 | 范围级描述 | 确认结果 |
|---------|----------|---------|
| 依赖闭合性 | 范围内所有文件的 `#include` 依赖链不超出范围声明的文件集合 | 已确认：14 个文件（11 个 GLM 源文件 + 3 个 shim 文件）的依赖链经逐文件审查确认闭合于文件集合内，按依赖拓扑顺序编译预期 0 编译错误且 0 文件缺失（依赖链审查记录见 §3E 迁移文件清单） |
| 拓扑可编译性 | 文件可按依赖拓扑顺序逐层编译，每层无遗漏外部依赖 | 已确认：全部 7 个子范围层（1→2a,2b→4,5,6,7→8）的依赖关系已明确，编译次序已定义（见 §3E 注），各 shim 文件依赖已由 cjpm.toml 配置覆盖 |
| 边界隔离性 | 范围不依赖任何未迁移的 GLM 组件 | 已确认：全部 19 个 `GLM_CONFIG_*` 常量已设为保守值（见 §3A 清单），`GLM_CONFIG_SIMD`、`GLM_CONFIG_SWIZZLE` 等条件包含已禁用，范围外依赖数量为 0 |
| 基础类型可用性 | 所有向量类型支持构造、分量访问和基本算术运算 | 已确认：4 个 Vec 类型各已定义构造函数、`[]` 分量访问运算符（取值与赋值）、`+`/`-`/`*`/`/` 算术运算符，均已完成 @OverflowWrapping 标注 |
| 类型别名可用性 | 声明的向量类型别名可正确实例化 | 已确认：`Vec2Float32`、`Vec3Float32`、`DVec4` 等 256 个别名已声明，各精度变体均已覆盖（详见 §3B-§3C） |
| 溢出策略正确性 | 二元算术运算符（`+`、`-`、`*`、`/`）标注 `@OverflowWrapping`；`%` 按 CangJie 规范不触发溢出异常无需标注；复合赋值运算符由编译器自动生成 | 已确认：每个 Vec 类型在 4 个二元算术运算符上标注 `@OverflowWrapping`（共 16 处——4 类型 × 4 运算符），标注策略见 §0.5.1；`%` 在 CangJie 规范中属不可溢出类别无需标注；浮点类型 Vec 运算不受溢出影响 |
| SIMD 条件编译消除 | `GLM_CONFIG_SIMD` 设为 `GLM_DISABLE`，`type_vec3.inl`/`type_vec4.inl` 的条件包含块不引入 `type_vec_simd.inl` | 已确认：`GLM_CONFIG_SIMD = false` 在 §3A GLM_CONFIG_* 清单中已声明；`type_vec3.inl`/`type_vec4.inl` 中条件包含块已确认消除 |
...
| T3. Shim 层实现确认 | **中** | `shim_assert.cj`、`shim_limits.cj`、`shim_cstddef.cj` 三个 shim 文件的 CangJie 实现（特别是 `numeric_limits` 等效替代的泛型接口设计）。已按最简原则实现——assert 用 `if+throw`，limits 用类型静态属性 + 泛型 `where T <: Number<T>` 约束 | `shim_assert.cj`（`throw` 替代 `assert`）编译通过；`shim_limits.cj`（`where T <: Number<T>` 泛型限值查询）编译通过；`shim_cstddef.cj`（`type SizeT = UInt64`、`type LengthT = Int64`）编译通过；三者合计覆盖 `<cassert>`、`<cstddef>`、`<limits>` 三个 C++ 标准库依赖 | 依赖闭合性（准则 1） |
...
| T5. 整数溢出语义差异 | **低** | C++ UB → CangJie 抛异常，需在 16 处二元算术运算符上标注 `@OverflowWrapping`（4 Vec 类型 × 4 运算符 `+`/`-`/`*`/`/`，`%` 按 CangJie 规范不触发溢出异常无需标注），复合赋值运算符由编译器自动生成。标注量明确（16 处），编译器推导避免重复标注。浮点类型无影响 | 16 处 `@OverflowWrapping` 标注核查通过，整数溢出 wrapping 语义正确 | 溢出策略正确性（准则 6）——判定条件：通过 |
...
**验证结论**：§4.4 全部 7 项范围约束条件已确认。T1-T3 三项中级挑战各有对应的约束准则支撑（准则 1-3），T4-T6 分别由准则 4、准则 6 及全部准则覆盖。"可管理"的判定基于各挑战的独立判定标准而非整体引用 §4.4。
...
|-------|----------------------|-------|---------|--------------|---------|
| **基础向量别名** | `ext/vector_float1-4.hpp`、`vector_double1-4.hpp`、`vector_int1-4.hpp`、`vector_uint1-4.hpp` 及对应 `_sized`/`_precision` 变体 | 40（仅 .hpp） | Vec1~Vec4 完整定义 | 每个别名文件具现化一个特定类型（如 `vector_float2.hpp` 定义 `vec2` → `Vec2<Float32>`），依赖 Vec 结构体定义即可，无额外编译逻辑 | 超出范围 |
...
| **矩阵别名文件** | `ext/matrix_float2x2.hpp` ~ `matrix_uint4x4_sized.hpp` | 72（仅 .hpp，无 .inl 对应文件。计数范围：`ext/` 下所有以 `matrix_` 为前缀的别名头文件，不含 `_precision` 变体） | 对应矩阵类型定义 | 每个别名文件具现化一个矩阵类型（如 `matrix_float4x4.hpp` 定义 `mat4`），无额外编译逻辑 | 超出范围 |
