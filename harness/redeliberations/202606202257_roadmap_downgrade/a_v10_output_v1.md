...
| T5. 整数溢出语义差异 | **低** | C++ UB → CangJie 抛异常，需在 16 处二元算术运算符上标注 `@OverflowWrapping`，复合赋值运算符由编译器自动生成。标注量明确（16 处），编译器推导避免重复标注。浮点类型无影响 | 16 处 `@OverflowWrapping` 标注核查通过，整数溢出 wrapping 语义正确 | 溢出策略正确性（准则 6）——判定条件：通过 |
