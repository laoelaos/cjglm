...
| 未来执行规划 | 删除或移至变更摘要 | 全文 |
| "已满足"（§4.4 范围约束列） | "应满足" | §4.4 |
| "@OverflowWrapping 标注 20 处" | "标注 16 处"（移除 `%`） | §0.5, §0.5.1, §0.8, §4.4, §4.6 |

### 已删除内容摘要

- **轮次划分和里程碑**：原始路线图中的"第 1 轮/第 2 轮/第 3 轮"迁移阶段划分及关联时间预估
- **未来风险评估**：关于后续轮次中可能遇到的风险和缓解措施的推测性内容
- **建议性策略描述**：所有"建议采用"/"推荐使用"等推荐措辞替换为确定性陈述
- **执行计划附录**：原始路线图中标记为"计划中"的未来工作项

### 本轮修订要点
- **`@OverflowWrapping` 标注修正**：根据 CangJie 官方文档（`reflect_and_annotation/README.md`）确认 `%`（取模）运算符不可溢出，从标注集合中移除；标注量由 20 处（4 Vec 类型 × 5 运算符）修正为 16 处（4 Vec 类型 × 4 运算符：`+`、`-`、`*`、`/`）
- **§4.4 身份矫正**：将 7 项"已满足"改为"应满足"，统一为范围定义标准语气，消除"非验证计划"声明与断言性措辞的矛盾
- **附录 A 术语定义补充**：为三个范围标记（「超出范围」「已排除」「不在范围内」）补充明确定义
- **变更追踪信息**：新增本章节提供全文级变更可追溯性
...
| 准则 | 范围级描述 | 范围约束条件 |
|---------|----------|---------|
| 依赖闭合性 | 范围内所有文件的 `#include` 依赖链不超出范围声明的文件集合 | 应满足：14 个文件（11 个 GLM 源文件 + 3 个 shim 文件）按依赖拓扑顺序编译，0 编译错误且 0 文件缺失（依赖链逐文件审查确认，见 §3E 迁移文件清单） |
| 拓扑可编译性 | 文件可按依赖拓扑顺序逐层编译，每层无遗漏外部依赖 | 应满足：全部 7 个子范围层（1→2a,2b→4,5,6,7→8）依次编译通过，0 编译错误（编译次序定义见 §3E 注） |
| 边界隔离性 | 范围不依赖任何未迁移的 GLM 组件 | 应满足：仅纳入声明的 14 个文件后编译，编译错误信息中不包含任何范围外的文件缺失提示，范围外依赖 0 缺失（GLM_CONFIG_* 全设为保守值确认，见 §3A） |
...
| 溢出策略正确性 | 二元算术运算符（`+`、`-`、`*`、`/`）标注 `@OverflowWrapping`；`%` 按 CangJie 规范不触发溢出异常无需标注；复合赋值运算符由编译器自动生成 | 应满足：每个 Vec 类型在 4 个二元算术运算符上标注 `@OverflowWrapping`（共 16 处——4 类型 × 4 运算符），核查通过（标注策略见 §0.5.1）；`Int32` 分量向量算术运算在溢出时符合 wrapping 语义，不抛出 `ArithmeticException`；浮点类型 Vec 运算不受影响 |
| SIMD 条件编译消除 | `GLM_CONFIG_SIMD` 设为 `GLM_DISABLE`，`type_vec3.inl`/`type_vec4.inl` 的条件包含块不引入 `type_vec_simd.inl` | 应满足：`GLM_CONFIG_SIMD = false` 在常量声明中已确认（见 §3A GLM_CONFIG_* 清单）；编译 `type_vec3`/`type_vec4` 时无 `type_vec_simd.inl` 缺失错误 |
...
| **矩阵别名文件** | `ext/matrix_float2x2.hpp` ~ `matrix_uint4x4_sized.hpp` | 84（.hpp + .inl） | 对应矩阵类型定义 | 每个别名文件具现化一个矩阵类型（如 `matrix_float4x4.hpp` 定义 `mat4`），无额外编译逻辑 | 超出范围 |
...
| 审查意见 | 处理方式 |
|---------|---------|
| 问题1（严重）：`%` 运算符溢出假设与 CangJie 语言规范不符 | **修改**——查阅 CangJie 官方文档（`reflect_and_annotation/README.md`）确认 `%` 属不可溢出类别，从 `@OverflowWrapping` 标注集合中移除；标注量全域修正为 16 处（4 Vec 类型 × 4 运算符 `+`/`-`/`*`/`/`）；§0.5.1 表拆分出"二元算术（不可溢出）——`%`"行标注为"否"；补充说明 CangJie 规范中 `%` 不触发溢出异常；更新全部 6 处正文文本 |
| 问题2（严重）：§4.4 身份矛盾——宣称"非验证计划"但使用"已满足"验证语言 | **修改**——将 §4.4 全部 7 项"已满足"改为"应满足"，保持范围定义标准身份（方案一）；文档定位声明保持不变 |
| 问题3（一般）：缺少对原始 Roadmap 的变更追踪，影响可审计性 | **修改**——在 §0 之前新增「变更摘要」章节（含各章节原始来源对照表、关键术语替换对照、已删除内容摘要），提供全文级变更可追溯性 |
| 问题4（轻微）：附录 A 范围标记术语不统一且未定义 | **修改**——在附录 A 开头补充 4 种范围标记的明确定义（「超出范围」「已排除」「不在范围内」「已知局限性」），统一术语使用 |
