...
| T4. `compute_vector_decl.hpp` 中 `std::size_t` 非类型模板参数映射 | **低-中** | 16 处 `std::size_t Size` 作为非类型模板参数，CangJie 中需确认非类型模板参数是否支持 `Int64` 类型。已分析确认：`Int64` 作为 CangJie 默认整数类型，为非类型模板参数的合法类型 | Vec 类型以 `Int64` 为非类型模板参数，设计层面预期编译运行正常 | 基础类型可用性（准则 4）——Int64 为非类型模板参数合法类型 |
...
| T6. GLM 源码量 vs CangJie 实现量对应关系 | **低** | C++ 模板代码与 CangJie 泛型代码行数比例可能存在差异。9 个核心文件 + 3 个 shim 文件体量有限（GLM 约 2000 行 C++ → CangJie 预估 1500-2500 行），已通过实践校准确认 | 11 个等效 GLM 文件 + 3 个 shim 文件合计编译无错误；核心 Vec 类型逻辑行数转换偏差 <30%（经实践校准确认） | 全部编译验证通过（准则 1-7） |
