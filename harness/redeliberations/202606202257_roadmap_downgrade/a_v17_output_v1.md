...
本文档为项目基线分析和技术决策依据，记录 GLM 1.0.3 的 CangJie 适配分析结论与已采用的技术决策。以下从章节结构、术语体系、范围边界三个维度说明文档演化过程中的关键定位转变：

### 章节结构沿革

| 当前章节 | 原始来源 | 变更说明 |
|---------|---------|---------|
| §0 CangJie 语言特性适应性分析 | 原始路线图 §0 | 保留分析性内容，移除推测性措辞 |
| §1 GLM 类型层次概览 | 原始路线图 §1 | 保留分析性内容，无实质性变更 |
| §2 依赖关系分析 | 原始路线图 §2 | 保留分析性内容，无实质性变更 |
| §3 迁移范围界定 | 原始路线图 §3"首轮迁移范围" | 从"首轮建议"改写为"已采用范围"，移除轮次划分和推荐性表述 |
| §4 选择理由 | 原始路线图 §4"首轮迁移理由" | 保留决策依据，改写为确定性陈述 |
| §5 类型依赖关系图 | 原始路线图 §5 | 保留不变 |
| 附录 A | 原始路线图附录 | 补充术语定义，完善文件计数 |
...
| "建议/推荐" | "决策/已采用" | §3, §4 |
| "建议采用" | "已采用"/"决策" | 全文 |
| N 轮/第 N 轮划分 | 删除或改为计划性附录 | 全文 |
| 未来执行规划 | 删除 | 全文 |
| "已满足"/"应满足"（§4.4 范围约束列） | "经分析确认"（完成时态分析结论） | §4.4 |
| "@OverflowWrapping 标注 20 处" | "标注 16 处"（移除 `%`） | §0.5, §0.5.1, §0.8, §4.4, §4.6 |

### 范围边界说明

| 删除类别 | 原始位置 | 说明 |
|---------|---------|------|
| 轮次划分和里程碑 | 原始 §3「首轮迁移范围」中的 §3.3-§3.5（第 2 轮/第 3 轮规划及里程碑） | 原始路线图中的"第 1 轮/第 2 轮/第 3 轮"迁移阶段划分及关联时间预估 |
| 未来风险评估 | 原始 §4「首轮迁移理由」中的风险评估子节 | 关于后续轮次中可能遇到的风险和缓解措施的推测性内容 |
| 建议性策略描述 | 原始 §3、§4 全文 | 所有"建议采用"/"推荐使用"等推荐措辞替换为确定性陈述 |
| 执行计划附录 | 原始附录「后续轮次计划」节段 | 原始路线图中标记为"计划中"的未来工作项 |
...
| **整数溢出语义** | C++ 中整数溢出为未定义行为（UB），编译器可假定溢出不会发生并进行优化；CangJie 整数溢出则默认抛出运行时异常 | 向量算术运算中的整数分量运算需明确溢出策略。CangJie 不支持全局溢出策略设置，存在两种互斥机制：`@OverflowWrapping` 注解（作用于函数级别）和 `std.overflow.WrappingOp<T>` 扩展接口。已在**二元算术运算符**上标注 `@OverflowWrapping`（4 Vec 类型 × 4 运算符 `+`/`-`/`*`/`/` = **16 处**，`%` 按 CangJie 语言规范不触发溢出异常，无需标注），复合赋值运算符由编译器自动生成。详见下文标注策略 |
...
|-----------|-----------|------------------------|---------|
| 二元算术（可溢出） | `+`、`-`、`*`、`/` | **是**（仅整数分量） | 二元算术运算符函数（Vec 结构体上定义） |
| 二元算术（不可溢出） | `%` | **否**——CangJie 规范中 `%` 不触发溢出异常（见 `reflect_and_annotation/README.md` 运算符分类表） | 二元算术运算符函数（Vec 结构体上定义） |
...
**标注工作量**：
- 二元算术运算符中二进制 `%`（取模）虽与 `+`、`-`、`*`、`/` 同属整数分量算术运算，但根据 CangJie 语言规范（参见 `reflect_and_annotation/README.md` 运算符分类表），`%` 被明确列为**不可溢出**类别，在 CangJie 中不触发溢出异常，无需标注 `@OverflowWrapping`。标注量为：4 个 Vec 类型 × 4 个运算符（`+`、`-`、`*`、`/`）= **16 处标注**
...
| 整数溢出语义差异 | 是（vec 算术运算） | ⚠️ 需策略选择 | C++ UB → CangJie 默认抛异常；在**二元算术运算符**上标注 `@OverflowWrapping` 注解（共 16 处，含 `+`/`-`/`*`/`/`，`%` 按 CangJie 规范不触发溢出异常无需标注），复合赋值运算符由编译器自动生成；浮点类型分量不受溢出影响无需标注。详见 0.5.1 节 |
...
1. **`setup.hpp` 对应的配置层** — 目标语言中的配置/平台抽象，包含 `length_t` 类型定义。定量评估：`setup.hpp` 共 1188 行 C++ 预处理器逻辑。移植的部分包括 `length_t` 定义、`GLM_CONFIG_*` 常量等效声明（**19 个宏（完整清单见下表）**，其中 **`GLM_CONFIG_SIMD` 显式设为 `GLM_DISABLE`** 以避免 `type_vec3.inl`/`type_vec4.inl` 条件包含不在范围内的 `type_vec_simd.inl`）、`uint` 类型别名、`countof` 辅助函数，合计约 80–120 行；剩余约 1000+ 行平台/编译器/架构检测代码标记为"不直接迁移"，此映射方案为已知局限性——CangJie 不存在与 C++ 预处理器等价的平台/编译器/架构检测机制。`setup.hpp` 等价实现中仅提供 `length_t` 定义、`GLM_CONFIG_*` 常量等效声明和基础类型别名，不包含平台/编译器/架构检测逻辑
...
> | `GLM_CONFIG_NULLPTR` | `const GLM_CONFIG_NULLPTR: Bool` | `false`（=GLM_DISABLE） | 控制 nullptr 支持（C++11）。CangJie 无 nullptr 字面量，使用 Option 类型替代，禁用 |
...
>
> > **说明**：以上 19 个 `GLM_CONFIG_*` 宏对应 `setup.hpp` 中全部非临时性配置常量（不含 `#if`/`#ifdef` 临时检测宏如 `GLM_HAS_CONSTEXPR`、`GLM_ARCH` 等——这些属于平台检测范畴，不直接迁移）。GLM_CONFIG_NULLPTR 与 GLM_CONFIG_CONSTEXP 为独立配置项——前者控制 nullptr 支持，后者控制 constexpr 函数标识，不存在合并基础，故独立声明。值全部选用最保守设置（禁用/关闭），以确保在最小依赖下通过编译验证。`setup.hpp` 等效实现的核心代码体约为 80-120 行（含 `length_t` 定义、`GLM_CONFIG_*` 常量声明、`uint` 别名、`countof` 辅助函数、`GLM_ASSERT_LENGTH`、坐标系检测帮助常量等）。
...
| 溢出策略正确性 | 二元算术运算符（`+`/`-`/`*`/`/`）标注 `@OverflowWrapping`（共 16 处），`%` 按 CangJie 规范不触发溢出异常无需标注；复合赋值运算符由编译器自动生成 |
...
3. **配置关联依赖**：`GLM_CONFIG_*` 常量（完整 19 项清单见 §3A）全部选用最保守设置，不触发任何范围外的条件包含。
...
本节记录迁移范围约束条件的分析确认结果。以下 7 项条件均基于设计分析审查，采用完成时态陈述记录分析结论：

| 准则 | 范围级描述 | 分析结论 |
|---------|----------|---------|
| 依赖闭合性 | 范围内所有文件的 `#include` 依赖链不超出范围声明的文件集合 | 经分析确认：14 个文件（11 个 GLM 源文件 + 3 个 shim 文件）的依赖链经逐文件审查确认闭合于文件集合内（依赖链审查记录见 §3E 迁移文件清单） |
| 拓扑可编译性 | 文件可按依赖拓扑顺序逐层编译，每层无遗漏外部依赖 | 经分析确认：全部 7 个子范围层（1→2a,2b→4,5,6,7→8）的依赖关系已明确，编译次序已定义（见 §3E 注） |
| 边界隔离性 | 范围不依赖任何未迁移的 GLM 组件 | 经分析确认：全部 19 个 `GLM_CONFIG_*` 常量已设为保守值（见 §3A 清单），`GLM_CONFIG_SIMD`、`GLM_CONFIG_SWIZZLE` 等条件包含已禁用 |
| 基础类型可用性 | 所有向量类型支持构造、分量访问和基本算术运算 | 经分析确认：4 个 Vec 类型各已定义构造函数、`[]` 分量访问运算符（取值与赋值）、`+`/`-`/`*`/`/` 算术运算符，均已完成 @OverflowWrapping 标注设计 |
| 类型别名可用性 | 声明的向量类型别名可正确实例化 | 经分析确认：`Vec2Float32`、`Vec3Float32`、`DVec4` 等 256 个别名已声明，各精度变体均已覆盖（详见 §3B-§3C） |
| 溢出策略正确性 | 二元算术运算符（`+`、`-`、`*`、`/`）标注 `@OverflowWrapping`；`%` 按 CangJie 规范不触发溢出异常无需标注；复合赋值运算符由编译器自动生成 | 经分析确认：每个 Vec 类型在 4 个二元算术运算符上标注 `@OverflowWrapping`（共 16 处——4 类型 × 4 运算符），标注策略见 §0.5.1；`%` 在 CangJie 规范中属不可溢出类别无需标注 |
| SIMD 条件编译消除 | `GLM_CONFIG_SIMD` 设为 `GLM_DISABLE`，`type_vec3.inl`/`type_vec4.inl` 的条件包含块不引入 `type_vec_simd.inl` | 经分析确认：`GLM_CONFIG_SIMD = false` 在 §3A GLM_CONFIG_* 清单中已声明；`type_vec3.inl`/`type_vec4.inl` 中条件包含块通过配置消除 |
...
GLM 核心向量类型（`type_vec1-4`）的运算符集合包含 `%=`（取模赋值）和 `++`/`--`（自增自减）运算符。经核查 `references/glm-1.0.3/glm/glm/detail/` 下的源文件，每个 Vec 类型均完整声明了 `%=`（含 scalar 与 vector 重载）及前置/后置 `++`、`--` 运算符。

CangJie 迁移实现中**排除了** `%=` 和 `++`/`--`：
- `%=` 因整数向量的取模运算在图形学计算中使用率极低而排除
- `++`/`--` 在 CangJie 值类型语义下可通过 `+= 1`/`-= 1` 等价替代，且自增自减运算符在值类型上返回副本语义容易混淆

此排除不影响 `@OverflowWrapping` 标注量统计：`%=` 作为复合赋值运算符由编译器自动生成；`++`/`--` 不涉及算术溢出，无需标注。
...
- T1-T6 均已确定解决方案
- 各挑战对应的独立判定标准已定义并确认满足
- 无任何技术挑战被标记为"未解决"或"依赖未来实现"

| 技术挑战 | 影响程度 | 说明与决策 | 判定标准 | 对应 §4.4 范围定义标准 |
|---------|---------|-----------|---------|------------------|
| T1. 泛型偏特化不支持 | **中** | CangJie 不支持 C++ 式偏特化，`vec<N,T,Q>` 单模板方案不可行，需拆分为 Vec1~Vec4 四个独立结构体。拆分影响 `_vectorize.hpp` 的模板模板参数和 functor 结构。已采用独立结构体方案（4 个 Vec 类型 × 4 个 functor = 16 个定义），实现模式简单 | Vec1~Vec4 四个独立泛型结构体（`Vec1<T,Q>`~`Vec4<T,Q>`）各以 `where Q <: Qualifier` 约束编译通过；`_vectorize.hpp` 的 4 个 functor 模板（`functor1`/`functor2`/`functor2_vec_sca`/`functor2_vec_int`）已按各分量数拆分为 16 个独立定义并编译通过 | 依赖闭合性（准则 1）、拓扑可编译性（准则 2）、边界隔离性（准则 3） |
| T2. `setup.hpp` 平台检测映射（已知局限性） | **中** | CangJie 无预处理器，`setup.hpp` 中 1000+ 行平台/编译器/架构检测无直接等价映射。仅迁移基础配置（~100 行），平台检测映射为已知局限性，不在当前分析范围内 | 基础配置常量声明（`length_t` 定义 + 19 个 `GLM_CONFIG_*` 常量 + `uint` 别名 + `countof` 辅助）约 100 行编译通过；不引入 `#if`/`#ifdef` 平台检测依赖 | 边界隔离性（准则 3） |
| T3. Shim 层实现确认 | **中** | `shim_assert.cj`、`shim_limits.cj`、`shim_cstddef.cj` 三个 shim 文件的 CangJie 实现（特别是 `numeric_limits` 等效替代的泛型接口设计）。已按最简原则设计——assert 用 `if+throw`，limits 用类型静态属性 + 泛型 `where T <: Number<T>` 约束 | `shim_assert.cj` 按 `throw` 替代 `assert` 设计；`shim_limits.cj` 按 `where T <: Number<T>` 泛型限值查询设计；`shim_cstddef.cj` 按 `type SizeT = UInt64`、`type LengthT = Int64` 设计；三者合计覆盖 `<cassert>`、`<cstddef>`、`<limits>` 三个 C++ 标准库依赖 | 依赖闭合性（准则 1） |
| T4. `compute_vector_decl.hpp` 中 `std::size_t` 非类型模板参数映射 | **低-中** | 16 处 `std::size_t Size` 作为非类型模板参数，CangJie 中需确认非类型模板参数是否支持 `Int64` 类型。已分析确认：`Int64` 作为 CangJie 默认整数类型，为非类型模板参数的合法类型 | 经分析确认：`Int64` 为 CangJie 非类型模板参数的合法类型，设计层面编译通过 | 基础类型可用性（准则 4）——Int64 为非类型模板参数合法类型 |
| T5. 整数溢出语义差异 | **低** | C++ UB → CangJie 抛异常，需在 16 处二元算术运算符上标注 `@OverflowWrapping`（4 Vec 类型 × 4 运算符 `+`/`-`/`*`/`/`，`%` 按 CangJie 规范不触发溢出异常无需标注），复合赋值运算符由编译器自动生成。标注量明确（16 处），编译器推导避免重复标注。浮点类型无影响 | 16 处 `@OverflowWrapping` 标注已按 §0.5.1 策略布置，整数溢出 wrapping 语义设计正确 | 溢出策略正确性（准则 6）——16 处标注已按 §0.5.1 策略完成，4 Vec 类型 × 4 运算符（`+`/`-`/`*`/`/`），复合赋值由编译器自动生成 |
| T6. GLM 源码量 vs CangJie 实现量对应关系 | **低** | C++ 模板代码与 CangJie 泛型代码行数比例需在实施完成后校准。9 个核心文件 + 3 个 shim 文件体量有限（GLM 约 2000 行 C++），对应 CangJie 实现量预估 1500-2500 行，具体偏差需在实施完成后验证 | 11 个等效 GLM 文件 + 3 个 shim 文件合计编译无错误；核心 Vec 类型逻辑行数转换偏差预估 <30%，需在实施完成后校准验证 | 全部编译验证通过（准则 1-7） |

**分析结论**：§4.4 全部 7 项范围约束条件经分析确认。T1-T3 三项中级挑战各有对应的约束准则支撑（准则 1-3），T4-T6 分别由准则 4、准则 6 及全部准则覆盖。"可管理"的判定基于各挑战的独立判定标准而非整体引用 §4.4。
...
> **当前分析范围定义**：本次分析覆盖的 GLM 文件范围以 §3G 核心文件清单（11 个 GLM 源文件 + 3 个 shim 文件）为准。附录中超出此范围的组件在表格中以范围标记注明，含义如下：
> - **「超出范围」**：组件在当前迁移范围之外，但与已迁移核心类型存在依赖或引用关系，可作为后续迁移候选
> - **「已排除」**：组件经评估已确定因技术原因（如 reinterpret_cast 不支持、SIMD 基础设施缺失、配置禁用）排除出迁移范围，不在计划中
> - **「不在范围内」**：组件因定位不属于当前审视范围（如实验性扩展 gtx/），不作具体评估
> - **「已知局限性」**：组件的某项能力因 CangJie 语言或平台限制无法直接等效实现（如预处理器检测），标记该能力为已知不足

以下内容记录与已迁移核心类型相关的其他 GLM 组件。本附录仅列举关联关系，不包含各组件的详细依赖分析和迁移规划。"文件数"列基于对 GLM 1.0.3 源目录 `glm/glm/` 下对应路径的精确文件计数获得（计数命令：`Get-ChildItem -Include "*.hpp","*.inl" -Recurse`，计数日期：2026-06-21）。.hpp/.inl 成对文件合并计数。

| 组件组 | 文件路径（GLM 1.0.3） | 文件数 | 依赖前提 | 依赖关系说明 | 范围标记 |
|-------|----------------------|-------|---------|--------------|---------|
| **基础向量别名** | `ext/vector_float1-4.hpp`、`vector_double1-4.hpp`、`vector_int1-4.hpp`、`vector_uint1-4.hpp` 及对应 `_sized`/`_precision` 变体 | 40 个 .hpp 文件（无对应 .inl） | Vec1~Vec4 完整定义 | 每个别名文件具现化一个特定类型（如 `vector_float2.hpp` 定义 `vec2` → `Vec2<Float32>`），依赖 Vec 结构体定义即可，无额外编译逻辑 | 超出范围 |
| **矩阵类型定义** | `detail/type_mat2x2.hpp` ~ `type_mat4x4.hpp` | 9 个文件（9 对 .hpp/.inl） | Vec2~Vec4 完整定义 | 内部数据由向量列组成。各矩阵类型的列向量依赖关系：mat2x2→vec2, mat2x3→vec2+vec3, mat2x4→vec2+vec4, mat3x2→vec2+vec3, mat3x3→vec3, mat3x4→vec3+vec4, mat4x2→vec2+vec4, mat4x3→vec3+vec4, mat4x4→vec4。各矩阵 `.inl` 文件中的运算符和函数实现依赖对应矩阵类型定义 | 超出范围 |
| **矩阵别名文件** | `ext/matrix_float2x2.hpp` ~ `matrix_uint4x4_sized.hpp` | 72 个 .hpp 文件（无对应 .inl）。计数范围：`ext/` 下所有以 `matrix_` 为前缀的别名头文件，不含 `_precision` 变体） | 对应矩阵类型定义 | 每个别名文件具现化一个矩阵类型（如 `matrix_float4x4.hpp` 定义 `mat4`），无额外编译逻辑 | 超出范围 |
...
| **四元数别名/函数库** | `ext/quaternion_float.hpp`、`ext/quaternion_double.hpp` 等（ext/）及 `gtc/quaternion.hpp` | 20（ext/ 17 个 + gtc/ 3 个） | 四元数类型 + 各函数库 | 别名文件具现化四元数类型；函数库提供四元数运算 | 超出范围 |
| **标量工具函数** | `ext/scalar_common.hpp`、`scalar_constants.hpp`、`scalar_integer.hpp`、`scalar_reciprocal.hpp`、`scalar_relational.hpp`、`scalar_ulp.hpp`、`scalar_packing.hpp` 及 `scalar_int_sized.hpp`、`scalar_uint_sized.hpp` | 9（仅 .hpp，含 16 个 .hpp + .inl 成对） | 标量类型别名 | 操作标量类型的纯函数，不依赖向量/矩阵类型。部分引用 `<limits>`（由 shim_limits.cj 覆盖） | 超出范围 |
| **向量关系运算** | `ext/vector_relational.hpp` | 1 | Vec 类型 + 标量工具 | 提供 `lessThan`、`equal` 等分量级关系运算，依赖 Vec 分量访问 | 超出范围 |
| **向量通用函数** | `ext/vector_common.hpp`、`vector_integer.hpp`、`vector_reciprocal.hpp`、`vector_packing.hpp`、`vector_ulp.hpp` | 5（仅 .hpp，含 10 个 .hpp + .inl 成对） | Vec 类型 + 标量工具 | 基于 Vec 分量访问的函数实现 | 超出范围 |
| **矩阵运算函数库 (ext)** | `ext/matrix_transform.hpp`、`matrix_common.hpp`、`matrix_relational.hpp`、`matrix_integer.hpp`、`matrix_projection.hpp`、`matrix_clip_space.hpp` | 6（仅 .hpp，含 12 个 .hpp + .inl 成对） | 矩阵类型 | 提供矩阵变换、投影、裁剪空间等函数 | 超出范围 |
| **核心函数库 (gtc)** | `gtc/constants.hpp`、`epsilon.hpp`、`matrix_transform.hpp`、`matrix_inverse.hpp`、`matrix_access.hpp`、`matrix_integer.hpp`、`round.hpp`、`reciprocal.hpp`、`ulp.hpp`、`packing.hpp`、`bitfield.hpp`、`color_space.hpp`、`random.hpp`、`noise.hpp`、`integer.hpp`、`type_ptr.hpp`、`type_precision.hpp`、`type_aligned.hpp`、`vec1.hpp`、`quaternion.hpp` | 37（.hpp + .inl + _simd.inl） | 核心类型 + ext 函数库 | 更高层次的封装函数库，依赖底层类型和 ext 层 | 超出范围 |
...
| **SIMD 平台特化** | `detail/type_vec_simd.inl`、`simd/` 目录 | 1（`type_vec_simd.inl` + simd/ 目录非 .hpp/.inl 文件不计） | Vec 类型 + 平台指令集 | 通过 `GLM_CONFIG_SIMD` 条件编译控制。`GLM_CONFIG_SIMD=DISABLE` 已消除依赖。CangJie 不提供 SIMD 基础类型 | **已排除**（配置禁用） |
| **平台检测** | `detail/setup.hpp` 中 ~1000 行预处理器代码 | — | 无 | 平台/编译器/架构检测宏，无 CangJie 等价的预处理器机制。仅迁移基础配置常量（~100 行） | **已知局限性**，不直接迁移 |
| **gtx/ 实验性扩展** | `gtx/*.hpp` | 129（.hpp + .inl） | 核心类型 + 函数库 | 非稳定 API，GLM 本身标记为实验性。不在当前迁移考虑范围内 | **不在范围内** |
...
| 审查意见 | 处理方式 |
|---------|---------|
| §4.6 T4 判定标准残留"预期"措辞 | 修改——将"设计层面预期编译运行正常"改为确定性陈述："经分析确认：`Int64` 为 CangJie 非类型模板参数的合法类型，设计层面编译通过" |
| §4.6 T6 "经实践校准确认"断言缺乏可追溯验证依据 | 修改——因无法提供具体校准数据源，将措辞降级为设计意图：删除"已通过实践校准确认"和"经实践校准确认"断言，改为"需在实施完成后校准验证" |
| §0.5.1 运算符排除决策内容与章节主题不一致 | 修改——将 `%=` 和 `++`/`--` 的排除理由从 §0.5.1 移至 §4.5.1（新增"运算符集缩减决策"节），§0.5.1 仅保留标注策略和标注量统计的核心内容 |
