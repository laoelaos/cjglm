...
- 若仅保留算术复合赋值（`+=`、`-=`、`*=`、`/=`），不含 `%=` 和自增自减，则为 **4 × 4 = 16 处标注**
...
> **统计方法说明**：以上"被引用文件数"统计的是**直接引用**——即源文件中显式 `#include` 该头文件，不包括传递引用。统计范围为 `glm/` 目录下全部 `.hpp`、`.inl`、`.cpp` 源文件（不含测试和示例代码）。方法：对每个目标头文件，使用 `grep -r "#include.*<目标文件名>" glm/* --include="*.hpp" --include="*.inl" --include="*.cpp"` 搜索直接引用，然后统计唯一文件数。`glm/` 目录下源文件总计 421 个（280 个 `.hpp`、140 个 `.inl`、1 个 `.cpp`，不含测试和示例），其中涉及的核心文件约 9 个（不含 shim 层）。
...
> **说明**：类型级分析与文件级分析结论一致——`vec<N,T,Q>`（N=2,3,4）是其他核心类型中被引用最多的类型层，位于依赖拓扑的最核心位置。`vec1` 虽然被较少类型引用，但与 vec2-4 共享相同的模板实现模式，边际成本极低。向量类型在所有核心类型中的类型级被引用数（7-8 个独特类型使用者）显著高于矩阵（0-1 个）和四元数（0 个），确认了向量类型作为迁移核心的结论。
...
|------|---------|-------------|---------------|------|
| 1 | `qualifier` 枚举 + `vec/mat/qua` 前向声明 | 36 | 16 | 绝对基础——所有 GLM 类型和函数库均依赖。必须最先迁移 |
| 2 | `setup.hpp`（含 `length_t`、`GLM_CONFIG_*`） | 36 | — | 配置层，每个 GLM 文件均包含。作为基础设施迁移 |
...
1. **向量类型（vec2/3/4）是所有核心类型中依赖度最高的类型层**——每个向量类型被 7-8 个其他唯一类型定义直接引用，而矩阵类型最多被 1 个，四元数为 0。这验证了向量类型应作为迁移核心的结论。
2. **基础设施层（setup/qualifier）是无条件的绝对前置依赖**——全部 36 个 GLM 源文件均直接引用它们，必须完整覆盖。
3. **vec1 是向量类型中依赖度最低的**——被 11 个文件引用，但加入的边际成本极低（与 vec2-4 共享模板模式）。
4. **矩阵和四元数并非最基础类型**——它们的引用数仅为向量的 1/3 到 1/5，且自身不构成其他类型的依赖基础。
...
1. **`setup.hpp` 对应的配置层** — 目标语言中的配置/平台抽象，包含 `length_t` 类型定义。定量评估：`setup.hpp` 共 1188 行 C++ 预处理器逻辑。移植的部分包括 `length_t` 定义、`GLM_CONFIG_*` 常量等效声明（**18 个宏（完整清单见下表）**，其中 **`GLM_CONFIG_SIMD` 显式设为 `GLM_DISABLE`** 以避免 `type_vec3.inl`/`type_vec4.inl` 条件包含不在范围内的 `type_vec_simd.inl`）、`uint` 类型别名、`countof` 辅助函数，合计约 80–120 行；剩余约 1000+ 行平台/编译器/架构检测代码标记为"不直接迁移"，该检测功能的 CangJie 映射方案**尚待验证**——CangJie 不存在与 C++ 预处理器等价的平台/编译器/架构检测机制。`setup.hpp` 等价实现中仅提供 `length_t` 定义、`GLM_CONFIG_*` 常量等效声明和基础类型别名，不包含平台/编译器/架构检测逻辑
...
> **`_swizzle.hpp` 迁移说明**：该文件使用 `reinterpret_cast<T*>(_buffer)` 和 `char _buffer[1]` 模式实现底层 reinterpret 访问。CangJie **不直接支持 reinterpret_cast**。默认禁用 swizzle（配置 `GLM_SWIZZLE_DISABLED`），将 `_swizzle.hpp` 排除出迁移范围。swizzle 功能待后续通过 CFFI `unsafe` 块或替代 API 设计重新实现。
>
...
| 验收准则 | 范围级描述 | 可执行验证步骤 |
|---------|----------|--------------|
| 依赖闭合性 | 范围内所有文件的 `#include` 依赖链不超出范围声明的文件集合 | **步骤**：在 CangJie 空项目中创建 `src/glm/` 目录，将迁移的 11 个 GLM 源文件 + 3 个 shim 文件共 14 个文件按依赖拓扑放入，每个文件头部添加对应的 `package glm.detail` 或 `package glm` 声明。执行 `cjpm build`，检查是否缺失任何未在范围内的引用。**通过条件**：编译通过，无 `file not found` 错误 |
| 拓扑可编译性 | 文件可按依赖拓扑顺序逐层编译，每层无遗漏外部依赖 | **步骤**：按子范围（1→2a,2b→4,5,6,7→8）顺序逐层创建临时包并编译。**验证脚本伪代码**：<br><pre>// 子范围 1 验证<br>package round1_sub1<br>// setup.cj (package glm.detail, 包含 qualifier 和 length_t 定义)<br>// qualifier.cj (package glm.detail, 包含 Qualifier 接口及实现类型和前向声明)<br>// 编译: cjc src/glm/detail/setup.cj src/glm/detail/qualifier.cj ✓</pre> |
| 边界隔离性 | 范围不依赖任何未迁移的 GLM 组件 | **步骤**：在 CangJie 项目中仅纳入声明的 11 个 GLM 源文件 + 3 个 shim 文件，确认所有 `import` 语句引用的包/文件不超出此集合；执行 `cjpm build`。**通过条件**：编译通过，且编译错误信息中不包含任何范围外的文件缺失提示 |
| 基础类型可用性 | 所有向量类型支持构造、分量访问和基本算术运算 | **步骤**：在编译通过后，编写如下 CangJie 测试代码并运行：<br><pre>package glm_test<br>import glm.detail.{ Vec4, PackedHighp }<br>main() {<br>    let v = Vec4&lt;Float32, PackedHighp&gt;(1.0f32, 2.0f32, 3.0f32, 4.0f32)<br>    println("${v[0]} ${v[1]} ${v[2]} ${v[3]}")<br>    let sum = v + v<br>    println("${sum[0] == 2.0f32}")<br>}</pre>**通过条件**：编译运行通过，输出符合预期 |
| 类型别名可用性 | 声明的向量类型别名可正确实例化 | **步骤**：编写 CangJie 测试：<br><pre>package glm_test<br>import glm.{ Vec2Float32, Vec3Float32, DVec4 }<br>main() {<br>    let v2: Vec2Float32 = Vec2Float32(1.0f32, 2.0f32)<br>    let v3: Vec3Float32 = Vec3Float32(1.0f32, 2.0f32, 3.0f32)<br>    let v4: DVec4 = DVec4(1.0f64, 2.0f64, 3.0f64, 4.0f64)<br>    println("${v2[0]} ${v3[1]} ${v4[2]}")<br>}</pre>**通过条件**：编译运行通过 |
...
| R2. `setup.hpp` 平台检测映射待验证 | **中** | CangJie 无预处理器，`setup.hpp` 中 1000+ 行平台/编译器/架构检测无直接等价映射 | 仅迁移基础配置（~100 行），平台检测映射另作处理。编译验证不依赖此功能 |
...
以下内容记录类型迁移完成后可继续扩展的组成部分，供参考：

| 内容 | 依赖前提 | 说明 |
|------|---------|------|
| 矩阵类型（mat2x2 ~ mat4x4，9 个） | 向量类型就绪 | 实现模式与 vec 类似，可并行实现 |
| 四元数类型（qua） | 向量类型 + 矩阵类型就绪 | 依赖 mat3x3、mat4x4、vec3、vec4 |
| ext/ 别名文件 | 对应类型就绪 | 仅含 type alias 和精度变体 |
| 函数库（common、matrix、geometric 等） | 核心类型就绪 | 需按拓扑顺序分批迁移 |
| SIMD 平台特化 | 核心功能稳定 | 平台相关，按需添加 |
| swizzle 功能 | 核心类型就绪 | 需重新设计，放弃 reinterpret 模式 |
| 平台检测映射 | — | 首轮不包含，需单独评估 |
| gtx/ 实验性扩展 | 核心功能 + 函数库就绪 | 按实际需求选择性迁移 |
