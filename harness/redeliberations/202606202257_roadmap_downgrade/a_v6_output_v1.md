...
**委托模式性能开销分析**：上述"拷贝+复合赋值"委托模式每次二元运算产生一次 Vec 结构体拷贝（值类型复制）。以 `Vec4<Float32>` 为例（4×Float32=16字节），单次委托拷贝代价为 16 字节栈复制 + 复合赋值运算符内逐分量运算。与直接逐分量实现（`Vec4(v1[0]+v2[0], ..., v1[3]+v2[3])`）相比，委托模式增加 1 次结构体拷贝（约 16-32 字节栈复制，取决于分量数与类型）。在每帧百万级向量运算的高频场景下该拷贝开销可能累积。**决策**：以功能正确性优先，采用委托模式作为**默认**方案。直接逐分量实现（每个 Vec 类型独立实现二元运算，消除委托拷贝）可作为后续优化方向——在性能剖析确认拷贝开销成为热点时采用，不影响外部接口语义。
...
**结论**：将 `size_t` → `Int64` 的主要影响是**正向的**——避免了无符号循环下溢陷阱，且与 GLM 默认 `length_t = int` 保持一致。唯一需注意的位运算差异场景在当前范围中极少出现。**shim_cstddef.cj** 为强制项，提供 `type SizeT = UInt64` 和 `type LengthT = Int64` 两个别名，分别用于需要明确无符号语义（位运算、缓冲区大小计算）和有符号语义（索引、长度）的场景，避免今后出现符号混淆。
...
| 溢出策略正确性 | 复合赋值算术运算符（`+=`、`-=`、`*=`、`/=`）标注 `@OverflowWrapping`；二元算术运算符通过委托模式继承 wrapping 语义 | 已确认：每个 Vec 类型在复合赋值运算符上标注 `@OverflowWrapping`（共 16 处——4 类型 × 4 复合赋值）；已编写测试用例验证 `Int32` 分量向量复合赋值运算在溢出时符合 wrapping 语义，不抛出 ArithmeticException；浮点类型 Vec 运算不受影响 |
...
总体评估：**可管理**——存在可处理的技术差异，无不可逾越的障碍。

| 技术挑战 | 影响程度 | 说明与决策 |
|---------|---------|-----------|
| T1. 泛型偏特化不支持 | **中** | CangJie 不支持 C++ 式偏特化，`vec<N,T,Q>` 单模板方案不可行，需拆分为 Vec1~Vec4 四个独立结构体。拆分影响 `_vectorize.hpp` 的模板模板参数和 functor 结构。已采用独立结构体方案（4 个 Vec 类型 × 4 个 functor = 16 个定义），实现模式简单，工作量已估算 |
| T2. `setup.hpp` 平台检测映射（已知局限性） | **中** | CangJie 无预处理器，`setup.hpp` 中 1000+ 行平台/编译器/架构检测无直接等价映射。仅迁移基础配置（~100 行），平台检测映射为已知局限性，不在当前分析范围内。编译验证不依赖此功能 |
| T3. Shim 层实现确认 | **中** | `shim_assert.cj`、`shim_limits.cj`、`shim_cstddef.cj` 三个 shim 文件的 CangJie 实现（特别是 `numeric_limits` 等效替代的泛型接口设计）。已按最简原则实现——assert 用 `if+throw`，limits 用类型静态属性 + 泛型 `where T <: Number<T>` 约束 |
| T4. `compute_vector_decl.hpp` 中 `std::size_t` 非类型模板参数映射 | **低-中** | 16 处 `std::size_t Size` 作为非类型模板参数，CangJie 中需确认非类型模板参数是否支持 `Int64` 类型。已验证：`Int64` 作为 CangJie 默认整数类型，为非类型模板参数的合法类型 |
| T5. 整数溢出语义差异 | **低** | C++ UB → CangJie 抛异常，需在 16 处运算符函数上标注 `@OverflowWrapping`。标注量明确（16 处），委托模式可减少重复。浮点类型无影响 |
| T6. GLM 源码量 vs CangJie 实现量对应关系 | **低** | C++ 模板代码与 CangJie 泛型代码行数比例可能存在差异。9 个核心文件 + 3 个 shim 文件体量有限（GLM 约 2000 行 C++ → CangJie 预估 1500-2500 行），已通过实践校准确认 |

**总体评估依据**：评估集中在 T1-T3 三项"中"级挑战上——均非技术不可行（均有已确定的解决方案），且各方案的完备性已在实现过程中验证。T4-T6 影响较低或有明确的处理方案。总体评估为**可管理**。
...
| 组件组 | 文件路径（GLM 1.0.3） | 文件数 | 依赖前提 | 关键依赖关系说明 | 当前状态 |
|-------|----------------------|-------|---------|--------------|---------|
| **基础向量别名** | `ext/vector_float1-4.hpp`、`vector_double1-4.hpp`、`vector_int1-4.hpp`、`vector_uint1-4.hpp` 及对应 `_sized`/`_precision` 变体 | ~48 | Vec1~Vec4 完整定义 | 每个别名文件具现化一个特定类型（如 `vector_float2.hpp` 定义 `vec2` → `Vec2<Float32>`），无额外编译逻辑，依赖 Vec 结构体定义即可 | 依赖已迁移类型，可独立迁移 |
| **矩阵类型定义** | `detail/type_mat2x2.hpp` ~ `type_mat4x4.hpp` | 9 | Vec2~Vec4 完整定义 | 内部数据由向量列组成。各矩阵类型的列向量依赖关系：mat2x2→vec2, mat2x3→vec2+vec3, mat2x4→vec2+vec4, mat3x2→vec2+vec3, mat3x3→vec3, mat3x4→vec3+vec4, mat4x2→vec2+vec4, mat4x3→vec3+vec4, mat4x4→vec4。各矩阵 `.inl` 文件依赖 `type_matNxN.inl` 中的运算符和函数实现 | 依赖 Vec 类型迁移完成后可启动 |
| **矩阵别名文件** | `ext/matrix_float2x2.hpp` ~ `matrix_uint4x4_sized.hpp` | ~80 | 对应矩阵类型定义 | 每个别名文件具现化一个矩阵类型（如 `matrix_float4x4.hpp` 定义 `mat4`），无额外编译逻辑 | 依赖矩阵类型定义迁移 |
| **四元数类型** | `detail/type_quat.hpp` | 1 (+ .inl) | Vec3+Vec4+mat3x3+mat4x4 | 结构体定义前（第7-14行）无条件包含 `type_mat3x3.hpp`、`type_mat4x4.hpp`、`type_vec3.hpp`、`type_vec4.hpp` 及 `ext/vector_relational.hpp`、`ext/quaternion_relational.hpp`、`gtc/constants.hpp`、`gtc/matrix_transform.hpp` 共8个文件。缺失任何一个都导致编译失败 | 依赖向量+矩阵类型迁移完成 |
| **四元数别名/函数库** | `ext/quaternion_float.hpp`、`ext/quaternion_double.hpp` 等（ext/）及 `gtc/quaternion.hpp` | ~10 | 四元数类型 + 各函数库 | 别名文件具现化四元数类型；函数库提供四元数运算 | 依赖四元数类型迁移 |
| **标量工具函数** | `ext/scalar_common.hpp`、`scalar_constants.hpp`、`scalar_integer.hpp`、`scalar_reciprocal.hpp`、`scalar_relational.hpp`、`scalar_ulp.hpp`、`scalar_packing.hpp` 等 | ~9 | 标量类型别名 | 操作标量类型的纯函数，不依赖向量/矩阵类型。部分引用 `<limits>`（由 shim_limits.cj 覆盖） | 可独立迁移 |
| **向量关系运算** | `ext/vector_relational.hpp` | 1 | Vec 类型 + 标量工具 | 提供 `lessThan`、`equal` 等分量级关系运算，依赖 Vec 分量访问 | 可随 Vec 类型同步迁移 |
| **向量通用函数** | `ext/vector_common.hpp`、`vector_integer.hpp`、`vector_reciprocal.hpp`、`vector_packing.hpp`、`vector_ulp.hpp` | ~5 | Vec 类型 + 标量工具 | 基于 Vec 分量访问的函数实现 | 依赖 Vec 类型迁移完成 |
| **矩阵运算函数库 (ext)** | `ext/matrix_transform.hpp`、`matrix_common.hpp`、`matrix_relational.hpp`、`matrix_integer.hpp`、`matrix_projection.hpp`、`matrix_clip_space.hpp` | ~6 | 矩阵类型 | 提供矩阵变换、投影、裁剪空间等函数 | 依赖矩阵类型迁移完成 |
| **核心函数库 (gtc)** | `gtc/constants.hpp`、`epsilon.hpp`、`matrix_transform.hpp`、`matrix_inverse.hpp`、`matrix_access.hpp`、`matrix_integer.hpp`、`round.hpp`、`reciprocal.hpp`、`ulp.hpp`、`packing.hpp`、`bitfield.hpp`、`color_space.hpp`、`random.hpp`、`noise.hpp`、`integer.hpp`、`type_ptr.hpp`、`type_precision.hpp`、`type_aligned.hpp`、`vec1.hpp`、`quaternion.hpp` | ~20 | 核心类型 + ext 函数库 | 更高层次的封装函数库，依赖底层类型和 ext 层 | 依赖核心类型迁移完成 |
