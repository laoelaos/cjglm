...
| 文件 | 操作 | 前置依赖 | 测试 |
|------|------|---------|------|
| gtc/matrix_transform.cj | 替换 stub → 完整实现 | ext/matrix_transform, ext/matrix_projection, ext/matrix_clip_space, geometric, trigonometric, matrix(detail) | 64 个函数抽样验证（每系族选 1~2 个）+ _slow 变体与常规版本输出一致验证 + gtc 与 ext 同名函数输出一致验证 |
