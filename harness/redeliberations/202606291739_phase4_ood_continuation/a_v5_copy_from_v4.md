...
| 审查意见 | 修改措施 |
|---------|---------|
| P1（严重）— gtc/noise.cj 周期性 Perlin 噪声仅覆盖 4D，遗漏 1D/2D/3D | §3.3 gtc/noise.cj 中将仅 4D 的周期重载扩展为 1D/2D/3D/4D 各一周期重载（`perlin1D(x, period)`、`perlin2D(v, period)`、`perlin3D(v, period)`、`perlin4D(v, period)`），签名约束及实现路径参照 4D 版本 |
| P2（严重）— gtc/random.cj 种子策略使用未经验证的 `Thread.currentThread().id` | §3.3 gtc/random.cj 种子生成策略中将 `Thread.currentThread().id` 替换为 `std.env.getProcessId()`（仓颉标准库 API，无需额外验证并将进程 ID 混入时间戳）。同步更新 D28 设计决策条目 |
| P3（中等）— gtc/ulp.cj float_distance 描述"个数"暗示非负，与有符号返回类型 Int32/Int64 矛盾 | §3.3 gtc/ulp.cj 中 `float_distance` 描述从"可表示浮点数的个数"修正为"有符号差值（以 ULP 为单位），结果可正可负" |
| P4（中等）— gtc/ulp.cj 未讨论 Float16 覆盖策略 | §3.3 gtc/ulp.cj 末尾新增 Float16 策略声明：当前不提供 Float16 重载（标注为已知缺失），理由为 Float16 ULP 在图形计算中需求近乎为零；`Float16` 的 `toBits()/fromBits()` 可支持未来补充 |
| P5（中等）— ext/quaternion_common.cj mix 描述存在不确定性措辞"可能需要" | §3.2 ext/quaternion_common.cj 中 mix 描述从"可能需要 common.cj 的 clamp"修正为"使用 common.cj 的 clamp 将 a 截断至 [0,1] 后执行插值（见 D10）" |
| P6（一般）— gtc/noise.cj 命名约定变更（perlin→perlin1D 等拆分）未在 §7 记录为设计决策 | §7 设计决策表中新增 D31 条目，记录噪声函数拆分命名的原因（仓颉不支持整数维度泛型参数，拆分命名是 C++ 模板维度参数的自然替代） |
| P7（一般）— fwd.cj 与 gtc/type_precision.cj 的别名关系未澄清 | §3.3 gtc/type_precision.cj 别名清单后新增「与 fwd.cj 的关系」段落，阐明 `fwd.cj` 定义泛型类型、`type_precision.cj` 定义具体别名，两者命名不重叠（`Vec2` vs `fvec2`），命名空间路径不同，无冲突 |

## 修订说明（v16 → v2 外部审查迭代）

| 审查意见 | 修改措施 |
|---------|---------|
| P1（严重）— `shear` 函数参数类型错误且与 GLM 1.0.3 不符 | §3.2 ext/matrix_transform.cj 中 `shear` 签名从 `(m, p, l_x, l_y, l_z)` 修正为 `shear<T, Q>(m: Mat4x4<T, Q>, p: Vec3<T, Q>, l_x: Vec2<T, Q>, l_y: Vec2<T, Q>, l_z: Vec2<T, Q>): Mat4x4<T, Q>`；§3.3 gtc/matrix_transform.cj 中 `shear_slow` 签名同理修正 |
| P2（严重）— `slerp` 第四参数 `k` 类型未指定 | §3.2 ext/quaternion_common.cj 中 `slerp(x, y, a, k)` 补充完整签名为 `slerp<T, Q>(x: Quat<T, Q>, y: Quat<T, Q>, a: T, k: T): Quat<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier` |
| P3（中等）— `shear_slow` 实现路径描述错误（声称使用 `tan`） | §3.3 gtc/matrix_transform.cj 中将 "`tan` 计算剪切因子" 删除，替换为 "从 `Vec2` 参数提取分量构造剪切矩阵" |
| P4（严重）— `stdmath_shim.cj` 未给出完整包装函数清单 | §1.4 中新增完整包装函数清单（24 个函数），标注每个包装函数的签名和委托路径（`std.math.<func>`），按指数/对数/幂、三角函数、双曲函数、舍入/绝对值四组分类列出 |
| P5（中等）— 阶段四未包含任何测试/验证策略 | §8 实施计划中为每批新增"测试"列，为每个文件定义具体的测试用例和验证目标；覆盖边界值、NaN/Inf 传播、互逆验证、编译验证等维度 |
| P6（中等）— `round.cj` NaN/Inf/零值边界行为未定义 | §3.3 gtc/round.cj 中新增边界行为表，为全部 6 个 round 函数按 NaN/+Inf/-Inf/±0/负数输入五类定义行为，参照 GLM 1.0.3 `gtc/round.inl` |
| P7（中等）— `noise.cj` 实现复杂度被低估（250~300 行） | §3.3 gtc/noise.cj 中将工作量评估从 "约 250~300 行" 修正为 "约 500~800 行"（含辅助函数和 8 个公共噪声函数） |
| P8（中等）— `lib.cj` 第 23 行修改缺少精确描述 | §8 lib.cj 更新中补充 lib.cj 第 23 行修改前的完整行内容（`public import glm.gtc.{identity, translate, rotate, rotate_slow, scale, scale_slow, shear, shear_slow, lookAt, lookAtRH, lookAtLH}`）和修改后的完整行内容（`public import glm.gtc.{identity, rotate_slow, scale_slow, shear_slow}`），含每项修改理由 |
| P9（轻微）— D16 向后不兼容影响的波及范围未分析 | §7 D16 设计决策条目补充波及范围分析结论：经搜索阶段一~三全部源码，未发现任何调用 `detail.geometric` 函数时使用整数向量类型的现成代码，理论上的向后不兼容变更在实际代码库中无受影响调用点 |
| P10（中等）— 覆盖验证报告的 F1-F3 问题未被产出修复 | F1（shear 签名）对应 P1 已修正；F2（slerp k 类型）对应 P2 已修正；F3（shear_slow 描述）对应 P3 已修正 |

## 修订说明（v2 → v3）

| 审查意见 | 修改措施 |
|---------|---------|
| P1（严重）— 阶段四范围定义未与 `docs/02_roadmap.md` 建立显式交叉验证链 | 在 §1.5 之后新增「阶段四范围与路线图的交叉验证映射」子章节，建立从路线图"阶段 4"章节到本设计方案各 § 的显式映射表，标注 `angleAxis` 文件归属差异（路线图归入 ext/quaternion_transform.cj，本设计置于 ext/quaternion_trigonometric.cj）及理由 |
| P2（中等）— §5 错误处理策略表遗漏 `stdmath_shim.cj` Float16 溢出异常场景 | 在 §5 错误处理策略表中新增一行，记录 `(result as T).getOrThrow()` 在 `T=Float16` 且中间值超过 ±65504 时的转型溢出异常场景及处理策略，标注消除此差异的可选方案 |
| P3（一般）— `ext/quaternion_exponential.cj` 最后 4 个 stub 缺少后续纳入规划 | 在 §1.5 末尾补充后续规划说明，明确选型：按路线图阶段 6（gtx 实验性扩展）的"按需实现"策略处理，不做强制迁移要求 |
| P4（一般）— `lib.cj` 第 23 行修改策略缺少 gtc↔ext 行为等价性验证项 | 在 §8 第四批 gtc/matrix_transform.cj 测试列中补充明确说明：需重点覆盖 lib.cj 第 23 行修改涉及的 translate/rotate/scale/shear/lookAt/lookAtRH/lookAtLH 的 gtc↔ext 输出一致性验证 |

## 修订说明（v14 → v15）

| 审查意见 | 修改措施 |
|---------|---------|
| 问题 1（严重—依赖表事实错误）：§2 依赖表第 248 行 `common.cj` 依赖条目遗漏 `stdmath_shim.cj` | §2 模块间依赖第 248 行将 `common.cj → qualifier, setup（泛型约束依赖）` 修正为 `common.cj → qualifier, setup, stdmath_shim.cj（frexp 委托 logT、ldexp 委托 powT）`，与 §3.1 common.cj 职责中声明的 frexp/ldexp 委托路径一致 |
| 问题 2（中等—实施批次规划自相矛盾）：§8 第 1013 行 trigonometric.cj 注释"独立于第一批，与第一批可并行"与依赖表第 250 行 `trigonometric.cj → stdmath_shim.cj`（第一批）矛盾 | §8 实施批次第二批 trigonometric.cj 的前置依赖注释从"（独立于第一批，与第一批可并行）"修正为"，stdmath_shim.cj（第一批）"，采用方案 A（依赖表正确，批次关系以依赖表为准） |
| 问题 3（轻微—计数误差）：§1.4 第 52 行声明"共 **24 个**包装函数"与第 57~88 行逐行计数 25 个不符 | §1.4 中将 "24 个" 修正为 "25 个"（实际 delegating wrappers 清单含 sqrtT/fmodT 共 25 个，inversesqrtT 非委托已排除） |

## 修订说明（v5 → v6）

本轮修订基于内部审查结果（P1/P2/P3，源自历史外部第 4 轮持续存在的问题）进行完善。

| 审查意见 | 修改措施 |
|---------|---------|
| P1（严重）— lib.cj 最终编译验证未纳入实施批次规划 | 在 §8「lib.cj 更新」末尾新增「lib.cj 编译验证步骤」子章节，包含 3 项验证：`cjpm build` 编译验证、关键跨包同名函数重载决议验证（mix/exp/log/pow/sqrt/translate/perspective）、lib.cj 第 23 行修改验证。归入「所有批次完成后」条目，与 lib.cj 修改同级。 |
| P2（一般）— ext/matrix_projection.cj unProject 系族隐式矩阵求逆的奇异场景未覆盖 | 在 §5 错误处理策略表中新增一行「unProject 系族隐式矩阵求逆（proj×model 奇异）」，说明其行为与 matrix.cj inverse 一致（IEEE 754 自然传播 NaN/Inf，不抛异常），并注明此场景不重复设计决策。 |
| P3（一般）— §4 关键数据流与控制流章节深度不足 | 在 §4 末尾新增 §4.7「深度数据流与控制流分析」子章节，为 slerp、lookAt、inverse(Mat4x4) 三个关键函数补充完整分析：(1) slerp：从 quaternion_common.slerp 到 common.clamp → trigonometric.acos/sin 的完整调用链，每步类型约束、控制流分支（sinOmega < epsilon 退化路径）和异常传播路径（NaN/Inf 在四元数中的填充传播）；(2) lookAt：从 eye/center/up Vec3 到 Mat4x4 的完整变换路径，含 normalize(零向量退化) → cross → 矩阵构造各步的控制流分支和异常传播；(3) inverse(Mat4x4)：余子式展开的 5 步数据流路径，重点展示 det=0 时 1/det → Inf → Inf×0 → NaN 在结果矩阵中的填充传播路径。 |
