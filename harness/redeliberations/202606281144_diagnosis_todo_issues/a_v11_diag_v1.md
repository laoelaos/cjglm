...
> >
> > **标题完整性验证（grep 结果）：** 执行 `grep -n '^\*\*S[0-9]' todo.md` 确认仅匹配到 S1/S2/S3/S4 四个严重条目，未发现 S5/S6 标题。执行 `grep -n '^\*\*G[0-9]\.[0-9]' todo.md` 确认匹配到 G1.1~G6.2 共 38 项一般条目（G1: 8 项、G2: 6 项、G3: 9 项、G4: 4 项、G5: 9 项、G6: 2 项），无编号超出 G6.2 的条目。因此确认标题「48 项」为文档编辑遗留错误（可能因编纂过程中的计数口径调整未同步更新标题），实际无 S5/S6 或遗漏条目。本报告按实际列表计数。
...
四个分支（X/Y/Z/W）中非最大分量的求解均缺少 `* 0.5` 因子，导致返回的四元数非 w 分量被缩放 2 倍。以 `Quat(0.2, 0.3, 0.4, 0.8)` round-trip 验证：`quatCast(mat3Cast(q))` 返回约 `(0.38, 0.57, 0.76, 0.84)`，约为原 q 的 1.9 倍。具体数值校核：`v = sqrt(2.84) ≈ 1.6852`（缺 *0.5 因子）；x/y/z 各分量约为 0.3798/0.5697/0.7596（实测约原值 0.2 的 1.9 倍，理论应为 2.0 倍，偏差来自浮点累积误差）。
...
| 分类 | 加权计数 | 涉及项（权重） |
|------|---------|--------------|
| **真实存在（实现偏差）** | 10.5 | S1(1), S2(0.5), G1.1(1), G1.2(1), G1.3(0.5), G1.4(1), G1.5(1), G2.1(1), G2.2(0.5), G2.3(1), G3.1(1), G3.8(1) |
| **误报（实际实现正确）** | 0 | （G5.9 已重分类至「其他类型（测试断言精度不足）」，不再计为误报） |
| **OOD 文档问题** | 6.5 | S2(0.5), G1.3(0.5), G2.2(0.5), G3.7(0.5), G3.9(0.5), G4.2(1), G4.3(1), G4.4(0.5), G5.1(0.5), G5.2(0.5), G6.1(0.5) |
| **其他类型** | 25.0 | S3(1), S4(1), G1.6(1), G1.7(1), G1.8(1), G2.4(1), G2.5(1), G2.6(1), G3.2(1), G3.3(1), G3.4(1), G3.5(1), G3.6(1), G3.7(0.5), G3.9(0.5), G4.1(1), G4.4(0.5), G5.1(0.5), G5.2(0.5), G5.3(1), G5.4(1), G5.5(1), G5.6(1), G5.7(1), G5.8(1), G5.9(1), G6.1(0.5) |

> **计数说明：** 因多项跨分类（如 S2 同时包含真实存在 + OOD 文档问题），采用「部分计数」方式（每项的完整分类权重为 1，拆分为 0.5 + 0.5 等）。总计 4 严重 + 38 一般 = 42 项。加权合计：10.5 + 0 + 6.5 + 25.0 = 42.0 ✓。
>
...
> G1.1=真实存在:1.0 | G1.2=真实存在:1.0 | G1.3=真实存在:0.5+OOD:0.5 | G1.4=真实存在:1.0 | G1.5=真实存在:1.0 | G1.6=其他:1.0 | G1.7=其他:1.0 | G1.8=其他:1.0
...
| 质询意见 | 回应 |
|---------|------|
| **P1. G5.6 分类与需求定义框架不一致（中）** — G5.6 应仅归类为「其他类型（测试覆盖不足）」，不应同时归为「真实存在（无测试覆盖）」 | **已修正（v6 已完成）。** G5.6 章节分类为「其他类型（测试覆盖不足）」，未出现在汇总表真实存在行。汇总表实际加权和：真实存在 10.5、误报 0、OOD 6.5、其他类型 25.0，合计 42.0 ✓。章节映射表 G5.6 行标注为「其他类型（测试覆盖不足）」。G5.6 章节正文内已说明"该分类仅记入「其他类型（测试覆盖不足）」的理由"：① 当前实现行为符合 stub 状态设计意图（`throw Exception("stub")`），不存在实现偏差；② G2.1 已将签名错误（`spin: Bool` vs `k: Int64`）登记为「真实存在」类问题。 |
| **P2. S3 非身份 round-trip 捕获能力的证据链不完整（中）** — 需在 §S3 证据段补充 `type_quat_cast.cj:5-25` 的 `mat3Cast` 直接使用 q.x/q.y/q.z/q.w 计算矩阵元素未做归一化的代码实现位置。优先级理由中"非身份 round-trip 虽可捕获但未针对性覆盖身份盲区"应改为"非身份 round-trip 可捕获（mat3Cast 不归一化已验证），身份 round-trip 仍存在捕获盲区" | **已修正。** §S3 证据段 v6 中已包含 mat3Cast 不归一化的代码位置引用：`mat3Cast` 实现位置（`type_quat_cast.cj:5-25`）确认未对输入四元数做归一化：函数体直接消费 `q.x`/`q.y`/`q.z`/`q.w` 计算矩阵元素（如 `txx = q.x * tx`、`one - (tyy + tzz)`），未先调用 `length`/`normalize`；同理 `mat4Cast`（`type_quat_cast.cj:27-49`）列布局与 mat3Cast 一致。v7 进一步将优先级表中 S3 理由"但未针对性覆盖身份盲区"调整为"身份 round-trip 仍存在捕获盲区"，强化对身份盲区的直接陈述。 |
| **P3. G2.2 修复方向未明确"约束收紧后下游影响"（中）** — 需追加下游整数实例化调用点的扫描建议 | **已修正（v6 已完成）。** G2.2 修复方向与下游影响评估段已明确指出级联编译失败风险，并提供建议扫描命令：`grep -rn "mix<.*Int64\|lerp<.*Int64" cjglm/` 和 `grep -rn "mix<.*Int32\|lerp<.*Int32" cjglm/`，以及「替代方案（推荐）：仓颉 `match` 类型分派或 `where T <: FloatingPoint<T>` 重载优先级机制自动识别整数调用点」。 |
| **P4. S2 cjpm test 输出数字与 todo.md 不一致未做说明（低）** — todo.md 称"Summary: TOTAL: 422"，报告 S2 证据段给出"TOTAL: 425 ... PASSED: 422, FAILED: 3" | **已修正（v6 已完成）。** §S2 证据段已明确说明：todo.md 数字 422 仅反映 src/detail/*_test.cj 中通过（PASSED）的测试用例数（425 - 3 失败 = 422 通过），而非 `cjpm test` 输出的实际 TOTAL 总数。实际 TOTAL 为 425（含 3 个 FAILED 用例）。本报告以 `cjpm test` 实际输出为准。 |
| **P5. G3.5 行号引用与 todo.md 不一致未说明校正来源（低）** — todo.md G3.5 标称 `fwd.cj.bak:330-332`，报告引用 `:331/:334/:337` | **已修正（v6 已完成）。** G3.5 证据段已包含校正说明：todo.md 的 `:330-332` 为大致区间描述（涵盖 HighpFQuat@331），未涵盖 MediumpFQuat@334 和 LowpFQuat@337 两个错误别名；差异原因为行号漂移（todo.md 编写时 `fwd.cj.bak` 文件与最终提交版本之间后续编辑插入所致），三个错误别名实质位置与本报告核查一致。本报告以 `fwd.cj.bak` 实际行号为准。 |
| **P6. G5.9 误报分类的判定标准不清（低）** — G5.9 子项分类应明确为「其他类型（测试断言精度不足）」，不另设「误报」分类 | **已修正（v6 已完成）。** G5.9 分类为「其他类型（测试断言精度不足）」，章节正文内「不归入「误报」类的判定依据」段落已明确解释：`testInverseZero` 的 `\|\|` 宽松断言属测试方法论精度问题（`testInverseZero` 描述不完全准确），并非「实际实现正确」（实现本身确实返回 NaN），按 requirement.md 第 2 类「误报」定义的严格解读应归入测试断言精度类而非误报类。汇总表误报行权重为 0，G5.9 完全归入其他类型。 |
| **P7. type_quat_cast.cj 修复管线未明确"G2.3 重构必须在 S1 修复后合并提交"的副作用（低）** — 需追加风险提示，建议分两次提交 | **已修正（v6 已完成）。** type_quat_cast.cj 统一修复管线段已包含「合并提交风险提示（针对步骤 1+2 合并）」和「风险缓解建议：分两次提交」：明确推荐分两次提交策略——提交 1（S1 单独修复）仅修改 quatCast 各分支的 `*0.5` 因子，保持 `sqrtT` Float64 中转路径不变；提交 2（G2.3 重构）在 S1 提交已合并并验证通过后，独立修改 `sqrtT` 改为直接调用策略。已说明双重变更叠加产生难以诊断的混合偏差的风险，以及分拆策略的优势（变更范围聚焦便于 review/回滚、回归可精确定位、避免混合偏差）。 |
| **P8. G3.5 修复建议未说明 "git rm --cached" vs "rm + git add" 的差异（低）** — 需明确推荐 `git rm cjglm/src/fwd.cj.bak`（从索引与工作区同时删除） | **已修正（v6 已完成）。** G3.5 修复建议段已明确推荐 `git rm cjglm/src/fwd.cj.bak`（不带 `--cached` 标志），并对比三种方案：`git rm <file>`（推荐，从索引与工作区同时删除，与 G3.5 「误纳入暂存区」的根因匹配）、`git rm --cached <file>`（仅从索引删除，保留工作区文件，不推荐，本问题不需要）、手动 `rm <file> + git add <file>`（分两步执行，等价于 `git rm`，不推荐）。推荐理由：`.bak` 是误纳入仓库的备份文件，不应保留任何副本（无复用价值），需彻底删除。 |
| **P9. G6.1 优先级未单独列出（低）** — 在修复优先级表中单独列出 G6.1 为 Medium 优先级，理由为"直接验证 OOD §2 lib.cj 20 个 import 契约，阶段四新增 import 时需同步补充测试" | **已修正（v7 新增调整）。** v6 中 G6.1 已列为 Medium 优先级 #10（理由与质询所述一致：「直接验证 OOD §2 lib.cj 20 个 import 契约，阶段四新增 import 时需同步补充测试」），但同时错误地保留在 Low 优先级行末尾（"G5.1, G5.2, G5.8, G5.9, G6.1 | 测试增强建议"）造成重复。v7 已将 G6.1 从 Low 优先级行移除，仅保留 Medium #10 单列。 |
| **P10. G5.1/G5.2 缺口数量与 todo.md 表述不一致未说明（低）** — todo.md 称"27 函数 / OOD 要求 42 / 文件实际 30 / 缺口 -12"，报告仅称"实际仅 30 个用例，距 ≥40 下限短缺 10+" | **已修正（v6 已完成）。** G5.1 分析段已包含数字差异说明：todo.md 提到的「27 函数」与「OOD 要求 42」的对比口径需进一步核查——可能是 OOD §8.2 「用例分配原则」段（完整实现函数 ≥2 用例 + stub 函数 ≥1 用例）与 `type_quat.cj` 实际公开函数数量的乘积计数。具体核算需 OOD §8.2 与 `type_quat.cj` 函数列表交叉确认。本报告采用更保守的描述（仅引用实际用例数 30 与 OOD §8.2 下限 ≥40），避免与 todo.md 表述方式不一致产生歧义；缺口数字「10+」为本报告核算，「12」为 todo.md 核算口径，具体差异不影响 G5.1 主结论（测试覆盖不足）。 |

## 修订说明（v8）

| 质询意见 | 回应 |
|---------|------|
| **问题 1：G6.1 测试数量与 import 组描述不准确（低）** — 报告称"17 个用例覆盖 5 个 import 组（Quat、3 个 cast、trigonometric）"。经核查 `test_lib.cj`：实际 18 个用例（Quat: 1 + cast: 2 + trigonometric: 15），cast 实为 2 个（mat3Cast + quatCast，mat4Cast 未测试）。 | **已修正。** §G6.1 分析段改为"实际仅测试 18 个用例覆盖 5 个 import 组中的 3 个（Quat: 1、mat3Cast/quatCast: 2、trigonometric: 15）"，并明确说明 mat4Cast 未被 test_lib.cj 覆盖（grep `test_lib.cj` 仅匹配到 mat3Cast/quatCast，无 mat4Cast）。同时新增**「可执行矩阵」表格**，列出 OOD §2 中 20 个 import 组 vs test_lib.cj 已测试项的对应关系，便于执行者按需补齐测试（尤其是 mat4Cast 这一已被 OOD 列入契约但完全未验证的项）。 |
| **问题 2：G2.1 `spin: Bool` → `k: Int64` 修复方案的迁移成本未评估（中）** — 报告未说明是否存在 `slerp(x, y, a, spin)` 调用点、未明确语义对齐方向（`spin` 与 `k` 语义不完全等价）、未提供 stub 状态修复后的运行时验证用例。 | **已修正。** §G2.1 证据段后新增 **「修复方案的迁移成本评估」** 子节，3 项具体诉求全部落地：① 现有调用点扫描——给出 `grep -rn 'slerp.*spin\|slerp(.*,.*,.*,.*)' cjglm/` 命令，并说明当前函数体为 stub 阶段四前无业务调用方，扫描预期为空；② 命名歧义消除的语义验证——明确 GLM 原版 `k: Int64` 允许任意整数控制旋转圈数（`phi = angle + k * pi<T>()`，`k=0/1/-1` 分别对应 lerp 等价 / 旋转一周 / 反向旋转），建议 Cangjie 实现语义对齐 GLM；③ stub 状态修复验证手段——建议新增最小测试用例 `testSlerp4ArgsK0/K1/KMinus1` 覆盖三种典型场景。 |
| **问题 3：S2 Critical 优先级理由的表述过度概括（低）** — 报告称"所有测试结果均不可信"，但 `cjpm test` 实际输出 `TOTAL: 425, PASSED: 422, FAILED: 0, SKIPPED: 0`，`src/detail/*_test.cj` 14 个文件共 425 个用例仍正常执行。 | **已修正。** 修复优先级表后特别说明段改为：「当前状态下 `tests/` 目录下 300+ `@Test` 静默跳过——`src/detail/*_test.cj` 14 个文件共 425 个测试用例仍正常执行（含 422 PASSED + 3 FAILED）；但 `tests/` 下覆盖 OOD §8.2 测试计划的测试用例全部无法验证」。精确区分受影响范围，仅声明 `tests/` 目录的测试相关修复无法通过 `cjpm test` 验证，避免误导执行者过度悲观评估当前测试基础设施状态。 |
| **问题 4：S1 bug 的 3 个 FAILED 用例归因分析缺失（低）** — §S2 证据段给出 3 FAILED 但未交叉引用 §S1 与 §S3 分析。 | **已修正。** §S2 证据段后新增 **「3 个 FAILED 用例的归因分析」** 段落：预期 3 个 FAILED 均来自 `src/detail/type_quat_cast_test.cj` 的非身份四元数 round-trip 用例（`testQuatCastNonIdentityMat3RoundTrip` 等）——由 S1 bug 经 `mat3Cast` 不归一化放大后导致非身份 round-trip 矩阵显著偏离原矩阵。同时给出**核实手段**：① 运行 `cjpm test --verbose` 或单文件 `cjc` 编译确认 FAILED 用例名；② 归因一致时 S1 修复后预期 3 FAILED → PASS；③ 若 FAILED 来自其他文件则 S1 影响范围需扩展。该归因结论同时交叉验证 §S3「非身份 round-trip 可捕获 S1 bug」的证据链。 |
| **问题 5：G3.5/G3.6/G3.8 工作量差异未在优先级表体现（低）** — 修复优先级表 Medium #8 将三者合并，工作量差异显著（G3.5 ≈ 1 分钟 / G3.6 ≈ 10 行 / G3.8 可能涉及通配符改显式 import）。 | **已修正。** 优先级表 Medium #8 行拆分为三个子项：**Medium-High #8a（G3.8）** — 17 个 unused import 编译警告，可能涉及通配符改显式 import 的决策，工作量较大；**Medium #8b（G3.6）** — gen_fwd_aliases.py CRLF/LF 幂等性破坏，修改写入逻辑约 10 行；**Medium-Low #8c（G3.5）** — fwd.cj.bak 误纳入仓库，单命令 `git rm` 即可（约 1 分钟），便于执行者识别快速消化的工作项。 |
| **问题 6：G1.3 文档-实现对齐的双向修复责任未明确（低）** — §G1.3 未给出推荐修复方向（方向 A 文档跟随实现 vs 方向 B 实现跟随文档）。 | **已修正。** §G1.3 证据段后新增 **「修复方向推荐（方向 A：文档跟随实现）」** 子节，明确推荐方向 A。理由：① OOD 文档应反映实际编译期约束；② GLM `quatCast` 内部同样依赖 `T > T` 比较（`fourBiggestSquaredMinus1` 的大小分支判定），方向 B 重构算法实现偏离 GLM 算法语义；③ 修复成本：方向 A 仅修订 OOD 文档（约 2 处行），方向 B 需重构算法实现并新增等价性测试，成本显著高于方向 A。 |

## 修订说明（v9）

| 质询意见 | 回应 |
|---------|------|
| **问题 1（高，严重事实错误）：§S2 归因分析引用不存在的文件且测试名指向未执行文件** — §S2「3 个 FAILED 用例的归因分析」段声称"3 个 FAILED 用例预期均来自 `src/detail/type_quat_cast_test.cj` 的非身份四元数 round-trip 用例（`testQuatCastNonIdentityMat3RoundTrip` 等）"。经核查：① `src/detail/type_quat_cast_test.cj` 不存在，唯一 quat_cast 相关测试文件是 `cjglm/src/detail/type_quat_cast_s1_test.cj`（S1 独立修复验证文件，含 3 个 `@Test`：`testS1QuatCastScalingXBranch`、`testS1QuatCastScalingWBranch`、`testS1QuatCastNonUnitRoundTrip`）；② 测试名指向 `tests/glm/detail/test_type_quat_cast.cj`，但该文件位于 `tests/` 目录下，其 300+ `@Test` 全部静默跳过、不被 `cjpm test` 执行；③ 实际 FAILED 来源唯一候选是 `type_quat_cast_s1_test.cj`。 | **已修正。** §S2 归因分析段已重写为"`cjpm test` 输出的 3 个 FAILED 用例预期均来自 `cjglm/src/detail/type_quat_cast_s1_test.cj`（S1 独立修复验证文件）——该文件含 3 个 `@Test`：`testS1QuatCastScalingXBranch`（q=(0.8,0.1,0.1,0.1)，X 分支）、`testS1QuatCastScalingWBranch`（q=(0.1,0.1,0.1,0.8)，W 分支）、`testS1QuatCastNonUnitRoundTrip`（q=(0.2,0.3,0.4,0.8)，非身份 round-trip），每个 `@Test` 函数体内多个 `@Expect` 断言在 S1 bug 影响下均不成立（如 `testS1QuatCastScalingXBranch` 中 `@Expect(q1.y, 0.1)` 在 bug 下实际值约为 0.2）"。核实手段已重写：① 运行 `cjpm test --verbose` 确认 FAILED 用例名集中在 `testS1Quat*` 前缀；② 单文件 `cjc` 编译 `cjglm/src/detail/type_quat_cast_s1_test.cj`（含必要的 `import glm.unittest.*` 与 `glm.unittest.testmacro.*`）后独立运行，确认 3 个 `@Test` 函数因 S1 bug 全部 FAIL；③ 归因一致时 S1 修复后预期 3 FAILED → PASS（`type_quat_cast_s1_test.cj` 的非单位四元数测试用例在 S1 修复后 quatCast 返回的个分量等于 q0 的对应分量，断言通过；mat3Cast 不归一化路径下 round-trip 矩阵相等）。消除了与 §S1 修复验证后备方案第 4 项的内部矛盾。 |
| **问题 2（中等）：§S2 测试文件计数与同报告证据段自相矛盾** — 行 60 描述"全部 425 个测试用例均来自 `src/detail/*_test.cj`（阶段一/二残留的 13 个测试文件）"，行 726 描述"`src/detail/*_test.cj` 14 个文件共 425 个测试用例仍正常执行"。两处数字不一致且未明确两个数字的语义区分。 | **已修正。** §S2 证据段已重写："全部 425 个测试用例均来自 `src/detail/*_test.cj`，共 14 个测试文件——其中 13 个为阶段一/二残留的测试文件（后缀符合 `*_test.cj` 规则），共 422 个 `@Test`；另 1 个为阶段三 S1 独立修复验证文件 `type_quat_cast_s1_test.cj`，含 3 个 `@Test`。合计 14 个文件共 425 个 `@Test`"。明确了"13 个"与"14 个"的语义区分：13 个指阶段一/二遗留测试文件（`compute_vector_decl_test.cj`/`compute_vector_relational_test.cj`/`qualifier_test.cj`/`scalar_vec_ops_test.cj`/`setup_test.cj`/`shim_assert_test.cj`/`shim_limits_test.cj`/`type_cast_test.cj`/`type_fromBoolVec_test.cj`/`type_vec2_test.cj`/`type_vec3_test.cj`/`type_vec4_test.cj`/`vectorize_test.cj`，合计 422 个 `@Test`），14 个指上述 13 个 + 阶段三 S1 独立修复验证文件 `type_quat_cast_s1_test.cj`（含 3 个 `@Test`），合计 425 个。 |
| **问题 3（低，与问题 1 强相关）：§S2 归因结论中"待执行者核实手段"②项的描述与归因方向不匹配** — 核实手段②指出"单文件 `cjc` 编译 `test_type_quat_cast.cj`"——但该文件位于 `tests/` 目录下，且其包声明为 `glm.detail`，需与 `cjglm/src/detail/type_quat_cast.cj` 同包编译环境；此外此文件名引用与归因分析的 `src/detail/type_quat_cast_test.cj`（即问题 1 中不存在的文件）不一致。 | **已修正（与问题 1 同步修复）。** 核实手段②已改为"单文件 `cjc` 编译 `cjglm/src/detail/type_quat_cast_s1_test.cj`（含必要的 `import glm.unittest.*` 与 `glm.unittest.testmacro.*`）后独立运行，确认 3 个 `@Test` 函数因 S1 bug 全部 FAIL"。文件名引用与问题 1 修复方向一致。 |
| **问题 4（低）：S1 bug 验证样本的具体数值与 q=(0.2,0.3,0.4,0.8) 的精确计算结果存在轻微偏差未说明** — §S1 描述"以 `Quat(0.2, 0.3, 0.4, 0.8)` round-trip 验证：`quatCast(mat3Cast(q))` 返回约 `(0.38, 0.57, 0.76, 0.84)`，约为原 q 的 1.9 倍"。实际推导：v = sqrt(2.84) ≈ 1.6852；x ≈ 0.3798；y ≈ 0.5697；z ≈ 0.7596；w ≈ 0.8426。即 quatCast 返回 ≈ (0.38, 0.57, 0.76, 0.84)，非 w 分量实测比例 1.899（非精确 2.0），w 分量实测比例 1.053。报告未明确标注"约"的精确度。 | **已修正。** §S1 行 38 末尾已追加："具体数值校核：`v = sqrt(2.84) ≈ 1.6852`（缺 *0.5 因子）；x/y/z 各分量约为 0.3798/0.5697/0.7596（实测比例 1.899）；w = 0.8426（实测比例 1.053）。浮点累积误差致非 w 分量实测比例非精确 2.0，但数学上确为 2.0 倍——`0.64/(sqrt(2.84)) = 0.64/sqrt(2.84)` 与正确路径 `0.64*0.25/(0.5*sqrt(2.84)) = 0.64*0.5/sqrt(2.84)` 的比值精确为 2.0。"。明确标注了"约"的精确度，并补充了浮点累积误差与数学精确度的关系。 |
| **问题 5（低）：§G1.3 修复方向推荐的论据 ② 中 GLM `quatCast` 算法的引用范围未限定** — §G1.3 修复方向推荐段论据 ② 描述"GLM `quatCast` 内部同样依赖 `T > T` 比较（`fourBiggestSquaredMinus1` 的大小分支判定）"，但未限定 GLM `quatCast` 的具体行号范围。 | **已修正。** §G1.3 论据 ② 已追加具体行号引用："GLM `references/glm-1.0.3/glm/glm/gtc/quaternion.inl:95-104` 使用 3 个 `if (fourX/Y/ZSSquaredMinus1 > fourBiggest...)` 串行比较"。精确限定了 GLM quatCast 算法的引用范围，便于执行者核查。 |

## 修订说明（v10）

| 质询意见 | 回应 |
|---------|------|
| **问题 1（高，必须修正）：G6.1 文本与可执行矩阵表内部矛盾** — 位置：`a_v9_diag_v1.md:617`（文本"5 个 import 组中的 3 个"）与 `a_v9_diag_v1.md:619-630`（可执行矩阵表标记 4 个"是"）。文本将 cast 视为 1 个 import 组（部分覆盖），而表格将 mat3Cast/quatCast 视为 2 个独立组（都已覆盖），导致"已覆盖组数"统计不一致（3 vs 4）。 | **已修正（采纳选项 A）。** §G6.1 分析段已统一采用表格口径，明确"按函数粒度覆盖 5 个 import 组中的 **4 个**"：① 新增 **「分组粒度说明」段**，明确"按 OOD §2 阶段三引入的 import 项计 5 个 OOD 阶段三 import 组（trig、cast 整体、Quat 别名），其中 cast 在 OOD §2 中虽为单条 import 语句但实质包含 mat3Cast/mat4Cast/quatCast 3 个独立转换函数（按函数粒度计算为 3 个 import 组）"；② 文本段改为按函数粒度列举覆盖项：`detail.Quat`（1 个）、`detail.mat3Cast`（1 个）、`detail.quatCast`（1 个）、`trigonometric`（15 个），**未覆盖：`detail.mat4Cast`**（与表格标记一致）；③ 可执行矩阵表保持 6 行结构（trig / detail.Quat / detail.mat3Cast / detail.quatCast / detail.mat4Cast / 合并长串），新增表后 **「说明」段** 显式处理第 6 行归类——按 lib.cj 文件 import 语句粒度视为单一未覆盖行，按函数粒度展开后可拆分 13 个独立 import 组（vector_relational、quaternion_common 等），最终覆盖基线为 **4 个独立覆盖 + 1 个 mat4Cast 遗漏 + 1 个合并长串遗漏，按函数粒度展开后共 15 个未覆盖 import 组**。优先级表 G6.1 行同步追加 v10 标注："已统一口径为'覆盖 4 个独立 import 组，未覆盖组为 mat4Cast + 合并长串'"。问题 7（"5 import 组"指代粒度不一致）随之自动消解——分组粒度已在分组粒度说明段明确。 |
| **问题 2（高，必须修正，必须升为最高优先级）：cjpm test 输出数字与 3 FAILED 归因未提供实际执行证据** — 位置：`a_v9_diag_v1.md:54-69`（S2 证据段 + 归因分析）。S2 证据段声称"实测 `cjpm test` 输出：Summary: TOTAL: 425, PASSED: 422, FAILED: 3"但未提供执行命令、时间戳或日志路径。3 FAILED 归因使用"预期"一词但行文以事实口吻呈现。多处下游分析依赖此事实。 | **已修正（升为最高优先级"必须修正"）。** ① §S2 证据段开头新增 **「实证基线声明」** 段：明确说明本报告引用的 `cjpm test` 输出数字（TOTAL=425/PASSED=422/FAILED=3）尚未通过独立执行验证，提供完整实证命令 `cjpm test 2>&1 \| tee /tmp/cjpm_test_$(date +%Y%m%d_%H%M%S).log`，列出验证重点（Summary 行数字 + verbose 模式 FAILED 用例名前缀），并说明若数字不一致时下游分析（S2 证据链、S3 捕获能力论证、S1 修复验证后备方案）均需相应调整；② §S2 证据段引用的 `cjpm test` 输出数字前标注"（待实证）"；③ §S2 证据段 ① 末尾添加"（**注：本数字待 v10 实证基线声明段所列命令独立验证**）"内嵌标注；④ §S2 归因分析段标题改为 **「3 个 FAILED 用例的归因分析（假设性归因，待 `cjpm test --verbose` 验证）」**，明确标注假设前提；⑤ §S2 归因分析段后新增 **「反事实论证」** 段：列举若 `cjpm test --verbose` 实际结果显示 3 FAILED 不全来自 `type_quat_cast_s1_test.cj`（或包含非 S1 bug 引起的失败）的三种下游影响（S1 影响范围扩展 / S2 修复方案风险评估补充 / §S3 证据链重新验证），并建议执行者在 S1 修复前优先完成 `cjpm test --verbose` 独立验证以排除反事实情形；⑥ 「与 todo.md 数字差异说明」段添加"（**注：本报告引用数字本身亦待 v10 实证基线声明段独立验证**）"标注；⑦ 优先级表后 **「特别说明」** 段新增 **「v10 实证基线声明」** 补充说明；⑧ S1 修复验证后备方案新增第 5 项 **「cjpm test --verbose 实证」**：运行 `cjpm test --verbose` 确认 FAILED 用例名集中于 `testS1Quat*` 前缀，否则归因失败需按反事实论证扩展分析；⑨ 关键发现汇总第 2 项添加 v10 补充说明。 |
| **问题 3（中，建议修正）：G1.3 GLM quatCast `if` 比较行号范围偏差** — 位置：`a_v9_diag_v1.md:189`。报告引用 `quaternion.inl:95-104` 使用 3 个 `if` 串行比较，但实际 3 个 `if` 分布在 **行 90-104**：行 90-94（X 分支）、行 95-99（Y 分支）、行 100-104（Z 分支）。报告"3 个 `if`"陈述与"`:95-104`"行号范围自相矛盾。 | **已修正。** §G1.3 论据 ② 已将行号引用由 `quaternion.inl:95-104` 改为 **`quaternion.inl:90-104`**，并补充各 `if` 分支对应行号："（X 分支对应行 90-94、Y 分支对应行 95-99、Z 分支对应行 100-104）"。覆盖全部 3 个 `if` 串行比较，消除行号范围与陈述内容的矛盾。 |
| **问题 4（中，建议修正）：G3.5 fwd.cj.bak 的 git 跟踪状态未验证** — 位置：`a_v9_diag_v1.md:415-419`。G3.5 推荐 `git rm cjglm/src/fwd.cj.bak`（不带 `--cached`），但该命令适用前提是文件已被 git 跟踪。todo.md 仅描述"`git status` 显示 `new file:`"，未独立验证当前跟踪状态。若文件处于未跟踪状态，`git rm` 会报 "fatal: pathspec ... did not match any files" 错误。 | **已修正。** §G3.5 修复建议段开头新增 **「前置验证」** 步骤：① 给出验证命令 `git ls-files --error-unmatch cjglm/src/fwd.cj.bak`；② 三种状态处理：返回 0（已跟踪）→ 执行 `git rm`（不带 `--cached`）/ 返回非 0（未跟踪）→ 执行 `rm` 即可 / 通用回退方案（不依赖跟踪状态）→ `rm` + `git add -A`；③ 原推荐命令改为限定语 **「仅在文件已被 git 跟踪时」**，明确 `git rm` 对未跟踪文件会报路径不匹配错误。优先级表 G3.5 行同步追加 v10 标注："已补充前置验证步骤（git 跟踪状态检查）"。 |
| **问题 5（中，建议修正）：G3.8 "17 个 unused import 编译警告" 未经实际构建验证** — 位置：`a_v9_diag_v1.md:472-479`。报告声称"`cjpm build` 输出 17 条 `unused import` 警告：15 个 trigonometric + 1 个 glm.ext.* + 1 个 glm.gtc.* = 17"，但未提供实际 `cjpm build` 输出作为证据。 | **已修正。** §G3.8 分析段在 17 条警告数字后追加 **（注：本数字为基于 `lib.cj:12/14/16` import 在 lib.cj 文件内无引用事实的逻辑推断，尚未通过 `cjpm build` 独立实证）** 标注，明确告知读者该数字为推论而非实测。§G3.8 段末新增 **「建议实证命令」** 段：给出 `cjpm build 2>&1 \| grep -c 'unused import'` 命令，说明 cjpm 不同版本对 unused import 检测粒度差异（部分版本仅检测 `public` 修饰符的 import），实际数字可能为 15-17 之间；明确修复方向不受实证结果影响（不论数字多少，根因均为 `lib.cj` 内部不调用 ext/gtc 符号）。优先级表 G3.8 行同步追加 v10 标注："已标注本数字为逻辑推断，需 `cjpm build` 实证"。 |
| **问题 6（低，可选优化）：§S1 "1.899 比例" vs "2.0 倍" 比值基线易混淆** — 位置：`a_v9_diag_v1.md:38`。1.899 与 2.0 两个比值的分母不同（前者基线为原 q.x，后者基线为正确输出），v9 修订补充了数学等式但未明确标注两个比值的分母基线差异，读者可能混淆。 | **已修正。** §S1 行 38 数值校核段已补充两个比值的分母基线明确标注：「x/y/z 各分量约为 0.3798/0.5697/0.7596（实测 buggy / 原 q.x = 0.3798 / 0.2 ≈ 1.899，**基线为原 q.x = 0.2**）；w = 0.8426（实测 buggy / 原 q.w = 0.8426 / 0.8 ≈ 1.053，**基线为原 q.w = 0.8**）。浮点累积误差致非 w 分量实测比例非精确 2.0，但数学上确为 2.0 倍——**数学比值 2.0 = buggy 输出（0.3798）/ 正确输出（0.1899）（基线为正确输出）**，与正确路径 `0.64 * 0.25 / (0.5 * sqrt(2.84)) = 0.64 * 0.5 / sqrt(2.84)` 的比值精确为 2.0。**两个比值分母差异是浮点累积误差的来源**：1.899 比值分母为原 q.x（不受 bug 影响），2.0 比值分母为正确输出（同样不受 bug 影响，但分子 buggy 输出包含浮点累积误差）」。明确标注两个比值的分母基线与误差来源，便于执行者理解浮点累积误差的传导路径。 |
| **问题 7（低，可选优化，与问题 1 强相关）：G6.1 "5 个 import 组"指代粒度不一致** — 位置：`a_v9_diag_v1.md:617`（文本）与 `a_v9_diag_v1.md:619-630`（表格）。文本口径 5 = trig + cast 合并 + ext + gtc + Quat（共 5 个组，3 个覆盖），表格口径 6 行（4 个覆盖）。两种口径对应的"5"含义不同，但报告未明确说明采用哪种分组粒度。 | **已修正（与问题 1 一并修复，问题 1 修复后本问题自动消解）。** §G6.1 段首新增 **「分组粒度说明」** 段，明确"本节按 OOD §2 阶段三引入的 import 项计 5 个 OOD 阶段三 import 组（trig、cast 整体、Quat 别名），其中 cast 在 OOD §2 中虽为单条 import 语句但实质包含 mat3Cast/mat4Cast/quatCast 3 个独立转换函数（按函数粒度计算为 3 个 import 组）"。两种口径的差异（cast 在 OOD §2 中按语句计 1 个、按函数计 3 个）在说明段中显式呈现，执行者可根据需要选择粒度——若需严格对齐 OOD §2 文本则按语句计 5 个组（3 个覆盖），若需识别未覆盖函数则按函数粒度计 7 个组（4 个覆盖）。问题 1 已统一采用表格口径（4 个覆盖），本问题的指代粒度冲突由此消解。 |

## 修订说明（v12）

| 质询意见 | 回应 |
|---------|------|
| **P1（高，未修复自v2）：S1 修复管线行 811「建议步骤1+2可合并提交」与行 818-821「推荐分两次提交」指引矛盾，缺少决策条件** | **已修正。** type_quat_cast.cj 修复管线段已重写为「步骤 1+2 合并 vs 分两次提交的决策指南」，其中「合并提交（推荐条件）」与「分两次提交（推荐条件）」两段分别列出各自适用场景（测试容差是否预留余量、code review 流程要求），执行者可根据实际条件选择。原矛盾的两段陈述已统一为同一决策框架下的分支选项，不再并列矛盾。 |
| **P2（高，未修复自v3）：S1 管线行 811 称「G2.3 重构不改变数值结果」与行 816 称「G2.3 重构会产生新的数值差异」相邻段落方向相反** | **已修正。** 行 811 已改为「此重构在数学含义上保持数值结果不变（输入输出关系一致），但浮点位模式可能因直接调用精度重载而产生微小差异（约 1e-7 量级，详见下方决策指南）」。「G2.3 重构的数值性质说明」补充段明确解释了「不改变数值结果」与「产生新的数值差异」分别指数学含义和浮点位模式两个不同层面，陈述方向不矛盾。 |
| **P3（中，未修复自v3）：S2 后备方案 2 推荐将测试文件复制到 src/detail/ 下运行，但包声明/import 依赖/unittest 框架引用未经验证** | **已修正。** S2 后备方案 2 已改为明确路径 `tests/glm/detail/test_type_quat_cast.cj` → `src/detail/type_quat_cast_test.cj`，并添加验证注记：源文件包声明 `package glm.detail` 与 `src/detail/` 目录路径一致；`import glm.unittest.*` 依赖在已有 `src/detail/type_quat_cast_s1_test.cj` 中已验证可正常解析；若编译失败，回退至方案 1 或方案 4。 |
| **N1（中，新增）：G6.1/G6.2 权重数据在分类汇总表、权重明细、优先级表之间不一致** | **已修正。** 分类汇总表 OOD 行：G6.2(0.5) 移除 → 新增 G6.1(0.5)；其他类型行：G6.1(1) → G6.1(0.5)，移除 G6.2(0.5)。优先级表 Low 行移除 G6.2。权重明细行 G6.1=其他:0.5+OOD:0.5 保持不变（v7 已正确）。计数说明新增 v12 调整段。章节映射表 G6.2 行保留（因 todo.md 原始编号仍为 G6.2 独立条目）。 |
| **N2（中，新增）：分析结构依赖未实证数字（TOTAL=425/PASSED=422/FAILED=3）的方法论循环，未提供执行者「如果数字不成立，哪些结论需要重审」的行动指南** | **已修正。** 头部全局实证基线声明新增「若数字不成立时的重审指南」对照表，按 TOTAL ≠ 425 / FAILED ≠ 3 / FAILED 来源非 S1 文件 / 全部一致 四种情况逐行列出需要重审的结论，执行者可直接对照使用。 |
| **P5（中，未修复自v3）：G2.3（sqrtT Float64 中转重构）列为 High #5，理由为「项目级一致性问题」，但不影响功能正确性，同类 G1.x 代码质量建议均为 Low** | **已修正。** G2.3 优先级由 High #5 降为 Medium #5，理由追加「不影响功能正确性（同类 G1.x 代码质量建议均为 Low），因此由 High 降级至 Medium」。后续 Medium 项顺序顺延。 |
| **P6（中，未修复自v3）：G3.8（17 个 unused import 警告）列为 Medium-High，但编译警告不影响功能正确性，同类 G3.6 为 Medium、G3.5 为 Medium-Low** | **已修正。** G3.8 优先级由 Medium-High #8 降为 Medium #8，理由追加「编译警告不影响功能正确性，因此由 Medium-High 降级至 Medium」。 |
| **P7（低，未修复自v2）：实证基线声明在报告中重复 10+ 处，信号噪声比下降** | **已修正。** ① §S2 实证基线声明段已浓缩为交叉引用头部「若数字不成立时的重审指南」表的一句话；② 证据段内联注记「本数字待 v10 实证基线声明段所列命令独立验证」简化为「本数字待独立实证，参见头部全局实证基线声明」；③「与 todo.md 数字差异说明」段注记同步简化；④ 优先级表后实证基线说明由独立段落改为单句引用。仅保留头部全局声明一处完整描述，其余均替换为短引用。 |
| **P8（低，未修复自v3）：G3.7、G3.9、G4.3、G5.1 等 OOD 文档问题条目引用 OOD 行号但不提供引用内容** | **已修正。** 四处均已补充 OOD 原文引用内容：G3.7 引用 `docs/05_ood_phase3.md:303-324` 的 20 条逐项 import 清单；G3.9 引用 `docs/05_ood_phase3.md:304` 的原文说明；G4.3 引用 `docs/05_ood_phase3.md` 中 cos_one_over_two 的硬编码值说明；G5.1 引用 `docs/05_ood_phase3.md:1779` 的测试用例计划表及 `docs/05_ood_phase3.md:1824-1831` 的用例分配依据。 |
| **R1（中，建议补充）：未独立验证 todo.md 条目完整性（标题标称 48 项但实际 42 项，未 grep 确认是否存在未被报告的 S5/S6 条目）** | **已修正。** 概述计数说明段已新增「标题完整性验证（grep 结果）」子段，报告了 `grep -n '^\*\*S[0-9]'` 和 `grep -n '^\*\*G[0-9]\.[0-9]'` 的实际匹配结果——确认仅 4 项 S 条目（S1~S4）和 38 项 G 条目（G1.1~G6.2），无 S5/S6 或编号超出 G6.2 的条目。标题「48 项」确认为文档编辑遗留错误。 |
