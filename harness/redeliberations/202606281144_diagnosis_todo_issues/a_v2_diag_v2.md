...
交叉验证依据：
- `docs/deviations.md` — 用户视角的 C++ GLM 与仓颉 GLM 偏差记录
- `docs/05_ood_phase3.md`（阶段三 OOD） — 注：requirement.md 指定的 `docs/02_ood_phase0.md` 不存在，经确认仓库中仅包含 `docs/03_ood_phase1.md`、`docs/04_ood_phase2.md`、`docs/05_ood_phase3.md` 三份阶段设计文档，`05_ood_phase3.md` 为阶段三设计文档，涵盖本报告涉及的全部 OOD 引用，故以此替代
- `references/glm-1.0.3` — GLM 参考实现
- `cjglm/` — 当前实现
...
**分类：真实存在（工具链兼容性）+ OOD 文档问题（OOD 对 cjpm 行为描述不准确）**
...
### S3. `test_type_quat_cast.cj` round-trip 测试覆盖不完整，仅部分用例可暴露 quatCast 因子 2 bug

**分类：其他类型（测试覆盖不足）**

**分析：** 8 个测试用例中 6 个采用 round-trip 形式 `mat3Cast(quatCast(M)) == M` 或 `mat4Cast(quatCast(M)) == M`，其余 2 个为直接值校验。6 个 round-trip 用例中：
- 2 个使用恒等四元数 `(0.0, 0.0, 0.0, 1.0)`：恒 etc 四元数的 round-trip 在因子 2 bug 下仍内部一致（quatCast 的 w 分支使用 `v * half` 正确计算 w，非 w 分量为零值不受缩放影响），**掩盖 bug**。
- 4 个使用非恒等四元数 `(0.2, 0.3, 0.4, 0.8)`：可直接暴露因子 2 bug（如 §S1 数值验证所示）。

OOD §8.2 明确推荐「旋转矩阵 × 向量 = 四元数 × 向量」等价性测试作为补充验证，但未采用。当前 round-trip 覆盖的局限在于：仅 4/6 用例使用非恒等四元数，且缺乏矩阵×向量等价性测试作为独立验证手段。

**证据：** `test_type_quat_cast.cj:18-93` 确认 6 个 round-trip 用例（lines 18-25, 46-53, 55-62, 64-75, 77-84, 86-93）和 2 个直接值校验（lines 3-16, 27-44）。
...
**误报说明：** todo.md 对 testInverseZero 的描述「测试断言未能正确区分 Inf 与 NaN」不完全准确。实际使用 `||` 是合理的防御性断言（任何一者出现即表明除以零问题），并非功能缺陷。此处不属于新发现的误报，而是 todo.md 描述精度不足的问题。
...
| 分类 | 加权计数 | 涉及项（权重） |
|------|---------|---------------|
| **真实存在（实现偏差）** | 13.0 项 | S1(1), S2(0.5), G1.1(1), G1.2(1), G1.3(0.5), G1.4(1), G1.5(1), G2.1(1), G2.2(0.5), G2.3(1), G3.1(1), G3.5(1), G3.6(1), G3.8(1), G5.6(0.5) |
| **OOD 文档问题** | 7.5 项 | S2(0.5), G1.3(0.5), G2.2(0.5), G3.7(0.5), G3.9(0.5), G4.2(1), G4.3(1), G4.4(0.5), G5.1(0.5), G5.2(0.5), G6.2(0.5) |
| **其他类型** | 21.5 项 | S3(1), S4(1), G1.6(1), G1.7(1), G1.8(1), G2.4(1), G2.5(1), G2.6(1), G3.2(1), G3.3(1), G3.4(1), G3.7(0.5), G3.9(0.5), G4.1(1), G4.4(0.5), G5.1(0.5), G5.2(0.5), G5.3(1), G5.4(1), G5.5(1), G5.6(0.5), G5.7(1), G5.8(1), G5.9(1), G6.1(1), G6.2(0.5) |

> **计数说明：** 每项问题权重和为 1（若跨分类则按 0.5+0.5 拆分）。S3/S4 的「测试覆盖不足」归入「其他类型」而非「真实存在」以对齐 requirement.md 定义的 4 类分类框架。G4.4 和 G6.2 按双分类各拆分 0.5。所有 42 项加权总和 = 13.0 + 7.5 + 21.5 = 42.0 ✓。「误报」作为子方面标注于所属条目（G5.9）内，不单独列项计数。

### todo.md 条目编号到报告章节映射

| todo.md 编号 | 报告章节 | 主要分类（权重分配） |
|-------------|---------|-------------------|
| S1 | §S1 | 真实存在(1) |
| S2 | §S2 | 真实存在(0.5) + OOD 文档问题(0.5) |
| S3 | §S3 | 其他类型(1) |
| S4 | §S4 | 其他类型(1) |
| G1.1 | §G1.1 | 真实存在(1) |
| G1.2 | §G1.2 | 真实存在(1) |
| G1.3 | §G1.3 | 真实存在(0.5) + OOD 文档问题(0.5) |
| G1.4 | §G1.4 | 真实存在(1) |
| G1.5 | §G1.5 | 真实存在(1) |
| G1.6 | §G1.6 | 其他类型(1) |
| G1.7 | §G1.7 | 其他类型(1) |
| G1.8 | §G1.8 | 其他类型(1) |
| G2.1 | §G2.1 | 真实存在(1) |
| G2.2 | §G2.2 | 真实存在(0.5) + OOD 文档问题(0.5) |
| G2.3 | §G2.3 | 真实存在(1) |
| G2.4 | §G2.4 | 其他类型(1) |
| G2.5 | §G2.5 | 其他类型(1) |
| G2.6 | §G2.6 | 其他类型(1) |
...
| G3.5 | §G3.5 | 真实存在(1) |
| G3.6 | §G3.6 | 真实存在(1) |
| G3.7 | §G3.7 | 其他类型(0.5) + OOD 文档问题(0.5) |
| G3.8 | §G3.8 | 真实存在(1) |
| G3.9 | §G3.9 | 其他类型(0.5) + OOD 文档问题(0.5) |
| G4.1 | §G4.1 | 其他类型(1) |
| G4.2 | §G4.2 | OOD 文档问题(1) |
| G4.3 | §G4.3 | OOD 文档问题(1) |
| G4.4 | §G4.4 | OOD 文档问题(0.5) + 其他类型(0.5) |
| G5.1 | §G5.1 | 其他类型(0.5) + OOD 文档问题(0.5) |
| G5.2 | §G5.2 | 其他类型(0.5) + OOD 文档问题(0.5) |
| G5.3 | §G5.3 | 其他类型(1) |
| G5.4 | §G5.4 | 其他类型(1) |
| G5.5 | §G5.5 | 其他类型(1) |
| G5.6 | §G5.6 | 真实存在(0.5) + 其他类型(0.5) |
| G5.7 | §G5.7 | 其他类型(1) |
| G5.8 | §G5.8 | 其他类型(1) |
| G5.9 | §G5.9 | 其他类型(1) |
| G6.1 | §G6.1 | 其他类型(1) |
| G6.2 | §G6.2 | OOD 文档问题(0.5) + 其他类型(0.5) |
...
| **High** | S3 | 测试设计缺陷——仅 4/6 非恒等 round-trip 用例可暴露 S1 bug，需补充矩阵×向量等价性测试 | `test_type_quat_cast.cj`（测试依赖 S1 修复结果） |
...
| **Low** | G5.1, G5.2, G5.8 | 测试增强建议 | 各测试文件 |
...
| 质询意见 | 回应 |
|---------|------|
| 1. 严重/一般问题计数与 todo.md 原始标称不符（48 vs 42），报告机械复制未识别 | 已在 v2 概述中增加计数说明段落，明确 todo.md 标称「6 项严重 + 42 项一般 = 48 项」与实际列表不符，并按实际列表 4 项严重 + 38 项一般 = 42 项报告。 |
| 2. G1.6 段落存在自相矛盾（先称「sqrtT 未调用」后称「quatCast 调用了 sqrtT」） | 核实 v2 报告原文：G1.6 分析点①描述的是「sqrtT 通过 Float64 中转实现 sqrt，与 OOD 方案 A 不一致」（代码质量观察），纠正段说明的是「todo.md 称 sqrtT 未被调用，经核查实际被调用」。两者无逻辑矛盾。为进一步消除歧义，已确认 v2 文本不包含「sqrtT 未在 type_quat_cast.cj 中被调用」的错误断言，保留现状。 |
| 3. deviations.md 交叉验证不充分，仅 G4.4 一处提及 | 核实 v2 报告：每个条目的「deviations.md 对照」小节均已存在。v3 维持该结构，对每个条目保持 deviations.md 对照结论。 |
| 4. 02_ood_phase0.md 不存在但未说明替代决策 | v2 概述已注明：「requirement.md 指定的 docs/02_ood_phase0.md 不存在，经确认仓库中仅包含 03/04/05 阶段文档，05_ood_phase3.md 为阶段三设计文档，涵盖本报告涉及的全部 OOD 引用，故以此替代」。v3 保持此说明。 |
| 5. 分类汇总表数据不一致（真实存在标称 11 实际 13+、其他类型标称 29 实际 23+、缺映射表） | v3 全面修正：统一加权计数口径，所有 42 项权重和 = 42.0 ✓。「真实存在」按加权值 13.0 列示，「OOD 文档问题」按 7.5 列示，「其他类型」按 21.5 列示（含 S3/S4 测试覆盖不足 2 项及 G4.4/G6.2 拆分 0.5+0.5）。新增完整的 todo.md 条目编号→报告章节映射表（见 §汇总）。移除「误报」独立计数行，改为子方面标注于所属条目内。S3/S4 统一归入「其他类型」以对齐 requirement.md 分类框架。 |
| 6. 缺乏修复优先级排序与依赖关系分析 | v2 已包含「修复优先级建议」小节。v3 在此基础上细化 S3 的优先级说明（与 S1 修复的耦合关系），维持 blocker/high/medium/low 分级及文件级依赖标注。 |
| 7. G4.2 分类边界模糊（应归 OOD 文档问题而非其他类型） | v2 中 G4.2 已分类为「OOD 文档问题（设计契约与约束策略不一致）」。v3 保持不变。 |
| 8. G1.6 的分类与纠正未厘清（「纠正」混在「代码质量」分类中） | v2 中「纠正」作为独立的二级段落（**纠正（todo.md 描述偏差）**）在 G1.6 下呈现，不与「代码质量」分类标签合并。v3 保持此结构。 |

### v3 较 v2 的增量修订

| 修订点 | 说明 |
|-------|------|
| 分类汇总表加权计数修正 | 统一按 42 项权重和 = 42 列示；真实存在 13.0、OOD 7.5、其他类型 21.5；G4.4/G6.2 按 0.5+0.5 拆分；S3/S4 移入其他类型 |
| 汇总表格式调整 | 移除「误报」独立行，改为子方面标注；增加「涉及项（权重）」权重列替换原「涉及项」列 |
| 映射表增加权重分配 | 每项标注完整权重分配，便于交叉验证加权和 |
| S3 分析修正 | round-trip 计数从 7 修正为 6（实测算 6 个 round-trip + 2 个直接值校验 = 8 总用例）；明确区分恒等四元数 round-trip（掩盖 bug）与非恒 etc round-trip（可暴露 bug）；结论从「round-trip 掩盖 bug」改为「round-trip 覆盖不全——仅 4/6 用例使用非恒等四元数」 |
| S3 分类变更 | 从「真实存在（测试覆盖不足）」变更为「其他类型（测试覆盖不足）」以对齐分类框架 |
| S4 分类变更 | 同上 |
| G5.9 误报标注方式 | 从「误报」独立行改为「误报说明」子段落 |
