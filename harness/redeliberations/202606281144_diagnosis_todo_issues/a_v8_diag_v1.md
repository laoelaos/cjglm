...
**3 个 FAILED 用例的归因分析：** `cjpm test` 输出的 3 个 FAILED 用例预期均来自 `src/detail/type_quat_cast_test.cj` 的非身份四元数 round-trip 用例（`testQuatCastNonIdentityMat3RoundTrip`、`testQuatCastNonIdentityMat4RoundTrip`、`testMat3CastNonIdentityQuat`/`testMat4CastNonIdentityQuat` 等）——由 S1 bug（quatCast 因子 2 缩放）经 `mat3Cast` 不归一化（详见 §S3）放大后导致非身份 round-trip 矩阵显著偏离原矩阵，`@Expect(m == m2, true)` 断言失败。**待执行者核实手段：** ① 运行 `cjpm test --verbose` 或单文件 `cjc` 编译 `test_type_quat_cast.cj`，确认 FAILED 用例名是否集中在非身份 round-trip 用例；② 若 FAILED 用例与 S1 bug 归因一致，则 S1 修复后预期 3 FAILED → PASS；若 FAILED 用例来自其他文件（如 `vector_relational.cj` 测试），则 S1 bug 影响范围需扩展分析。该归因结论同时交叉验证 §S3「非身份 round-trip 可捕获 S1 bug」的证据链。
...
> **特别说明：** S2 列为唯一 Critical 优先级的理由是：当前状态下 `tests/` 目录下 300+ `@Test` 静默跳过——`src/detail/*_test.cj` 14 个文件共 425 个测试用例仍正常执行（含 422 PASSED + 3 FAILED）；但 `tests/` 下覆盖 OOD §8.2 测试计划的测试用例全部无法验证。这意味着任何针对 `tests/` 目录的测试相关修复（包括 S3、S4、G5、G6 的修复正确性）无法通过 `cjpm test` 验证。S2 修复（测试文件命名重构）不依赖 S1，可独立先行。
