...
glm.detail（同包直接可见）
  ...
  exponential.cj → std.math（pow/exp/log/sqrt 等）
   trigonometric.cj → std.math + qualifier + scalar_constants（pi<T>() 函数，radians/degrees 使用）
...
- 约束策略：
  ...
  - `mod`：`T <: Integer<T>`（整数取模，当前约束为 Integer<T>；未来可考虑补充 FloatingPoint<T> 浮点重载以扩展 GLSL 语义覆盖，但此变更为向后不兼容——现有 Integer<T> 约束下的调用码在收紧为 FloatingPoint<T> 后将编译报错）
...
- **modf/frexp/ldexp 签名设计**：
  ...
  - `frexp<T>(x: T): (T, Int64)` where T <: FloatingPoint<T>：返回 (mantissa, exponent) 元组。mantissa 为 `[0.5, 1)` 范围的归一化尾数，exponent 为 2 的幂指数。《=64 位浮点指数范围在 Int64 内。实际上实现时应避免将 exponent 保持为泛型 T（因指数是整数语义而非浮点），使用 Int64 作为指数类型
  - `ldexp<T>(x: T, exp: Int64): T` where T <: FloatingPoint<T>：实现为 `x * pow(T(2), T(exp))`（不委托 `std.math.ldexp`，仓颉标准库不提供此函数）。`T(exp)` 转换需使用 `T(Float64(exp))` 按 §1.4 的表示约定处理。当 `T = Float16` 时依赖 `pow` 的 `exp(T(exp) * log(T(2)))` 回退路径
...
- 实现路径：
  - `exp/log/exp2/log2/sqrt`：直接委托 `std.math` 对应函数（已验证 Float16/Float32/Float64 三重载均可用）
  - `pow`：`std.math.pow` 提供 Float32 和 Float64 重载，但不提供 Float16 重载。当 `T = Float16` 时使用 `exp(T(exp) * log(base))` 恒等式回退（`std.math.exp` 和 `std.math.log` 均提供 Float16 重载）。此约束策略使 `pow<T>(base, exp)` 的泛型参数 `T <: FloatingPoint<T>` 保持一致，所有三种浮点类型均可实例化
...
func mirrorRepeat<T>(Texcoord: T): T where T <: FloatingPoint<T>       // 分片纹理循环

// 浮点→整数舍入（非负输入，+0.5 后截断）
...
- 实现路径：3/4 输入 `min`/`max` 委托 core common.cj 的 2 输入 `min`/`max` 层叠调用；`fmin` 系列实现为 `if isnan(a) return b` 后委托 `min`；`fclamp` 实现为 `fmin(fmax(x, minVal), maxVal)`；纹理环绕函数直接使用 core common.cj 的 `clamp`/`fract`/`abs`/`mod`/`floor`/`mix`；`iround`/`uround` 实现为 `Int64(x + T(0.5))`/`UInt64(x + T(0.5))`
...
- 替换 stub 为完整实现，对标 GLM 1.0.3 `ext/matrix_clip_space.hpp`（第 42~572 行）的全部 46 个函数：
  - **ortho 系族（10 个）**：`ortho(left,right,bottom,top)`（2D）、`orthoLH_ZO`、`orthoLH_NO`、`orthoRH_ZO`、`orthoRH_NO`、`orthoZO`、`orthoNO`、`orthoLH`、`orthoRH`、`ortho(left,right,bottom,top,zNear,zFar)`（默认）
...
- **实现路径**：内部使用 `toBits()`/`fromBits()` 操作浮点位模式。`next_float(x)` 将 x 的位模式增加 1（正数）或减少 1（负数）；`ulp(x)` 对归一化数返回 `T.fromBits(T(1).toBits() - T(1))` 的差值，对非归一化数直接返回最小正数
...
| D15 | **`mod` 函数当前约束为 `Integer<T>`（与 GLM 1.0.3 的浮点语义不完全对应）** | 当前阶段三 stub 中 `mod` 的实际约束为 `Integer<T>`（`func mod<T>(a: T, b: T): T where T <: Integer<T>`）。若后期收紧为 `FloatingPoint<T>` 以匹配 GLM 语义，此变更为向后不兼容——现有 `Integer<T>` 约束下调用了 `mod` 的代码将编译报错。设计保持当前 `Integer<T>` 约束不变，编码阶段可根据实际需求补充 `FloatingPoint<T>` 重载（通过函数重载共存），无需变更已有约束 |
...
| D29 | **`exponential.cj` 的 `pow` 为 Float16 使用 `exp(b*log(a))` 回退** | `std.math.pow` 仅提供 Float32 和 Float64 重载，无 Float16 重载。若将 `pow` 约束收紧为 `T <: (Float32 | Float64)` 则破坏 `FloatingPoint<T>` 泛型一致性，且与其他函数的约束不统一。采用 `exp(T(exp) * log(base))` 恒等式回退（`std.math.exp` 和 `std.math.log` 均提供 Float16 重载），使 `pow` 的泛型约束 `T <: FloatingPoint<T>` 对所有三种浮点类型一致可用。`ldexp` 的 `x * pow(T(2), T(exp))` 实现同样受益于此回退机制 |
