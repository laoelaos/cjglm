...
| gtc/ulp.cj | ULP（Units in the Last Place）浮点比较函数 | 包级具体类型重载函数（Float32/Float64） |
...
package glm.gtc                 — GTC 扩展函数库（修改/新建文件以 ★ 标记）
  ...
  │                               （高精度类型别名定义）
  ├── ulp.cj ★                   — 新建完整实现
  │                               （next_float/prev_float/float_distance/ulp，Float32/Float64 具体类型重载）
  ├── round.cj ★                 — 新建完整实现
...
glm.detail（同包直接可见）
  ...
  trigonometric.cj → std.math + qualifier + scalar_constants.cj（radians/degrees 使用 pi<T>()）+ Vec 重载
...
glm.gtc（单向依赖 glm.detail，无循环依赖）
  ...
  ulp.cj → glm.detail（Float32/Float64 具体类型 + 位操作 API）
...
- 约束策略：
  ...
  - `floor/ceil/trunc/round/roundEven/fract/fma`：`T <: FloatingPoint<T>`（需要浮点特定运算）
  - `mix`：三参数版本 `T <: Number<T>`（线性插值），Bool 选择版本独立重载
  - `isnan/isinf`：`T <: FloatingPoint<T>`（需要 isNaN/isInf 实例方法）
  - `mod`：`T <: FloatingPoint<T>`（阶段四完整实现后使用 `x - y * floor(x / y)` 公式）。注意：当前（阶段三）stub 的 `mod` 约束为 `Integer<T>`，阶段四替换为完整实现后将变更为 `FloatingPoint<T>`，此为向后不兼容变更——调用方依赖整数 `mod` 行为的代码将在阶段四编译报错。编码阶段可评估是否需要保留整数 `mod` 的重载以满足向后兼容，仅当实测现有代码存在整数 `mod` 调用时方需增加
  - `floatBitsToInt/floatBitsToUint/intBitsToFloat/uintBitsToFloat`：具体类型重载（仅 Float32/Int32/UInt32）

- **modf/frexp/ldexp 的仓颉签名设计**：
  - `modf`：C/GLM 使用 `modf(x, &i)` 通过输出参数返回整数部分，仓颉无引用参数。采用元组返回：`func modf<T>(x: T): (T, T) where T <: FloatingPoint<T>`，返回 `(fractional, integer)` 元组。采用向零截断公式：`integer = trunc(x)`，`fractional = x - trunc(x)`。此公式对所有 x（含负数）与 C 标准 modf 的向零截断行为一致
  - `frexp`：C/GLM 使用 `frexp(x, &exp)` 通过输出参数返回指数，仓颉采用元组返回：`func frexp<T>(x: T): (T, Int64) where T <: FloatingPoint<T>`，返回 `(mantissa, exponent)` 元组。实现通过位操作（对 Float32 使用 `toBits()` 提取指数位，对 Float64 使用 `toBits()` 提取指数位）或通过数学变换 `frexp(x) = { exp = floor(log2(abs(x))) + 1; mantissa = x / pow(2, exp) }` 实现。位操作为首选路径，若泛型下不可行则采用数学公式
  - `ldexp`：自行实现：`func ldexp<T>(x: T, exp: Int64): T where T <: FloatingPoint<T>`。实现公式为 `x * pow(T(2), T(exp))`。标量版本直接计算，向量版本逐分量计算。注：std.math 不提供 ldexp 函数，因此无法委托标准库，需自行实现
...
- `radians(x)` 实现为 `x * pi<T>() / T(180)`，`degrees(x)` 实现为 `x * T(180) / pi<T>()`。`pi<T>()` 来自同包的 `scalar_constants.cj`
...
- `normalize/length/distance/reflect/refract/faceforward`：Vec2~Vec4 版本
...
- **奇异矩阵行为**：奇异矩阵求逆的结果由 IEEE 754 浮点运算自然决定：`T(1)/det` 在 `det == T(0)` 时产生 `+Inf` 或 `-Inf`，`Inf * 0` 产生 NaN，最终可能得到 NaN 或 Inf 填充的矩阵。函数不抛出异常，不设置显式零矩阵保护。调用方需自行通过行列式检查矩阵的奇异性。此行为与 GLM 1.0.3 一致
...
**职责**：
- 替换 stub 为完整实现，对标 GLM 1.0.3 `ext/matrix_projection.hpp`（第 49~144 行）的全部 8 个函数：
  - `projectZO(obj, model, proj, viewport)`：对象坐标映射到窗口坐标（Direct3D Z 范围 [0,1]）
  - `projectNO(obj, model, proj, viewport)`：对象坐标映射到窗口坐标（OpenGL Z 范围 [-1,1]）
  - `project(obj, model, proj, viewport)`：默认方式的 project
  - `unProjectZO(win, model, proj, viewport)`：窗口坐标逆映射到对象坐标（D3D）
  - `unProjectNO(win, model, proj, viewport)`：窗口坐标逆映射到对象坐标（OpenGL）
  - `unProject(win, model, proj, viewport)`：默认方式的 unProject
  - `pickMatrix(center, delta, viewport)`：拾取区域矩阵
...
- 替换 stub 为完整实现，对标 GLM 1.0.3 `ext/matrix_clip_space.hpp`（第 42~572 行）的全部 46 个函数：
  - **ortho 系族（11 个）**：`ortho(left,right,bottom,top)`（2D）、`orthoLH_ZO`、`orthoLH_NO`、`orthoRH_ZO`、`orthoRH_NO`、`orthoZO`、`orthoNO`、`orthoLH`、`orthoRH`、`ortho(left,right,bottom,top,zNear,zFar)`（默认）
  - **frustum 系族（9 个）**：`frustumLH_ZO`、`frustumLH_NO`、`frustumRH_ZO`、`frustumRH_NO`、`frustumZO`、`frustumNO`、`frustumLH`、`frustumRH`、`frustum`（默认）
  - **perspective 系族（9 个）**：`perspectiveRH_ZO`、`perspectiveRH_NO`、`perspectiveLH_ZO`、`perspectiveLH_NO`、`perspectiveZO`、`perspectiveNO`、`perspectiveRH`、`perspectiveLH`、`perspective`（默认）
  - **perspectiveFov 系族（9 个）**：`perspectiveFovRH_ZO`、`perspectiveFovRH_NO`、`perspectiveFovLH_ZO`、`perspectiveFovLH_NO`、`perspectiveFovZO`、`perspectiveFovNO`、`perspectiveFovRH`、`perspectiveFovLH`、`perspectiveFov`（默认）
  - **infinitePerspective 系族（7 个）**：`infinitePerspectiveLH_ZO`、`infinitePerspectiveLH_NO`、`infinitePerspectiveRH_ZO`、`infinitePerspectiveRH_NO`、`infinitePerspectiveLH`、`infinitePerspectiveRH`、`infinitePerspective`（默认）
  - **tweakedInfinitePerspective（2 个）**：`tweakedInfinitePerspective(fovy,aspect,near)`、`tweakedInfinitePerspective(fovy,aspect,near,ep)`
...
- **随机数引擎管理**：使用仓颉 `std.random.Random` 作为底层随机数源。引擎采用 `ThreadLocal<Random>` 线程本地存储策略——每个线程持有独立的 `Random` 实例，通过仓颉 `ThreadLocal<T>`（来自 `core` 包，已验证可用，见 H5）管理，避免加锁争用
  - **初始化时机**：在 `linearRand`/`gaussRand` 首次被调用时创建 `Random` 实例（惰性初始化），存入 `ThreadLocal<Random>`。后续调用直接从 `ThreadLocal.get()` 获取实例并调用 `nextFloat64()` / `nextGaussianFloat64()` 等方法推进状态
  - **种子生成策略**：组合 `DateTime.now().toUnixMillisecond()` 与线程标识（`Thread.currentThread().getId()`）作为种子输入，即 `seed = DateTime.now().toUnixMillisecond() ^ Thread.currentThread().getId()`。异或组合确保同一毫秒内不同线程获得不同种子，降低种子碰撞概率。此策略虽不能完全消除碰撞（不同毫秒 + 不同线程的组合可能巧合产生相同种子），但与 GLM 1.0.3 的单一种子策略相比已有显著改善。若调用方需要可重复的随机序列，可扩展为支持外部种子注入，但当前设计不暴露种子设置 API（与 GLM 1.0.3 行为一致——GLM 的 `linearRand`/`gaussRand` 内部维护全局引擎，不提供种子控制）
  - **已知的种子碰撞风险**：尽管采用组合种子策略，仍存在两个线程在不同毫秒但巧合得到相同种子的微小可能性（碰撞概率约 1/2^32 量级）。当前设计接受此风险，与 GLM 1.0.3 行为一致——GLM 也不提供种子控制。如需完全确定性种子隔离，编码阶段可通过扩展 API 支持外部种子注入
  - **竞态保护**：`ThreadLocal<T>` 保证每个线程独立存储，无共享状态，无需加锁。初始化时线程间不存在竞态——如果两个线程同时首次调用，各自独立创建 `Random` 实例
...
- `ulp(x)`：返回 x 的 ULP 大小
- **实现路径分析**：ULP 函数需要操作浮点数的位表示（将浮点数位模式解释为整数进行加减运算再转回浮点数）。仓颉 `FloatingPoint<T>` 接口不提供位级操作能力，`toBits()`/`fromBits()` 方法仅存在于具体类型 `Float32`/`Float64` 上，不在泛型约束中。因此无法以 `T <: FloatingPoint<T>` 泛型方式实现
- **签名设计**：改为具体类型重载（Float32/Float64 各一套）：
  ```
  func next_float(x: Float32): Float32    // Float32 > Float64 > Float16，使用 toBits/fromBits
  func next_float(x: Float64): Float64
  func prev_float(x: Float32): Float32
  func prev_float(x: Float64): Float64
  func float_distance(x: Float32, y: Float32): Float32
  func float_distance(x: Float64, y: Float64): Float64
  func ulp(x: Float32): Float32
  func ulp(x: Float64): Float64
  ```
  - `next_float/prev_float`：使用 `toBits()` 将浮点数位模式转为无符号整数，执行 `+1`/`-1` 操作，再通过 `fromBits()` 转回浮点数。需处理符号位和 NaN/Inf 边界条件
  - `float_distance`：对两个浮点数分别 `toBits()`，差值即 ULP 距离。需统一符号位处理
  - `ulp(x)`：对 x > 0 返回 `next_float(x) - x`，对 x < 0 返回 `x - prev_float(x)`。使用具体类型直接计算
- 不包括 Float16 版本（GLM 1.0.3 也不为 half 类型提供完整的 ULP 函数）
- 约束：具体类型直接声明，无泛型约束
...
流程：
  ...
  3. 奇异矩阵行为：inverse 的结果由 IEEE 754 浮点运算自然决定
     - 1/det 在 det == T(0) 时产生 ±Inf
     - Inf × 余子式中的零分量 → Inf × 0 → NaN（经 IEEE 754 NaN 传播）
     - 最终可能产生 NaN 或 Inf 填充的矩阵
     - 函数不抛出异常，不设置显式零矩阵保护
     - 调用方需自行通过行列式检查矩阵的奇异性
...
| 场景 | 策略 | 说明 |
|------|------|------|
| **奇异矩阵求逆** | 结果由 IEEE 754 浮点运算自然决定（1/det → ±Inf，Inf × 0 → NaN），最终可能产生 NaN 或 Inf 填充的矩阵 | 与 GLM 1.0.3 行为一致，函数不抛出异常，调用方需自行通过行列式检查矩阵的奇异性 |
...
**例外 — `gtc/random.cj`**：`linearRand` 和 `gaussRand` 需要可变状态的随机数引擎以推进随机数序列，因此非纯函数。线程安全性通过 `ThreadLocal<Random>`（已验证 H5）保证——每个线程持有独立的 `Random` 实例，无共享状态，无需加锁。惰性初始化策略确保首次使用时创建引擎实例，种子采用时间戳与线程 ID 组合以降低碰撞概率。此例外不影响其他函数库的纯函数声明。
...
| D15 | **`mod` 函数当前（阶段三 stub）约束为 `Integer<T>`，阶段四完整实现后改为 `FloatingPoint<T>`** | 当前 stub 文件（`common.cj` 第 12 行）的 `mod` 约束为 `Integer<T>`，此为阶段三为快速编译采用的临时约束。阶段四替换为完整实现后，`mod` 使用 `x - y * floor(x / y)` 公式（需要浮点除法），约束变更为 `FloatingPoint<T>`。此变更为向后不兼容——现有使用整数类型调用 `mod` 的代码将在阶段四编译报错。编码阶段应评估现有代码是否在 `Integer<T>` 下调用了 `mod`，若存在整数 mod 的调用需求，可增加整数版本的重载以恢复向后兼容。整数取模在 GLSL 中通过 `%` 运算符而非 `mod` 函数实现 |
...
| D19 | **`gtc/random.cj` 使用 `ThreadLocal<Random>` 管理随机数引擎，种子采用时间戳与线程 ID 组合** | `linearRand`/`gaussRand` 每次调用修改引擎内部状态，非纯函数。仓颉 `ThreadLocal<Random>`（来自 `core` 包，已验证可用 H5）确保每个线程持有独立实例，无需加锁。惰性初始化：首次调用时以 `DateTime.now().toUnixMillisecond() ^ Thread.currentThread().getId()` 为种子创建 `Random` 实例。异或组合降低同一毫秒内多线程的种子碰撞概率。已知的种子碰撞风险：不同毫秒 + 不同线程的巧合组合仍可能产生相同种子（概率约 1/2^32），当前设计接受此风险，与 GLM 1.0.3 行为一致。备选方案 `Mutex<Random>` 可用但性能较差，仅在 `ThreadLocal` 方案经编译验证不可行时启用 |
...
| D25 | **modf/frexp 采用元组返回替代 C 引用参数，ldexp 自行实现** | 仓颉无引用/输出参数，C/GLM 中 `modf(x, &i)` 和 `frexp(x, &exp)` 的第二个输出参数通过引用传递。仓颉采用元组返回：`modf(x) → (fractional, integer)`，`frexp(x) → (mantissa, exponent)`。ldexp 无输出参数问题，但 std.math 不提供 ldexp 函数，因此改为自行实现：`x * pow(T(2), T(exp))`。此设计消除了阶段三 stub 中缺失的签名定义 |
| D26 | **奇异矩阵求逆行为采用"由 IEEE 754 浮点运算自然决定"的统一定义** | 修正此前三处描述不一致的问题。统一为：`T(1)/det` 在 `det == T(0)` 时产生 ±Inf，Inf × cofactor 中的零分量经 IEEE 754 传播产生 NaN，最终结果可能是 NaN 或 Inf 填充的矩阵。函数不抛出异常，不设保护分支。与 GLM 1.0.3 实现一致 |
| D27 | **gtc/ulp.cj 改为具体类型重载（Float32/Float64），放弃泛型 `T <: FloatingPoint<T>`** | `FloatingPoint<T>` 接口不包含 `toBits()`/`fromBits()` 等位操作方法，这些方法仅存在于具体类型 `Float32`/`Float64` 上。ULP 函数的核心算法依赖浮点数的位模式操作（将浮点数解释为整数进行 ±1 操作），无法通过泛型代码实现。改为 Float32/Float64 具体类型重载后，内部直接调用 `Float32.toBits().wrappingAdd(1U).toFloat32()` 等位运算，实现路径清晰 |
...
| gtc/ulp.cj | 新建完整实现 | Float32/Float64 类型 + 位操作 API |
...
| 审查意见 | 修改措施 |
|---------|---------|
| P1（严重）— modf/frexp/ldexp 签名设计完全缺失 | 在 §3.1 common.cj 职责中新增「modf/frexp/ldexp 的仓颉签名设计」段落，完整描述三个函数的仓颉签名形态：`modf<T>(x: T): (T, T)` 返回 (fractional, integer) 元组；`frexp<T>(x: T): (T, Int64)` 返回 (mantissa, exponent) 元组；`ldexp<T>(x: T, exp: Int64): T` 直接委托 `std.math.ldexp`。补充实现路径说明（modf 通过 fract/截断组合，frexp 以位操作为首选路径）。新增 D25 设计决策条目记录选择理由 |
| P2（严重）— 奇异矩阵求逆行为在三处自相矛盾 | 统一三处描述：§3.1 matrix.cj 职责中的"奇异矩阵行为"段落；§4.3 核心场景中的第 3 步流程；§5 错误处理策略表中的奇异矩阵行。全部统一为"结果由 IEEE 754 浮点运算自然决定（1/det → ±Inf，Inf × 0 → NaN），最终可能产生 NaN 或 Inf 填充的矩阵。函数不抛出异常。调用方需自行通过行列式检查矩阵的奇异性。"新增 D26 设计决策条目记录统一定义 |
| P3（严重）— mod 约束的当前状态描述与代码事实不符 | 修正 §3.1 common.cj 约束策略中的 `mod` 行：明确标注阶段三当前 `mod` 约束为 `Integer<T>`（引用 `common.cj` 第 12 行），阶段四替换为完整实现后将改为 `FloatingPoint<T>`，并标注此变更为向后不兼容。修正 D15 设计决策条目：从"当前约束为 FloatingPoint<T>"改为"当前（阶段三 stub）约束为 Integer<T>"，补充评估建议（编码阶段检查现有代码是否使用整数 mod 并考虑保留重载） |
| P4（一般）— trigonometric.cj 依赖图中遗漏 scalar_constants.cj | 在 §2 模块间依赖表中 trigonometric.cj 行追加 `+ scalar_constants.cj（radians/degrees 使用 pi<T>()）`。在 §3.1 trigonometric.cj 职责描述中明确标注 `pi<T>()` 来自同包的 `scalar_constants.cj` |
| P5（一般）— gtc/random.cj 种子初始化竞态风险评估不足 | 在 §3.3 random.cj 种子生成策略中增强说明：种子组合策略改为 `DateTime.now().toUnixMillisecond() ^ Thread.currentThread().getId()`，通过异或操作降低同一毫秒内多线程的碰撞概率。新增"已知的种子碰撞风险"段落明确标注"碰撞概率约 1/2^32 量级，当前设计接受此风险，与 GLM 1.0.3 行为一致"。同步更新 D19 设计决策条目中的种子策略描述 |
| P6（严重）— gtc/ulp.cj 泛型函数在仓颉中缺少实现路径 | 重写 §3.3 ulp.cj 职责：原 `T <: FloatingPoint<T>` 泛型签名改为 Float32/Float64 具体类型重载，提供完整签名清单（4 个函数 × 2 种精度 = 8 个重载）。新增"实现路径分析"段落解释放弃泛型的原因（`FloatingPoint<T>` 接口不包含 `toBits()`/`fromBits()`）及具体类型重载的实现策略（`toBits()` → 整数 ±1 → `fromBits()`）。更新 §1 核心抽象表中 ulp.cj 的类型形态为"包级具体类型重载函数（Float32/Float64）"。更新 §2 包组织和模块间依赖中的 ulp.cj 依赖描述。更新 §8 实施批次中 ulp.cj 的前置依赖。新增 D27 设计决策条目记录选择理由 |
| 问题 1（一般）— modf 公式在负数情况下不正确 | 修正 §3.1 common.cj 中 modf 的公式：从 `fract(x)` 基值 + 负数分段修正改为 `trunc(x)` 基值的统一公式。新公式 `integer = trunc(x)`、`fractional = x - trunc(x)` 对所有实数均符合向零截断语义，与 C 标准 modf 行为一致 |
| 问题 2（一般）— ldexp 声称直接委托 std.math.ldexp 但该函数不存在 | 修正 §3.1 common.cj 中 ldexp 的实现路径：删除"直接委托 std.math.ldexp"的描述，改为自行实现为 `x * pow(T(2), T(exp))`。同步更新 D25 设计决策条目中的理由描述，从"ldexp 直接委托 std.math"改为"ldexp 自行实现" |
