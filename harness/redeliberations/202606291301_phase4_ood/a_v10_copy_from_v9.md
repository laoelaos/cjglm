...
func mirrorRepeat<T>(Texcoord: T): T where T <: FloatingPoint<T>       // 分片纹理循环：Abs = abs(Texcoord) → Floor = floor(Abs) → Rest = Abs - Floor → Mirror = clamp(Floor, 0, 1) → mix(Rest, 1 - Rest, Mirror % 2 >= 1)。GLM 1.0.3 源码参考：`ext/scalar_common.inl` 第 78~86 行
...
// Simplex 噪声系列（1D 返回标量 T，2D/3D/4D 返回对应向量）
func simplex1D<T, Q>(x: T): T where T <: FloatingPoint<T>, Q <: Qualifier
func simplex2D<T, Q>(v: Vec2<T, Q>): Vec2<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier
func simplex3D<T, Q>(v: Vec3<T, Q>): Vec3<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier
func simplex4D<T, Q>(v: Vec4<T, Q>): Vec4<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier
```
