...
- **modf/frexp/ldexp 签名设计**：
  ...
  - `ldexp<T>(x: T, exp: Int64): T` where T <: FloatingPoint<T>：直接实现为 `x * pow(T(2), T(exp))`，不委托 `std.math.ldexp`（仓颉标准库不提供此函数）。`T(exp)` 转换需使用 `T(Float64(exp))` 按 §1.4 的表示约定处理
...
**职责**：对标 GLM 1.0.3 `ext/vector_common.hpp` 的全部 20 个函数，按分组列出函数签名映射。所有函数为 Vec 泛型版本，参数 `L`（维度 1~4）、`T`（标量类型）、`Q`（限定符）：
