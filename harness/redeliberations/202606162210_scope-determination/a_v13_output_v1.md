...
| 偏特化（Partial Specialization） | ❌ 不支持：CangJie 泛型类型在所有类型参数上**不变**（invariant），无 C++ 式偏特化/全特化机制 | **关键差异**：`vec<N,T,Q>` 的 4 个分量数变体无法通过模板偏特化实现。替代方案：① 分别定义 `Vec1<T, Q>`、`Vec2<T, Q>`、`Vec3<T, Q>`、`Vec4<T, Q>` 四个独立泛型结构体（不使用分量数参数），其中 Q 以 `where Q <: Qualifier` 约束；② 使用宏生成各分量数的结构体定义；③ 若保留 `N` 作为类型参数，可通过 `VArray<T, N>`（标准语法，`N` 为编译期常量）配合宏实现编译期分量数分发。首轮范围建议采用方案①或方案② |
| 型变 | 用户自定义泛型类型在所有类型参数上为**不变** | 不影响首轮核心类型设计（`vec` 无须协变/逆变） |

> **`qualifier` 在 CangJie 中的类型映射**：C++ GLM 中 `qualifier` 是一个枚举类型（含 `packed_highp`、`packed_mediump`、`packed_lowp`、`aligned_highp` 等枚举值），作为**枚举值**传递给泛型类型参数（如 `vec<L,T,qualifier Q>`）。CangJie 泛型类型参数**仅接受类型作为实参**，不接受枚举值或枚举构造器作为实参。因此首轮采用以下设计方案（**接口多态方案**）：
...
| 5 | `glm/detail/_vectorize.hpp` | 向量化工具模板（`functor1`/`functor2` 等） | 无 `#include`，使用 `length_t`/`qualifier` 作为模板参数；含 template template parameter，CangJie 需拆分为 4 组 functor（共 16 个定义），详见 0.1 节说明 | — | 2a |
...
2. 所有首轮核心结构体定义（`Qualifier` 接口及实现类型、`Vec1`～`Vec4` 结构体、辅助文件中的工具结构体）均放置在 `package glm.detail` 中
...
| 独立可编译性 | 首轮文件仅依赖自身集合，不引入任何未迁移的 GLM 组件；可按子范围逐层编译 |
...
| 验收准则 | 范围级描述 | 可执行验证步骤 |
|---------|----------|--------------|
| 依赖闭合性 | 首轮范围内所有文件的 `#include` 依赖链不超出首轮范围声明的文件集合 | **步骤**：在 CangJie 空项目中创建 `src/glm/` 目录，将首轮迁移的 11 个 GLM 源文件 + 3 个 shim 文件共 14 个文件按依赖拓扑放入，每个文件头部添加对应的 `package glm.detail` 或 `package glm` 声明。执行 `cjpm build`，检查是否缺失任何未在首轮范围内的引用。**通过条件**：编译通过，无 `file not found` 错误 |
| 拓扑可编译性 | 首轮文件可按依赖拓扑顺序逐层编译，每层无遗漏外部依赖 | **步骤**：按子范围（1→2a,2b→4,5,6,7→8）顺序逐层创建临时包并编译。**验证脚本伪代码**：<br><pre>// 子范围 1 验证<br>package round1_sub1<br>// setup.cj (package glm.detail, 包含 qualifier 和 length_t 定义)<br>// qualifier.cj (package glm.detail, 包含 Qualifier 接口及实现类型和前向声明)<br>// 编译: cjc src/glm/detail/setup.cj src/glm/detail/qualifier.cj ✓</pre> |
| 边界隔离性 | 首轮范围不依赖任何未迁移的 GLM 组件 | **步骤**：在 CangJie 项目中仅纳入首轮声明的 11 个 GLM 源文件 + 3 个 shim 文件，确认所有 `import` 语句引用的包/文件不超出此集合；执行 `cjpm build`。**通过条件**：编译通过，且编译错误信息中不包含任何首轮范围外的文件缺失提示 |
| 基础类型可用性 | 所有首轮向量类型支持构造、分量访问和基本算术运算 | **步骤**：在首轮编译通过后，编写如下 CangJie 测试代码并运行：<br><pre>package glm_test<br>import glm.detail.{ Vec4, PackedHighp }<br>main() {<br>    let v = Vec4&lt;Float32, PackedHighp&gt;(1.0f32, 2.0f32, 3.0f32, 4.0f32)<br>    println("${v[0]} ${v[1]} ${v[2]} ${v[3]}")  // 分量访问<br>    let sum = v + v  // 算术运算<br>    println("${sum[0] == 2.0f32}")<br>}</pre>**通过条件**：编译运行通过，输出符合预期 |
...
>
> **`_swizzle.hpp` 依赖链分析**（已排除出首轮范围，见 4.5 节）：经审查确认，`_swizzle.hpp` 自身不含额外 `#include` 语句（仅定义 swizzle 构造器、重载操作符等纯类型工具），其依赖链闭合于首轮基础设施层——该文件不引入任何向量/矩阵/函数库类型。但 `_swizzle.hpp` 中定义的结构体继承自 `vec<L, T, Q>`，其编译需依赖 vec 的完整类型定义，因此随对应 `type_vecN.hpp` 一同编译而非独立编译。
...
| 审查意见 | 处理方式 |
|---------|---------|
| 问题1（一般）：3A 节 setup.hpp 描述中 “约 10 个宏” 与完整清单 18 个宏存在事实矛盾 | **修改**：将 3A.1 节中 “约 10 个宏” 修改为 “**18 个宏（完整清单见下表）**” |
| 问题2（一般）：4.4 节验证步骤中的文件总数与 3D/3G 节正式统计口径不一致 | **修改**：将 4.4 节两处 “9 个核心文件 + 2~3 个 shim 文件共 11~12 个文件” 统一修正为 “11 个 GLM 源文件 + 3 个 shim 文件共 14 个文件”，与 3G 节口径完全一致 |
| 问题3（微小）：GLM_CONFIG_* 完整清单表格嵌套在 qualifier.hpp 段落后而非 setup.hpp 段落后，结构逻辑不通 | **修改**：将 GLM_CONFIG_* 完整清单表格从嵌套 blockquote 中移至 setup.hpp 条目后的独立 blockquote 位置，使表格紧接 setup.hpp 描述文本出现，脱离嵌套结构；storage SIMD 特化依赖和 GLM_CONFIG_SIMD 条件包含的 blockquote 自然移后 |
...
| 审查意见 | 处理方式 |
|---------|---------|
| 问题1（严重）：`Vec<T, Q>` 泛型参数 Q 设计存在根本性语法错误——CangJie 泛型类型参数仅接受类型作为实参，不接受枚举值 | **修改**：全文采用方案 A（接口多态）重新设计 qualifier 映射。在 0.1 节新增 "`qualifier` 在 CangJie 中的类型映射" 说明段，定义 `interface Qualifier {}` 及 6 个实现空结构体（`PackedHighp`、`PackedMediump`、`PackedLowp`、`AlignedHighp`、`AlignedMediump`、`AlignedLowp`），Vec 泛型中的 Q 改为 `where Q <: Qualifier` 接口约束。同步更新：0.1 节表格（泛型 struct/class 行、偏特化行）、0.1 节 `_vectorize.hpp` 注文中的 functor 模板参数描述、0.3 节类型别名说明（别名不能重复 `where` 约束）、3A.2 节 `qualifier.hpp` 描述、3F 节包映射代码示例（`qualifier.packed_highp` → `PackedHighp`）、4.4 节验证示例代码（同上）、3F 节包结构描述（`qualifier` 枚举 → `Qualifier` 接口及实现类型）、4.4 节拓扑验证伪代码（同上）。全文所有 `qualifier.packed_highp` 枚举值作为泛型实参的写法均移除，统一替换为类型名 `PackedHighp` |
| 问题2（中）：文档标题版本号写为 (v11) 但最后修订为 v12 | **修改**：标题从 `(v11)` 更新为 `(v13)` |
| 问题3（中）：`_vectorize.hpp` 文件清单条目未体现模板模板参数的设计影响 | **修改**：在 3E 节文件清单第 5 行 "C++ 依赖" 列补充注释 "含 template template parameter，CangJie 需拆分为 4 组 functor（共 16 个定义），详见 0.1 节说明" |
| 问题4（低）：`@OverflowWrapping` 示例代码中的注解被放置在行尾注释位置 | **修改**：将 0.5.1 节代码示例中的 `// @OverflowWrapping` 行尾注释改为 `@OverflowWrapping` 独立属性行置于函数声明之前 |
| 问题5（低）：4.4 节边界隔离性验证步骤的 C++ 环境倾向 | **修改**：将 4.4 节两处 "禁用全部 mat/qua/ext/gtc/gtx 头文件路径" 分别改为 CangJie 环境适用描述——验证标准表中 "独立可编译性" 行改为 "首轮文件仅依赖自身集合，不引入任何未迁移的 GLM 组件"；边界隔离性步骤改为 "在 CangJie 项目中仅纳入首轮声明的 11 个 GLM 源文件 + 3 个 shim 文件，确认所有 import 语句引用的包/文件不超出此集合" |
| 问题6（低）：`_swizzle.hpp` 依赖链分析中 "本版已将其排除" 的时效性表述过时 | **修改**：将第 5 节注文中 "（本版已将其排除出首轮范围，此处为记录）" 改为 "（已排除出首轮范围，见 4.5 节）" |
