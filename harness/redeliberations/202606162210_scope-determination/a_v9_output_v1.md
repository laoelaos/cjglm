...
| 4 | `glm/detail/compute_vector_decl.hpp` | 向量声明辅助模板（`functor` 等） | `setup.hpp` + `qualifier.hpp`（包含上下文中解析）、`<cstddef>`（`std::size_t` 非类型模板参数，16 处）、`<functional>`（`std::plus<T>` 等）、`<limits>`（经 `compute_vector_relational.hpp` 传递） | `shim_cstddef.cj`、`shim_limits.cj`（传递，由 compute_vector_relational 覆盖） | 2a |
