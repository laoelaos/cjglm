...
| 欧拉角 | `eulerAngles`/`roll`/`pitch`/`yaw` | **完整实现**（v3 修正） | `trigonometric.cj`(`atan`/`asin`) + `common.cj`(`clamp`) + `vector_relational.cj`(`equal`) + `scalar_constants.cj`(`epsilon<T>()`) — 但这些都是依赖 stub，所以**改为 stub 占位**（v3 最终决策） |
...
- `lib.cj`：新增 Quat 类型、type_quat_cast 函数、ext/gtc 函数和常量的 public import
...
### glm/detail/type_quat_cast.hpp（v3 新增）

| 函数 | 覆盖状态 | 对应设计方案章节 |
|------|---------|----------------|
| `mat3Cast(q): Mat3x3<T,Q>` | 本阶段实现 | §3.2 |
| `mat4Cast(q): Mat4x4<T,Q>` | 本阶段实现 | §3.2 |
| `quatCast(m: Mat3x3<T,Q>): Quat<T,Q>` | 本阶段实现 | §3.2 |
| `quatCast(m: Mat4x4<T,Q>): Quat<T,Q>` | 本阶段实现 | §3.2 |
...
| `mat3_cast`/`mat4_cast`/`quat_cast`（detail） | ✅ 可用 | ✅ 可用 |
| `mat3_cast`/`mat4_cast`/`quat_cast`（gtc，重导出） | ✅ 可用 | ✅ 可用 |
