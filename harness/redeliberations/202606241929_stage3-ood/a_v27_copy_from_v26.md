...
> **修订日期**：2026-06-26
> **修订定位**：v26 设计的迭代修订（对应本轮审查 a_v27_iteration_requirement.md）。基于 v26 输出修订，针对审查报告识别的 10 项问题（3 严重 + 6 一般 + 1 轻微）逐项落实。本轮重点修复：(1) §1 计数口径拆分——✅ 行拆分为"独立函数范式"和"含重载/常量展开"两行，删除误导性"约 47%"数字；(2) §8.3 H 节新增 H4 核验项——修订说明的修改措施必须有正文修改位置标记；(3) §3.13.1 新增 trigonometric.cj 完整签名展开模板及 gtc/matrix_transform 各类别签名模板；(4) §8.2 新增逐函数测试映射模板与合计值校验方法；(5) 清理正文中非决策性位置残留的 v14~v25 版本标注约 100+ 处；(6) §11.5 表前新增权威声明与质量状态段、表后新增合计行交叉核验；(7) 文件名/头部/正文版本号统一为 v27，消除版本号混淆；(8) §3.16 新增阶段四回归测试策略子节；(9) O-06 优先级提升为"下一轮必须实施"、§1 新增验证时序声明；(10) §8.2 末尾明确 7 个增补用例的具体分配计划。
...
### 关于版本标注（v27 新增，Problem 5 响应）

本设计文档采用迭代演进式修订方式，正文中残留了大量跨轮次的版本标注（"**v14 关键修订**"、"**v15 修订，Issue 6 响应**"等），共计约 100+ 处。这些标注虽在历史迭代中起过追踪作用，但对当前设计的首次阅读者构成视觉噪音和考古负担。

**v27 清理原则**：
1. **§7 设计决策表**保留各决策的版本号（D01~D37 均标注首次引入的版本号），这是决策溯源必需的元数据
2. **非决策性位置的版本标注**（正文段落中的 "**v{N} 关键修订**"、"**v{N} 修订**"、"**v{N} 新增**" 等）已逐一清理——保留标注内包含的设计决策内容（约束说明、策略选择、Issue 引用等），移除版本标记本身
3. **§3.13.2/§8/§11.5 等计数或表格**中保留少量版本标记仅用于计数变更解释，不从正文中移除

读者如需追溯特定版本的完整变更历史，请查阅独立历史文件 `a_v27_revision_history.md`（本版本修订说明见文件末尾「修订说明（v27）」章节）。---
...
| O-06 | 验证前置化流程（核心假设在设计版本固化前完成验证）尚未制度化 | v25 | **下一轮必须实施**（v27 升级） | v28 |
