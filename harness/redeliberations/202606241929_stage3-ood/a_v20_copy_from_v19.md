...
- **实施路径（v19 关键修订，Issue 3 响应——补充可执行实施细节）**：`fwd.cj` 是自动生成文件（`cjglm/src/fwd.cj:1-2` 文件头注释为中文「`// fwd.cj — GLM 兼容类型别名（自动生成）`」+「`// 注意：此文件由脚本自动生成，手动修改请谨慎`」），**手动添加将随重新生成丢失**。v13 决策采用**方案 A（推荐，修改生成脚本）**。以下给出可直接执行的完整修改方案：
  ...
  **说明**：`QUAT_FAMILIES` 是独立于 `VEC_FAMILIES` 的单独字典，避免在 Vec 循环的 `DIMS = [1,2,3,4]` 中误生成 `Quat1`/`Quat2`/`Quat3` 三个不存在项；`QUAT_PRECISIONS` 与 `VEC_PRECISIONS` 值一致但独立声明，消除循环间意外副作用。
  ...
  grep -c "type Quat = Quat<" cjglm/src/fwd.cj
  # 预期输出：9（对应 Quat/FQuat/DQuat + 3×Float32 精度 + 3×Float64 精度）
  
  # 验证不包含 Vec1/Vec2/Vec3 变体
  grep "type.*Quat.*Vec" cjglm/src/fwd.cj
  # 预期输出：无匹配（不应存在 Quat1/Quat2/Quat3）
...
27. **Mat4x4 列字段名 `c0`/`c1`/`c2`/`c3` 存在性验证（v19 修订，Issue 6 响应——补充修复责任人标注）**：验证 `Mat4x4<T,Q>` 的列字段命名为 `c0`/`c1`/`c2`/`c3`（而非 `col0`/`col1`/`col2`/`col3` 或其他命名），确保 §3.3 item 7 `fromMat4` 手动提取策略中 `m.c0`/`m.c1`/`m.c2` 编译通过——(a) 查阅阶段二 `type_mat4x4.cj` 确认列字段的实际命名；(b) 若实际命名与设计假设不一致，更新 §3.3 item 7 描述中的字段名引用；(c) 验证 `Vec3<T,Q>(m.c0.x, m.c0.y, m.c0.z)` 表达式 `T` 类型推断正确。**修复责任人**：编码者——验证后按实际字段名修改 §3.3 item 7 实现注释中的字段名引用；仅涉及实现注释文字修正，不涉及 API 签名变更，无需设计审批。
...
|------|----------|--------|
| `identity` | ✅ 可用 | ✅ 可用 |
| `fromMat3` | ✅ 可用（**约束：`where T <: FloatingPoint<T>`（签名显式声明，v15 修订，Issue 5 响应），仅浮点 T；整型 T 实例化时编译失败；编译错误直接定位在 `fromMat3` 签名约束处而非内部 callee**） | ✅ 可用（约束同上） |
| `fromMat4` | ✅ 可用（**约束：`where T <: FloatingPoint<T>`（签名显式声明，v15 修订，Issue 5 响应），仅浮点 T；整型 T 实例化时编译失败；编译错误直接定位在 `fromMat4` 签名约束处而非内部 callee**） | ✅ 可用（约束同上） |
| `fromVec3` | ❌ stub | ✅ 可用 |
| `fromEuler` | ❌ stub | ✅ 可用 |
| `fromQuat`（**v20 新增，Issue 1 响应**：跨类型构造工厂函数，通过 `conv` 闭包转换分量类型） | ✅ 可用 | ✅ 可用 |
| `wxyz`（**v20 新增，Issue 1 响应**：参数顺序 w,x,y,z 与 GLM `qua::wxyz()` 一致） | ✅ 可用 | ✅ 可用 |
| `[]` 下标运算符（**v20 新增，Issue 1 响应**：取值 + 赋值双版本，i < 0 或 i >= 4 抛 Exception） | ✅ 可用 | ✅ 可用 |
| `static func length(): Int64`（**v20 新增，Issue 1 响应**：编译期查询函数，返回常量 4） | ✅ 可用 | ✅ 可用 |
| `q * v` (Quat×Vec3) | ⚠️ 抛 stub 异常 | ✅ 可用 |
| `v * q` (Vec3×Quat) | ✅ 可用（**v18 修订：内联 conjugate/dot 计算路径**——`(Quat(-q.x, -q.y, -q.z, q.w) / (q.w*q.w + q.x*q.x + q.y*q.y + q.z*q.z)) * v`，纯算术运算，无需 `glm.ext` 跨包 import） | ✅ 可用 |
| `v * q` (Vec4×Quat) | ✅ 可用（通过 Vec3 中间路径，与 Vec3×Quat 一致） | ✅ 可用 |
| `+=`/`-=`/`*=`（四元数乘法）/`*=`（标量乘法）/`/=`（**v20 新增，Issue 1 响应**：复合赋值运算符，由编译器自动生成，与阶段二一致） | ✅ 可用 | ✅ 可用 |
| `add`（`scalar_quat_ops.cj`） | ✅ 可用 | ✅ 可用 |
| `sub`（`scalar_quat_ops.cj`） | ✅ 可用 | ✅ 可用 |
| `mul`（`scalar_quat_ops.cj`） | ✅ 可用 | ✅ 可用 |
| `div`（`scalar_quat_ops.cj`） | ✅ 可用 | ✅ 可用 |
...
| `axis` | ✅ 可用 | ✅ 可用 |
| `equal`（Quat） | ✅ 可用 | ✅ 可用 |
| `equal`（Quat, epsilon） | ✅ 可用 | ✅ 可用 |
| `notEqual`（Quat） | ✅ 可用 | ✅ 可用 |
| `notEqual`（Quat, epsilon） | ✅ 可用 | ✅ 可用 |
| `isnan` | ✅ 可用（**约束：`where T <: FloatingPoint<T>`（D29）**，仅浮点 T，整型 T 编译失败） | ✅ 可用（**约束：`where T <: FloatingPoint<T>`（D29）**，仅浮点 T） |
| `isinf` | ✅ 可用（**约束：`where T <: FloatingPoint<T>`（D29）**，仅浮点 T，整型 T 编译失败） | ✅ 可用（**约束：`where T <: FloatingPoint<T>`（D29）**，仅浮点 T） |
| `slerp` | ❌ stub | ✅ 可用 |
| `mix` | ❌ stub | ✅ 可用 |
| `angle` | ❌ stub | ✅ 可用 |
| `angleAxis` | ❌ stub | ✅ 可用 |
...
| `yaw` | ❌ stub | ✅ 可用 |
| `quatLookAt` | ❌ stub | ✅ 可用 |
| `quatLookAtRH` | ❌ stub | ✅ 可用 |
| `quatLookAtLH` | ❌ stub | ✅ 可用 |
| `epsilon`（浮点） | ✅ 可用 | ✅ 可用 |
| `pi`（浮点） | ✅ 可用 | ✅ 可用 |
| `cos_one_over_two`（浮点） | ✅ 可用 | ✅ 可用 |
| `epsilon`（整型） | ❌ 抛异常 | ❌ 抛异常（保持契约） |
| `pi`（整型） | ❌ 抛异常 | ❌ 抛异常（保持契约） |
| `cos_one_over_two`（整型） | ❌ 抛异常 | ❌ 抛异常（保持契约） |
| `mat3Cast`（detail） | ✅ 可用（**约束：`where T <: FloatingPoint<T>`（D32）**，仅浮点 T，整型 T 编译失败，对应 GLM `GLM_STATIC_ASSERT(is_iec559, ...)`） | ✅ 可用（**约束：`where T <: FloatingPoint<T>`（D32）**，仅浮点 T） |
| `mat4Cast`（detail） | ✅ 可用（同上） | ✅ 可用（同上） |
| `quatCast(Mat3)`（detail） | ✅ 可用（同上） | ✅ 可用（同上） |
| `quatCast(Mat4)`（detail） | ✅ 可用（同上） | ✅ 可用（同上） |
| `mat3Cast`（gtc，重导出） | ✅ 可用（**约束：`where T <: FloatingPoint<T>`（D32，约束源自 detail 端原始定义，通过 public import 透明传递）**，仅浮点 T，整型 T 编译失败，**v12 关键修订，Issue 1 响应**：gtc 端采用 camelCase 命名而非 GLM snake_case 习惯，因 `public import` 不做命名转换） | ✅ 可用（同上） |
| `mat4Cast`（gtc，重导出） | ✅ 可用（同上） | ✅ 可用（同上） |
| `quatCast(Mat3)`（gtc，重导出） | ✅ 可用（同上） | ✅ 可用（同上） |
| `quatCast(Mat4)`（gtc，重导出） | ✅ 可用（同上） | ✅ 可用（同上） |
| `lessThan` | ✅ 可用（**约束：`where T <: Comparable<T>`, `Q <: Qualifier`**） | ✅ 可用（同上） |
| `lessThanEqual` | ✅ 可用（同上） | ✅ 可用（同上） |
| `greaterThan` | ✅ 可用（同上） | ✅ 可用（同上） |
| `greaterThanEqual` | ✅ 可用（同上） | ✅ 可用（同上） |
