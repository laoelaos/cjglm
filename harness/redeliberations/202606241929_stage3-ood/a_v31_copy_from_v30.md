...
> **修订日期**：2026-06-28
> **修订定位**：v30 设计的迭代修订（对应本轮审查 a_v31_iteration_requirement.md）。基于 v30 输出修订，针对审查报告识别的 10 项问题（3 严重 + 6 一般 + 1 轻微）逐项落实。本轮重点修复：(1) 版本标注噪音系统性清理——全文扫描 `**v{N}` 模式，非决策性标注移除外层版本包装保留设计内容；(2) 函数状态信息去重——选择 §11.5 为唯一权威来源，其余位置替换为交叉引用；(3) 运行时调用链依赖分析——对所有 ⚠️/✅ 但依赖 stub 的函数新增分析子段，§8.3 H 新增运行时调用链门禁；(4) §11.5 合计行新增总函数数口径说明行；(5) §5.3 新增 NaN/Inf 组合场景 4 行边界契约；(6) 三角函数 `FloatingPoint<T>` 约束增加阶段三编译行为说明与整数 T 降级路径；(7) §8 验证项状态汇总声明；(8) §3.16 阶段四批次编号改为「批次 A~D」消除与 §8.4 冲突；(9) §3.13.2 审计结论补充计数口径公式；(10) §11.7.1 ⚠️ 计数引用全文同步确认。
...
| **gtc/quaternion.cj** | **gtc 四元数扩展函数库（欧拉/比较/看向函数 + 转换函数重导出）** | **混合型：4 函数完整实现 + 7 函数 stub + 4 函数重导出**（：原「4 函数完整实现 + 7 函数 stub + 4 函数重导出」中 4 函数完整实现仅含比较函数，与 §3.15 一致；§3.2 策略段之前误声明的「8 个完整实现」含欧拉函数已修正） |
...
**验证时序声明**：H1（`T(Float64(n))` 语法可行性）和 H2（`FloatingPoint<T>` 接口方法可用性）两项 P0 核心假设的验证已完成于 v25 轮次（验证日期：2026-06-26）。验证结果：✅ **两项均已通过**。这意味着 v1~v24 中基于未验证假设做出的所有设计决策（涉及 normalize/axis/mat3Cast/mat4Cast/Quat×Vec3/Quat×Vec4/slerp/pow/log 等函数的签名选择与实现策略）已在轮次 v25（即第 25 轮设计迭代）时得到实际编译验证的支持。为彻底解决「验证发生在设计完成之后」的时序矛盾，O-06（验证前置化流程制度化）已在 v27 从「待评估」升级为「下一轮必须实施」——未来版本中，P0 核心假设的验证将前置为设计版本固化前的门禁条件，不再出现 24 轮设计建立在未经验证假设之上的情况。
...
本设计文档采用迭代演进式修订方式。v31 已对全部非决策性版本标注执行统一清理：正文中所有 `（v{N} 修订）`、`（v{N} 关键修订）`、`（v{N} 新增）`、`（Issue {N} 响应）` 等模式已被移除，保留了标注内的设计决策内容（约束说明、策略选择、计数变更解释等可实施性信息）。**§7 设计决策表**保留各决策的版本号（D01~D38），§3.13.2/§8/§11.5 中保留少量版本标记仅用于计数口径解释。历史变更追踪由独立文件 `a_v27_revision_history.md` 承载。§8.3 H5 核验门禁已将此清理纳入版本发布前置检查。
...
| `radians` | `radians<T>(x: T): T where T <: FloatingPoint<T>` | `radians<T, Q>(x: VecN<T, Q>): VecN<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier` | **例外（stdlib 不存在）**：硬编码 π 字面量，见本表下实现段。**交叉引用**：硬编码 π 值与 §3.12 `pi<T>()` 的 π 值应一致，建议在 `detail/scalar_constants.cj` 中定义 `PIVAL<T>` 共享常量，同时被 `pi<T>()`、`radians`、`degrees` 引用，消除潜在不一致（对应 Problem 5 建议） |
| `degrees` | `degrees<T>(x: T): T where T <: FloatingPoint<T>` | `degrees<T, Q>(x: VecN<T, Q>): VecN<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier` | **例外（stdlib 不存在）**：硬编码 π 字面量，见本表下实现段。**交叉引用**：硬编码 π 值与 §3.12 `pi<T>()` 的 π 值应一致，建议在 `detail/scalar_constants.cj` 中定义 `PIVAL<T>` 共享常量，同时被 `pi<T>()`、`radians`、`degrees` 引用，消除潜在不一致（对应 Problem 5 建议） |
...
**完整签名模板示例（v27 新增，Problem 3 响应）**：以下以 `sin` 函数为例给出 trigonometric.cj 中 14 个单参数三角函数之一的完整展开签名模板。其余 13 个单参数函数（`cos`/`tan`/`asin`/`acos`/`atan`/`sinh`/`cosh`/`tanh`/`asinh`/`acosh`/`atanh`/`radians`/`degrees`）和双参数 `atan2` 按相同模式展开：
...
**双曲类函数示例（v29 新增，Problem 4 响应）**：以 `cosh` 为例给出双曲类函数的完整展开签名模板。其余 5 个双曲函数（`sinh`/`tanh`/`asinh`/`acosh`/`atanh`）按相同模式展开：
```cangjie
// ===== 标量版本 =====
...
**反三角类函数示例（v29 新增，Problem 4 响应）**：以 `acosh` 为例给出反三角类函数的完整展开签名模板。其余 5 个反三角/反双曲函数（`asin`/`acos`/`atan`/`asinh`/`atanh`）按相同模式展开：
```cangjie
// ===== 标量版本 =====
...
**坐标转换类函数示例（v29 新增，Problem 4 响应）**：以 `degrees` 为例给出坐标转换类函数的完整展开签名模板。另一个坐标转换函数 `radians` 按相同模式展开：
```cangjie
// ===== 标量版本 =====
...
> **阶段三约束影响说明（v31 新增，Problem 6 响应）**：`where T <: FloatingPoint<T>` 约束收紧在阶段三**仅影响编译行为**，不改变运行时行为（函数体均为 `throw Exception("stub")`）。整数 T（如 `Int64`）实例化 trig 函数在阶段三会编译失败。**降级路径**：若下游需要整数 T 实例化通过编译，可在阶段三暂时移除 `where T <: FloatingPoint<T>` 约束改为 `where T <: Number<T>`，并在函数体内添加 `match` 分派：对浮点 T 保留 `throw Exception("stub")`，对整数 T 抛更友好的错误消息如 `Exception("trigonometric functions not defined for integer types")`。该降级路径仅用于阶段三编译通过性需求；阶段四实现函数体时必须恢复 `FloatingPoint<T>` 约束以确保整数 T 编译期拒绝。**当前阶段三所有 stub 函数保持 `FloatingPoint<T>` 约束不变**，降级路径由需要整数 T 编译的下游客自行评估实施。
...
- **核验项 H6（v31 新增，Problem 3 响应）**：运行时调用链无 stub 依赖验证门禁。所有标注为 ⚠️ 或 ✅ 的函数，在编码实施时必须在函数实现描述段中附带「运行时调用链依赖分析」子段，明确列出函数体内部的所有运行时调用链（包括间接调用），标注每级调用是否依赖 stub。验证方式：逐函数审查 §3.x 描述段，确认对每个 ⚠️/✅ 函数的运行时调用链分析完备；对遗漏分析子段的函数阻止版本发布。此门禁确保系统化的运行时依赖分析成为版本发布前置门禁，避免再次出现 Vec3×Quat 状态错误跨越 12 轮的流程性缺陷。
...
| O-06 | 验证前置化流程（核心假设在设计版本固化前完成验证）尚未制度化 | v25 | （详见下方 O-06 可执行落地计划） | v28 |
| O-07 | I 节非空时文档版本号标记质量门禁尚未实施 | v25 | 待评估 | v28 |

**O-06 可执行落地计划（v28 新增，Problem 8 响应）**：验证前置化流程（核心假设在设计版本固化前完成验证）的制度化方案，包含以下可执行要素：
...
：本表目标为**函数级状态追踪的权威来源**（每函数一行，标注本阶段/阶段四状态）。§3.13.2 审计节、§8 产出物清单、§9a 覆盖矩阵、§11.6 可达性矩阵的函数状态描述均以本表为主要基准。§9a「GLM 1.0.3 Stage 3 API 覆盖矩阵」提供聚合统计信息，与本表构成互补关系——本表聚焦逐函数精细追踪，§9a 提供聚合统计视图与 GLM 1.0.3 基准声明。
