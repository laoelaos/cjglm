...
> **修订日期**：2026-06-24
> **修订定位**：v2 设计的迭代修订（v3）。依据组件B诊断报告（19 项问题）与质询报告（LOCATED）开展修订，重点解决 3 项严重问题（`gtc/quaternion.cj` 文件规划缺失、`Quat×Vec3` 公式错误、`mat3_cast` 等函数归属矛盾）与 10 项一般问题。
...
| ext/quaternion_common.cj | 四元数公共函数库（slerp/lerp/conjugate/inverse） | 3 函数完整实现 + 5 函数 stub |
...
| **gtc/quaternion.cj** | **gtc 四元数扩展函数库（mat3_cast/mat4_cast/quat_cast/eulerAngles/roll/pitch/yaw/lessThan*/greaterThan*/quatLookAt*）** | **混合型：14 函数完整实现 + 3 函数 stub** |
...
沿用阶段一/阶段二的双层包结构 + ext 子包 + gtc 子包：glm.detail 包含四元数泛型结构体定义和运算符重载，glm 包通过类型别名暴露常用具现化，glm.ext 包含四元数/向量关系运算等扩展函数库和别名文件，glm.gtc 包含数学常量和 gtc_quaternion 扩展函数库。四元数类型以 Vec3+标量(w) 为内部数据布局，与 GLM 的 xyzw 布局一致（非 wxyz 布局），通过 `const init` 构造函数支持编译期求值。
...
### gtc/quaternion.cj 设计决策（v3 新增）

`gtc/quaternion.hpp` 包含 17 个函数（`eulerAngles`/`roll`/`pitch`/`yaw`/`mat3_cast`/`mat4_cast`/`quat_cast(Mat3)`/`quat_cast(Mat4)`/`lessThan`/`lessThanEqual`/`greaterThan`/`greaterThanEqual`/`quatLookAt`/`quatLookAtRH`/`quatLookAtLH`，其中 `quat_cast` 两个重载共 17 个声明）。本设计决策如下：

- **独立文件策略**：在 `src/gtc/quaternion.cj`（`package glm.gtc`）独立创建该文件，沿用 GLM 原始头文件路径，确保 GLM 函数到仓颉文件的 1:1 映射，便于追溯
- **文件归属决策**：`mat3_cast`/`mat4_cast`/`quat_cast` **不放在 `glm.detail.type_quat.cj`**，而是统一归入 `gtc/quaternion.cj`。GLM `type_quat.inl` 中的构造函数 `qua(mat3x3)`/`qua(mat4x4)` 通过 `#include "../gtc/quaternion.hpp"` 访问这些函数；仓颉版本通过 `package glm.gtc.quaternion` 路径访问
- **跨包依赖**：type_quat.cj 通过 `import glm.gtc.quaternion.*` 引用 `mat3Cast`/`mat4Cast`/`quatCast`，gtc/quaternion.cj 通过 `import glm.detail.*` 引用 `Quat`/`Mat3x3`/`Mat4x4`——形成 `glm.detail → glm.gtc` 的单向依赖（detail 不依赖 gtc，但 type_quat 的构造函数依赖 gtc 的转换函数）

**为何不把转换函数放入 detail**：GLM 中 `qua(mat3x3)`/`qua(mat4x4)` 构造函数在 `type_quat.inl` 内联，但其内部调用 `quat_cast`（gtc 函数）；仓颉中如把 `quatCast` 放入 detail 将违反 GLM 文件归属约定（gtc 函数应在 gtc 包）。本设计选择「gtc_quaternion.cj 独立承载 17 函数，type_quat.cj 跨包引用」，与 GLM 原始组织一致。
...
package glm/detail          — 核心实现层（新增/修改文件以 ★ 标记）
  ...
  │                          (构造函数内部跨包引用 glm.gtc.quaternion 的 mat3Cast/mat4Cast/quatCast)
...
package glm.ext             — 扩展函数库（新增文件以 ★ 标记）
  ...
  ├── quaternion_common.cj ★      — 四元数公共函数（3 函数完整实现 + 5 函数 stub）
...
package glm.gtc             — GTC 扩展函数库（新增目录和文件以 ★ 标记）
  ...
  ├── quaternion.cj ★       — gtc 四元数扩展函数（17 函数混合型：14 完整 + 3 stub）
```
...
glm.detail（同包直接可见）
  ...
  type_quat.cj → glm.gtc.quaternion（mat3Cast/mat4Cast/quatCast，跨包引用）
...
glm.gtc
  ...
  quaternion.cj → glm.detail（Quat, Mat3x3, Mat4x4, Vec3）
  quaternion.cj → glm.ext.scalar_constants（epsilon<T>()）
  quaternion.cj → glm.ext.vector_relational（equal，用于 roll/pitch/yaw 边界检测）
...
- **验证项 2（跨包引用）**：验证 `type_quat.cj`（package glm.detail）能否通过 `import glm.gtc.quaternion.*` 引用 gtc 包级函数
...
**`@Derive[Hashable]` 约束核验**（v3 新增）：仓颉 `@Derive[Hashable]` 要求泛型 type parameter 满足 `Hashable` 接口。`Quat<T, Q>` 中 `T <: Number<T>` 约束下所有数字类型均实现 `Hashable`；`Q <: Qualifier` 约束下 6 个 Qualifier 实现类型均无数据成员（标记类型），其 `Hashable` 派生由编译器自动支持。需在编码启动前验证项中确认 `@Derive[Hashable]` 对 `Q <: Qualifier` 的派生编译通过（[阶段一 Vec3 已实践使用 @Derive[Hashable]，模式一致，参见 `type_vec3.cj` 验证]）。
...
| Mat3←Quat mat3_cast | 四元数转 3×3 旋转矩阵 | gtc/quaternion.cj 包级函数 |
| Mat4←Quat mat4_cast | 四元数转 4×4 旋转矩阵 | gtc/quaternion.cj 包级函数 |
| Quat←Mat3 quat_cast | 旋转矩阵转四元数 | gtc/quaternion.cj 包级函数 |
| Quat←Mat4 quat_cast | 4×4 旋转矩阵转四元数 | gtc/quaternion.cj 包级函数 |
...
**本阶段对 gtc/quaternion 函数的处理策略（v3 修订）**：`gtc/quaternion.hpp` 中共 17 个函数声明（含 `quat_cast` 两个重载）。本阶段采取以下分批策略：

- **完整实现（归入 `gtc/quaternion.cj`）**：
  - `mat3_cast`/`mat4_cast`/`quat_cast(Mat3)`/`quat_cast(Mat4)` — 4 个函数，纯算术与 sqrt，不依赖 stub
  - `lessThan`/`lessThanEqual`/`greaterThan`/`greaterThanEqual` — 4 个比较函数，纯算术
- **stub 占位（归入 `gtc/quaternion.cj`，依赖 trigonometric.cj/geometric.cj stub）**：
  - `eulerAngles`/`roll`/`pitch`/`yaw` — 4 个函数，依赖 `trigonometric.cj` 的 `atan`/`asin` 和 `common.cj` 的 `clamp`，均为 stub
  - `quatLookAt`/`quatLookAtRH`/`quatLookAtLH` — 3 个函数，依赖 `geometric.cj` 的 `cross`/`dot`/`normalize`/`inversesqrt`/`max`（均为 stub）

**type_quat.cj 与 gtc/quaternion.cj 的协作关系**：type_quat.cj 中 `fromMat3`/`fromMat4` 工厂函数内部调用 `glm.gtc.quaternion.mat3Cast`/`mat4Cast`/`quatCast`，形成 `glm.detail → glm.gtc` 的单向依赖。仓颉包依赖 DAG：`glm.detail` 不依赖 `glm.gtc`（仅 type_quat.cj 通过 `import glm.gtc.quaternion.*` 引用单个转换函数，不构成循环依赖）。
...
6. **从旋转矩阵构造 fromMat3(m: Mat3x3<T,Q>)** — 辅助工厂函数，内部调用 `gtc/quaternion.quatCast(m)`。非 const。**边界行为**（v3 新增契约声明）：仅对纯旋转矩阵产生有意义的四元数；对非旋转矩阵（如含缩放/平移/剪切矩阵）行为未定义，结果可能产生非单位四元数或语义错误的旋转。

7. **从旋转矩阵构造 fromMat4(m: Mat4x4<T,Q>)** — 辅助工厂函数，内部先降维至 3×3 矩阵（提取左上 3×3 子矩阵或按 GLM 策略转换为 Mat3x3）再调用 `quatCast`。非 const。**边界行为**：同上，依赖 `fromMat3` 的输入合法性。
...
| 二元 `*` (Quat×Vec3) | 四元数旋转向量 | `Number<T>` | 实现：`v + (cross(QuatVector, v) * q.w + cross(QuatVector, cross(QuatVector, v))) * 2`（两次 Vec3 叉乘 `uv`/`uuv`）。**依赖 `geometric.cj` 向量 `cross`（阶段三 stub），调用将抛 `Exception("stub")`** |
...
- `axis(x: Quat<T,Q>): Vec3<T,Q>` — 依赖 `sqrt`（std.math 提供）和 T(1) 演算。可完整实现。**边界行为（v3 新增契约）**：零四元数输入（q.x == q.y == q.z == 0）时，内部 `normalize(Vec3(0, 0, 0))` 返回 `Vec3(T(1), T(0), T(0))`（沿用 `quaternion_geometric.cj.normalize` 零向量保护行为，与 GLM `quaternion_geometric.inl:20-21` 一致）。
- `angleAxis(angle: T, axis: Vec3<T,Q>): Quat<T,Q>` — 依赖 `sin`/`cos`（trigonometric.cj stub）。**stub 占位**。

**修正本阶段策略**：
- `axis` — **完整实现**（仅依赖 `std.math.sqrt`，零四元数保护行为见上）
...
**依赖分析**：
- `exp(q: Quat<T,Q>): Quat<T,Q>` — 依赖 `length`（quaternion_geometric，已实现）、`cos`/`sin`（trigonometric.cj stub）。**stub 占位**。
- `log(q: Quat<T,Q>): Quat<T,Q>` — 依赖 `length`、`atan`、`log`（trigonometric.cj/exponential.cj 接口）。**stub 占位**。
- `pow(x: Quat<T,Q>, y: T): Quat<T,Q>` — 依赖 `abs`/`clamp`/`acos`/`sin`/`cos`/`sqrt`（其中 `abs`/`clamp` 来自 common.cj stub，`acos`/`sin`/`cos` 来自 trigonometric.cj stub，`sqrt` 来自 std.math）+ `epsilon<T>()`（scalar_constants.cj 已实现）。**stub 占位**。
- `sqrt(x: Quat<T,Q>): Quat<T,Q>` — 委托 `pow(x, 0.5)`。**stub 占位**。

**v3 修订**：4 个函数均依赖 trigonometric.cj/common.cj stub，本阶段全部 **stub 占位**。`pow` 的依赖关系以 GLM `ext/quaternion_exponential.inl:24-43` 实际实现为准。
...
- `inverse(q: Quat<T,Q>): Quat<T,Q>` — `conjugate(q) / dot(q, q)`。**完整实现**（依赖 dot 和除法运算符）。**边界行为（v3 新增契约）**：
...
- `slerp(x, y, a): Quat<T,Q>` — 依赖 `dot`、`acos`/`sin`/`mix`（标量版）+ `epsilon<T>()`（v3 修订）。`acos`/`sin` 属 trigonometric.cj stub，`mix` 属 common.cj stub。**stub 占位**。
- `mix(x, y, a): Quat<T,Q>` — 依赖 `dot`、`acos`、`sin`、`epsilon<T>()`（v3 修订）。`epsilon<T>()` 用于 `cosTheta > 1 - epsilon<T>()` 的退化检测分支。**stub 占位**。
- `slerp(x, y, a, k)` — 多旋转 slerp。**stub 占位**。**`k` 类型决策（v3 明确）**：采用 `slerp<T, Q>(x: Quat<T,Q>, y: Quat<T,Q>, a: T, k: Int64)` 简化版签名。GLM 4-arg `slerp` 使用独立整型参数 `S`（通过 `static_assert(is_integer)` 约束），仓颉版本固定为 `Int64`（与 deviations.md DV-03「移位运算右操作数固定为 Int64」风格一致），牺牲泛型灵活性换取签名简洁性。**理由**：阶段三 slerp 函数本身为 stub，固定 `Int64` 简化签名不影响本阶段实施；阶段四 slerp 完整实现时若需泛型 K 可扩展。
- `isnan(q: Quat<T,Q>): Vec4<Bool,Q>` — 依赖 `x.isNaN()` 实例方法（仓颉 `std.math` 仅提供实例方法，**不提供**顶层 `std.math.isNaN(x)` 函数）。**可完整实现**（采用实例方法路径：`Vec4(q.x.isNaN(), q.y.isNaN(), q.z.isNaN(), q.w.isNaN())`），无需顶层函数验证。
- `isinf(q: Quat<T,Q>): Vec4<Bool,Q>` — 依赖 `x.isInf()` 实例方法。**可完整实现**（同上实例方法路径）。
...
- **整数类型 T 的行为契约（v3 新增）**：约束收紧为 `T <: FloatingPointNumber<T>` 或等价约束（如仓颉版本不支持，则采用 `T <: Float32 | Float64`）。**若仓颉泛型不支持窄约束**，则在 `match` 中增加 `case _ => throw Exception("epsilon/pi/cos_one_over_two not defined for non-floating-point types")` 显式错误分支。GLM 1.0.3 `scalar_constants.inl:36-43` 使用 `static_assert(is_iec559, ...)` 静态断言非浮点类型时编译失败——仓颉版本采用运行时异常等价行为。
...
**角色**：承载 GLM `gtc/quaternion.hpp` 的全部 17 个函数（4 个转换 + 4 个比较 + 9 个欧拉/看向函数），与 GLM 原始头文件路径保持 1:1 映射。

**职责分组**：

| 函数组 | 函数 | 本阶段状态 | 依赖 |
|--------|------|----------|------|
| 矩阵-四元数互转 | `mat3_cast`/`mat4_cast`/`quat_cast(Mat3)`/`quat_cast(Mat4)` | 完整实现 | 纯算术 + `std.math.sqrt` |
| 四元数比较 | `lessThan`/`lessThanEqual`/`greaterThan`/`greaterThanEqual` | 完整实现 | 纯比较 |
| 欧拉角 | `eulerAngles`/`roll`/`pitch`/`yaw` | stub | `trigonometric.cj`(`atan`/`asin`) + `common.cj`(`clamp`) + `vector_relational.cj`(`equal`) + `scalar_constants.cj`(`epsilon<T>()`) |
| 看向 | `quatLookAt`/`quatLookAtRH`/`quatLookAtLH` | stub | `geometric.cj`(`cross`/`dot`/`normalize`/`inversesqrt`/`max`) + `quat_cast`(本文件内) |

**完整实现函数（合计 8 个）**：
- `mat3_cast(q: Quat<T,Q>): Mat3x3<T,Q>` — 计算 3×3 旋转矩阵元素 `Result[i][j] = ...`（详见 GLM `quaternion.inl:47-72`），`T(1)` 通过调用方演算或参数传入
- `mat4_cast(q: Quat<T,Q>): Mat4x4<T,Q>` — 内部委托 `Mat4x4(mat3_cast(q))`
- `quat_cast(m: Mat3x3<T,Q>): Quat<T,Q>` — Shepperd's method（最大迹算法），4 分支 switch，依赖 `std.math.sqrt`
- `quat_cast(m: Mat4x4<T,Q>): Quat<T,Q>` — 降维至 3×3 后委托 `quat_cast(m)`
...
**与 type_quat.cj 的协作**：type_quat.cj 中 `fromMat3`/`fromMat4` 工厂函数通过 `import glm.gtc.quaternion.*` 跨包调用 `mat3Cast`/`mat4Cast`/`quatCast`。这构成 `glm.detail → glm.gtc` 的单向依赖（detail 不依赖 gtc，仅 type_quat.cj 单文件依赖），无循环依赖。
...
```
let m3 = mat3Cast(q)     // 四元数转 3×3 旋转矩阵 [本阶段可验证]
...
| 边界条件 | 函数 | 行为契约 |
|---------|------|---------|
| 零四元数 | `axis(q)` | 返回 `Vec3(T(1), T(0), T(0))`（沿用 `normalize` 零向量保护行为） |
...
| D11 | **mat3_cast/mat4_cast/quat_cast 定义于 glm.gtc.quaternion，type_quat.cj 跨包引用** | 与 GLM 原始头文件归属一致（gtc 函数在 gtc 包），type_quat.cj 通过 `import glm.gtc.quaternion.*` 单向引用 gtc 包级函数 |
...
| D20 | **`gtc/quaternion.cj` 独立文件承载 17 函数（v3 新增）** | 与 GLM `gtc/quaternion.hpp` 1:1 映射，便于 GLM 函数到仓颉文件的追溯；与阶段二 gtc 子包策略一致 |
| D21 | **`mix`/`slerp`/`pow` 的依赖明确包含 `epsilon<T>()`（v3 新增）** | 与 GLM `quaternion_common.inl`/`quaternion_exponential.inl` 实际依赖一致，包括 `cosTheta > 1 - epsilon<T>()` 退化检测分支 |
...
- `detail/type_quat.cj`（四元数核心类型 + 运算符，跨包引用 gtc/quaternion 的 mat3Cast/mat4Cast/quatCast）
...
- **`gtc/quaternion.cj`（v3 新增，14 函数完整实现 + 7 函数 stub）**
- 四元数别名文件（4 个 ext/ 文件）
- 四元数测试文件（详见 §8.2）
...
- `fwd.cj`：新增 8 个四元数类型别名（Quat/FQuat/DQuat/3×Float32 精度/3×Float64 精度）
...
| **`tests/glm/gtc/quaternion_test.cj`（v3 新增）** | **gtc 四元数转换/比较/欧拉/看向** | **≥20** |
| **合计** | — | **≥163** |
...
4. **stub 函数异常路径**：mix/slerp(2 版本)/angle/angleAxis/exp/log/pow/sqrt/rotate/eulerAngles/roll/pitch/yaw/quatLookAt* 调用时验证抛 `Exception("stub")`
5. **跨 Qualifier 实例化**：PackedHighp/PackedMediump/PackedLowp/AlignedHighp/AlignedMediump/AlignedLowp 6 种精度的 Quat 运算符测试
6. **跨类型实例化**：Float32/Float64 的 Quat 完整测试，整型 Quat 仅测试非依赖 stub 的运算符（边界处理）
...
2. **type_quat.cj 跨包引用 gtc/quaternion.cj 验证**：验证 `import glm.gtc.quaternion.*` 在 detail 包中能正确解析 gtc 包级函数（避免仓颉包间循环依赖检测误报）
3. **Quat 构造 fromMat3/fromMat4 依赖 quatCast 跨包引用验证**：type_quat.cj 中构造函数体调用 gtc/quaternion.cj 的 quatCast，验证跨包函数引用延迟解析
4. **Vec3/Vec4 extend 块中 Quat 类型前向引用验证**：Vec3×Quat 和 Vec4×Quat 运算符需在 Vec3/Vec4 的 extend 块中引用 Quat 类型，验证同包延迟解析（已通过阶段二原型验证，仅为防御性确认）
5. **scalar_constants 的泛型 `match` 类型模式匹配验证**：验证 `func epsilon<T>(hint: T): T where T <: Number<T>` 函数体内 `match (hint) { case _: Float32 => ... }` 编译通过
6. **scalar_constants 对整数类型 T 的行为契约验证**：验证 `epsilon<Int64>()` 调用时按 D25 决策抛出运行时异常（约束收紧若失败时的 fallback 路径）
7. **标量-四元数与标量-向量/标量-矩阵同名重载消歧验证**：`add(s: T, q: Quat<T,Q>)` 与 `add(s: T, v: Vec2<T,Q>)`/`add(s: T, m: Mat2x2<T,Q>)` 等的编译器消歧
8. **`x.isNaN()`/`x.isInf()` 实例方法路径验证**：确认仓颉标准库为所有浮点类型提供 `isNaN()`/`isInf()` 实例方法（影响 quaternion_common.cj 中 isnan/isinf 的实现可行性）
9. **ext/vector_relational 内联 abs 验证**：验证 `if (d >= T(0)) { d } else { -d }` 模式在 `Number<T> & Comparable<T>` 约束下的编译通过性
10. **`@Derive[Hashable]` 对 `Q <: Qualifier` 的支持验证**：验证 Hashable 派生宏对 6 个 Qualifier 实现类型的编译通过性
11. **`length(q)` 在 Float32 实例的 sqrt 转换验证**：验证 `T(Float64.sqrt(Float64(dot_qq)))` 路径或额外 Float32 重载的编译可行性
...
| **从旋转矩阵构造改为具名工厂函数** | C++ GLM 中 `qua(mat3x3)`/`qua(mat4x4)` 为构造函数，仓颉改为 `Quat.fromMat3`/`Quat.fromMat4`（跨包引用 gtc/quaternion.cj 的 quat_cast） |
| **`gtc/quaternion.cj` 独立承载 17 函数（v3 明确）** | 与 GLM `gtc/quaternion.hpp` 1:1 映射，type_quat.cj 通过 `import glm.gtc.quaternion.*` 跨包引用 |
| **`mat3_cast`/`mat4_cast`/`quat_cast` 位于 gtc/quaternion.cj（v3 明确）** | 与 GLM 头文件归属一致；type_quat.cj 通过跨包 import 引用 |
...
| **`axis` 零四元数返回 `Vec3(1, 0, 0)`（v3 契约）** | 沿用 `normalize` 零向量保护行为，与 GLM `quaternion_geometric.inl:20-21` 一致 |
...
| mix(Quat, Quat, T) | 本阶段 stub（依赖 trigonometric.cj/common.cj stub + epsilon<T>()） |
| slerp(Quat, Quat, T) | 本阶段 stub |
| slerp(Quat, Quat, T, Int64) | 本阶段 stub |
| isnan(Quat) | 本阶段实现（实例方法路径） |
| isinf(Quat) | 本阶段实现（实例方法路径） |

### glm/ext/quaternion_trigonometric.hpp

| 函数 | 覆盖状态 |
|------|---------|
| axis(Quat) | 本阶段实现（零四元数返回 Vec3(1,0,0)） |
| angle(Quat) | 本阶段 stub |
...
| exp(Quat) | 本阶段 stub |
| log(Quat) | 本阶段 stub |
| pow(Quat, T) | 本阶段 stub（依赖 abs/clamp/acos/sin/cos/sqrt + epsilon<T>()） |
| sqrt(Quat) | 本阶段 stub |
...
### glm/gtc/quaternion.hpp（v3 独立矩阵）

| 函数 | 覆盖状态 |
|------|---------|
| mat3_cast(Quat) | 本阶段实现 |
| mat4_cast(Quat) | 本阶段实现 |
| quat_cast(Mat3) | 本阶段实现 |
| quat_cast(Mat4) | 本阶段实现 |
| lessThan(Quat, Quat) | 本阶段实现 |
| lessThanEqual(Quat, Quat) | 本阶段实现 |
| greaterThan(Quat, Quat) | 本阶段实现 |
| greaterThanEqual(Quat, Quat) | 本阶段实现 |
| eulerAngles(Quat) | 本阶段 stub |
| roll(Quat) | 本阶段 stub |
| pitch(Quat) | 本阶段 stub |
| yaw(Quat) | 本阶段 stub |
...
| 审查意见 | 修改措施 |
|---------|---------|
| **问题 1（严重）**：`gtc/quaternion.cj` 文件未完整规划——§2 包组织、§3.2 协作关系表、§8 产出物清单均未将该文件纳入 | **已采纳**。新增 §3.15 独立小节「gtc/quaternion.cj」完整定义 17 函数分组与依赖；§1 核心抽象表新增 `gtc/quaternion.cj` 行（混合型：14 完整 + 3 stub——v3 精确统计为 14 完整 + 3 stub + 3 看向 stub = 8 + 4 + 3 = 15 个声明，其中 `quat_cast` 两个重载）；§2 包组织 `glm.gtc` 块下新增 `quaternion.cj ★` 条目；§8 产出物清单「完整实现」和「大部分实现」两段均补充 `gtc/quaternion.cj`；§10 覆盖矩阵新增「glm/gtc/quaternion.hpp（v3 独立矩阵）」表 |
| **问题 4（严重）**：§3.4 第 216 行 `Quat×Vec3` 旋转运算符实现公式错误——设计文档的 `v + 2.0 * cross(cross(QuatVector, v) + QuatVector * q.w, v)` 与 GLM `type_quat.inl:359-366` 实际公式 `v + (cross(QuatVector, v) * q.w + cross(QuatVector, cross(QuatVector, v))) * 2` 不等价 | **已采纳**。§3.4 `Quat×Vec3` 行公式修正为 `v + (cross(QuatVector, v) * q.w + cross(QuatVector, cross(QuatVector, v))) * 2`（两次 Vec3 叉乘 uv 和 uuv）；D26 设计决策新增；§9 差异声明新增「`Quat×Vec3` 旋转公式采用两次 Vec3 叉乘」条目 |
| **问题 5（严重）**：§3.2 协作关系表与策略段落对 `mat3_cast` 等函数归属矛盾——表行说「gtc/quaternion.cj（随 type_quat.cj 编码）」，策略段说「随 type_quat.cj 一同实现」 | **已采纳**。§1「整体架构思路」段落新增「gtc/quaternion.cj 设计决策」小节，明确「独立文件策略」+「`mat3_cast`/`mat4_cast`/`quat_cast` 不放在 detail，统一归入 gtc/quaternion.cj」+「跨包依赖单向性」；§3.2 协作关系表行 167-170 的「实现位置」列从「gtc/quaternion.cj（随 type_quat.cj 编码）」改为「gtc/quaternion.cj 包级函数」；§3.2 策略段落「本阶段对 gtc/quaternion 函数的处理策略」明确分组（4 转换 + 4 比较完整实现 / 4 欧拉 + 3 看向 stub）；D11 设计决策完整重写文件归属理由；§9 差异声明新增「`gtc/quaternion.cj` 独立承载 17 函数」和「`mat3_cast`/`mat4_cast`/`quat_cast` 位于 gtc/quaternion.cj」两个条目 |
| **问题 2（一般）**：§8 完整实现清单末尾提及「四元数测试文件」一行但未展开 | **已采纳**。§8 新增「8.2 测试设计」子章节，包含测试文件清单与位置（13 个测试文件，≥163 用例）、测试覆盖维度（5 类）、浮点比较策略、阶段三可验证 vs 待阶段四拆分；§8 编码启动前验证项保留并补充测试相关条目 |
| **问题 3（一般）**：§2 模块间依赖图缺少 `gtc/quaternion.cj` 的依赖方向 | **已采纳**。§2 模块间依赖图 `glm.gtc` 块下新增 `quaternion.cj → glm.detail(Quat, Mat3x3, Mat4x4, Vec3), glm.ext.scalar_constants, glm.ext.vector_relational, glm.detail.common(stub), glm.detail.trigonometric(stub), glm.detail.geometric(stub)` 依赖方向说明；§2 末尾 cjpm 子包构建预验证段同步引用 |
| **问题 6（一般）**：§3.2 策略段落对 `eulerAngles`/`roll`/`pitch`/`yaw` 分类逻辑矛盾——标为「实现」但实际依赖未就绪的 stub | **已采纳**。§3.2 策略段落明确分组：「完整实现」组（mat3_cast/mat4_cast/quat_cast + lessThan*）vs「stub 占位」组（eulerAngles/roll/pitch/yaw + quatLookAt*）；§10 覆盖矩阵 gtc/quaternion 表与 §3.15 gtc/quaternion.cj 小节的状态标注完全对齐（4 欧拉函数 + 3 看向函数均为 stub） |
| **问题 7（一般）**：§3.10 第 339 行 `pow` 依赖关系不准确——错误列 `asin`/`pow`、遗漏 `abs`/`clamp`/`cos` | **已采纳**。§3.10 `pow` 依赖关系修订为「依赖 `abs`/`clamp`/`acos`/`sin`/`cos`/`sqrt`（其中 abs/clamp 来自 common.cj stub，acos/sin/cos 来自 trigonometric.cj stub，sqrt 来自 std.math）+ `epsilon<T>()`」；D21 设计决策明确引用；§10 覆盖矩阵 quaternion_exponential 表同步更新 |
| **问题 8（一般）**：§3.11 第 354 行 `mix` 依赖关系遗漏 `epsilon<T>()`；`slerp` 描述同样遗漏 | **已采纳**。§3.11 `mix` 依赖修订为「依赖 `dot`、`acos`、`sin`、`epsilon<T>()`」；`slerp` 描述修订为「依赖 `dot`、`acos`/`sin`/`mix`（标量版）+ `epsilon<T>()`」；D21 设计决策明确引用 |
| **问题 10（一般）**：§3.11 第 355 行 `slerp(x, y, a, k)` 4 参数版本 `k` 类型未定 | **已采纳**。§3.11 `slerp` 4 参数版本明确签名 `slerp<T, Q>(x: Quat<T,Q>, y: Quat<T,Q>, a: T, k: Int64)`；D22 设计决策说明采用简化版（与 deviations.md DV-03 一致）；§9 差异声明新增对应条目；§10 覆盖矩阵 quaternion_common 表更新 |
| **问题 11（一般）**：§3.4 `Quat×Vec3`/`Quat×Vec4` 行「备注」列为空，未说明依赖 `geometric.cj stub` | **已采纳**。§3.4 `Quat×Vec3` 行备注列补充「实现：`v + (...) * 2`（两次 Vec3 叉乘 uv/uuv）。**依赖 `geometric.cj` 向量 `cross`（阶段三 stub），调用将抛 `Exception("stub")`**」；`Quat×Vec4` 行备注列补充「通过 Vec3 中间路径间接依赖 `Quat×Vec3`，阶段三调用同样抛 stub 异常」；§4.2 行为契约示例标注「本阶段调用抛 stub 异常，待阶段四 geometric.cj 完整实现后生效」；§11 函数可用性对照表新增对应行 |
| **问题 14（一般）**：§5 错误处理策略覆盖不足（5 类边界条件缺失） | **已采纳**。§5.3 新增「边界条件与异常场景」表格，列出 8 类边界条件的行为契约：① `axis` 零四元数返回 `Vec3(1,0,0)`；② 浮点 `inverse` 零四元数产生 Inf/NaN；③ 整数 `inverse` 触发除零异常；④ `mix`/`slerp` cosTheta 退化检测；⑤ `fromMat3`/`fromMat4` 非纯旋转矩阵行为未定义；⑥ `lerp` 断言失败；⑦ `equal` epsilon=0 返回 false；⑧ 整型 `epsilon<T>()` 抛运行时异常 |
| **问题 18（一般）**：§3.12 第 367-371 行 `epsilon<T>()`/`pi<T>()`/`cos_one_over_two<T>()` 对整数类型 T 的行为未定义 | **已采纳**。§3.12 「整数类型 T 的行为契约」段落新增，明确约束收紧策略（`T <: FloatingPointNumber<T>`）+ 运行时 fallback 异常分支；D25 设计决策明确行为；§5.3 边界条件表新增对应行；§8 编码启动前验证项新增验证项 6 |
| **问题 9（轻微）**：§3.11 第 352 行 `lerp` 实现策略遗漏 `assert(a >= 0 && a <= 1)` 断言，且未明确「非 const」约束 | **已采纳**。§3.11 `lerp` 描述修订为「`x * (1 - a) + y * a`，含 `assert(a >= 0 && a <= 1)` 断言与 GLM `ext/quaternion_common.inl:28-38` 一致」+「`const` 约束说明：lerp 不可声明为 `const func`，因函数体内 `assert` 不是 `const` 函数（与 deviations.md IF-03 一致）」；D23 设计决策明确；§5.4 const 上下文约束段落新增 |
| **问题 12（轻微）**：§3.5 第 252 行向量 `equal` 容差比较语义 `\|x-y\| <= epsilon` 与 GLM `<` 严格小于不一致 | **已采纳**。§3.5 epsilon 版本语义从「`\|x-y\| <= epsilon`」修订为「`\|x-y\| < epsilon`（严格小于，与 GLM 1.0.3 `func_vector_relational.inl:18-22` 一致）」；`notEqual` 对应改为「`\|x-y\| >= epsilon`」；D24 设计决策明确；§5.3 边界条件表新增「`epsilon = T(0)`」行；§9 差异声明新增「向量 `equal` 采用严格 `<` 语义」条目 |
| **问题 13（轻微）**：§3.4 与 §3.7 中 `cross` 命名歧义未充分处理 | **已采纳**。§3.4 `Quat×Vec3` 行后新增「命名约定说明」段：「上表中 `Quat×Vec3` 行公式中的 `cross(QuatVector, v)` 调用的是 **Vec3 叉乘**（定义于 `geometric.cj`，参数为 `Vec3<T,Q>`），**非 §3.7 中的四元数 `cross`（Hamilton 乘积）**」；§3.7 命名歧义说明段强化「下游消费者判别方法」三段说明（参数类型消歧 + §3.4 旋转公式内部调用明确） |
| **问题 15（轻微）**：§3.1 第 149 行 `@Derive[Hashable]` 在阶段一/二的实践依据未在本设计核验 | **已采纳**。§3.1 第 149 行后新增「`@Derive[Hashable]` 约束核验」段，明确 `T <: Number<T>` 和 `Q <: Qualifier` 的 Hashable 接口要求；引用阶段一 Vec3 实践（`type_vec3.cj` 验证）；§8 编码启动前验证项新增验证项 10「`@Derive[Hashable]` 对 `Q <: Qualifier` 的支持验证」 |
| **问题 16（轻微）**：§3.14 与 §8.5 对 fwd.cj 别名清单的细节未明确（`BQuat`/`IQuat` 是否包含、`Quat`/`FQuat` 双别名机制） | **已采纳**。§3.14 fwd.cj 别名表重写为完整 8 行（Quat/FQuat/DQuat/3×Float32 精度/3×Float64 精度）+「排除项」段（不含 BQuat/IQuat/I64Quat/BQuat 精度变体，理由与阶段二策略一致）；「`Quat` 与 `FQuat` 双别名机制」段新增说明（与阶段二 `Mat2x2`/`FMat2x2` 模式一致）；D27 设计决策明确 |
| **问题 17（轻微）**：§8 验证项与 §2 末尾的 cjpm 子包构建预验证策略未对齐（阶段二 ext/ 子包验证结果、阶段三 gtc/ 子包回退方案） | **已采纳**。§2 末尾 cjpm 子包构建预验证段重写为「阶段二已通过验证：[已通过 cjpm 验证]」+「阶段三新增 gtc/ 需原型验证」+ 三个验证项（gtc 子包 + 跨包引用 + 回退方案）；§8 编码启动前验证项 1 与之对齐 |
| **问题 19（轻微）**：部分接口边界契约对下游消费者的影响未充分说明（`identity` 调用提示、`fromQuat` 闭包示例、`Quat×Vec3` stub 标注、stub 函数对照表） | **已采纳**。§3.3 各工厂函数新增「调用示例」代码块（identity / fromQuat）；§3.3 fromMat3/fromMat4 新增「边界行为」契约声明（仅对纯旋转矩阵有效）；§3.11 inverse 新增「边界行为」（浮点 Inf/NaN vs 整数除零异常）；§11 新增「下游消费者迁移指南」独立章节（4 类迁移场景 + 函数可用性对照表，覆盖 16 个函数的本阶段/阶段四状态） |
| **附带改进**：跨包依赖单向性确认 | **已采纳**。§1「gtc/quaternion.cj 设计决策」段落与 §3.2 协作关系表均明确 type_quat.cj 通过 `import glm.gtc.quaternion.*` 跨包引用，构成 `glm.detail → glm.gtc` 的单向依赖；无循环依赖。§8 编码启动前验证项新增验证项 2 跨包引用验证 |
| **附带改进**：`length(q)` 在 Float32 实例的 sqrt 处理 | **已采纳**。§3.7 `length` 描述新增「Float32 实例的 sqrt 处理」说明（`std.math.sqrt` 仅支持 Float64 输入，需显式转换路径或额外 Float32 重载）；§8 编码启动前验证项新增验证项 11 |
