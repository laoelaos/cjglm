...
**审查重点**：本产出已通过前 7 轮内部审议（v2~v11 共 51+ 项审查意见 + 14 项 v7 审查意见）。本轮审查侧重通过独立源码核查，识别前序审议未充分验证的"事实准确性"维度质量问题。
...
**关键发现**：本轮独立源码核查发现 **6 项严重事实错误**，其中 4 项直接引用了不存在或与项目实际不符的源文件位置/行号/路径。这些错误在前序 7 轮内部审议（侧重设计完整性与 GLM 行为对齐）中均未被识别，因为前序审议未对设计文档中引用的"项目文件路径/行号/字段"进行独立的源码核查。本轮通过以下独立验证路径识别：
1. 实际查阅 `cjglm/src/lib.cj` / `cjglm/src/fwd.cj` / `cjglm/scripts/gen_fwd_aliases.py` 文件内容
2. 实际查阅 `references/glm-1.0.3/glm/glm/gtc/quaternion.hpp` / `gtc/matrix_transform.hpp` / `ext/matrix_transform.inl` / `ext/matrix_clip_space.inl` / `ext/matrix_projection.inl` / `detail/type_quat.inl` / `ext/quaternion_geometric.inl` 实际源码
3. 实际查阅 `.opencode/skills/cangjie-original-docs/std/math/math_package_api/math_package_interfaces.md` 与仓颈 stdlib 文档
...
### 问题 2（严重）：§3.13「`gtc/matrix_transform.cj` 函数清单」与 GLM 1.0.3 实际函数列表显著偏差 — 包含不存在的 `scaleAlongAxis`，遗漏多个实际函数
...
**`references/glm-1.0.3/glm/glm/ext/matrix_transform.inl`** 实际包含 11 个函数：
- `identity()`、`translate`、`rotate`、`rotate_slow`、`scale`、`scale_slow`、`shear`、`shear_slow`、`lookAtRH`、`lookAtLH`、`lookAt`

**`references/glm-1.0.3/glm/glm/ext/matrix_clip_space.inl`** 实际包含 46 行 `GLM_FUNC_QUALIFIER`（44 个唯一函数名）：
- ortho 系列：ortho, orthoLH, orthoLH_NO, orthoLH_ZO, orthoNO, orthoRH, orthoRH_NO, orthoRH_ZO, orthoZO（9 个）
- frustum 系列：frustum, frustumLH, frustumLH_NO, frustumLH_ZO, frustumNO, frustumRH, frustumRH_NO, frustumRH_ZO, frustumZO（9 个）
- perspective 系列：perspective, perspectiveFov, perspectiveFovLH, perspectiveFovLH_NO, perspectiveFovLH_ZO, perspectiveFovNO, perspectiveFovRH, perspectiveFovRH_NO, perspectiveFovRH_ZO, perspectiveFovZO, perspectiveLH, perspectiveLH_NO, perspectiveLH_ZO, perspectiveNO, perspectiveRH, perspectiveRH_NO, perspectiveRH_ZO, perspectiveZO（18 个）
- infinitePerspective 系列：infinitePerspective, infinitePerspectiveLH, infinitePerspectiveLH_NO, infinitePerspectiveLH_ZO, infinitePerspectiveRH, infinitePerspectiveRH_NO, infinitePerspectiveRH_ZO（7 个）
- tweakedInfinitePerspective 系列：tweakedInfinitePerspective 等 4 个变体
...
**`gtc/matrix_transform.hpp` 自身不声明任何函数**，仅 `#include "matrix_transform.inl"`（即 3 行内容），实际通过 include `ext/matrix_transform.hpp` + `ext/matrix_clip_space.hpp` + `ext/matrix_projection.hpp` 等文件暴露函数。
...
| 缩放 2：`scale`、`scaleAlongAxis` | `scale` 存在；**`scaleAlongAxis` 在 GLM 1.0.3 中不存在** | 严重错误（虚构函数） |
| 视口变换 5：`ortho` + 4 LH/RH ZO/NO 变体 | 9 个 ortho 变体 + 9 个 frustum 变体 = 18 个 | 严重遗漏（13 个未列） |
| 透视投影 10：`perspective` + 9 个变体 | 18 个 perspective/perspectiveFov 变体 | 严重遗漏（8 个未列） |
| 无穷远透视 10：`infinitePerspective` + 9 个变体 | 7 个 infinitePerspective 变体 + 4 个 tweakedInfinitePerspective 变体 = 11 个 | 函数计数偏差，名称部分缺失 |
| 拾取矩阵 1：`pickMatrix` | 存在 | 正确 |
| 看向与矩阵合成 4：`lookAt` + `lookAtRH` + `lookAtLH` + `matrixCompMult` | `lookAt`/`lookAtRH`/`lookAtLH` 已在基础变换；`matrixCompMult` 实际属于 `glm/detail/func_matrix.inl` 而非 matrix_transform 系列 | 归属错误 |

**总计**：GLM 1.0.3 实际通过 `gtc/matrix_transform.hpp` 暴露约 62 个函数（11 + 44 + 7 = 62），设计文档声称 35 个，**实际偏差 27 个函数 + 1 个虚构函数 `scaleAlongAxis`**。

**所在位置**：§3.13 表格下「`gtc/matrix_transform.cj` 函数清单（v12 新增，Issue 4 响应）」段（约第 568-579 行）

**严重程度**：严重（下游按设计清单 stub 占位 35 个函数后，阶段四仍需补齐 27 个未列入的函数 + 删除 1 个虚构的 `scaleAlongAxis`，影响阶段四实施计划）

**改进建议**：
1. §3.13 函数清单修订为基于实际 GLM 1.0.3 源码核查的 62 个函数完整列表，按类别（基础变换 11 / 视口与裁剪空间 18 / 透视投影 18 / 无穷远透视 11 / 投影工具 7 / 拾取矩阵 1）分组。
2. 删除虚构的 `scaleAlongAxis` 函数。
3. 明确指出「`matrixCompMult` 实际定义在 `glm/detail/func_matrix.inl`，不属于 `gtc/matrix_transform` 系列」，避免归属错误。
4. 同步修订 §10 覆盖矩阵 `gtc/matrix_transform.hpp` 表、§8.3 验收项 C 覆盖矩阵表 `gtc/matrix_transform.hpp` 行的「函数总数」从 35 修订为 62（或按设计决策明确仅 stub 部分核心函数，并说明阶段四如何处理未列入函数）。
...
`references/glm-1.0.3/glm/glm/detail/type_quat.inl` 第 196-217 行：
```cpp
196: GLM_FUNC_QUALIFIER qua<T, Q>::qua(vec<3, T, Q> const& u, vec<3, T, Q> const& v)
197: {
198:     T norm_u_norm_v = sqrt(dot(u, u) * dot(v, v));
199:     T real_part = norm_u_norm_v + dot(u, v);
200:     vec<3, T, Q> t;
201: 
202:     if(real_part < static_cast<T>(1.e-6f) * norm_u_norm_v)
203:     {
204:         // If u and v are exactly opposite, rotate 180 degrees
205:         // around an arbitrary orthogonal axis. ...
206:         real_part = static_cast<T>(0);
207:         t = abs(u.x) > abs(u.z) ? vec<3, T, Q>(-u.y, u.x, static_cast<T>(0)) : vec<3, T, Q>(static_cast<T>(0), -u.z, u.y);
208:     }
209:     else
210:     {
211:         // Otherwise, build quaternion the standard way.
212:         t = cross(u, v);
213:     }
214: 
215:     *this = normalize(qua<T, Q>::wxyz(real_part, t.x, t.y, t.z));
216: }
```
...
### 问题 6（严重）：§3.7 normalize 函数 v11 修订后边界行为契约与 GLM 实际行为虽然描述正确，但相关引用细节（如行号、调用模式）需进一步验证

**问题描述**：v11 修订（Issue 2 响应）在 §3.7 normalize 函数末尾新增「实现策略（v11 补强）」段，明确「实现公式与 GLM `ext/quaternion_geometric.inl:17-24` 完全一致——`tmp1 = length(q); if (tmp1 <= T(Float64(0))) { return identity_q } else { return q / tmp1 }`」。

**事实核查**：`references/glm-1.0.3/glm/glm/ext/quaternion_geometric.inl` 实际 normalize 实现为：
```cpp
...
- 行号引用 `ext/quaternion_geometric.inl:17-24`——实际 GLM 文件起始行为 `namespace glm {` + `template<typename T, qualifier Q>`，normalize 函数体大致在第 16-25 行附近，**行号大致正确**（具体行号需精确核对，但函数归属与文件名正确）。

**此问题严重程度判定**：经核查后，本问题描述的实际行为正确，行号大致准确。但 §3.7 normalize 函数末尾 v12 修订（Issue 6 响应）将 T(0)/T(1) 获取路径描述统一指向 §1 单一权威来源，逻辑闭环。但下游若需精确查找 GLM 源码，应确认行号。

**所在位置**：§3.7 normalize 函数末尾「T(0)/T(1) 获取路径（v12 关键修订，Issue 6 响应，引用 §1 作为单一权威来源）」段（约第 460 行附近）

**严重程度**：一般（描述与 GLM 实际行为一致，行号大致准确，不构成严重事实错误；本问题在本次审查中识别但实际并非严重问题，仅作为 v11 修订的核查验证）

**改进建议**：
1. 明确标注「GLM 实际返回的 `identity_q = qua<T,Q>::wxyz(1, 0, 0, 0)`（wxyz 顺序），与仓颉 `Quat(T(0), T(0), T(0), T(1))`（xyzw 顺序）等价」，帮助下游理解 GLM 与仓颉命名约定的对应关系。
2. 行号精确化：核实 GLM 1.0.3 `ext/quaternion_geometric.inl` 中 normalize 函数的精确行号（实际约第 16-25 行）。

---

## 三、一般问题（2 项）

### 问题 7（一般）：§3.7 normalize 函数边界行为契约 + §11.6 四命名空间可达性矩阵与 §3.13.2 审计节存在功能重叠

**问题描述**：
- §3.7 normalize 函数末尾「T(0)/T(1) 获取路径」段描述 normalize 在零四元数时的返回单位四元数契约；
...
### 问题 8（一般）：设计文档总行数 1887 行，§修订说明（v2~v12）章节占比 36%+，下游查阅效率严重受影响

**问题描述**：v12 修订后设计文档总行数达 1887 行，其中 §修订说明（v2）~§修订说明（v12）共 11 个章节约 680 行（占比 36%+）。设计实质内容（§1~§11）约 1200 行。质询报告 D 项曾建议拆分至独立附录文件，v11/v12 修订说明均标记「设计意图保留，未采纳」，但实际下游查阅 §1~§11 时需翻越大量修订历史，影响编码启动效率。

**所在位置**：§修订说明（v2）~§修订说明（v12），共 11 个章节

**严重程度**：一般（属于文档组织偏好，不直接影响设计正确性）

**改进建议**：
1. 采纳质询报告 D 项建议，将 §修订说明（v2）~§修订说明（v11）共 10 个章节拆分至独立文件 `cjglm/docs/05_ood_phase3_revision_history.md`，设计文档主体仅保留 §1~§11 + §修订说明（v12 当前轮）。
...
### 问题 9（轻微）：§修订说明（v12）问题 1 描述仍存在「snake_case 命名约定」原始问题措辞，与已采纳的方案 C（camelCase 命名）相距较远
...
### 问题 10（轻微）：§11.5 函数可用性对照表 gtc 重导出行 §3.13/§10 中 35 个 gtc/matrix_transform 函数未纳入对照表
...
**改进建议**：
1. §11.5 函数可用性对照表新增 gtc/matrix_transform 区块，列出 35（或实际 62）个函数均为 `❌ stub` 状态。
...
本轮（v8 迭代第 8 轮）新识别质量问题共 **10 项**（6 严重 + 2 一般 + 2 轻微）。
...
- **问题 2**：gtc/matrix_transform.cj 函数清单——偏差 27 个函数 + 1 个虚构函数
...
- **问题 6**：normalize 函数引用核查——经核查后实际正确

**整体设计质量评价**：经过 11 轮迭代（v2~v12）累计 65+ 项审查意见落实，v12 设计文档在设计完整性、GLM 行为对齐、约束一致性、命名约定、依赖闭环等维度已较为完善；本轮识别的 6 项严重问题均集中于「项目源码/文件引用的独立性事实核查」维度，建议下一轮迭代（v9）重点开展对设计文档所有外部引用（文件路径、行号、字段名、API 名称）的独立源码自动化核查，确保设计与项目实际状态完全对齐。

**建议优先级**：
1. **必须修复**（问题 1~5 严重）：直接影响下游编码落地。
2. **建议修复**（问题 7~8 一般）：提升设计文档的内部一致性与下游查阅效率。
3. **可选修复**（问题 9~10 轻微）：属于文档组织偏好，可在后续迭代中酌情处理。
