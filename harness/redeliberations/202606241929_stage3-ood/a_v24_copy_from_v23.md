...
> **修订日期**：2026-06-26
> **修订定位**：v24 设计的迭代修订（对应本轮审查 a_v24_iteration_requirement.md）。基于 v23 输出修订，针对审查报告识别的 8 项问题（2 严重 + 5 一般 + 1 轻微）逐项落实。本轮重点修复：(1) §5.4 `inverse` const 拒绝理由从「运行时抛算术异常」修正为「整数除法非 constexpr 兼容」；(2) §1 `T(Float64(n))` 语法对整型 T 的支持声明从「均编译可行」限定为浮点类型，整型降级为 P0 假设 H1 并增加回退路径；(3) §3.13.1 三角函数表依赖标注增加阶段四依赖说明和格式修订；(4) §3.13.2 审计结论 18 函数后添加「按行粒度展开为 21 行」附注；(5) §1 回退方案末尾增加验证时限承诺和受 H1/H2 影响函数清单标注；(6) §3.13 gtc/matrix_transform 表格下新增 6 大类代表性函数完整签名模板；(7) §5.3 边界条件表补充 NaN/Inf 传播等 5 处边界条件；(8) §11.5 权威声明从「单一权威来源」修订为「函数级权威来源，与 §9a/§10 互补」。
...
**验证时限承诺（v24 新增，Issue 5 响应）**：以下 P0 验证项应在编码启动后 **5 个工作日内**完成最小仓颉测试文件（~20 行/项）验证：
- **验证项 20（H2：`FloatingPoint<T>` 接口方法）**：最小测试文件实例化 `FloatingPoint<Float32>.getMinDenormal()`/`getInf()`/`isInf()`/`isNaN()`/`getNaN()`，确认编译通过
- **验证项 25（H1：`T(Float64(n))` 语法）**：最小测试文件实例化 `Float32(Float64(0))`/`Float64(Float64(0))`/`Int32(Float64(1))`/`Int64(Float64(0))`，确认编译通过
- **验证项 29（`glm.detail` 导入 `glm.ext` 验证）**：最小测试文件验证 `glm.detail` 包无 `import glm.ext` 声明，避免包间循环依赖

验证结果反馈至 §8 验证结果记录表模板。若验证失败，按回退方案决策树执行对应回退路径。
...
29. **`glm.detail` 包对 `glm.ext` 的导入可行性验证（v18 新增，Issue 1 响应）**：验证 `detail/type_quat.cj` 中 Vec3×Quat/Vec4×Quat 等运算符是否引入 `glm.detail → glm.ext` 跨包 import——(a) 在 `type_quat.cj` 中 grep 所有 `import glm.ext` 声明；(b) 若存在此类 import，则构成 `glm.detail → glm.ext → glm.detail` 包间循环依赖，违反仓颉 cjpm 构建系统约束。**修复指引**：对 Vec3×Quat/Vec4×Quat 采用内联 conjugate/dot 计算路径（见 §3.4 v18 修订）；对其他场景将跨包访问函数内联或移至 `glm.ext` 层。**受影响章节**：§2 模块间依赖图、§3.4 Vec extend 块、§8 编码启动前验证项合计
...
- **验证项 29（P0，v18 新增，Issue 1 响应）**：`glm.detail` 包内 `type_quat.cj` 对 `glm.ext` 的导入可行性验证——要求在编码阶段前先编写最小仓颉测试文件（~20 行），在 `type_quat.cj` 中 `import glm.ext.quaternion_geometric.*`，同时在 `quaternion_geometric.cj` 中 `import glm.detail.*`，确认 `cjpm build` 报循环依赖错误。随后移除 `type_quat.cj` 中的 `glm.ext` import，使用内联 conjugate/dot 计算路径替代 Vec3×Quat/Vec4×Quat 中原本的 `inverse(q)` 调用路径，确认 `cjpm build` 通过。验证结果反馈到 §2 模块间依赖图的修正。**受影响章节**：§2 模块间依赖图、§3.4 Vec extend 块实现路径、§8.3 验收项 E 验证项清单合计。
...
| **20（P0）** | **FloatingPoint<T> 接口方法可用性** | | | | §1 回退方案 H2 全部路径（类型分派/字面量 fallback） | 编码者 |
...
| **25（P0）** | **T(Float64(n)) 语法编译可行性** | | | | §1 回退方案 H1 全部路径（追加 `one: T` 形参/`match` 分派/收紧约束） | 设计者 |
...
| **29（P0）** | **glm.detail 导入 glm.ext 验证** | | | | §3.4 确认内联 conjugate/dot 路径实施；如编译失败则退回原 inverse 路径 | 设计者 |
...
**C. 覆盖矩阵验收（来源：§10 GLM 1.0.3 API 阶段覆盖矩阵）。v15 修订，Issue 2 响应：「本阶段实现」列已统一使用三档分类**：
...
`§11.5 函数可用性对照表` 是阶段三验证的**函数级权威来源**（v23 修订，Issue 2 响应；v24 修订，Issue 8 响应：从「单一权威来源」修订为「函数级权威来源」——与 §9a 基准声明和 §10 覆盖矩阵构成互补关系）：✅/⚠️/❌ 三档符号标记每个函数本阶段状态（实现在本文件其他章节有详细描述）。下游验证时按 §11.5 表逐项核验函数级状态，按 §10 表查阅文件级覆盖视图，按 §9a 表核对 GLM 1.0.3 基准。
...
新增以下文档内一致性自动核验项，防止后续版本再次出现 §10 合计行计数与逐行累加值偏差的问题：
- **核验项 H1（v23 修订，Issue 7 响应：关联自动化工具）**：§10 覆盖矩阵合计行 ❌/⚠️ 计数 = 逐行累加各文件 ❌/⚠️ 计数之和。验证方式：建议使用 Spreadsheet 模板（`docs/templates/coverage_matrix_checklist.xlsx`）或 Python 脚本（`scripts/check_coverage_matrix.py`）自动化完成累加验证——逐行提取 ✅/⚠️/❌ 数值，逐行累加后与合计行声明值对照，偏差为 0。如自动化工具尚未就绪，人工核验模板见 §8 验证结果记录表模板。
...
- **§9a 与 §10 互补关系声明（v24 修订，Issue 8 响应）**：§9a「GLM 1.0.3 Stage 3 API 基准声明」与 §10「GLM 1.0.3 API 阶段覆盖矩阵」定位为互补关系而非冗余——§9a 提供 GLM 1.0.3 基准函数总数和推迟理由清单（按头文件维度聚合），§10 提供按文件分组的覆盖状态视图和设计章节映射，§11.5 提供逐函数精细追踪。三者形成「基准→文件→函数」多层次覆盖体系，不再合并。§10 的合计行计数以 §9a 为基准，两表应保持一致。

**H 节核验执行者与周期（v23 新增，Issue 7 响应）**：上述 H1/H2/H3 三项核验由**设计者在每轮版本输出前**执行，核验结果记录于版本发布检查清单中。核验失败时，阻止版本发布直至全部核验项通过。

**I. 已知未解决问题列表（v23 新增，Issue 1 响应）**

本节记录当前轮次（v23）已知但尚未解决的问题，作为 §8.3 验收项的补充。与历史修订说明中已闭环的问题不同，以下问题的修复计划在后续轮次中落实。

| 编号 | 问题描述 | 首次检出轮次 | 当前状态 | 计划修复轮次 |
|------|---------|------------|---------|------------|
| O-01 | §11.5 对齐 §9a 基准的逐函数覆盖率统计尚未完成自动化 | v23 | 待评估 | v24 |
| O-02 | §9a 与 §10 两张覆盖矩阵在 v24 中已明确为互补关系（§11.5 定位声明已修订），不再需要合并——§9a 提供聚合基准计数，§10 提供按文件分组覆盖视图，§11.5 提供逐函数精细追踪；三者形成多层次覆盖体系 | v23 | ✅ 已闭环（v24 修订） | — |
| O-03 | δ 覆盖矩阵合计的 H1 核验尚未绑定自动化脚本 | v23 | 待评估 | v24 |
| O-04 | 验证项修复责任人列已补齐，但编码启动前验证流程尚未与 CI/CD 管线集成 | v23 | 待评估 | v24 |
...
## 9a. GLM 1.0.3 Stage 3 API 基准声明（v21 新增）
...
**v15 修订，Issue 2 响应**：本矩阵「覆盖状态」列统一使用与 §11.5 对齐的三档分类：✅ 可用（无 stub 依赖）、⚠️ 抛 stub 异常、❌ stub。函数可用状态以 §11.5 为最终基准。
...
| Vec3×Quat/Vec4×Quat | ✅ 可用（**v18 修订：内联 conjugate/dot 计算路径**——纯算术运算，无 stub 依赖，无 `glm.ext` 跨包 import） | §3.4 |
...
**v23 修订，Issue 2 响应；v24 修订，Issue 8 响应**：本表目标为**函数级状态追踪的权威来源**（每函数一行，标注本阶段/阶段四状态）。§3.13.2 审计节、§8 产出物清单、§10 覆盖矩阵、§11.6 可达性矩阵的函数状态描述均以本表为主要基准。v22 已补齐全部 5 个缺失 API 条目（`wxyz`/`fromQuat`/`[]`/`static func length`/复合赋值运算符），当前版本已具备完整性要求。§9a「GLM 1.0.3 基准函数总数」和 §10「按头文件分组的覆盖状态视图」提供聚合统计信息，与本表构成互补关系——本表聚焦逐函数精细追踪，§9a/§10 提供聚合统计视图与 GLM 1.0.3 基准声明。
...
<!-- 修订说明(v2)~(v11) 已剥离至独立文件 a_v11_revision_history_v2_v14.md；修订说明(v12)及以上保留在正文中 -->
