...
> **修订日期**：2026-06-26
> **修订定位**：v27 设计的迭代修订（对应本轮审查 a_v28_iteration_requirement.md）。基于 v27 输出修订，针对审查报告识别的 8 项问题（2 严重 + 4 一般 + 2 轻微）逐项落实。本轮重点修复：(1) §1 新增核心用户用例（旋转/插值）前置风险声明；(2) §3.16 D35-D37 量化偏差累积影响、补充下游风险评估与临时降级路径；(3) 验证项 29（P0）完成验证并补充验证记录；(4) gtc/matrix_transform.cj 64 个函数签名模板全面展开为完整签名列表；(5) §8.2 新增"编译期函数签名正确性验证"覆盖维度；(6) 彻底清理正文中残留的非决策性版本标注约 50+ 处；(7) 文档头部与实际角色统一、消除"copy_from"误导性命名描述；(8) O-06 补充可执行落地计划（责任人/里程碑/产出物）。
...
读者如需追溯特定版本的完整变更历史，请查阅独立历史文件 `a_v27_revision_history.md`（本版本修订说明见文件末尾「修订说明（v27）」。当前版本 v28 的修订说明见文件末尾「修订说明（v28）」章节）。---
...
下游编码者按此模式将表中 15 个函数名（14 单参数 + 1 双参数）逐一展开为标量 + Vec1~Vec4 共 5 个签名 = 75 个函数签名。全部 75 个签名的约束统一为 `where T <: FloatingPoint<T>` 或 `where T <: FloatingPoint<T>, Q <: Qualifier`，本阶段函数体统一为 `{ throw Exception("stub") }`。
...
**临时降级路径一览表（v28 新增，Problem 2 响应）**：

| 所需功能 | 受影响函数 | 本阶段降级方案 | 行为差异 | 实施成本 |
|---------|-----------|--------------|---------|---------|
| 球面插值 | `slerp` | 改用 `lerp` 线性插值 | 大角度差（>45°）时角速度不均匀，可能导致视觉上的"弹跳"感 | 0 人日（`lerp` 已 ✅ 可用） |
| 摄像机看向 | `quatLookAt*`（3 个） | 自行构造旋转矩阵 + `fromMat3` 转换 | 需手动实现 gaze→up→right 正交化步骤，约 10 行代码 | 1-2 人日（含测试） |
| 四元数混合 | `mix` | 改用 `lerp` 或自行实现 `nlerp`（归一化 lerp） | `nlerp` 近似 slerp 精度，代价为短弧插值时需额外归一化 | 0.5-1 人日 |
...
| **合计行（与 §9a 交叉核验）** | **✅ ~18 个（独立函数）** + **⚠️ 1 个** + **❌ ~87 个** = **合计 88 个 ⚠️/❌** | — | — |
