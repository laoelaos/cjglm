...
> **修订日期**：2026-06-25
> **修订定位**：v16 设计的迭代修订（对应本轮审查 a_v16_iteration_requirement.md）。基于 v15 输出修订，针对审查报告识别的 4 项问题（1 严重 + 2 一般 + 1 轻微）逐项落实。本轮重点修复：(1) 全文档版本号核验——全文替换「v16 修订」为「v15 修订」（v15 正文中残留的 12 处错误版本号标记全部修正），建立版本发布前 `grep -n "v[0-9]"` 核验机制；(2) 函数可用计数统一——§8 产出物清单「真完整实现函数」计数从「20」（含 quatCast 重载拆分）统一为「18」（按函数名计），与 §3.13.2 审计结论口径一致；§8.3 验收项 A 表同步修正；(3) §3.16 路线图三项不一致补充临时实施建议与工期影响评估，并升级为正式设计决策 D35~D37；(4) §8 编码启动前验证项 20/25 补充对 §1 回退方案子节的引用，确保开发者可立即定位到具体回退路径代号。
...
| **合计** | — | **≥194**（v15 修订，Issue 5 响应：与下方用例分配表对齐，`test_ext_quaternion_common.cj` 从 ≥16 修正为 ≥13，`test_ext_quaternion_trigonometric.cj` 从 ≥8 修正为 ≥4；按上述每行 ≥值求和：40+8+8+12+13+4+4+2+16+13+9+28+27+8=194；下方用例分配表逐项求和同样为 194，通过增补 5 个边界用例可达 ≥199） |
...
| | **全表总计** | — | — | ≥194 | 逐项相加：42+8+8+12+13+4+4+2+16+13+9+28+27+8 = 194（下限）；通过增补三角函数/geometric 边界用例 5 个可轻松达到 ≥199 |
...
|------|----------|--------|
| `identity` | ✅ 可用 | ✅ 可用 |
| `fromMat3`/`fromMat4` | ✅ 可用（**约束：`where T <: FloatingPoint<T>`（签名显式声明，v15 修订，Issue 5 响应），仅浮点 T；整型 T 实例化时编译失败；编译错误直接定位在 `fromMat3`/`fromMat4` 签名约束处而非内部 callee**） | ✅ 可用（约束同上） |
...
> **修订定位**：v14 设计的迭代修订（v13，对应本轮审查 a_v13_iteration_requirement.md）。依据审查报告识别 6 项问题（6 一般），逐项落实到对应章节，并追加修订说明（v13）章节。本轮 6 项问题在迭代 12 的审查中均已识别但 v12 未有效修复，本轮逐一落实。
...
> **修订定位**：v11 设计的迭代修订（v12，对应本轮审查 a_v12_iteration_requirement.md）。依据审查报告识别 8 项问题（3 严重 + 4 一般 + 1 轻微），逐项落实到对应章节。
