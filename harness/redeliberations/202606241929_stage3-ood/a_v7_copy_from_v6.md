...
> **修订日期**：2026-06-25
> **修订定位**：v10 设计的迭代修订（v11）。依据本轮审查报告（a_v7_iteration_requirement.md 对应 v6 诊断报告 b_v6_diag_v1.md）从需求响应充分度、整体深度与完整性、设计可落地性维度识别 13 项质量问题（2 严重 + 6 一般 + 5 轻微），逐项落实到对应章节，并补充修订说明（v11）章节。本轮 2 项严重问题均属「事实错误/直接阻塞编码」类：① §3.13.1 trigonometric.cj 15 个函数签名模板未声明 `where T <: FloatingPoint<T>` 约束，与 §3.2.1 D32 等已统一策略不一致；② §3.7 normalize 描述未涵盖 GLM `quaternion_geometric.inl:17-24` 的零四元数保护分支（`if (len <= 0) return identity`），下游按字面实现时零四元数产生 NaN 与 §5.1 契约不符。
...
| 二元 `*` (Quat×Vec4) | 四元数旋转向量（保留 w） | `Number<T>` | 实现：`Vec4(q * Vec3(v), v.w)`（其中 `q * Vec3(v)` 是上一行 `Quat×Vec3` 运算符的复用，保留 `v.w` 作为输出 Vec4 的 w 分量；**v11 关键修订，Issue 3 响应，明确 `Vec4(q * Vec3(v), v.w)` 公式含义**：第一参数 `q * Vec3(v)` 是 `Quat×Vec3` 运算符返回的 Vec3 结果，第二参数 `v.w` 是输入 Vec4 的 w 分量原样传递，构造输出 Vec4）。**通过 Vec3 中间路径间接依赖 `Quat×Vec3`，阶段三调用同样抛 stub 异常** |
...
- `normalize(q: Quat<T,Q>): Quat<T,Q>` — 归一化四元数。**v11 关键修订，Issue 2 响应**（补充 GLM 零四元数保护行为）：实现公式与 GLM `ext/quaternion_geometric.inl:17-24` 完全一致——`tmp1 = length(q); if (tmp1 <= T(Float64(0))) { return identity_q } else { return q / tmp1 }`，其中 `tmp1 <= T(0)` 时返回**单位四元数**（`Quat(T(0), T(0), T(0), T(1))`，T(0) 通过 `one - one` 演算，T(1) 通过 `T(Float64(1))` 字面量替代）。**设计意图**：与 §5.1「normalize 零四元数返回单位四元数」契约对齐，避免下游按字面实现时零四元数除零产生 NaN 分量。**length 极小值边界行为（v11 新增）**：`tmp1` 为极小正数（如浮点最小次正规数）时不触发零保护分支，返回 `q / tmp1` 结果各分量趋向 Inf/NaN（与 GLM 行为一致，GLM 不做 length 极小值保护）；如需防止此类溢出，应在调用 `normalize` 前自行检查 `length(q) >= epsilon<T>()`。**实现策略（v11 补强）**：`where T <: Number<T>` 约束下调用 `length(q)`，根据 §3.7 `length` 函数约束收紧策略（与 `length` 函数本身的 `T <: Number<T>` 约束保持一致，不引入额外 `T <: FloatingPoint<T>` 约束，使整数四元数 `normalize` 仍可调用——返回 `q / tmp1` 整除结果）。
...
| 函数 | 标量签名（v11 关键修订，Issue 1 响应，所有签名统一标注 `where T <: FloatingPoint<T>`） | 向量签名（占位符，v11 修订列标题：明确按 Vec1~Vec4 展开为 4 个独立函数） | 内部依赖（v11 修订：删除冗余的「T 必须为 FloatingPoint<T>」标注，统一在签名 where 子句约束） |
|------|---------|---------|---------|
| `sin` | `sin<T>(x: T): T where T <: FloatingPoint<T>` | `sin<T, Q>(x: VecN<T, Q>): VecN<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier` | `std.math.sin`（v11 修订：v10 描述「受 §1 约束管辖」错误——`std.math.sin` 自身已提供 Float16/Float32/Float64 重载，T=Float32 实例化时直接调用即可，无需 `T(Float64.xxx(Float64(value)))` 转换） |
| `cos` | `cos<T>(x: T): T where T <: FloatingPoint<T>` | `cos<T, Q>(x: VecN<T, Q>): VecN<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier` | `std.math.cos`（v11 修订同上：Float32 重载已提供） |
| `tan` | `tan<T>(x: T): T where T <: FloatingPoint<T>` | `tan<T, Q>(x: VecN<T, Q>): VecN<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier` | `std.math.tan`（v11 修订同上） |
| `asin` | `asin<T>(x: T): T where T <: FloatingPoint<T>` | `asin<T, Q>(x: VecN<T, Q>): VecN<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier` | `std.math.asin`（v11 修订同上） |
| `acos` | `acos<T>(x: T): T where T <: FloatingPoint<T>` | `acos<T, Q>(x: VecN<T, Q>): VecN<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier` | `std.math.acos`（v11 修订同上） |
| `atan` | `atan<T>(x: T): T where T <: FloatingPoint<T>` | `atan<T, Q>(x: VecN<T, Q>): VecN<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier` | `std.math.atan`（v11 修订同上） |
| `sinh` | `sinh<T>(x: T): T where T <: FloatingPoint<T>` | `sinh<T, Q>(x: VecN<T, Q>): VecN<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier` | `std.math.sinh`（v11 修订同上） |
| `cosh` | `cosh<T>(x: T): T where T <: FloatingPoint<T>` | `cosh<T, Q>(x: VecN<T, Q>): VecN<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier` | `std.math.cosh`（v11 修订同上） |
| `tanh` | `tanh<T>(x: T): T where T <: FloatingPoint<T>` | `tanh<T, Q>(x: VecN<T, Q>): VecN<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier` | `std.math.tanh`（v11 修订同上） |
| `asinh` | `asinh<T>(x: T): T where T <: FloatingPoint<T>` | `asinh<T, Q>(x: VecN<T, Q>): VecN<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier` | `std.math.asinh`（v11 修订同上） |
| `acosh` | `acosh<T>(x: T): T where T <: FloatingPoint<T>` | `acosh<T, Q>(x: VecN<T, Q>): VecN<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier` | `std.math.acosh`（v11 修订同上） |
| `atanh` | `atanh<T>(x: T): T where T <: FloatingPoint<T>` | `atanh<T, Q>(x: VecN<T, Q>): VecN<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier` | `std.math.atanh`（v11 修订同上） |
| `radians` | `radians<T>(x: T): T where T <: FloatingPoint<T>` | `radians<T, Q>(x: VecN<T, Q>): VecN<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier` | **v11 关键修订，Issue 10 响应**：无 std.math 依赖（stdlib 不存在 `radians`），硬编码 π 字面量 `Float64(3.141592653589793)`。完整实现公式 `radians(x) = x * Float64(3.141592653589793) / Float64(180.0)`；T 实例化为 Float32 时需 `T(Float64(3.141592653589793)) / T(Float64(180.0))` 转换；T 实例化为 Float64 时直接使用 `x * Float64(3.141592653589793) / Float64(180.0)` |
| `degrees` | `degrees<T>(x: T): T where T <: FloatingPoint<T>` | `degrees<T, Q>(x: VecN<T, Q>): VecN<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier` | **v11 关键修订，Issue 10 响应**：无 std.math 依赖（stdlib 不存在 `degrees`），硬编码 π 字面量 `Float64(3.141592653589793)`。完整实现公式 `degrees(x) = x * Float64(180.0) / Float64(3.141592653589793)`；T 实例化为 Float32 时需 `T(Float64(180.0)) / T(Float64(3.141592653589793))` 转换；T 实例化为 Float64 时直接使用 `x * Float64(180.0) / Float64(3.141592653589793)` |

**双参数三角函数（v8 新增 `atan2` 双参数标量版本）**：

| 函数 | 标量签名（v11 关键修订，Issue 1 响应） | 向量签名（占位符，v11 修订列标题） | 内部依赖（v11 修订） |
|------|---------|---------|---------|
| `atan2` | `atan2<T>(y: T, x: T): T where T <: FloatingPoint<T>`（v8 明确要求；GLM `std::atan2(T, T)` 对应物） | `atan2<T, Q>(y: VecN<T, Q>, x: VecN<T, Q>): VecN<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier` | `std.math.atan2`（v11 修订：v10 描述「受 §1 约束管辖」错误——`std.math.atan2` 自身已提供 Float16/Float32/Float64 重载，T=Float32 实例化时直接调用即可） |
...
**Float32 实例化影响（v11 修订，Issue 7 关联）**：基于 `cangjie-original-docs/std/math/math_package_api/math_package_funcs.md` 全文检索（v11 关键修订）：**所有 `std.math.*` 三角函数（`sin`/`cos`/`tan`/`asin`/`acos`/`atan`/`sinh`/`cosh`/`tanh`/`asinh`/`acosh`/`atanh`/`atan2`/`sqrt`/`exp`/`log`）均提供 Float16/Float32/Float64 三种重载**，`std.math.pow` 提供 Float32/Float64 重载（4 个重载）；T=Float32 实例化时**直接调用** std.math 函数即可（与 §1 v11 修订策略方案 A 一致），无需 `T(Float64.xxx(Float64(value)))` 显式转换。**例外**：`radians`/`degrees` 是 std.math 不存在的函数（仓颉 stdlib 仅 `func getPI()` 接口静态方法提供 π 常量，无弧度/角度转换函数），实现阶段四 trigonometric.cj 时需自行硬编码 π 字面量；T=Float32 实例化时 `radians`/`degrees` 函数体内需应用 `T(Float64(...))` 转换。
...
**完整实现函数（4 个）**：
- `lessThan(x, y): Vec4<Bool,Q>` — 逐分量 `<` 比较，**约束：`where T <: Comparable<T>`（v10 新增，Issue 16 响应，依赖 `<` 运算符）**。**v11 关键修订，Issue 9 响应，补充实现公式模板**：`func lessThan<T, Q>(x: Quat<T,Q>, y: Quat<T,Q>): Vec4<Bool,Q> where T <: Comparable<T>, Q <: Qualifier { Vec4<Bool,Q>(x.x < y.x, x.y < y.y, x.z < y.z, x.w < y.w) }`
- `lessThanEqual(x, y): Vec4<Bool,Q>` — 逐分量 `<=` 比较，**约束：`where T <: Comparable<T>`（v10 新增，Issue 16 响应，依赖 `<=` 运算符）**。**v11 修订，Issue 9 响应，补充实现公式模板**：`func lessThanEqual<T, Q>(x: Quat<T,Q>, y: Quat<T,Q>): Vec4<Bool,Q> where T <: Comparable<T>, Q <: Qualifier { Vec4<Bool,Q>(x.x <= y.x, x.y <= y.y, x.z <= y.z, x.w <= y.w) }`
- `greaterThan(x, y): Vec4<Bool,Q>` — 逐分量 `>` 比较，**约束：`where T <: Comparable<T>`（v10 新增，Issue 16 响应，依赖 `>` 运算符）**。**v11 修订，Issue 9 响应，补充实现公式模板**：`func greaterThan<T, Q>(x: Quat<T,Q>, y: Quat<T,Q>): Vec4<Bool,Q> where T <: Comparable<T>, Q <: Qualifier { Vec4<Bool,Q>(x.x > y.x, x.y > y.y, x.z > y.z, x.w > y.w) }`
- `greaterThanEqual(x, y): Vec4<Bool,Q>` — 逐分量 `>=` 比较，**约束：`where T <: Comparable<T>`（v10 新增，Issue 16 响应，依赖 `>=` 运算符）**。**v11 修订，Issue 9 响应，补充实现公式模板**：`func greaterThanEqual<T, Q>(x: Quat<T,Q>, y: Quat<T,Q>): Vec4<Bool,Q> where T <: Comparable<T>, Q <: Qualifier { Vec4<Bool,Q>(x.x >= y.x, x.y >= y.y, x.z >= y.z, x.w >= y.w) }`
...
| `gtc/matrix_transform.hpp` | 全部 | 0 | 全部（空桩占位） | 完整实现（阶段四） |
