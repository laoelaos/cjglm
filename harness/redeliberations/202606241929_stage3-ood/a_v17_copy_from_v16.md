...
> **修订日期**：2026-06-25
> **修订定位**：v17 设计的迭代修订（对应本轮审查 a_v17_iteration_requirement.md）。基于 v16 输出修订，针对审查报告识别的 6 项问题（2 严重 + 3 一般 + 1 轻微）逐项落实。本轮重点修复：(1) §11.5 函数可用性对照表——补充遗漏的 4 个「真完整实现」函数（`axis`/`cross(Quat)`/`equal(Quat)`/`notEqual(Quat)`），同时将分组行统一拆分为逐函数行以消除粒度不一致；(2) §8.2 测试用例计数——文件清单合计「≥194」修正为「≥192」（实际逐项加和为 192），分配表 `test_type_quat.cj` 列值 42 修正为 40 以与文件清单对齐，合计行 194→192、"增补 5 个"→"增补 7 个"；(3) 版本号体系——§修订说明(v13) 首行「v14 设计的迭代修订」修正为「v13 设计的迭代修订」，§修订说明(v12) 首行「v11 设计的迭代修订」修正为「v12 设计的迭代修订」；(4) 修订说明剥离注释声明范围从 (v2)~(v14) 修正为 (v2)~(v11)，与正文保留状态一致；(5) §8.2 新增同类 stub 函数抽样策略说明，明确按类别分组抽样条件与覆盖依据。
...
| 测试用例总数 | ≥199 | 40+8+8+12+13+4+4+2+16+13+9+28+27+8 = 192（下限）；通过增补 7 个边界用例可达 ≥199 |
