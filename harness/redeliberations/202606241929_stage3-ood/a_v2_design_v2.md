...
**`@Derive[Hashable]` 约束核验（v3 新增）**：仓颉 `@Derive[Hashable]` 要求泛型 type parameter 满足 `Hashable` 接口。`Quat<T, Q>` 中 `T <: Number<T>` 约束下所有数字类型均实现 `Hashable`；`Q <: Qualifier` 约束下 6 个 Qualifier 实现类型均无数据成员（标记类型），其 `Hashable` 派生由编译器自动支持。需在编码启动前验证项中确认 `@Derive[Hashable]` 对 `Q <: Qualifier` 的派生编译通过（[阶段一 Vec3 已实践使用 @Derive[Hashable]`，参见 `type_vec3.cj:6` 验证]）。
