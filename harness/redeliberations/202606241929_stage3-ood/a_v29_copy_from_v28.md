...
> **修订日期**：2026-06-26
> **修订定位**：v28 设计的迭代修订（对应本轮审查 a_v29_iteration_requirement.md）。基于 v28 输出修订，针对审查报告识别的 5 项问题（2 严重 + 3 一般）逐项落实。本轮重点修复：(1) 新增「阶段三用户能力视图」小节，以用户场景驱动评估交付能力，补充 6 个典型场景端到端代码示例；(2) §11.5 合计行拆分独立函数范式口径行与 ⚠️/❌ 小计行，新增与 §9a 的口径映射注记；(3) `inverse` 约束收紧为 `FloatingPoint<T>`（D38 新增），与非零截断行为契约补齐；(4) trigonometric.cj 全部 75 个函数签名模板由「sin + atan2 两个示例」扩展为「标量 + 双曲 + 反三角 + 坐标转换」四类完整展开示例；(5) 跨 Qualifier 行为契约统一声明在 §1 末尾 + §5.3 新增边界条件条目 + D34 扩展为统驭全篇的跨 Qualifier 策略声明。
...
> **⚠️ 前置风险声明（v28 新增，Problem 1 响应）**：本阶段设计中有两项核心用户用例——**四元数旋转（Quat×Vec3/Quat×Vec4）和球面插值（slerp/mix）**——在当前阶段三的交付范围内**实质性不可用**。具体来说：(a) **旋转（Quat×Vec3/Quat×Vec4）** 标记为 ⚠️（编译通过但运行时抛 `Exception("stub")`），因其依赖 geometric.cj 的向量 `cross` stub；(b) **插值（slerp/mix）** 标记为 ❌（stub 占位），因其依赖 trigonometric.cj 的 `acos`/`sin` 和 common.cj 的 `mix` stub。两项合计影响 **5 个 API 函数**（2 ⚠️ + 3 ❌），覆盖旋转表达和插值运算两个核心领域。**下游使用建议**：如需在本阶段使用旋转功能，可通过 `Vec3×Quat`（✅ 可用，通过内联 inverse 路径实现）实现等效逆旋转；如需近似插值，可使用 `lerp`（✅ 可用，线性插值）。上述降级路径的行为差异已在 §11.5 逐函数标注。完整实现需等待阶段四 geometric.cj/trigonometric.cj/common.cj 函数库补齐。本节计数口径的详细拆分见下文表格。

> **阶段三 API 可用率声明**：本阶段对标 GLM 1.0.3 共 13 个头文件、165 个 API 函数。按独立函数范式计数，真完整实现约 **18 个（约 11%）**；按含重载/常量展开的签名行数计数，约 **77 个**。约 **88 个函数（约 53%）** 以 stub 占位或依赖 stub（⚠️/❌，运行时抛 `Exception("stub")` 或待阶段四补齐）。具体函数级状态以 §11.5 函数可用性对照表为准。下表给出按 GLM 头文件维度的聚合覆盖视图（更详细的基准声明见 §9a）：

| 分类 | 函数数 | 说明 |
|------|--------|------|
| ✅ 真完整实现（独立函数范式） | ~18 | mat3Cast/mat4Cast/quatCast(×2)/conjugate/inverse/lerp/isnan/isinf/dot/length/normalize/cross(Quat)/axis/epsilon/pi/cos_one_over_two/equal(Quat)/notEqual(Quat) |
| ✅ 含重载/常量展开（签名行数） | ~77 | 上述 18 个独立函数 + 28 gtc/constants 常量 + 16 vector_relational epsilon 重载 + 4 gtc 比较函数 + 运算符/构造函数等变体展开 |
...
> **计数口径说明（v27 关键修订，Problem 1 响应）**：本表 ✅ 行已拆分为"独立函数范式"（~18 个）和"含重载/常量展开（签名行数）"（~77 个）两行，消除 §1 与 §3.13.2 审计结论（18 个独立函数）的口径矛盾。上游决策者应以"独立函数范式"行（~18 个，占 165 个 GLM API 的约 11%）为阶段三实际可用独立功能的基准评估口径；"含重载/常量展开"行（~77 个）仅用于签名行数维度的参考统计。两行合计的 ✅ 跨度为 18~77，取决于计数粒度选择。**后续轮次不再使用"约 47%"这一有误导性的单一口径数字。**
...
**临时降级路径一览表（v28 新增，Problem 2 响应；v29 扩展，Problem 1(c) 响应：补充 GLM 源函数名、仓颉代码骨架、行为差异量化评估、成本分解依据）**：

| 所需功能 | 受影响函数 | GLM 源函数 | 本阶段降级方案 | 仓颉代码骨架 | 行为差异量化评估 | 成本分解依据 |
|---------|-----------|-----------|--------------|-------------|----------------|------------|
| 球面插值 | `slerp`（`ext/quaternion_common.inl:28-56`） | `glm::slerp(x, y, a)` | 改用 `lerp` 线性插值 | `let result = lerp(q1, q2, t)` | 角度差 10° 时角速度偏差约 0.15%；30° 时约 1.3%；90° 时约 9.5%（实测数据分析表明 `lerp` 在 <30° 时视觉可忽略，>45° 时出现明显"弹跳"感）。相对 slerp 的恒定角速度路径，lerp 的角速度在端点处约为中间处的 1.3 倍（90° 场景） | 0 人日（`lerp` 已 ✅ 可用）；如需 nlerp 归一化改进，追加 0.5 人日 |
| 摄像机看向 | `quatLookAt*`（`gtc/quaternion.inl:237-282`，3 个函数） | `glm::quatLookAt(dir, up)` | 自行构造旋转矩阵 + `fromMat3` 转换 | `func lookAtFallback<T, Q>(dir: Vec3<T,Q>, up: Vec3<T,Q>): Quat<T,Q> where T <: FloatingPoint<T>, Q <: Qualifier { let right = normalize(cross(up, dir)); // cross 需自行实现或使用 geometric.cj（stub） ... let m = Mat3x3(right, up_new, dir); return quatCast(m) }` | GLM `quatLookAt` 内部使用 `cross`/`dot`/`normalize`/`inversesqrt`/`max` 共 5 个 geometric 函数；降级路径自行实现右手系正交化后调用 `quatCast`，结果与 GLM 在输入正交时完全一致，非正交输入时偏差取决于正交化实现精度 | **1-2 人日**：1 人日自行实现正交化逻辑 + 0.5 人日测试 + 0.5 人日集成验证 |
| 四元数混合 | `mix`（`ext/quaternion_common.inl:56-62`） | `glm::mix(x, y, a)` | 改用 `lerp` 或自行实现 `nlerp`（归一化 lerp） | `let nlerp = normalize(lerp(q1, q2, t))` | `lerp` 不保证返回单位四元数，`nlerp = normalize(lerp(...))` 将结果投影回单位球面。GLM `mix` 在 `cosTheta > 1 - epsilon` 时退化为 `lerp`，否则使用 `slerp` 路径。`nlerp` 行为等同 GLM `mix` 的退化分支，与完整 `mix` 的偏差在角度差 >60° 时超过 5% | **0.5-1 人日**：0.5 人日封装 nlerp 辅助函数 + 0.5 人日测试；如直接使用 `lerp` 则为 0 人日 |
...
| **合计行：独立函数范式口径** | **✅ ~18 个** | **⚠️ 1 个** | **❌ ~87 个（含 3 个整数 T 抛异常条目）** | ✅ 独立函数 18 个指不依赖重载展开的函数名计数；⚠️+❌ 小计 = **88 个** |
