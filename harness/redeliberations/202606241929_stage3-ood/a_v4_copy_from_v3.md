...
> **修订日期**：2026-06-24
> **修订定位**：v7 设计的迭代修订（v8）。依据本轮审查报告（a_v4_iteration_requirement.md 对应 v3 诊断报告）识别 4 项严重问题（`gtc/constants` 函数数量自相矛盾 / `axis` 函数 T(1) 获取方式描述错误 / `fromMat4` 降维策略两可 / `FMat2x2` 先例引用错误）+ 5 项一般问题（测试文件命名约定不一致 / `trigonometric.cj` 函数清单未区分标量/向量重载 / `axis` 函数 Float32 sqrt 处理策略缺失 / `fromVec3` 工厂函数边界行为契约缺失 / `mat3Cast`/`mat4Cast` 边界行为未与 §3.3 对齐）+ 2 项轻微问题（`gtc/quaternion_test.cj` 用例数比例失衡 / §11.5 `isnan`/`isinf` 与 `mat3_cast`/`mat4_cast`/`quat_cast` 行 where 子句标注缺失）开展修订。本轮共 11 项审查意见（4 严重 + 5 一般 + 2 轻微），逐项落实到对应章节，并补充修订说明（v8）章节。
...
- **常量型 T(1) 字面量替代**（v8 新增）：对于公式中固定为常数 1 的场景（如 `axis` 公式中的 `T(1) - x.w*x.w`），可采用 `T(Float64(1))` 显式转换路径（编译期 `Float64` 字面量 → `T` 类型），无需 `one: T` 形参；该路径对 `T = Float32`/`Float64` 均成立（`Float32(Float64(1.0))` 与 `Float64(Float64(1.0))` 均返回正确字面量）
...
7. **从旋转矩阵构造 fromMat4(m: Mat4x4<T,Q>)** — 辅助工厂函数，**v8 决策：采用手动提取策略**——直接读取 `m.c0`/`m.c1`/`m.c2` 三列构建 `Mat3x3<T,Q>(Vec3<T,Q>(m.c0.x, m.c0.y, m.c0.z), Vec3<T,Q>(m.c1.x, m.c1.y, m.c1.z), Vec3<T,Q>(m.c2.x, m.c2.y, m.c2.z))`（参考阶段二 `cjglm/src/detail/type_mat2x2.cj:163-165` 中 `Mat2x2.fromMat(Mat3x3, one)` 的列提取模式，**无需 `one: T` 参数**——因为我们构造的是 `Mat3x3` 实例，不是从更小矩阵向上转换），再调用同包 `quatCast(mat3x3)` 转换为四元数。非 const。**v8 修订说明**：v7 文档给出"手动提取 vs GLM 策略转换"两种"或"的选择，但两种方式实现语义不同（手动提取无需 `one` 参数；GLM 策略转换依赖阶段二未提供的 `Mat3x3<T,Q>(Mat4x4<T,Q>)` 构造重载），下游无法决策。v8 明确选择手动提取策略，因其：①与阶段二 `type_mat2x2.cj:163-165` 的列提取模式一致；②不依赖 Mat3x3 构造重载扩展；③无 `one: T` 参数污染（与 `fromMat3` 无 one 参数的签名风格对齐）。**边界行为**：同上，依赖 `fromMat3` 的输入合法性——若 `m` 的左上 3×3 子矩阵非纯旋转矩阵，结果未定义。
...
- `axis(x: Quat<T,Q>): Vec3<T,Q>` — 实现公式（与 GLM `ext/quaternion_trigonometric.inl:20-27` 一致）：`tmp1 = T(Float64(1)) - x.w * x.w`；若 `tmp1 <= T(Float64(0))` 返回 `Vec3(T(Float64(0)), T(Float64(0)), T(Float64(1)))`（z 轴单位向量）；否则 `tmp2 = T(Float64(1)) / T(Float64(std.math.sqrt(Float64(tmp1)))`，返回 `Vec3(x.x*tmp2, x.y*tmp2, x.z*tmp2)`。依赖 `std.math.sqrt`（受 §1「Float32 与 std.math 的交互约束」管辖，统一使用 `T(Float64.xxx(Float64(value)))` 模式兼容 `T = Float32`/`Float64`）和 `T(Float64(1))` 字面量替代（无需 `one: T` 形参），**可完整实现**。**T(1) 获取方式（v8 修订）**：v7 文档描述「T(1) 演算通过 `x.w*x.w` 取得」是错误的——`x.w*x.w` 是 w² 而非 1，无法作为 T(1) 的演算路径（v8 明确删除该错误描述）。本函数采用 §1「系统性设计约束」中 v8 新增的「常量型 T(1) 字面量替代」策略：`T(Float64(1))` 显式转换路径——`Float64(1.0)` 是字面量，`T(Float64(1.0))` 在 `T = Float64` 时返回 1.0，在 `T = Float32` 时返回 1.0f，无需 `one: T` 形参污染函数签名。**边界行为（v3 新增契约，v5 修订，v8 强化）**：触发 `Vec3(T(0), T(0), T(1))` 返回值的条件为 `T(Float64(1)) - x.w*x.w <= T(Float64(0))`（典型场景：单位四元数 `(0, 0, 0, 1)` 即 `|w| >= 1`）；对于真正的零四元数 `(0, 0, 0, 0)`，`tmp1 = 1 > 0` 进入 else 分支返回 `Vec3(0, 0, 0)`（**注意：v3 原描述的「内部 `normalize(Vec3(0, 0, 0))` 返回 `Vec3(T(1), T(0), T(0))`」为虚构实现，GLM 源码不调用 `normalize`，v5 修订纠正**）。
...
#### 3.13.1 trigonometric.cj 函数清单（v8 新增标量/向量重载区分）
...
| `radians` | `radians<T>(x: T): T` | `radians<T, Q>(x: VecN<T, Q>): VecN<T, Q>` | 纯算术（度→弧度，无 std.math 依赖） |
| `degrees` | `degrees<T>(x: T): T` | `degrees<T, Q>(x: VecN<T, Q>): VecN<T, Q>` | 纯算术（弧度→度，无 std.math 依赖） |
...
| `tests/glm/ext/test_vector_relational.cj` | 向量 epsilon/ULP 比较 | ≥16 |
| `tests/glm/ext/test_quaternion_relational.cj` | 四元数关系运算 | ≥8 |
| `tests/glm/ext/test_quaternion_geometric.cj` | 四元数几何函数 | ≥12 |
| `tests/glm/ext/test_quaternion_common.cj` | 四元数公共函数 | ≥16 |
| `tests/glm/ext/test_quaternion_trigonometric.cj` | 四元数三角函数 | ≥8 |
| `tests/glm/ext/test_quaternion_exponential.cj` | 四元数指数函数（stub） | ≥4 |
| `tests/glm/ext/test_quaternion_transform.cj` | 四元数变换函数（stub） | ≥2 |
| `tests/glm/ext/test_scalar_constants.cj` | 标量常量 | ≥6 |
| `tests/glm/ext/test_quaternion_float.cj` | 四元数别名 | ≥4 |
...
| **合计** | — | **≥178**（v8 修订：原 ≥171，常量 +3 + quaternion +4 = +7） |
...
| `mat3_cast`/`mat4_cast`/`quat_cast`（gtc，重导出） | ✅ 可用（**约束：`where T <: FloatingPoint<T>`（D32）**，仅浮点 T，整型 T 编译失败，v5 约束收紧） | ✅ 可用（**约束：`where T <: FloatingPoint<T>`（D32）**，仅浮点 T） |
