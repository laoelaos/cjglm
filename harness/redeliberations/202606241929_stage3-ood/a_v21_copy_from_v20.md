...
> **修订日期**：2026-06-26
> **修订定位**：v21 设计的迭代修订（对应本轮审查 a_v21_iteration_requirement.md）。基于 v20 输出修订，针对审查报告识别的 8 项问题（4 严重 + 3 一般 + 1 轻微）逐项落实。本轮重点修复：(1) 新增 GLM 1.0.3 Stage 3 API 基准声明（§9a），覆盖 13 个头文件共 165 个 API 的逐项对比与推迟理由；(2) 修复 §8.3 D 节「权威基准」与 §11.5「主要参考来源」的长期矛盾——统一降级为「主要参考基准」；(3) 修正 gen_fwd_aliases.py 验证命令模板（匹配模式与代码格式对齐，新增 HighpFQuat 冗余前缀检出不期望模式）；(4) 为 normalize/axis/mat3Cast/mat4Cast 四个 P0 函数补充「集中参考清单」，聚合跨 6 个章节的分散信息；(5) 新增验证结果记录表模板覆盖全部 29 项验证项（含 3 项 P0），使核验结果可追溯；(6) 核验确认 Mat4x4 列字段名 `c0`/`c1`/`c2`/`c3` 假设正确，验证项 27 升级为「已核验确认」；(7) §11.5 对照表新增「设计章节」列，每函数行标注其 §3.x 主要描述位置，stub 函数同步标注 §1 约束位置；(8) 修订说明全部版本表的「审查意见」列统一标注前序版本号前缀。
