...
> **修订日期**：2026-06-26
> **修订定位**：v22 设计的迭代修订（对应本轮审查 a_v22_iteration_requirement.md）。基于 v21 输出修订，针对审查报告识别的 6 项问题（3 一般 + 3 轻微）逐项落实。本轮重点修复：(1) §3.7 `length` 函数 Float32 路径描述——消除 `Float64(dot_qq)` 显式转换与 §1 方案 A 的矛盾，统一为直接调用 `std.math.sqrt` 的 Float32 重载；(2) §8 验证项 2 snake_case 命名残留——`mat3_cast` 修正为 `mat3Cast`；(3) §8 验证项 12 验证路径优先级——明确方案 A（直接调用 Float32 重载）为首选路径，原显式 Float64 转换路径降级为备选；(4) §11.7 新增 ✅ 可用函数集中参考索引——25 个 ✅ 函数的聚合跨节引用表（§3.x / §5.3 / §3.13.2 / §8 / §11.5 五维映射）；(5) §3.16 新增阶段三→阶段四演进指南——明确签名兼容性承诺、⚠️ 函数自动过渡路径、测试迁移规则、责任边界总表；(6) §3.12 新增 epsilon 硬编码参考值 ground truth 子段——显式记录 Float32/Float64/整数类型的 epsilon 数值。
...
**审计结论**：
- **本阶段可调用的「真完整实现」函数**（无 stub 依赖）：`epsilon<T>()`/`pi<T>()`/`cos_one_over_two<T>()`/`axis`/`length`/`normalize`/`inverse`/`isnan`/`isinf`/`mat3Cast`/`mat4Cast`/`quatCast`/`conjugate`/`lerp`/`dot`/`cross(Quat)`/`equal(Quat)`/`notEqual(Quat)`（共 **18 个函数**【v15 修订，Issue 4 响应：上一轮误标为 17 个，实际列表含 18 个函数名，本轮修正为 18】）
...
| 职责 | 阶段三 | 阶段四 |
|------|--------|--------|
| API 签名设计 | 全部完成（含 `where` 子句） | **不可修改**（仅可新增重载） |
...
| 验证项 | 验证假设 | 验证失败后受影响章节 |
|-------|---------|-------------------|
| 1-2（gtc 子包构建） | cjpm 识别 `src/gtc/` + `package glm.gtc` | §2「cjpm 子包构建预验证」回退方案；§2 包组织结构；§8.3 验收项 E |
| 3（包间无循环依赖） | `glm.gtc → glm.detail` 单向 | §2 模块间依赖图；§3.2.1 type_quat_cast 包归属决策 D11；§3.15 跨包引用 |
| 4（同包调用 type_quat_cast） | `fromMat3`/`fromMat4` 调用同包函数 | §3.3 item 6/7；§3.2 协作关系表 |
| 5（Vec extend 块前向引用） | Vec3/Vec4 extend 块中 Quat 引用延迟解析 | §3.4 Vec extend 块 |
| 6-7（scalar_constants match 模式） | 泛型 `match` 类型模式匹配编译 | §3.12 实现策略；§7 D09 |
| 8（标量运算同名重载消歧） | add/sub/mul/div 与向量/矩阵版本消歧 | §3.4 全局具名函数表；§7 D18 |
| 9（isNaN/isInf 实例方法） | 浮点类型提供 `isNaN()`/`isInf()` | §3.11 isnan/isinf 实现路径；§7 D29 |
| 10（内联 abs 编译通过） | `Number<T> & Comparable<T>` 内联模式 | §3.5 依赖分析段 |
| 11（Hashable 派生对 Qualifier） | `@Derive[Hashable]` 对 `Q <: Qualifier` 支持 | §3.1 `@Derive[Hashable]` 约束核验段；§7 D30 |
| 12（length sqrt 转换） | `T(Float64.sqrt(Float64(dot_qq)))` 编译 | §3.7 length 函数；§1 Float32/std.math 约束 |
| 13（isnan/isinf 约束收紧） | `where T <: FloatingPoint<T>` 编译期生效 | §3.11 isnan/isinf 签名；§7 D29；§5.3 边界条件表 |
| 14（public var 可见性） | `public var` 字段满足 Hashable 派生 | §3.1 数据成员声明；§7 D30 |
| 15（pow 命名消歧） | 四元数 pow 与 std.math.pow 区分 | §3.10 pow 依赖描述；§1 系统性约束 |
| 16（getMinDenormal 静态方法） | `FloatingPoint<T>.getMinDenormal()` 存在 | §3.10 pow 次正规数边界；§1 回退方案 H2 路径 A/B |
| 17（type_quat_cast 约束） | `where T <: FloatingPoint<T>` 编译生效 | §3.2.1 函数签名规范；§7 D32；§5.3 边界条件表 |
| 18（getInf 静态方法） | `FloatingPoint<T>.getInf()` 存在 | §3.10 log w=0 退化分支；§1 回退方案 H2 路径 A/B |
| 19（epsilon 一致性） | `epsilon<T>()` 与 `epsilonOf<T>(hint)` 一致 | §3.12 epsilon 实现；§8.2 测试覆盖维度 |
| 20（FloatingPoint 接口方法）→ P0 | 6 静态 + 3 实例方法均存在 | §1 回退方案 H2 全部路径；§3.10/§3.11 依赖描述；§7 D21/D29 |
| 21（std.math Float32 重载） | 三角函数均提供 Float32 重载 | §1 Float32/std.math 约束段；§3.13.1 函数表；§3.10 pow/log/exp |
| 22（trigonometric T 约束） | 75 展开函数均标注 `FloatingPoint<T>` | §3.13.1 函数签名表；§8.3 验收项 E |
| 23（fwd.cj 生成脚本） | `gen_fwd_aliases.py` 正确生成 9 别名 | §2 fwd.cj 段落；§3.14 别名清单；§8.3 验收项 F |
| 24（conjugate const func） | `const func` 编译通过 | §3.11 conjugate 实现；§5.4 const 上下文约束 |
| 25（T(Float64(n)) 语法）→ P0 | 全部受影响的类型编译通过 | §1 回退方案 H1 全部路径；§3.7 normalize；§3.9 axis；§3.2.1 mat3Cast/mat4Cast；§3.4 Quat×Vec3/Vec4 |
| 26（Mat4x4 [] 返回类型） | `[]` 返回可赋值引用 | §3.3 item 7 fromMat4 实现 |
| 27（Mat4x4 列字段名） | 列字段名为 `c0`/`c1`/`c2`/`c3` | §3.3 item 7 fromMat4 实现（编码者：验证后按实际字段名修改 §3.3 item 7 实现注释中的字段名引用；不涉及 API 签名变更，无需设计审批） |
| 28（Vec4 双参数构造） | `Vec4(Vec3, T)` 在 6 个 Qualifier 上均可用 | §3.4 Quat×Vec4 运算符；§11.5 可用性表 |
| 29（glm.detail 导入 glm.ext）→ P0 | `detail/type_quat.cj` 无 `import glm.ext.*` | §2 模块间依赖图；§3.4 Vec extend 块（v18 修订内联路径）|
...
为确保编码启动前的假设核验可追溯，以下提供标准化验证结果记录表模板。对 3 项 P0 验证项（20、25、29），要求验证通过后才可启动编码；验证失败时按「失败后处置路径」列指引的章节触发设计修订流程。

| 验证项编号 | 假设描述 | 验证结果（通过/失败） | 验证日期 | 验证人 | 失败后处置路径 |
|-----------|---------|---------------------|---------|-------|-------------|
| 1 | cjpm 识别 `src/gtc/` + `package glm.gtc` | | | | §2 回退方案：迁移至 `src/` 根目录降级为 `package glm` |
| 2 | `public import` 重导出 detail 函数可行 | | | | D11 重新评估：detail 端函数下沉至 gtc 端复制实现 |
| 3 | 包间无循环依赖（`glm.gtc → glm.detail` 单向） | | | | §2 模块间依赖图重绘：D28 包间依赖策略修订 |
| 4 | type_quat.cj 同包调用 type_quat_cast.cj 函数 | | | | §3.3 item 6/7 fromMat3/fromMat4 改为跨包 import |
| 5 | Vec extend 块中 Quat 前向引用 | | | | §3.4 Vec extend 块运算符改为全局函数（scalar_quat_ops 扩展） |
| 6-7 | scalar_constants match 模式 | | | | D09 重复评估：改用 if-else 链替代 match |
| 8 | 标量运算同名重载消歧 | | | | §3.4 全局函数改名避免冲突（如 addTQuat/scalarAddQuat） |
| 9 | isNaN/isInf 实例方法 | | | | §3.11 改为 match 运行时分派 |
| 10 | 内联 abs 编译通过 | | | | §3.5 回退至调用 common.cj abs stub |
| 11 | Hashable 派生对 Qualifier | | | | §3.1 移除 `@Derive[Hashable]`，手动实现 Hashable |
| 12 | length sqrt 转换 | | | | §3.7 改为 `T(Float64.sqrt(Float64(dot_qq)))` + 类型转换 |
| 13 | isnan/isinf 约束收紧 | | | | §3.11 放宽约束，运行时 match 分派 |
| 14 | public var 可见性 | | | | §3.1 确认字段标注 public，否则修正 |
| 15 | pow 命名消歧 | | | | §3.10 pow 函数使用全限定名 `std.math.pow` |
| 16 | getMinDenormal 静态方法 | | | | §1 回退方案 H2 路径 A/B：类型分派或字面量 fallback |
| 17 | type_quat_cast 约束 | | | | §3.2.1 放宽为 Number<T>，运行时 match 分派 |
| 18 | getInf 静态方法 | | | | §1 回退方案 H2 路径 A/B：类型分派或 T(1)/T(0) 构造 |
| 19 | epsilon<T>() 与 epsilonOf<T>(hint) 一致性 | | | | §3.12 统一为单实现路径，消除分支 |
| **20（P0）** | **FloatingPoint<T> 接口方法可用性** | | | | §1 回退方案 H2 全部路径（类型分派/字面量 fallback） |
| 21 | std.math Float32 重载 | | | | §1 返回「必选 Float64 转换」策略：`T(Float64.xxx(Float64(value)))` |
| 22 | trigonometric T 约束 | | | | §3.13.1 放宽为 Number<T>，运行时 match 分派 |
| 23 | fwd.cj 生成脚本 | | | | §2 备选方案 B：手动维护 lib.cj 或新建别名文件 |
| 24 | conjugate const func | | | | §3.11 移除 const 声明，降级为普通 func |
| **25（P0）** | **T(Float64(n)) 语法编译可行性** | | | | §1 回退方案 H1 全部路径（追加 `one: T` 形参/`match` 分派/收紧约束） |
| 26 | Mat4x4 [] 返回类型 | | | | §3.3 item 7 退化为 Mat4x4 主构造函数构造 |
| 27 | Mat4x4 列字段名 `c0`/`c1`/`c2`/`c3` | ✅ 已核验 | 2026-06-26 | 设计方 | **本轮已核验确认**：§3.3 item 7 描述正确，无需编码者重复核验 |
| 28 | Vec4 双参数构造函数 | | | | §3.4 Quat×Vec4 改为手动 Vec4(x,y,z,w) 构造 |
| **29（P0）** | **glm.detail 导入 glm.ext 验证** | | | | §3.4 确认内联 conjugate/dot 路径实施；如编译失败则退回原 inverse 路径 |
...
`§11.5 函数可用性对照表` 是阶段三验证的**主要参考基准**：✅/⚠️/❌ 三档符号标记每个函数本阶段状态（实现在本文件其他章节有详细描述）。下游验证时按 §11.5 表逐项核验即可。

**E. 验证项清单（来源：§8 编码启动前验证项 1-28）**

阶段三编码启动前需完成 §8 编码启动前验证项 1-29 全部 29 项验证，其中验证项 20、25、29 兼为高优先级编译前验证项（P0）。**v18 修订，Issue 1 响应**：新增验证项 29（`glm.detail` 包对 `glm.ext` 的导入可行性验证），独立验证目标从 28 项修正为 29 项。确保：
...
- **核验项 H1**：§10 覆盖矩阵合计行 ❌/⚠️ 计数 = 逐行累加各文件 ❌/⚠️ 计数之和。验证方式：每行提取 ✅/⚠️/❌ 数值，逐行累加后与合计行声明值对照，偏差为 0。
...
### 11.5 函数可用性对照表（v3 新增，**v20 关键修订，Issue 1 响应：补齐 5 个缺失 API 条目，降级权威声明**）

**v20 修订，Issue 1+4 响应**：本表目标为函数阶段状态追踪的**主要参考来源**。当前版本因缺少 `wxyz`/`fromQuat`/`[]`/`static func length`/复合赋值运算符共 5 个 API 条目，尚不具备"单一权威来源"的完整性要求。§3.13.2 审计节、§8 产出物清单、§10 覆盖矩阵、§11.6 可达性矩阵的函数状态描述仍以本表为主要基准，但下游查阅时需注意补充上述缺漏项。待本轮补齐并通过审查后，下个版本升级为"单一权威来源"。
...
### 11.7 ✅ 可用函数集中参考索引（v22 新增）

本节为所有 ✅ 可用（无 stub 依赖）函数提供聚合参考索引，将每个函数分散在 §3.x、§5.3、§3.13.2、§8、§11.5 中的跨节引用信息集中列示。已在本节 §3.x 描述段末尾拥有独立「集中参考清单」的 4 个 P0 函数（normalize/axis/mat3Cast/mat4Cast）同样在本索引中汇总列出，便于统一查阅。

| 函数 | §3.x 描述 | §5.3 边界条件 | §3.13.2 审计行 | §8 验证项 | §11.5 行 |
|------|----------|-------------|--------------|----------|---------|
...
- 「§3.13.2 审计行」列标注该函数是否在 §3.13.2 审计表中拥有独立行（"无独立行"表示该函数在本节 §3.13.2 段落中仅被汇总提及但无单独行标注）
...
经过多轮审查迭代，本设计文档的审查意见修复率累计达 100%。本轮 7 项问题全部落实后，文档可进入下游验证环节。
...
经过多轮审查迭代，本设计文档的审查意见修复率累计达 100%。本轮 4 项问题全部落实后，文档可进入下游验证环节。
...
经过多轮审查迭代，本设计文档的审查意见修复率累计达 100%。本轮 6 项问题全部落实后，文档可进入下游验证环节。
...
经过多轮审查迭代，本设计文档的审查意见修复率累计达 100%。本轮 8 项问题全部落实后，文档可进入下游验证环节。
