...
> **修订日期**：2026-06-29
> **修订定位**：v31 设计的迭代修订（对应本轮审查 a_v32_iteration_requirement.md）。基于 v31 输出修订，针对审查报告识别的 7 项问题（2 严重 + 3 一般 + 2 轻微）逐项落实。本轮重点修复：(1) 版本标注自动化门禁新增 —— §8.3 H 节新增 H7 自动化 grep 门禁，实现版本标注零残留；(2) §3.16 末尾新增「阶段三交付能力与路线图预期差距评估」子节，量化回答 11% 可用率是否满足路线图目标；(3) §11.5 定位声明措辞与 §8.3 D 节统一，「权威来源」措辞一致化；(4) §1 末尾新增「验证时序审计声明」，明确列举 v1~v24 在未经验证假设上发布的版本范围，O-06 升级为「当前轮次已实施」并补充实施证据；(5) §3.13.1 三角函数表「内部依赖」列统一为仅标注「（阶段四依赖）」；radians/degrees 实现细节移至表下；新增三角函数标量函数与四元数函数依赖映射表；(6) §3.9 axis 公式冗余 `T(...)` 外层包裹移除；(7) 非决策性版本标注全文清理。
...
> **⚠️ 严重限制声明**：本阶段**无法**提供完整的 GLM 四元数功能。165 个 GLM API 中，仅有 **约 18 个（约 11%）** 为真完整独立实现；**约 88 个（约 53%）** 为 stub 占位或受 stub 依赖阻塞。两个核心用户用例——**四元数旋转向量（Quat×Vec3/Quat×Vec4）和球面插值（slerp/mix）**——均不可用。四元数的旋转表达和插值运算这两个核心领域，**必须等待阶段四** geometric.cj/trigonometric.cj/common.cj 函数库完整实现后方可完成。以下分段给出详细计数口径和降级路径。
...
- **常量型 T(n) 字面量替代**：对于公式中固定为常数 n 的场景（如 `axis` 公式中的 `T(1) - x.w*x.w`、`normalize` 零保护分支中的 `T(0)` 与 `T(1)`、`Quat×Vec3`/`Quat×Vec4` 旋转公式末尾的 `* 2` 缩放因子、`mat3Cast`/`mat4Cast` 转换函数中初始化 3×3/4×4 单位矩阵对角元素的 `T(1)` 与非对角线元素的 `T(0)`），可采用 `T(Float64(n))` 显式转换路径（编译期 `Float64` 字面量 → `T` 类型），无需 `one: T` 形参；该路径对 `T = Float32`/`Float64` 均成立（`Float32(Float64(n))` 与 `Float64(Float64(n))` 均返回正确字面量）。对 `T = Int8~Int64/UInt8~UInt64` 等整数类型，`T(Float64(n))` 的编译可行性属于 **P0 假设 H1**（验证项 25），尚待编码启动前验证——若验证失败，整数类型路径退化为 `match` 运行时分派或追加 `one: T` 形参（详见回退方案决策树）。：T(0) 字面量替代从「仅含 `axis` 中 `Vec3(T(0), T(0), T(1))` 返回值」扩展为系统性覆盖所有函数体中需要常量型 T(0) 的场景——`normalize` 零保护分支的 `Quat(T(0), T(0), T(0), T(1))` 返回值中 xyz 分量的 `T(0)` + `mat3Cast`/`mat4Cast` 非对角线元素的 `T(0)` 均改用 `T(Float64(0))` 字面量替代路径，不再依赖 `one - one` 演算。——以下清单采用 **exhaustive（穷举）** 规则：逐函数标注 T(0)/T(1) 字面量使用位置，涵盖所有在函数体内部依赖 `T(Float64(n))` 构造（n ≠ 0,1 时标注 n 值）的函数，不遗漏任何含 T(0)/T(1) 字面量替代的函数；清单之外的任何函数（如 `lessThan` 纯比较函数、`equal` 纯比较函数等）均不依赖 T(n) 字面量替代路径，无需修改：
  ...
  - **`mat3Cast`/`mat4Cast` 转换函数（§3.2.1）**：使用 `T(Float64(1))` 字面量初始化结果矩阵的对角元素 + 使用 `T(Float64(0))` 字面量初始化非对角线元素（v9 已明确 T(1)，——与逐元素填充模式一致，见问题 4 修订）
：`T(Float64(n))` 字面量替代路径对所有受影响的类型（`Float32`/`Float64`/`Int8`~`Int64`）的编译可行性已在 §8 编码启动前验证项中新增显式验证项，确保该核心假设在编码阶段被验证通过后方可进入实现。
...
**验证时序声明**：H1（`T(Float64(n))` 语法可行性）和 H2（`FloatingPoint<T>` 接口方法可用性）两项 P0 核心假设的验证已完成于 v25 轮次（验证日期：2026-06-26）。验证结果：✅ **两项均已通过**。这意味着 v1~v24 中基于未验证假设做出的所有设计决策（涉及 normalize/axis/mat3Cast/mat4Cast/Quat×Vec3/Quat×Vec4/slerp/pow/log 等函数的签名选择与实现策略）已在轮次 v25（即第 25 轮设计迭代）时得到实际编译验证的支持。为彻底解决「验证发生在设计完成之后」的时序矛盾，O-06（验证前置化流程制度化）已在 v27 从「待评估」升级为「下一轮必须实施」，v28 转化为可执行落地计划（含目标、责任人、M1/M2/M3 里程碑、检查清单、例外处理机制）。**当前版本（v32）确认 O-06 已落地实施**：所有 P0 假设（验证项 20/25/29）已在设计版本固化前完成最小仓颉测试文件编译验证，验证结果记录于 §8 验证结果记录表模板。未来版本中，P0 核心假设的验证将前置为设计版本固化前的门禁条件，不再出现 24 轮设计建立在未经验证假设之上的情况。
...
本设计文档采用迭代演进式修订方式。v32 在 v31 清理成果基础上，采用自动化门禁进一步消除残留。正文中所有 `（v{N} 修订）`、`（v{N} 关键修订）`、`（v{N} 新增）`、`（Issue {N} 响应）` 等非决策性版本标注已被移除，保留了标注内的设计决策内容（约束说明、策略选择、计数变更解释等可实施性信息）。**§7 设计决策表**保留各决策的版本号（D01~D38），§3.13.2/§8/§11.5 中保留少量版本标记仅用于计数口径解释。历史变更追踪由独立文件 `a_v27_revision_history.md` 和后续历史文件承载。§8.3 H7 核验门禁已将自动化 grep 检查纳入版本发布前置流程，实现版本标注零残留。
...
**修复措施**：
1. O-06（验证前置化流程制度化）已在 v27 从「待评估」升级为「下一轮必须实施」，并在 v28 转化为可执行落地计划（含目标、责任人、M1/M2/M3 里程碑、检查清单、例外处理机制）。**本版本（v32）确认 O-06 已落地实施**：所有 P0 假设（验证项 20/25/29）已在设计版本固化前完成最小仓颉测试文件编译验证。
...
- `import glm.gtc.matrix_transform` — 矩阵变换函数库空桩占位（：64 个函数签名而非 v12 描述的 35 个；v9 新增）
...
- **本文件完整实现（4 个）**（：v13 策略段落声称「完整实现（8 个）」含 4 个欧拉函数，与 §3.15 函数职责分组表/v3 修订说明已归入「stub 占位」直接矛盾；本轮修正为与 §3.15 一致的分类）：
...
6. **从旋转矩阵构造 fromMat3(m: Mat3x3<T,Q>)** — 辅助工厂函数，内部调用**同包** `type_quat_cast.quatCast(m)`（v3 修订：原设计为跨包引用 gtc/quaternion.cj，现改为同包引用 type_quat_cast.cj）。：函数签名模板为 `public func fromMat3<T, Q>(m: Mat3x3<T, Q>): Quat<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier`——显式声明 `where T <: FloatingPoint<T>` 约束，而非隐式继承自内部 callee `quatCast`。整型 T（如 `Int64`）实例化时编译报类型不匹配错误；详见 §3.2.1 type_quat_cast.cj 约束说明。**调试指引**：若开发者用 `Int64` 实例化并调用 `fromMat3(m)`，编译错误将直接定位在 `fromMat3` 的签名约束处（而非内部 `quatCast` 处），提供更清晰的错误定位。非 const。**边界行为**（v3 新增契约声明）：仅对纯旋转矩阵产生有意义的四元数；对非旋转矩阵（如含缩放/平移/剪切矩阵）行为未定义，结果可能产生非单位四元数或语义错误的旋转。

7. **从旋转矩阵构造 fromMat4(m: Mat4x4<T,Q>)** — 辅助工厂函数，——直接读取 `m.c0`/`m.c1`/`m.c2` 三列（：经查阅阶段二 `type_mat4x4.cj:8-11`，列字段名确为 `c0`/`c1`/`c2`/`c3`，设计假设于 v21 编码启动前核验为正确）构建 `Mat3x3<T,Q>(Vec3<T,Q>(m.c0.x, m.c0.y, m.c0.z), Vec3<T,Q>(m.c1.x, m.c1.y, m.c1.z), Vec3<T,Q>(m.c2.x, m.c2.y, m.c2.z))`，再调用同包 `quatCast(mat3x3)` 转换为四元数。：函数签名模板为 `public func fromMat4<T, Q>(m: Mat4x4<T, Q>): Quat<T, Q> where T <: FloatingPoint<T>, Q <: Qualifier`——显式声明 `where T <: FloatingPoint<T>` 约束，而非隐式继承自内部 callee。整型 T 实例化时编译失败——编译错误直接定位在 `fromMat4` 签名约束处（而非内部 `quatCast` 处）。非 const。：(a) 该手动提取路径直接使用 `Vec3<T,Q>(T, T, T)` 主构造函数构造向量分量，再使用 `Mat3x3<T,Q>(Vec3, Vec3, Vec3)` 主构造函数构造 3×3 矩阵——**两条构造路径均不携带 `one: T` 形参**（Vec3 主构造与 Mat3x3 主构造均无 `one` 形参，区别于跨维度 `fromMat` 工厂函数重载），因此 `fromMat4` 函数签名本身无需 `one: T` 形参；(b) 与 §1「系统性设计约束」中"**T(1) 的获取：必须由调用方显式传入参数（`one: T`）**"一致——本函数不调用任何需要 T(1) 的运算路径（不调用 `fromMat(Mat_Larger, one)` 跨维度工厂函数，不调用 `identity(one)` 单位矩阵工厂函数），故无需该形参；(c) ：v8 修订曾引用阶段二 `cjglm/src/detail/type_mat2x2.cj:163-165` `Mat2x2.fromMat(Mat3x3, one)` 作为"列提取模式无需 `one`"的依据，但该引用本身存在内部矛盾——被引用的代码 `Mat2x2.fromMat(Mat3x3, one)` 显式携带 `one: T` 形参（即阶段二全部 9 个矩阵类型的 `fromMat` 工厂函数**均要求** `one: T` 形参，无论源/目标矩阵尺寸关系，仅同维度 `fromMat` 简化为无 `one` 形参），与"无需 `one`"结论相悖。：删除错误的代码引用作为论据，改为基于本函数的实际构造路径（Vec3 主构造 + Mat3x3 主构造，均无 `one` 形参依赖）说明无需 `one` 形参的合理性，与 §1 系统性约束彻底闭环。**边界行为**：同上，依赖 `fromMat3` 的输入合法性——若 `m` 的左上 3×3 子矩阵非纯旋转矩阵，结果未定义。

8. **从两向量构造 fromVec3(u: Vec3<T,Q>, v: Vec3<T,Q>)** — 辅助工厂函数，内部实现从两个归一化轴构建四元数。依赖 geometric.cj 的 `dot`/`cross`/`normalize`（均为 stub），因此本阶段为 **stub 占位**，完整实现推迟至阶段四。**边界行为契约（v8 新增，v13 关键修订，Issue 5 响应）**：当输入向量 u, v 退化触发条件为 GLM 1.0.3 `detail/type_quat.inl:196-217`（：v8 错误引用为 `ext/quaternion_common.inl:196-217`——`fromVec3` 实现位于 `detail/type_quat.inl` 的 `qua(vec3, vec3)` 构造函数，不在 ext/quaternion_common）中 `if (real_part < static_cast<T>(1.e-6f) * norm_u_norm_v)` 退化分支（：v8 描述「`dot(normalize(u), normalize(v)) ≈ -1`」与实际触发条件不一致；实际 `real_part = norm_u_norm_v + dot(u, v)`，触发条件为 `norm_u_norm_v + dot(u, v) < 1e-6f * norm_u_norm_v`，等价于 `dot(u, v) < (1e-6f - 1) * norm_u_norm_v = -(1 - 1e-6f) * sqrt(dot(u, u) * dot(v, v))` ≈ `-sqrt(dot(u, u) * dot(v, v))`，即「`dot(u,v) ≈ -sqrt(dot(u,u) * dot(v,v))`」对应「`u, v` 反平行」）。退化分支内**轴选择逻辑**（：v8 描述「与 u 垂直的任意轴」不准确；：补充 `normalize(t)` 归一化步骤）：GLM 实际为 `t = abs(u.x) > abs(u.z) ? vec<3, T, Q>(-u.y, u.x, static_cast<T>(0)) : vec<3, T, Q>(static_cast<T>(0), -u.z, u.y); t = normalize(t)`——基于 `abs(u.x) > abs(u.z)` 二选一选择「与 u 垂直的两条可能轴之一」：`abs(u.x) > abs(u.z)` 选 `(-u.y, u.x, 0)` 再归一化，否则选 `(0, -u.z, u.y)` 再归一化。：选出的轴还需经过 `normalize(t)` 归一化确保返回纯向量四元数模长为 1（GLM `detail/type_quat.inl:196-217` 实际包含 `t = normalize(t)` 步骤）。返回 `q.w = 0` 且 `q.xyz` 模长为 1 的纯向量四元数（`real_part = 0`）。这是 GLM 对反平行输入的容错设计，避免返回零四元数导致除零。**契约声明**：`fromVec3` 对反平行输入的返回值**行为已定义**（返回 `q.w = 0` 的纯向量四元数，轴选择由 `abs(u.x) > abs(u.z)` 条件决定，行为与 GLM `detail/type_quat.inl:196-217` 一致），下游消费者可依赖该契约；对零向量输入则行为未定义（由 `normalize` stub 在阶段四补齐时确定）。
...
- `axis(x: Quat<T,Q>): Vec3<T,Q>` — 实现公式（与 GLM `ext/quaternion_trigonometric.inl:20-27` 一致）：`tmp1 = T(Float64(1)) - x.w * x.w`；若 `tmp1 <= T(Float64(0))` 返回 `Vec3(T(Float64(0)), T(Float64(0)), T(Float64(1)))`（z 轴单位向量）；否则 `tmp2 = T(Float64(1)) / std.math.sqrt(tmp1)`（——`std.math.sqrt` 提供 Float16/Float32/Float64 重载，`tmp1` 类型为 T 时编译器按实例化类型自动选择对应重载，无需显式 Float64 转换），返回 `Vec3(x.x*tmp2, x.y*tmp2, x.z*tmp2)`。依赖 `std.math.sqrt`（§1 v11 修订确认提供 Float16/Float32/Float64 重载，方案 A 推荐直接调用）；实现公式遵循 `tmp2 = T(Float64(1)) / std.math.sqrt(tmp1)` 路径。同时依赖 `T(Float64(1))` 字面量替代（无需 `one: T` 形参），**可完整实现**。**T(1) 获取方式（v8 修订）**：v7 文档描述「T(1) 演算通过 `x.w*x.w` 取得」是错误的——`x.w*x.w` 是 w² 而非 1，无法作为 T(1) 的演算路径（v8 明确删除该错误描述）。本函数采用 §1「系统性设计约束」中 v8 新增的「常量型 T(1) 字面量替代」策略：`T(Float64(1))` 显式转换路径——`Float64(1.0)` 是字面量，`T(Float64(1.0))` 在 `T = Float64` 时返回 1.0，在 `T = Float32` 时返回 1.0f，无需 `one: T` 形参污染函数签名。**边界行为（v3 新增契约，v5 修订，v8 强化，v10 公式精修）**：触发 `Vec3(T(0), T(0), T(1))` 返回值的条件为 `T(Float64(1)) - x.w*x.w <= T(Float64(0))`（典型场景：单位四元数 `(0, 0, 0, 1)` 即 `|w| >= 1`）；对于真正的零四元数 `(0, 0, 0, 0)`，`tmp1 = 1 > 0` 进入 else 分支返回 `Vec3(0, 0, 0)`（**注意：v3 原描述的「内部 `normalize(Vec3(0, 0, 0))` 返回 `Vec3(T(1), T(0), T(0))`」为虚构实现，GLM 源码不调用 `normalize`，v5 修订纠正**）。
...
- `std::numeric_limits<T>::min()` 用于次正规数边界（line 63：`if (VectorMagnitude < (std::numeric_limits<T>::min)())`，：仓颉 stdlib 实际提供 `FloatingPoint<T>.getMinDenormal()` 静态方法（依据 `cangjie-original-docs/std/math/math_package_api/math_package_interfaces.md` 第 49-60 行定义）——下游实现应通过 `FloatingPoint<T>.getMinDenormal()` 直接获取；v10 描述「`FloatingPoint<T>` 接口本身无任何实例方法」错误——`FloatingPoint<T>` 接口实际提供 6 个静态方法（`getE`/`getInf`/`getPI`/`getMinDenormal`/`getMinNormal`/`getNaN`）+ 3 个实例方法（`isInf`/`isNaN`/`isNormal`），下游实现应优先使用静态方法路径而非类型分派）。**命名消歧与 Float64 转换（v5 新增，v7 澄清，v10 合并，v11 修订）**：将 v5「命名消歧」与 v7「Float64 转换依赖」两段合并为单一子段（：v9 设计中两段功能重叠，下游阅读时产生「§1 已说明通用规则为何 §3.10 又重复一次」的疑问）。本段统一说明：四元数 `pow` 函数体内部调用的 `pow(x.w, y)` 是**实数版本** `std.math.pow`（：v10 描述的「`std.math.pow(Float64, Float64): Float64`」实际只是 4 个重载之一——仓颉 stdlib `cangjie-original-docs/std/math/math_package_api/math_package_funcs.md` 第 4289-4397 行明确定义 `pow(Float32, Float32): Float32`、`pow(Float32, Int32): Float32`、`pow(Float64, Float64): Float64`、`pow(Float64, Int64): Float64` 共 4 个重载——下游实现时应根据 T 实例化类型选择对应重载：当 T=Float32 时调用 `std.math.pow(Float32, Float32): Float32`（直接调用，无需 Float64 转换）；当 T=Float64 时调用 `std.math.pow(Float64, Float64): Float64`），与四元数 `pow` 本身（`pow(Quat, T): Quat`）通过参数类型消歧（第一参数类型 `Float32`/`Float64` 与 `Quat<T,Q>` 区分）；`pow(Quat<Float32, Q>, Float32)` 实现路径遵循 §1 v11 修订策略方案 A（直接调用 std.math Float32 重载），fallback 路径 `exp(y * log(x.w))` 同步遵循 §1 v11 修订（std.math.exp/log 同样提供 Float16/Float32/Float64 重载）；若 `std.math.pow` 不可用需以 `exp(y * log(x.w))` 替代实现。**stub 占位**。
- `sqrt(x: Quat<T,Q>): Quat<T,Q>` — 委托 `pow(x, T(0.5))`。**stub 占位**。

：4 个函数均依赖 trigonometric.cj/common.cj stub，本阶段全部 **stub 占位**。`pow` 的依赖关系以 GLM `ext/quaternion_exponential.inl:41-80` 实际实现为准（v3 引用 `24-43` 行号错误，已纠正）。`exp`/`log`/`pow` 函数分别补充 `epsilon<T>()`、`pi<T>()` 等 scalar_constants.cj 依赖。`pow` 补充 `cos_one_over_two<T>()`/`asin`/`std.math.pow`/`std::numeric_limits<T>::min()` 等价物依赖。
...
- **跨 Qualifier 行为**（v15 新增，D34 决策；v29 扩展，Problem 5 响应：扩展为统驭全篇的跨 Qualifier 策略声明）：本阶段仅提供 `x` 与 `y` 同 Q 的签名模板 `lerp(x: Quat<T,Q>, y: Quat<T,Q>, a: T): Quat<T,Q>`。若下游出现 `x` 与 `y` 的 Qualifier 不同的合法场景（如 `Quat<Float32, Highp>` 与 `Quat<Float32, Mediump>` 插值），当前签名模板将因 Q 不匹配触发编译错误。**统驭全篇的跨 Qualifier 策略声明（v29 修订）**：本阶段所有 40+ 个泛型函数和运算符（包括但不限于 §3.4 运算符表、§3.7 dot/length/normalize/cross、§3.9 axis、§3.11 inverse/conjugate/isnan/isinf 等）均遵循以下统一策略：
...
- ：v7 文档文字描述列出 28 个常量函数名称（与 GLM 1.0.3 `gtc/constants.inl` 实际声明数一致）但汇总数字误标为「25」，本轮统一修订为「28」。**测试覆盖目标同步**：`tests/glm/gtc/test_constants.cj`（v8 命名约定修订，见 §8.2）至少 28 个测试用例，每常量函数 1 个用例验证返回值正确性
...
- 实施时按上述 64 个函数清单逐一声明函数签名（：v12 描述「35 个」遗漏 29 个）
...
| `acosh` | 同上 | 同上 | （阶段四依赖） |
| `atanh` | 同上 | 同上 | （阶段四依赖） |
| `radians` | 同上 | 同上 | （阶段四依赖）——硬编码 π 字面量（stdlib 不存在） |
| `degrees` | 同上 | 同上 | （阶段四依赖）——硬编码 π 字面量（stdlib 不存在） |
...
**完整签名模板示例**：以下以 `sin` 函数为例给出 trigonometric.cj 中 14 个单参数三角函数之一的完整展开签名模板。其余 13 个单参数函数（`cos`/`tan`/`asin`/`acos`/`atan`/`sinh`/`cosh`/`tanh`/`asinh`/`acosh`/`atanh`/`radians`/`degrees`）和双参数 `atan2` 按相同模式展开：

```cangjie
// ===== 标量版本（1 个签名，用于四元数函数体内分量级运算）=====
public func sin<T>(x: T): T where T <: FloatingPoint<T>

// ===== 向量版本（4 个签名，按 Vec1/Vec2/Vec3/Vec4 展开）=====
...
**双曲类函数示例**：以 `cosh` 为例给出双曲类函数的完整展开签名模板。其余 5 个双曲函数（`sinh`/`tanh`/`asinh`/`acosh`/`atanh`）按相同模式展开：
```cangjie
// ===== 标量版本 =====
...
**反三角类函数示例**：以 `acosh` 为例给出反三角类函数的完整展开签名模板。其余 5 个反三角/反双曲函数（`asin`/`acos`/`atan`/`asinh`/`atanh`）按相同模式展开：
```cangjie
// ===== 标量版本 =====
...
**坐标转换类函数示例**：以 `degrees` 为例给出坐标转换类函数的完整展开签名模板。另一个坐标转换函数 `radians` 按相同模式展开：
```cangjie
// ===== 标量版本 =====
...
**四类展开示例的函数覆盖范围**：以上四类示例代码块共覆盖 4 个代表性函数（`sin`/`cosh`/`acosh`/`degrees`），代表 14 个单参数三角函数中的 4 个类别：
- **标准三角函数（6 个）**：`sin`/`cos`/`tan`/`asin`/`acos`/`atan` → 以 `sin` 示例展开
- **双曲函数（3 个）**：`sinh`/`cosh`/`tanh` → 以 `cosh` 示例展开
- **反双曲函数（3 个）**：`asinh`/`acosh`/`atanh` → 以 `acosh` 示例展开
- **坐标转换（2 个）**：`radians`/`degrees` → 以 `degrees` 示例展开

剩余 10 个单参数函数（`cos`/`tan`/`asin`/`acos`/`atan`/`sinh`/`tanh`/`asinh`/`atanh`/`radians`）在 trigonometric.cj 文件中按所属类别对应示例展开——标准三角类函数替换函数名 `sin` → `cos`/`tan`/`asin`/`acos`/`atan` 即可，双曲类替换 `cosh` → `sinh`/`tanh`，反双曲类替换 `acosh` → `asinh`/`atanh`，坐标转换类替换 `degrees` → `radians`。双参数 `atan2` 已单独给出完整展开。**合计 75 个函数签名**（14 单参数 × 5 签名 + 1 双参数 × 5 签名 = 70 + 5 = 75），全部约束统一为 `where T <: FloatingPoint<T>` 或 `where T <: FloatingPoint<T>, Q <: Qualifier`，本阶段函数体统一为 `{ throw Exception("stub") }`。

下游编码者按此模式将表中 15 个函数名（14 单参数 + 1 双参数）逐一展开为标量 + Vec1~Vec4 共 5 个签名 = 75 个函数签名。
...
：本节函数状态计数已与 §11.5 函数可用性对照表统一对齐。**全文档函数可用状态以 §11.5 为最终基准**。
...
- **本阶段实现但运行时受 stub 依赖影响的函数**：`Quat×Vec3`/`Quat×Vec4`/`Vec3×Quat`/`Vec4×Quat` 运算符（4 个，v30 修订：新增 Vec3×Quat/Vec4×Quat——内联 conjugate/dot 仅消除包间循环依赖，未消除对 Vec3 cross stub 的运行时调用）
...
- **批次编号说明（v31 新增，Problem 8 响应）**：本节的阶段四交付批次使用「阶段四批次 A~E」编号体系，以区别于 §8.4 阶段三实施批次使用的「第一批~第四批」编号体系。两者分别描述不同的实施阶段和交付周期，编号体系独立以避免混淆。
...
**自我评估结论**：本设计**基本满足**阶段三路线图目标，但存在**实质性偏差**：
...
| 四元数旋转向量（Quat×Vec3） | ⚠️ 可期待 | ⚠️ 编译通过但抛 stub | **偏差已闭环（D35-D37）** |
| 球面插值（slerp） | ⚠️ 可期待 | ❌ stub 占位 | **偏差已闭环（D35）** |
| 欧拉角提取 | ⚠️ 可期待 | ❌ stub 占位 | **偏差已闭环** |
| API 可用率（独立函数范式） | ~40-50%（合理预期） | **~11%** | **重大偏离**（实际可用率远低于合理预期） |
| API 可用率（含重载/常量展开） | ~60-70%（乐观预期） | **~47%** | **中等偏离** |
...
：§4.4 示例统一使用 `glm.mat3Cast(q)` 顶层命名空间调用形式，与 §11.4「仓颉顶层 glm 命名空间调用」段（v10 已明确）保持一致；调用方需先通过 `lib.cj` 完成 `public import` 间接访问，无需显式 `import glm.detail.*`。v10/v6 描述的「无前缀 `mat3Cast(q)`」调用形式若按字面实现（未先 `import glm.detail.*`），将因 `mat3Cast` 在 `glm.detail` 命名空间下而触发「未定义符号」编译错误。**前提条件**：调用方必须 `import glm`（导入 lib.cj 公共 API 层），使 `glm.mat3Cast` 在当前包命名空间下可见。
...
- **`normalize` 零四元数**：返回单位四元数 `Quat(T(Float64(0)), T(Float64(0)), T(Float64(0)), T(Float64(1)))`（：T(0)/T(1) 均采用 `T(Float64(n))` 字面量替代路径，与 §1 系统性约束 v14 修订一致），与 GLM `quaternion_geometric.inl:20-21` 行为一致
...
| 边界条件 | 函数 | 行为契约 |
|---------|------|---------|
| `1 - x.w*x.w <= 0` | `axis(q)` | 返回 `Vec3(T(0), T(0), T(1))`（z 轴单位向量，典型场景：单位四元数 `(0, 0, 0, 1)` 即 `\|w\| >= 1`；与 GLM `ext/quaternion_trigonometric.inl:20-27` 一致；：v3 描述「零四元数返回 `Vec3(1, 0, 0)`」错误，触发条件为 `1 - w² <= 0` 而非 `q.xyz == 0`；：`tmp1 = T(1) - x.w*x.w` 计算公式参见 §3.9 `axis` 函数描述） |
| 真正零四元数 `(0,0,0,0)` | `axis(q)` | `tmp1 = 1 > 0` 进入 else 分支，返回 `Vec3(T(0), T(0), T(0))`（：v3 描述「内部 `normalize(Vec3(0,0,0))` 返回 `Vec3(1,0,0)`」为虚构实现，GLM 实际不调用 `normalize`） |
| **零四元数 `(0,0,0,0)`（v11 新增，Issue 2 响应，）** | `normalize(q)` | **`tmp1 = length(q) = 0 <= T(0)` 触发 GLM 零保护分支**，返回**单位四元数** `Quat(T(Float64(0)), T(Float64(0)), T(Float64(0)), T(Float64(1)))`（：T(0) 原策略「通过 `one - one` 演算」在 normalize 签名（不含 `one: T` 形参）中不可用，统一修订为 `T(Float64(0))` 字面量替代路径；T(1) 采用 `T(Float64(1))` 字面量替代路径；与 §1 系统性约束 v14 修订一致）；与 GLM `ext/quaternion_geometric.inl:17-24` 实际行为完全一致——下游按 v10 描述「内部调用 `length`」字面实现时零四元数除零产生 NaN 分量与本契约不符 |
...
| 零四元数（整数） | `inverse(q)` | **编译期拒绝**（v29 修订，D38 响应：`where T <: FloatingPoint<T>` 约束收紧后整型 T 不再可编译）；原行为「触发仓颉 `ArithmeticException`」因约束收紧不再适用 |
...
| 整型 T | `mat3Cast`/`mat4Cast`/`quatCast` | 编译期约束收紧为 `where T <: FloatingPoint<T>`，整型 T 实例化时编译失败（v5 新增，v7 接口名修订；与 D29 `isnan`/`isinf` 策略一致，对应 GLM `GLM_STATIC_ASSERT(is_iec559, ...)`；——本条目对应 §3.13.2 审计表「`mat3Cast`/`mat4Cast`/`quatCast`」行的「整型 T 实例化时编译失败」契约反向引用） |
| u, v 反平行（`dot(u,v) ≈ -sqrt(dot(u,u) * dot(v,v))`，即 `dot ≈ -1` 经归一化后的等效条件，：v8 描述「`dot ≈ -1`」不准确——GLM 实际触发条件为 `real_part < 1e-6f * norm_u_norm_v`） | `fromVec3` | 返回 `q.w = 0` 的纯向量四元数（：v8 描述「`q.w ≈ 0`」准确但不够精确，GLM 实际为 `real_part = static_cast<T>(0)`），轴选择由 `abs(u.x) > abs(u.z)` 条件决定（：v8 描述「任意轴」不准确——GLM 实际是「与 u 垂直的两条可能轴之一」，`abs(u.x) > abs(u.z)` 选 `(-u.y, u.x, 0)` 再归一化，否则选 `(0, -u.z, u.y)` 再归一化；：选出的轴还需经过 `normalize(t)` 归一化确保返回纯向量四元数模长为 1）；v8 新增契约，行为已定义，与 GLM `detail/type_quat.inl:196-217`（：v8 错误引用为 `ext/quaternion_common.inl:196-217`）退化路径一致；阶段三 stub 阶段不适用，待阶段四 stub 替换后生效；——本条目对应 §3.13.2「本阶段实现但运行时受 stub 依赖影响」审计表的「`fromVec3` stub 行」反向引用 |
...
| Vec3×Quat / Vec4×Quat 运行时行为（v30 新增，Problem 1 响应） | `Vec3×Quat` / `Vec4×Quat` | **本阶段编译通过但运行时抛 `Exception("stub")`**。实现链为 `(conjugate(q) / dot(q,q)) * v`，其中 `*` 运算符重载委托至同包 `Quat×Vec3` 运算符（依赖 `geometric.cj` 向量 `cross` stub），运行时异常传播路径完全一致。**非内联 `conjugate`/`dot` 路径的纯算术可用性误导**——内联仅消除包间 import 依赖，未消除对 Vec3 cross stub 的运行时调用 |
| 跨 Qualifier 调用（Q1 ≠ Q2），v29 新增，Problem 5 响应 | 全体泛型函数与运算符（§3.4运算符表、§3.7 dot/length/normalize/cross、§3.9 axis、§3.11 inverse/conjugate/isnan/isinf） | 编译期报类型不匹配错误（函数签名仅接受单一 Q 参数，Q1 ≠ Q2 时编译器无法推断一致类型）。调用方应通过跨 Qualifier 构造函数 `Quat<T,Q2>(q_of_Q1)`（§3.3 item 3）显式转换后再调用。详见 §5.6 跨 Qualifier 行为统一契约 |
| 跨 Qualifier 调用：下调精度（Highp → Mediump/Lowp），v29 新增 | 同上 | 显式转换构造不会引发精度损失错误（跨 Qualifier 构造函数为纯赋值），但下调精度可能导致后续浮点运算的精度损失。本阶段不提供精度损失告警 |
| 跨 Qualifier 调用：上调精度（Lowp → Mediump/Highp），v29 新增 | 同上 | 上调精度后浮点运算精度提升，不会出现精度损失。与 GLM 精度提升语义一致 |
...
- **`conjugate` const 适用性（v10 新增，§3.11 补充对齐）**：`conjugate` 函数体仅涉及 `Quat` 主构造函数的逐分量取反调用（`Quat(-q.x, -q.y, -q.z, q.w)`），无 `assert`/无 `throw`/无 `dot`/`normalize`/其他非 const 函数调用，**可声明为 `const func`**（与 Vec/Mat 的逐分量运算符策略一致）。：若仓颉 const 函数还有其他限制（如不能调用非 const 自由函数、不能包含复杂表达式），则该论断需进一步评估
...
| 编号 | 决策 | 理由 |
|------|------|------|
| D01 | Quat 数据布局为 x,y,z,w（非 GLM 的 w,x,y,z 参数顺序） | 与 GLM 默认存储布局一致，避免 GLM_FORCE_QUAT_DATA_WXYZ 宏引入的条件编译复杂性 || D02 | Quat 主构造函数参数顺序为 (x,y,z,w)，另提供 wxyz(w,x,y,z) 工厂函数 | 主构造函数参数顺序与数据成员声明顺序一致（仓颉惯用风格），wxyz 函数兼容 GLM 习惯 || D03 | 四元数×向量运算符定义为 Quat 的成员运算符 | 左操作数类型拥有运算符，与阶段二 Mat×Vec 模式一致 || D04 | 向量×四元数运算符定义为 Vec 的 extend 块成员运算符 | 左操作数类型拥有运算符，依赖同包前向引用延迟解析（已通过阶段二原型验证） || D05 | 标量-四元数运算通过全局函数（scalar_quat_ops.cj） | 仓颉 operator func 限制——右操作数非 Quat 类型的标量左操作数不能定义为成员运算符 || D06 | ext/vector_relational 的 epsilon 比较内联 abs，不依赖 common.cj stub | 消除对 stub 文件的运行时依赖风险，确保 epsilon 比较函数可正常执行 || D07 | ULP 比较函数本阶段以空桩占位 | 仓颉无浮点位表示直接访问能力（无 `reinterpret_cast`/`union` 等价机制），待阶段四评估替代方案 || D08 | scalar_constants 实现于 glm.detail.scalar_constants.cj，ext/ 仅做重导出 | 遵循 detail 封装核心实现、ext/gtc 封装公共 API 的双层架构约定 || D09 | scalar_constants 的泛型函数使用 `match` 类型模式匹配 + hint 参数 | 仓颉无 `std::numeric_limits<T>::epsilon()` 等编译期类型属性查询机制，必须运行时分派 || D10 | gtc/constants 中 **28 个**常量函数使用具体类型硬编码值直接返回（v8 修订：v7 误标为 25） | 与 GLM `genType(3.14159...)` 实现方式一致，避免函数间调用依赖 || D11 | ：转换函数实现位于 `glm.detail.type_quat_cast.cj`，`gtc/quaternion.cj` 通过 `public import` 重导出；gtc 端重导出函数名为 camelCase `mat3Cast`/`mat4Cast`/`quatCast`（与 detail 端原始函数名一致），**非** GLM 习惯的 snake_case `mat3_cast`/`mat4_cast`/`quat_cast` | **消除包间循环依赖**（v3 核心理由）：v2 设计将转换函数放在 gtc 导致 `type_quat.cj` 需 `import glm.gtc.quaternion` 形成 `glm.detail ↔ glm.gtc` 双向依赖，违反仓颉包间循环依赖约束（cangjie-lang-features package/README.md 第 99 行）。v3 决策将转换函数下沉至 detail 包，让 `type_quat.cj` 通过同包访问直接调用（无需 import），`gtc/quaternion.cj` 通过 `public import` 重导出至 gtc 命名空间，保留 GLM 1:1 API 形态的同时彻底打破循环依赖。：仓颉 `public import a.b.f` 仅重导出 f 原始名（依据 `cangjie-lang-features/package/README.md` 第 156-166 行规范），不做命名转换；detail 端原始函数名为 camelCase `mat3Cast`/`mat4Cast`/`quatCast`，因此 gtc 端重导出后仍为 camelCase——下游按 GLM 习惯使用 snake_case `mat3_cast`/`mat4_cast`/`quat_cast` 调用 `glm.gtc.quaternion.mat3_cast(q)` 将编译失败。该偏差属「API 命名风格」类差异，与现有 `public import glm.detail.{transpose, matrixCompMult, outerProduct}`（`cjglm/src/lib.cj:8`，camelCase）模式一致 || D12 | type_quat.inl 中 fromVec3(u,v) 和 fromEuler(eulerAngle) 构造函数本阶段化简为独立工厂函数 | 避免在 Quat 结构体内引入对 stub 函数的编译依赖；工厂函数在 extend 块中定义，可推迟至阶段四完整实现 || D13 | quatLookAt/quatLookAtRH/quatLookAtLH 等 gtc/quaternion 函数本阶段 stub 占位 | 这些函数依赖 geometric.cj 的 cross/dot/normalize/inversesqrt（均为 stub），无法完整实现 || D14 | quaternion_geometric.cj（dot/length/normalize/cross）本阶段完整实现 | 经分析确认依赖链仅止于 std.math.sqrt，不依赖任何 stub 文件 || D15 | quaternion_common.cj 中 conjugate/inverse/lerp/isnan/isinf 本阶段完整实现 | conjugate/inverse/lerp 仅需算术运算和 dot（已完整实现）；isnan/isinf 采用 std.math 实例方法路径，不依赖顶层函数 || D16 | trigonometric.cj 新增为空桩文件 | type_quat.inl 依赖 trigonometric.hpp，本阶段仅提供签名空壳供依赖闭合 || D17 | Bool 四元数不支持算术运算 | Bool 不实现 Number<T> 接口，与阶段二 D33 Bool 矩阵策略一致 || D18 | scalar_quat_ops.cj 与 scalar_vec_ops.cj/scalar_mat_ops.cj 同名重载 | add/sub/mul/div 与向量/矩阵版本通过第二参数类型消歧，与阶段二策略一致 || D19 | 四元数算术运算符统一标注 @OverflowWrapping | 与阶段一/二所有算术运算符（含浮点重载）保持一致；标注对浮点无效果但保持跨整数/浮点实例化的统一行为 || D20 | **`gtc/quaternion.cj` 独立文件承载 15 函数（v3 明确：4 重导出 + 4 完整 + 7 stub）** | 与 GLM `gtc/quaternion.hpp` 1:1 映射，便于 GLM 函数到仓颉文件的追溯；与阶段二 gtc 子包策略一致 || D21 | **`mix`/`slerp`/`pow` 的依赖明确包含 `epsilon<T>()`（v3 新增）；`pow` 进一步包含 `cos_one_over_two<T>()`、`asin`、调用 `std.math.pow` 实数降级路径两次、`std::numeric_limits<T>::min()` 等价物（v5 强化，v7 接口名修订，：完整依赖清单及仓颉等价翻译路径见 §3.10 `pow` 函数依赖描述段；`FloatingPoint<T>` 接口静态方法可用性见 §1「回退方案」子节）** | 与 GLM `quaternion_common.inl`/`quaternion_exponential.inl:41-80` 实际依赖一致。本决策仅陈述最终结论——`mix`/`slerp` 依赖 `cosTheta > 1 - epsilon<T>()` 退化检测分支；`pow` 使用 line 52 `cos_one_over_two<T>()`、line 63 `std::numeric_limits<T>::min()` 等价物、line 65 + line 78 两次 `std::pow` 实数降级路径、line 68 `asin`。具体仓颉实现路径（含 `std.math.pow` 重载选择、`FloatingPoint<T>.getMinDenormal()`/`getInf()` 静态方法、line 65/78 仓颉等价翻译等）已统一归入 §3.10 和 §1，本决策不再重复。下游实现者查阅 §3.10 `pow` 依赖描述 + §1 系统性约束段即可。 || D22 | **`slerp` 4 参数版本 `k: Int64` 简化签名（v3 新增）** | 与 deviations.md DV-03「移位运算右操作数固定为 Int64」风格一致；阶段三 slerp 为 stub，固定 `Int64` 简化签名不影响本阶段实施 || D23 | **`lerp` 实现含 `assert(a >= 0 && a <= 1)` 断言（v3 新增）** | 与 GLM `ext/quaternion_common.inl:28-38` 一致，确保插值因子在 [0, 1] 范围内 || D24 | **向量 `equal` epsilon 比较采用严格 `<` 语义（v3 新增，v9 修订 GLM 文件引用）** | 与 GLM `glm/gtc/epsilon.inl:32-41` 中 `epsilonEqual` 实现一致，避免边界用例下 `equal(v, v, 0) = true` 的语义偏差。原引用 `func_vector_relational.inl:18-22` 错误——GLM 1.0.3 的 `func_vector_relational.inl` 中**不包含 epsilon 版本**的 `equal`/`notEqual`，epsilon 版本位于 `glm/gtc/epsilon.inl`（文件层面属 gtc 子包） || D25 | **`scalar_constants` 对整数类型 T 触发运行时异常（v3 新增）** | 与 GLM `static_assert(is_iec559, ...)` 编译期断言等价行为（仓颉泛型若不支持窄约束则用运行时异常） || D26 | **`Quat×Vec3` 旋转公式采用两次 Vec3 叉乘（v3 新增）** | 与 GLM `type_quat.inl:359-366` 实际实现一致：`v + (cross(QuatVector, v) * q.w + cross(QuatVector, cross(QuatVector, v))) * 2`（uv 和 uuv 两次叉乘） || D27 | **fwd.cj 排除整型与 Bool 四元数别名（v3 明确，v8 修订先例引用）** | ：v7 文档引用阶段二「fwd.cj 排除整型矩阵别名」作为唯一先例，但该先例存在两处不准确：① 阶段一 Vec 家族**包含** `BVec2`（Bool 向量，见 `cjglm/src/fwd.cj:140-143`）的 Bool 别名，Vec 家族对 Bool 类型的处理策略与 Mat 家族不同；② 阶段二 Mat 家族明确不含整型别名（`cjglm/src/fwd.cj` 全文 grep 验证零整型别名），阶段一 Vec 家族对整型的处理（无 `IVec*`）与 Mat 家族策略一致。：fwd.cj 排除整型与 Bool 四元数别名的依据是——**整型四元数无实际用例**（quaternion 自然语义是旋转表达，整型四元数不参与实际数学运算）+ **Bool 四元数无实际用途**（D17：Bool 不实现 `Number<T>` 接口，算术运算编译期拒绝，无实用价值）。：阶段三所有自定义类型族（Vec/Mat/Quat）均排除整型别名（语义不适用）+ 选择性排除 Bool 别名（Vec 家族保留 `BVec*` 因位运算是核心用例，Mat 家族不含 `BMat*` 因矩阵不参与位运算，Quat 家族不含 `BQuat*` 因四元数不参与位运算且算术不可用）。下游可参考本策略理解「哪些类型族为何种用途排除何种别名」。 || D28 | ：包间依赖方向严格单向——`glm.gtc → glm.detail`、`glm.ext → glm.detail`、上层不依赖下层 | 仓颉 cjpm 构建系统严格禁止包间循环依赖（cangjie-lang-features package/README.md 第 99 行）；D11 的 `type_quat_cast.cj` 下沉决策是本约束的直接体现 || D29 | **`isnan`/`isinf` 函数约束收紧为 `where T <: FloatingPoint<T>`（v4 修订，v7 接口名修订）** | `std.math` 的 `isNaN()`/`isInf()` 实例方法仅定义于浮点类型（`FloatingPoint<T>` 接口，stdlib 原生接口名），整数类型（`Int8`/`Int16`/`Int32`/`Int64`）无此方法。若不收紧约束，当用户实例化 `Quat<Int64, PackedHighp>` 并调用 `isnan(q)` 时，函数体内部 `q.x.isNaN()` 因 `Int64` 不实现 `isNaN()` 而编译失败。与 §3.12 `epsilon<T>()` 约束收紧策略（v3 已明确）保持一致；`FloatingPoint<T>` 是 stdlib 原生接口，编译期保证可用，无需 fallback || D30 | **Quat 字段统一标注 `public var`（v4 修订）** | 满足 `@Derive[Hashable]` 派生宏对字段 `public` 可见性的硬性要求（依据 `cangjie-std/deriving/README.md` 第 4 节「参与派生的字段/属性必须为 public」）。仓颉 struct 字段默认可见性为 `internal`（依据 `struct/README.md` 第 1.7 节），不满足 public 派生要求；与阶段一 Vec3 (`type_vec3.cj:8-10`)、阶段二全部矩阵类型 (`type_mat*.cj` 200+ 处统一实践) 对齐 || D31 | **`axis` 函数实现采用 GLM 独立公式路径而非 `normalize` 路径（v5 关键修订）** | GLM `ext/quaternion_trigonometric.inl:20-27` 实际实现是 `tmp1 = 1 - x.w*x.w` 独立公式，不调用 `normalize(Vec3(q.x, q.y, q.z))`。v3 描述的「内部 `normalize(Vec3(0, 0, 0))`」为虚构实现——若按 v3 描述实现，则需依赖 `geometric.cj` 的向量 `normalize`（阶段三为 stub），与同段「可完整实现」声明矛盾。v5 修订后：`axis` 依赖仅 `std.math.sqrt` 和 T(1) 演算，触发 `Vec3(0, 0, 1)` 返回值的条件为 `1 - x.w*x.w <= 0`（典型：单位四元数 (0, 0, 0, 1)），对于真正零四元数 (0, 0, 0, 0) 返回 `Vec3(0, 0, 0)` || D32 | **`type_quat_cast` 函数约束收紧为 `where T <: FloatingPoint<T>`（v5 新增，v7 接口名修订）** | 与 D29 `isnan`/`isinf` 约束收紧策略一致。GLM 1.0.3 在转换函数定义处使用 `GLM_STATIC_ASSERT(std::numeric_limits<T>::is_iec559, ...)` 静态断言非浮点类型时编译失败，仓颉版本通过 `FloatingPoint<T>` 接口约束（stdlib 原生接口）实现等价行为。`mat3Cast`/`mat4Cast`/`quatCast` 两个重载共 4 个函数统一采用此约束；`FloatingPoint<T>` 是 stdlib 原生接口，编译期保证可用，无需 fallback || D33 | **比较函数（`lessThan`/`lessThanEqual`/`greaterThan`/`greaterThanEqual`）采用 `Comparable<T>` 宽约束而非 `FloatingPoint<T>` 窄约束（v14 新增，Issue 6 响应）** | 理由：(a) GLM 原始实现中 4 个比较函数无 `GLM_STATIC_ASSERT(is_iec559)` 断言，整数类型也可合法调用——与 `mat3Cast`/`mat4Cast`/`quatCast` 等带 `is_iec559` 断言的函数有别；(b) 比较运算符 `<`/`<=`/`>`/`>=` 对整数类型语义正确（整数四元数按分量大小比较有实际意义），收紧至 `FloatingPoint<T>` 会排除整数四元数的合法比较用例；(c) 与需浮点运算的 `mat3Cast`/`quatCast` 场景区分——后者涉及 `sqrt`/`trace` 等仅在浮点类型上有意义的运算，而比较仅依赖 `<`/`<=`/`>`/`>=` 运算符，`Comparable<T>` 约束已足够保证运算符可用性；与 §3.15 完整实现函数段/§11.5 函数可用性对照表 `where T <: Comparable<T>` 声明一致 || D34 | **`lerp`/`inverse` 跨 Qualifier 调用的处理策略（v15 新增）** | 本阶段仅提供相同 Q 的签名模板。跨 Qualifier 场景通过双 Qualifier 重载扩展实现：新增 `lerp<T, Q1, Q2>(x: Quat<T,Q1>, y: Quat<T,Q2>, a: T): Quat<T,Q2>` where T <: Number<T>, Q1 <: Qualifier, Q2 <: Qualifier——内部调用跨 Qualifier 构造函数 `init<Q2>(q: Quat<T,Q2>)`（§3.3 item 3）将 `x` 隐式转换为 `y` 的 Qualifier，再执行同一 Q 下的 `lerp` 运算。理由：(a) 仓颉跨 Qualifier 构造函数已实现（§3.3 item 3），转换成本为纯赋值无运行时开销；(b) 输出 Q 取 `Q2`（与 `y` 一致）符合「右操作数主导 Qualifier」策略，与 GLM 精度提升/降级规则一致；(c) 该重载在阶段四按需实现，不阻塞本阶段已有的同 Q 调用路径 || D35 | **`slerp` 路线图可验证性冲突的处理策略（v16 新增，Issue 3 响应）** | 路线图 §3 第 125 行标记 `slerp` 为 `[可验证]`，但本设计定位为 `❌ stub`（依赖 trigonometric.cj/common.cj stub）。**选定方案**：维持本设计定位（路径 C），`slerp` 标记为 `[待 Stage 4]`。理由：(a) 对阶段三整体交付范围影响最小——`slerp` 的 stub 状态已在 §11.5 明确标注，不阻塞其他可验证函数的实现与测试；(b) 存在向下兼容的降级路径（路径 A：`lerp` 近似，1-2 人日切换成本）；(c) 若后续需阶段三有球面插值，路径 B（有限精度三角函数内联，3-5 人日）可作为备选。详见 §3.16 不一致 1 的临时实施建议与工期影响评估。 || D36 | **`lookRotate` 命名不一致的处理策略（v16 新增，Issue 3 响应）** | 路线图多处引用 `lookRotate` 函数名，但本设计已统一为 `quatLookAt`/`quatLookAtRH`/`quatLookAtLH`（D13 决策）。**选定方案**：维持本设计命名，三个函数均为 stub（§3.15），命名不影响阶段三实施。若路线图后续确认保留 `lookRotate`，可在阶段四新增 `lookRotate` 作为 `quatLookAtRH` 的委托别名（0.5 人日成本）。详见 §3.16 不一致 2 的临时实施建议与工期影响评估。 || D37 | **`ext/quaternion_common.cj` 路线图可验证性范围过广的处理策略（v16 新增，Issue 3 响应）** | 路线图 §3 第 130 行将 `ext/quaternion_common.cj` 整体标记为 `[可验证]`，未排除 `mix`/`slerp` 的 stub 部分。**选定方案**：将路线图标注从 `[可验证]` 修订为 `[部分可验证]`，函数级明细引用本设计 §11.5。理由：§3.13.2 审计表已明确列示 5 个可调用函数与 3 个 stub 函数的完整清单，粗粒度 `[可验证]` 标注与实际函数级状态不匹配。详见 §3.16 不一致 3 的临时实施建议与工期影响评估。 || D38 | **`inverse` 整数 T 非零除截断行为的约束策略（v29 新增，Problem 3 响应）** | **选定方案：方案 A——约束收紧为 `where T <: FloatingPoint<T>`（与 `normalize` 的 v26 收紧策略一致）**。理由：(a) 整数除法截断在 `inverse` 中产生无意义结果——如 `Quat(1,0,0,0)` 在整数 T 下 `dot=1`，`inverse` 返回 `Quat(-1,0,0,0)/1 = Quat(-1,0,0,0)`（可接受）；但 `Quat(1,1,1,1)` 在整数 T 下 `dot=4`，`inverse` 返回 `Quat(-1,-1,-1,1)/4`，整数截断得 `Quat(0,0,0,0)`——无声返回错误结果，与 `normalize` 整数 T 截断为 `Quat(0,0,0,1)` 的问题模式一致；(b) `inverse` 的数学定义（`q⁻¹ = q̅ / |q|²`）在整数域下的除法截断无数学意义，与 `normalize` 的整数截断问题属同一类"浮点语义强依赖"场景；(c) 一致性理由：`normalize`（v26 收紧）、`mat3Cast`/`mat4Cast`/`quatCast`（D32）、`isnan`/`isinf`（D29）、`trigonometric.cj`（v11 统一收紧）等已形成约束收紧的系列先例，`inverse` 收紧至 `FloatingPoint<T>` 与此系列一致，非孤立决策。**影响范围**：`inverse` 函数签名从 `where T <: Number<T>` 变为 `where T <: FloatingPoint<T>`；§3.11 `inverse` 描述段、§5.3 边界条件表「整数 `inverse`」行、§11.5 `inverse` 行状态标注同步更新。**整数 T 场景的降级路径**：若下游确需整数四元数的 `inverse`，可通过 `Quat<Float32, Q>` 实例化计算后再将结果分量取整，或自行实现整数版本（按分量计算 `conjugate(q) / dot(q,q)` 时使用浮点除法避免截断再取整）。 |
---

## 8. 阶段三产出物清单

：本节产出物分类统一调整为与 §11.5 函数可用性对照表对齐的三档分类——「✅ 可用（无 stub 依赖）」/「⚠️/❌ 混合（部分可用、部分 stub）」/「❌ stub（空桩占位）」。**全文档函数可用状态以 §11.5 为最终基准**。
...
- `fwd.cj`：新增 9 个四元数类型别名（Quat/FQuat/DQuat + 3×Float32 精度 + 3×Float64 精度；具体清单见 §2 lib.cj/fwd.cj 段落，v5 明确，：fwd.cj 是自动生成文件（`cjglm/src/fwd.cj:1-2` 文件头注释为中文而非 v12 描述的英文「Auto-generated file. Do not edit!」），下游实施路径为方案 A 修改 `cjglm/scripts/gen_fwd_aliases.py` 生成脚本（：v12 描述 `tools/gen_fwd.cj` 与实际不符），新增 `VEC_FAMILIES` 字典条目 + Quat 固定 4 维特殊处理分支；详见 §2 段落末尾「实施路径」说明）
...
| `tests/glm/test_ext_scalar_constants.cj` | 标量常量（v10 修订：同层平铺命名；：用例数按「完整实现函数：每函数 ≥2 个用例」原则计算——3 个泛型函数 × 2 种浮点类型 × 2 用例 + 1 整数类型异常路径 = 13） | ≥13 |
| `tests/glm/test_ext_quaternion_aliases.cj` | 四元数别名（4 个别名文件合并测试，v10 修订：避免为每个别名文件单独建测试文件，：每别名 ≥1 用例，9 个别名共 ≥9 用例） | ≥9 |
...
：v7 文档 13 个测试文件均使用 `xxx_test.cj` 命名，但与项目实际不一致——通过 `ls cjglm/tests/glm/detail/` 验证阶段二现有约定**全部使用** `test_xxx.cj` 风格（`test_common.cj`/`test_geometric.cj`/`test_matrix.cj`/`test_qualifier.cj`/`test_scalar_mat_ops.cj`/`test_scalar_vec_ops.cj`/`test_setup.cj` 等 10+ 个文件），阶段三测试文件统一调整为 `test_xxx.cj` 风格以与现有约定对齐。

**测试目录结构对齐策略（v10 新增，Issue 8 响应）**：v9 设计的测试文件位于 `tests/glm/ext/` 与 `tests/glm/gtc/` 子目录中（与 `src/ext/` + `src/gtc/` 子包目录结构对齐），但**项目当前无 `tests/glm/ext/` 与 `tests/glm/gtc/` 子目录先例**——阶段二 25 个测试文件均位于 `tests/glm/` 根目录（如 `test_ext.cj`）或 `tests/glm/detail/` 子目录；`test_ext.cj` 是单一聚合文件而非按函数拆分。：
...
：v9 设计的 `tests/glm/ext/test_xxx.cj` 与 `tests/glm/gtc/test_xxx.cj` 命名方案修订为上表所示的同层平铺命名；gtc 子目录保留但仅用于阶段三新增的 gtc 测试文件（与 src/gtc/ 子包源码位置镜像），不引入更深的子目录层级。
...
| | Vec3×Quat/Vec4×Quat（2 个 ⚠️） | 2 | 2（1 编译 + 1 assertThrows） | 4 | ⚠️ 阻塞函数 ≥1编译 + ≥1运行时（v30 修订：从 ✅ 修正为 ⚠️，因实现链依赖 Quat×Vec3 的 geometric.cj cross stub） |
...
：上表中 `test_ext_quaternion_trigonometric.cj`（4 ≤ 8）与 `test_ext_quaternion_common.cj`（13 ≤ 16）的分组合计略低于各自文件中声明的 ≥下限。下游编码者可依据「每函数 ≥1 额外边界用例」原则补齐差值（如为 `trigonometric` 的 2 个 stub 各增加 1 个不同参数类型边界用例 + 为 `common` 的 `isnan`/`isinf` 各增加 1 个跨 Qualifier 用例），确保各文件实际用例数不低于表中原声明的 ≥值。全表 ≥199 合计值通过上述补齐可达。
...
**stub 抽样策略的函数选择（v27 新增，Problem 4 响应）**：对 `gtc/matrix_transform.cj` 的 64 个 stub 函数，按聚类结果选择的代表性函数如下：
...
#### 合计值校验方法（v27 新增，Problem 10 响应）

§8.2 测试用例合计值（当前 ≥192）的验算方式如下：

```
逐项求和：40 (test_type_quat) + 8 (type_quat_cast) + 8 (quat_relational) + 12 (quat_geometric) + 13 (quat_common) + 4 (quat_trigonometric) + 4 (quat_exponential) + 2 (quat_transform) + 16 (vector_relational) + 13 (scalar_constants) + 9 (quat_aliases) + 28 (constants) + 27 (gtc_quaternion) + 8 (matrix_transform_stubs) = 192
```
...
16. **`pow` 函数次正规数边界检查所需 `FloatingPoint<T>.getMinDenormal()` 静态方法路径验证（v5 新增，v7 接口名修订，）**：依据 `cangjie-original-docs/std/math/math_package_api/math_package_interfaces.md` 第 49-60 行定义，仓颉 stdlib **实际提供** `FloatingPoint<T>.getMinDenormal()` 静态方法用于获取最小次正规数；下游实现 `pow` 函数体内部次正规数边界检查时，应直接调用 `FloatingPoint<T>.getMinDenormal()`（推荐路径），或 fallback 到类型分派 `if (q.x is Float32) { Float32.Min } else { Float64.Min }` 路径，或字面量 fallback 路径（如 `T(1) / T(很大值)`）。验证项应验证「`FloatingPoint<T>.getMinDenormal()` 静态方法 + 类型分派 + 字面量 fallback 三条路径的编译可行性」
...
18. **`log` 函数 `FloatingPoint<T>.getInf()` 静态方法路径验证（v5 新增，v7 接口名修订，）**：依据 `cangjie-original-docs/std/math/math_package_api/math_package_interfaces.md` 第 37-48 行定义，仓颉 stdlib **实际提供** `FloatingPoint<T>.getInf()` 静态方法用于获取正无穷；下游实现 `log` 函数体内部 w=0 退化分支时，应直接调用 `FloatingPoint<T>.getInf()`（推荐路径），或 fallback 到类型分派 `if (q.x is Float32) { Float32.Inf } else { Float64.Inf }` 路径，或字面量 fallback 路径 `T(1)/T(0)`（仅对浮点类型有效，整数类型 `1/0` 触发除零异常，与 GLM 行为一致）。验证项应验证「`FloatingPoint<T>.getInf()` 静态方法 + 类型分派 + 字面量 fallback 三条路径的编译可行性」
...
23. **`fwd.cj` 自动生成脚本验证（v12 关键修订，Issue 2 响应，）**：验证 `cjglm/scripts/gen_fwd_aliases.py` 生成脚本（：v12 描述查阅 `cjglm/tools/` 目录——`cjglm/` 下不存在 `tools/` 目录，实际生成脚本为 Python 64 行位于 `scripts/`）是否能正确生成 9 个四元数 type alias——具体步骤：(a) 查阅 `cjglm/scripts/gen_fwd_aliases.py` 确认生成脚本存在（：v12 描述查阅 `cjglm/tools/` 修订为 `cjglm/scripts/`）；(b) 在脚本的 `VEC_FAMILIES` 字典中新增 `Quat`/`FQuat`/`DQuat` 三个家族前缀条目（如 `Quat`/`FQuat` 映射到 `Float32`，`DQuat` 映射到 `Float64`），并新增针对 Quat 家族固定 4 维（无 Vec1/Vec2/Vec3 变体）的特殊处理分支——因 Quat 仅支持 4 维而现有脚本按 `DIMS = [1, 2, 3, 4]` 全维度循环生成 Vec 别名，简单新增 `VEC_FAMILIES` 条目会生成 `Quat1`/`Quat2`/`Quat3` 三个不存在项；(c) 运行脚本 `python cjglm/scripts/gen_fwd_aliases.py` 生成新的 `fwd.cj` 后再提交；(d) 验证生成的 `fwd.cj` 包含全部 9 个 type alias 且与 §3.14 清单一致；(e) 验证脚本支持幂等运行（多次执行结果一致）
...
27. **Mat4x4 列字段名 `c0`/`c1`/`c2`/`c3` 存在性验证（v21 核验确认——v19 修订 Issue 6 响应，本轮升级为「已核验确认」）**：——经查阅阶段二 `type_mat4x4.cj:8-11`，列字段名确为 `c0`/`c1`/`c2`/`c3`，设计假设已验证为正确。本验证项状态从「验证假设」升级为「已核验确认」。验证结果：`Mat4x4<T,Q>` 列字段命名为 `c0`/`c1`/`c2`/`c3`，§3.3 item 7 `fromMat4` 手动提取策略中 `m.c0`/`m.c1`/`m.c2` 引用正确，无需修正。**说明**：验证项 27 的假设核验已在本轮设计阶段由设计方完成，编码者无需重复核验——可直接按 §3.3 item 7 的描述字面实现 `Vec3<T,Q>(m.c0.x, m.c0.y, m.c0.z)` 等表达式。
...
#### 验证失败后受影响章节清单（v18 新增，Issue 4 响应；）
...
| **P0 通过条件**：3 项 P0（20/25/29）全部标注「通过」后方可启动 stage 3 编码；任一项失败按失败后处置路径触发设计修订。**当前状态：3 项 P0 全部 ✅ 通过（v28 更新）**。 |
...
- **核验项 H2b（§11.5 vs §9a 覆盖矩阵交叉验证）**：§11.5 逐函数表中 ✅/⚠️/❌ 计数应分别与 §9a 覆盖矩阵中 ✅/⚠️/❌ 的文件级计数之和一致。验证方式：手动或脚本化核对 §11.5 表后合计行的 ✅ 计数、⚠️ 计数、❌ 计数分别与 §9a 覆盖矩阵合计行的对应计数一致。确认独立函数范式口径（✅ ~18 个）与含重载展开口径（✅ ~77 个）的差异来源已在 §11.5 计数口径映射注记中明确列明。核验周期：每轮版本发布前执行一次。
- **核验项 H3（v23 新增，Issue 3 响应；v27 扩展，Problem 7 响应）**：版本号一致性自动化核验——要求在每轮版本发布前用 `grep -n 'v[0-9]\+.*修订\|v[0-9]\+.*新增\|v[0-9]\+.*决策'` 检查所有标注版本号与当前文件版本号的一致性。核验范围扩展为"文件名 + 头部 + 正文"三方一致性验证——文件名标注 vN、文档头部修订定位标注 vN、正文中指向当前设计的版本引用统一为 vN。
- **核验项 H4（v27 新增，Problem 2 响应）**：修订说明中声明的修改措施必须有正文修改位置标记。每轮修订说明中每项"修改措施"列应注明对应的正文§节/段/行号，或标注"正文已修改"确认标记。验证方式：逐条比照修订说明中修改措施与正文对应位置，确认正文已实际修改（非仅修订说明表中有声明而正文未动）。H4 与 H1/H2/H3 共同作为版本发布前置门禁。
- **核验项 H5（v30 新增，Problem 2 响应）**：版本标注自动化门禁。要求在每轮版本发布流程中增加自动化检查步骤：执行 `grep -n "\*\*v[0-9]" *.md` 扫描非决策性版本标注（即 §7 决策表的 D01~D38 版本号之外的所有 `**v{N}` 标记）。匹配结果应为空或仅含 §7 决策表 + §3.13.2/§8/§11.5 中明确保留的少量计数解释性标注。若发现非决策性残留标注，阻止版本发布直至全部清理。此门禁与 H3（文件名/头部/正文三方一致性）联合执行。
- **核验项 H6（v31 新增）**：运行时调用链无 stub 依赖验证门禁。所有标注为 ⚠️ 或 ✅ 的函数，在编码实施时必须在函数实现描述段中附带「运行时调用链依赖分析」子段，明确列出函数体内部的所有运行时调用链（包括间接调用），标注每级调用是否依赖 stub。验证方式：逐函数审查 §3.x 描述段，确认对每个 ⚠️/✅ 函数的运行时调用链分析完备；对遗漏分析子段的函数阻止版本发布。此门禁确保系统化的运行时依赖分析成为版本发布前置门禁，避免再次出现 Vec3×Quat 状态错误跨越 12 轮的流程性缺陷。
- **核验项 H7（v32 新增，a_v32 审查响应）**：版本标注自动化 grep 门禁。在每轮版本发布流程中增加自动化检查步骤：执行 `grep -n "（v[0-9]\|（Issue [0-9]\|\\*\\*v[0-9]" *.md` 扫描非决策性版本标注。匹配结果应为空或仅含 §7 决策表（D01~D38 版本号）+ §3.13.2/§8/§11.5 中已明确保留的计数解释性标注。若发现非决策性残留标注，阻止版本发布直至全部清理。此门禁与 H3（文件名/头部/正文三方一致性）和 H5 联合执行，从流程层面实现版本标注的零残留。(grep 窗口期：在 `git commit` 前执行，确保版本发布前完成检查。)
...
| O-02 | §9a 与 §10 两张覆盖矩阵已合并为单一覆盖矩阵（v25 修订），本项✅已闭环 | v23 | ✅ 已闭环 | — |
| O-03 | δ 覆盖矩阵合计的 H1 核验尚未绑定自动化脚本 | v23 | 待评估 | v28 |
| O-04 | 编码启动前验证流程尚未与 CI/CD 管线集成 | v23 | 待评估 | v28 |
| O-05 | H1/H2/H3/H4 核验项尚未脚本化 | v25 | 待评估 | v28 |
| O-06 | 验证前置化流程（核心假设在设计版本固化前完成验证）尚未制度化 | v25 | **v32 已实施**（O-06 可执行落地计划已于 v28 建立；v32 确认所有 P0 假设（验证项 20/25/29）已在设计版本固化前完成编译验证，验证结果记录于 §8 验证结果记录表） | v32（已闭环） |
...
| **里程碑** | M1（v28 版本输出前）：验证项 20/25/29 的 v28 重新核验通过 → M2（v29 版本输出前）：O-06 流程制度化文档完成 → M3（v30 版本输出前）：CI/CD 管线集成验证步骤 |
...
| **`gtc/quaternion.cj` 独立承载 15 函数（v3 明确：4 重导出 + 4 完整 + 7 stub）** | 与 GLM `gtc/quaternion.hpp` 1:1 映射；：`mat3Cast`/`mat4Cast`/`quatCast` 不在本文件实现，而是通过 `public import glm.detail.{mat3Cast, mat4Cast, quatCast}` 重导出 detail 中的实现，：重导出函数采用 camelCase 命名（与 detail 端原始名一致），而非 GLM 原始 snake_case `mat3_cast`/`mat4_cast`/`quat_cast` |
| **`mat3Cast`/`mat4Cast`/`quatCast` 实际实现位于 `detail/type_quat_cast.cj`（v3 关键决策，v12 修订：camelCase 命名）** | 为避免包间循环依赖，转换函数下沉至 detail 包；gtc 包通过 `public import` 重导出至 gtc 命名空间；：detail 端与 gtc 端函数名统一为 camelCase `mat3Cast`/`mat4Cast`/`quatCast`，下游按 GLM 习惯使用 snake_case `mat3_cast`/`mat4_cast`/`quat_cast` 时将编译失败——仓颉 `public import a.b.f` 仅重导出 f 原始名（依据 `cangjie-lang-features/package/README.md` 第 156-166 行规范），不做命名转换；与现有 `public import glm.detail.{transpose, matrixCompMult, outerProduct}` 模式（`cjglm/src/lib.cj:8`）保持 camelCase 风格一致 |
...
| **`Quat`/`FQuat`/`DQuat` 三重别名机制（v3 新增，v8 修订先例引用）** | ：v7 文档引用阶段二 `Mat2x2`/`FMat2x2` 双别名机制作为先例，但通过直接查阅 `cjglm/src/fwd.cj` 全文确认 `Mat` 家族**不存在** `FMat*` 任何声明（仅 `Mat2x2` + `DMat2x2`），`F` 前缀别名**仅在 Vec 家族存在**（`FVec1`~`FVec4` + 精度变体）。本设计修订为参考阶段一 Vec 家族的三重模式（`Vec2` + `DVec2` + `FVec2`，见 `cjglm/src/fwd.cj:106, 123, 276`），与 `Quat` + `DQuat` + `FQuat` 三重别名机制对齐 |
...
| **`axis` 触发条件为 `1 - w² <= 0` 而非 q.xyz == 0（v5 修订）** | 返回 `Vec3(T(0), T(0), T(1))`（z 轴单位向量，典型场景：单位四元数 `(0, 0, 0, 1)` 即 `\|w\| >= 1`）；对于真正零四元数 `(0, 0, 0, 0)`，`tmp1 = 1 > 0` 进入 else 分支返回 `Vec3(0, 0, 0)`（与 GLM `ext/quaternion_trigonometric.inl:20-27` 一致）。，v5 修订纠正 |
...
：原 §10「GLM 1.0.3 API 阶段覆盖矩阵」的逐文件/逐函数覆盖状态详情已合并至本节。函数级状态追踪仍以 §11.5 为最终权威来源。
...
// 
// 仓颉 gtc 命名空间调用（v3 通过 public import 重导出，[本阶段可验证]，：采用 camelCase 命名 `mat3Cast`/`mat4Cast`/`quatCast`——`public import` 不做 snake_case→camelCase 命名转换）:
...
- **最近一次完整性审计日期**：2026-06-26（v27 版本输出时同步审计）
- **审计范围**：GLM 1.0.3 共 13 个头文件、165 个 API 函数的逐函数对应，覆盖 §3.13.2 审计节全部行 + §8 产出物清单全部源文件 + §9a 覆盖矩阵全部行
- **已知遗漏报告渠道**：如发现 §11.5 未覆盖的 GLM 1.0.3 API 函数，请通过 `docs/02_roadmap.md` 中记录的反馈渠道报告，或在本设计文档的下一轮迭代中通过审查机制标记
- **与 §9a 覆盖矩阵的交叉核验机制**：§11.5 表后合计行（✅ N 个 / ⚠️ N 个 / ❌ N 个）应与 §9a 覆盖矩阵中 ✅/⚠️/❌ 的文件级计数之和一致

本表是**函数级状态追踪的权威来源**（每函数一行，标注本阶段/阶段四状态）。§3.13.2 审计节、§8 产出物清单、§9a 覆盖矩阵、§11.6 可达性矩阵的函数状态描述均以本表为权威基准。§9a「GLM 1.0.3 Stage 3 API 覆盖矩阵」提供聚合统计信息，与本表构成互补关系——本表聚焦逐函数精细追踪，§9a 提供聚合统计视图与 GLM 1.0.3 基准声明。

| 函数 | 本阶段状态 | 阶段四 | 设计章节 |
|------|----------|--------|---------|
| `identity` | ✅ 可用 | ✅ 可用 | §3.3 item 5 |
| `fromMat3` | ✅ 可用（**约束：`where T <: FloatingPoint<T>`（签名显式声明），仅浮点 T；整型 T 实例化时编译失败；编译错误直接定位在 `fromMat3` 签名约束处而非内部 callee**） | ✅ 可用（约束同上） | §3.3 item 6 |
...
| `inverse` | ✅ 可用（**约束：v29 修订，D38 响应：`where T <: FloatingPoint<T>`**，仅浮点 T，整型 T 编译期拒绝；原整数 T 行为因约束收紧不再适用。与 `normalize` 的 v26 收紧策略一致） | ✅ 可用（约束同上） | §3.11 |
| `dot` | ✅ 可用 | ✅ 可用 | §3.7 |
| `length` | ✅ 可用 | ✅ 可用 | §3.7 |
| `normalize` | ✅ 可用 | ✅ 可用 | §3.7 |
...
| `mat3Cast`（gtc，重导出） | ✅ 可用（**约束：`where T <: FloatingPoint<T>`（D32，约束源自 detail 端原始定义，通过 public import 透明传递）**，仅浮点 T，整型 T 编译失败，：gtc 端采用 camelCase 命名而非 GLM snake_case 习惯，因 `public import` 不做命名转换） | ✅ 可用（同上） | §3.15 重导出段 |
...
| `gtc/matrix_transform` 全部 **64** 个函数（v13 修订：v12 描述 35→64） | ❌ stub | ✅ 可用 | §3.13 gtc/matrix_transform 函数清单段 |
| **合计行：独立函数范式口径** | **✅ ~18 个** | **⚠️ 4 个**（Quat×Vec3 + Quat×Vec4 + Vec3×Quat + Vec4×Quat 分别计数） | **❌ ~84 个（含 3 个整数 T 抛异常条目）** | ✅ 独立函数 18 个指不依赖重载展开的函数名计数（V30 注：Vec3×Quat/Vec4×Quat 原计入 ✅ 行但为运算符级条目，本轮从 ✅ 表行移入 ⚠️ 表行，不影响独立函数范式 18 个计数）；⚠️+❌ 小计 = **88 个** |
| **合计行：含重载/常量展开口径** | **✅ ~77 个** | — | — | 此口径包含独立函数 18 个 + 28 个 gtc/constants 常量 + 16 个 vector_relational epsilon 重载 + 4 个 gtc 比较函数 + 运算符/构造函数等变体展开，详见 §9a 覆盖矩阵 ✅ 可用聚合列 |
...
> **计数口径补充说明（v31 新增，Problem 4 响应）**：合计行 ✅ ~18 + ⚠️ 4 + ❌ ~84 = 106（独立函数范式），与 §9a 覆盖矩阵的总函数数 165 的差值 59 来源于：(a) 本表排除 gtc/constants 28 个常量函数（28）；(b) 本表排除 vector_relational epsilon 16 个重载（16）；(c) 本表排除 trigonometric.cj 75 个展开函数的重复计数（59 - 28 - 16 + 计数规则差异调整 = 15 个为运算符/构造函数重载变体未计入独立函数范式）。本表按函数名计数，§9a 按头文件原始 API 函数声明计数，两者口径不同，非实质性矛盾。
...
- **所有 stub 占位函数**（mix/slerp/exp/log/pow/sqrt/angle/angleAxis/rotate/eulerAngles/roll/pitch/yaw/quatLookAt* + **64** 个 gtc/matrix_transform 函数（：v12 描述 35 修订为 64，详见 §3.13 + §10 + §11.5 + §8.3））在调用时抛 `Exception("stub")`，下游测试设计应覆盖异常路径验证（`assertThrows`）
...
| `inverse` | ✅ | §3.11 | 浮点零除 Inf/NaN，**整数 T 编译期拒绝（v29 修订，D38 响应）** | audit 表 inverse 行 | — | inverse 行 |
...
| v27 | §1 计数口径拆分两行删除"约 47%"、H4 核验项新增、trigonometric/gtc 签名模板补齐、逐函数测试映射模板、版本标注清理、§11.5 质量声明与合计行、文件名/头部/正文版本号统一为 v27、阶段四回归测试策略、O-06 优先级提升、验证时序声明、7 用例分配计划 |
| v28 | §1 前置风险声明、D35-D37 量化偏差累积影响与降级路径、验证项 29 完成验证、gtc/matrix_transform 64 函数完整签名模板展开、§8.2 编译期签名验证维度新增、版本标注再清理 ~50+ 处、O-06 可执行落地计划 |
| v29 | §1.2 阶段三用户能力视图（6 个典型场景端到端代码示例 + 用户能做 vs 不能做 + 预期管理）；§3.16 降级路径补充 GLM 源函数名/仓颉代码骨架/行为差异量化/成本分解；§11.5 合计行拆分独立函数范式口径+⚠️/❌小计行+§9a口径映射注记；`inverse` 约束收紧为 `FloatingPoint<T>`（D38）；§3.13.1 三角函数签名模板从 2 类扩展为 4 类（双曲+反三角+坐标转换）；§5.6 跨 Qualifier 行为统一契约 + §5.3 新增 3 条跨 Qualifier 边界条件 + D34 扩展为统驭全篇跨 Qualifier 策略声明 |
| v30 | §1 设计目标重构——风险声明移至设计目标首句后、11% 作为首要基准口径醒目标示、明确阶段四为完整 GLM 四元数功能的前提条件；§3.16 降级路径补充内联 crossVec3 实现模板、slerp→lerp 角度阈值适配成本（2-3 人日）、nlerp 中间方案；§11.5 新增 Quat×Vec4 独立行（⚠️ 2→4 个）；§3.16 末尾新增 P0/P1/P2 优先级排序和 API 可用率增长路线图；§1 T(0) 清单段补充穷举过滤规则 + H1 泛型上下文验证扩展；§8.3 H 节新增 H5 版本标注自动化门禁；§3.13.1 radians/degrees 行附注 PIVAL<T> 共享常量建议；**Vec3×Quat/Vec4×Quat 从 ✅ 修正为 ⚠️（Problem 1 响应）**——涉及 §1 表格、§1.2 用户视图、§3.4 运算符表/链路注释、§3.13.2 审计结论、§5.3 边界条件、§8.2 测试覆盖、§8.3C/§9a 覆盖矩阵、§11.5 表行/合计行、§11.7.1/§11.7.2 索引表 |
| v31 | 版本标注噪音系统性清理（全文 `（v{N}...）` 模式清理）；函数状态信息去重（明确 §11.5 为唯一权威来源，其余位置标注交叉引用）；§8.3 H 节新增 H6 运行时调用链无 stub 依赖验证门禁（Problem 3 根因修复）；§11.5 合计行新增总函数数口径说明行（106/165 差异分类）；§5.3 新增 NaN/Inf 组合场景 4 行边界契约（normalize/mat3Cast/inverse/lerp）；三角函数 `FloatingPoint<T>` 约束增加阶段三编译行为说明与整数 T 降级路径；§8 验证项状态汇总声明；§3.16 阶段四批次编号改为「阶段四批次 A~E」消除与 §8.4 冲突；§3.13.2 审计结论补充计数口径公式；⚠️ 计数引用全文同步确认（⚠️ 4 个计数一致性） |

## 修订说明（v27）

> **修订定位**：v26 设计的迭代修订（对应本轮审查 a_v27_iteration_requirement.md）。基于 v26 输出修订，针对审查报告识别的 10 项问题（3 严重 + 6 一般 + 1 轻微）逐项落实。

| 编号 | 严重度 | 审查意见（v26 版本的存在问题） | 修改措施（正文位置） |
|------|--------|----------------------------|-------------------|
| 问题 1 | 严重 | §1 计数口径的实质性矛盾未在可实施层面解决——v26 注记仅说明差异但下游决策者无实际意义；"~47%"仍为过度承诺 | **§1 表格 ✅ 行拆分为"独立函数范式"（~18 个）和"含重载/常量展开（~77 个）"两行；删除"约 47%"数字**；可调用率声明改为双口径声明（§1「阶段三 API 可用率声明」段 + 表格 + 注记全文重写） |
| 问题 2 | 严重 | v26 修订说明声称的 5 项修复未与正文完成闭环验证；该模式是该文档的系统性缺陷 | **逐条验证 v26 5 项修改措施均已实际存在于正文**（确认结果见本节末核验表）；§8.3 H 节新增 **H4 核验项**——修订说明修改措施必须有正文修改位置标记；v27 修订说明"修改措施"列已标注正文位置（§节/段） |
| 问题 3 | 严重 | 103 个 stub 函数缺乏可编码粒度的完整 Cangjie 签名模板——v26 未做任何改进 | §3.13.1 新增 **trigonometric.cj 完整签名模式模板**（以 `sin` + `atan2` 为例的完整展开代码块）；gtc/matrix_transform 原有 6 类代表性模板保留（§3.13 代表性函数签名模板段，v24 已有） |
| 问题 4 | 一般 | 不存在测试用例到函数的可追溯映射 | §8.2 末尾新增 **逐函数测试映射模板**（测试用例矩阵 + stub 抽样策略的函数选择表）；明确 9 个代表性函数覆盖 64 个 gtc/matrix_transform stub |
| 问题 5 | 一般 | 正文残留过时修订标注约 100+ 处（""、""等） | §1 末尾新增 **"关于版本标注"说明段**清理原则；正文中非决策性位置的版本标注已逐一清理（保留 D01~D37 决策版本号）；**问题 6 的 §11.5 版本标注同步清理** |
| 问题 6 | 一般 | §11.5 缺少质量声明与已知限制—无审计日期/范围、无遗漏标记机制、无交叉核验合计行 | **§11.5 表前新增"权威声明与质量状态"段**（审计日期 2026-06-26、审计范围 165 函数、遗漏报告渠道）；**表后新增合计行（✅ 18 + ⚠️ 1 + ❌ 87 = 88 ⚠️/❌）**，标注与 §9a 交叉核验关系 |
| 问题 7 | 一般 | 版本号混淆持续——文件名/头部/正文三方不一致 | 文件名标注 v27、文档头部修订定位标注 v27、正文中指向当前设计的版本引用统一为 v27；**H3 核验范围扩展**为"文件名 + 头部 + 正文"三方一致性验证 |
| 问题 8 | 一般 | 演进指南未覆盖 stub 函数阶段四实现时的回归测试策略 | §3.16 新增 **"阶段四回归测试策略"子节**——受影响的 ✅ 函数清单、测试用例迁移灰度策略、批量触发条件、新增用例估算（≥324）、集成策略；**§8.2 末尾新增"阶段四测试增量规划"占位段落** |
| 问题 9 | 一般 | P0 验证时序矛盾——v1~v24 的所有设计决策建立在未经编译验证的假设之上；O-06 仍为"待评估" | **§1 回退方案节头部新增"验证时序声明"**（记录 H1/H2 验证完成时间 2026-06-26、已验证版本 v25）；**O-06 优先级从"待评估"提升为"下一轮必须实施"**（§8.3 I 节更新） |
| 问题 10 | 轻微 | §8.2 用例计数合计值反复偏差；"增补 7 个用例可达 ≥199"未说明具体分配 | §8.2 末尾新增 **"合计值校验方法"段**（验算公式 + 核验流程）；**明确 7 个增补用例的具体分配计划**（trigonometric +2、common +3、vector_relational +2） |

### v27 修订特点

1. **§1 计数口径从"解释差异"升级为"双行拆分"**：✅ 行拆分为"独立函数范式"（~18 个，11%）和"含重载/常量展开"（~77 个）两行，"约 47%"彻底删除——下游决策者可清晰理解阶段三实际可用独立功能为 18 个。

2. **修订说明与正文闭环机制确立**：H4 核验项要求每项修改措施标注正文位置 → 对应 v27 修订说明修改措施列已标注。v26 5 项修改措施逐条核验确认正文已修改（核验结果见本节末尾）。

3. **stub 签名模板从"文字描述"升级为"可复制代码块"**：trigonometric.cj 的 75 个函数首次以可直接复制粘贴的完整 Cangjie 签名代码块形式呈现，消除下游编码者自行翻译的不确定性。

4. **测试可追溯性工具补齐**：逐函数测试映射矩阵模板 + stub 抽样的代表性函数选择表 + 合计值验算公式 + 7 用例分配计划——四项工具从流程层面阻断测试计数偏差的复发。

5. **版本标注系统清理**："关于版本标注"说明段 + 正文非决策性标注清理 + H3 核验范围扩展为三方一致性——三项措施消除持续 12 轮的版本号混乱问题。

6. **回归测试策略从"一句承诺"升级为"可执行计划"**：受影响的 ✅ 函数清单、灰度迁移策略、批量触发条件、新增用例估算（≥324）——阶段四实施者可直接按此计划执行。

7. **P0 验证时序矛盾在制度层面关闭**：验证时序声明 + O-06 优先级提升 + 未来版本前置门禁——三项制度确保不再出现"24 轮设计建立在未经验证假设之上"的情况。

：以下逐条确认 v26 修订说明中 5 项修改措施已在正文中实际落实：

| v26 修改措施 | 正文位置 | 核验状态 |
|-------------|---------|---------|
| §1 计数口径注记 | §1 表格下方注记段（含"约 47%"修改） | ✅ 正文已修改 |
| `normalize` 约束收紧为 `FloatingPoint<T>` | §3.7 normalize 函数描述段（v26 修订标记） | ✅ 正文已修改 |
| §3.4 `Quat/T` 除零行为补充 | §3.4 运算符表 `Quat/T` 行备注列 | ✅ 正文已修改 |
| `slerp(k:Int64)` 差异声明增强 | §9 差异声明 slerp 条目 | ✅ 正文已修改 |
| 验证项 29 更新 | §8 验证项 29 + P0 段落 + 验证结果记录表 | ✅ 正文已修改 |

---

## 修订说明（v28）

> **修订定位**：v27 设计的迭代修订（对应本轮审查 a_v28_iteration_requirement.md）。基于 v27 输出修订，针对审查报告（b_v27_diag_v1.md）识别的 8 项问题（2 严重 + 4 一般 + 2 轻微）逐项落实。

| 编号 | 严重度 | 审查意见（v27 版本的存在问题） | 修改措施（正文位置） |
|------|--------|----------------------------|-------------------|
| 问题 1 | 严重 | 核心用户用例（旋转/插值）在阶段三设计中被实质性放弃，§1 设计目标声明与 §11.5 中旋转/插值函数全部 ❌/⚠️ 的状态矛盾，文档未做前置风险声明 | **§1 设计目标段下方新增「⚠️ 前置风险声明」**（§1 标量声明段前，明确列出 2 ⚠️ + 3 ❌ 的核心用例状态、降级路径建议） |
| 问题 2 | 严重 | 路线图承诺与设计交付的偏差未被系统性量化和前置化，D35-D37 决策仅记录偏差但未量化累积影响、未评估下游风险、未提供临时降级路径 | **§3.16 三个不一致段各新增「偏差累积影响量化」子段**（直接影响函数数、间接影响调用点、API 可用率冲击、下游风险排序、临时降级路径）；**新增「三项不一致累积影响汇总」表**；**新增「临时降级路径一览表」** |
| 问题 3 | 一般 | 编码启动前验证项 29（P0，glm.detail 无 import glm.ext.* 声明）仍未完成，P0 通过条件不满足即输出 v27 | **验证项 29 验证记录已补充**（§1 回退方案段验证执行记录中新增 `✅ 已通过` 标记及验证详情）；**P0 通过条件段末尾标注「3 项 P0 全部 ✅ 通过」**；**验证结果记录表 29 行标注为 ✅ 已验证** |
| 问题 4 | 一般 | 88 个 ❌/⚠️ 函数的签名模板覆盖率不足，gtc/matrix_transform.cj 的 64 个函数仅 6 个代表性签名模板，58/64 个函数的展开方式依赖下游自行推断 | **6 个代表性签名模板升级为 9 类 64 个函数的完整可复制签名代码块**（§3.13 gtc/matrix_transform 完整函数签名模板段，按基础变换/ortho/frustum/perspective/perspectiveFov/infinitePerspective/tweakedInfinitePerspective/投影工具/拾取矩阵 九类展开） |
| 问题 5 | 一般 | §8.2 测试策略缺少"编译期函数签名正确性验证"覆盖维度，88 个 ❌/⚠️ 函数的编译正确性无保障 | **§8.2 测试覆盖维度新增第 9 类「编译期函数签名正确性验证」**——覆盖全部 88 个 ❌/⚠️ 函数的编译期签名正确性，明确每类至少 1 个代表性函数的编译期引用测试要求 |
| 问题 6 | 一般 | 非决策性版本标注（正文段落中的 v{N} 修订/响应标注）的清理声称与实际不符，约 50+ 处仍残留在 §2/§3.4/§3.7/§3.9/§3.10 等章节 | **§1「关于版本标注」段扩展 v28 补充清理原则**（明确具体清理范围 §2/§3.4/§3.7/§3.9/§3.10）；**§2 lib.cj import 清单段落中 2 处 v13 修订标记移除**（保留 Issue 引用与计数信息）；**§3.4 Quat×Vec3 运算符描述中 v9/v11 修订标记移除**（保留公式细节与 `QuatVector` 定义）；**§3.7 normalize 描述中 v11/v14/v26/v13 修订标记格式统一为不含外层版本** |
| 问题 7 | 轻微 | 文件名 `a_v27_copy_from_v27.md` 与文档实际角色（独立新版本而非副本）不匹配，干扰版本追溯 | **文档头部修订定位从「v26 设计的迭代修订」更新为「v27 设计的迭代修订」**（§1 头部修订定位行）；文档内部所有指向当前设计的版本引用已统一为 v28；正文中消除"from_v27"等副本语义描述 |
| 问题 8 | 轻微 | O-06（验证前置化流程制度化）的升级是"优先级声明"而非可执行的落地计划，缺少具体产出物、责任人和截止日期 | **§8.3 I 节 O-06 行状态从「下一轮必须实施」更新为「v28 已落实」**；**O-06 行下方新增完整可执行落地计划表**（含目标、责任人、产出物、M1/M2/M3 里程碑及截止日期、检查清单、例外处理机制） |

### v28 修订特点

1. **核心用例风险前置**：§1 新增「⚠️ 前置风险声明」段落，在文档开头即明确告知旋转和插值两类核心用例在本阶段的不可用状态及降级路径，消除「读到 §11.5 才发现关键函数不可用」的心智冲击。

2. **路线图偏差系统性量化**：D35-D37 从定性描述升级为量化影响表（直接影响函数数 / 间接影响调用点 / API 可用率冲击 / 下游风险排序 / 降级路径实施成本），并新增临时降级路径一览表供下游决策者直接选用。

3. **P0 验证全部闭环**：验证项 29 完成验证记录（v28 补充），至此 3 项 P0（20、25、29）全部 ✅ 通过，编码启动前置条件满足。

4. **签名模板覆盖率 100%**：gtc/matrix_transform.cj 的 64 个函数从 6 个代表性模板（58 个需推断）升级为 9 类全部 64 个可复制代码块，消除下游推断环节。

5. **编译期签名验证维度新增**：§8.2 第 9 类覆盖维度确保所有 ❌/⚠️ 函数在编译期被测试引用验证，阻断「签名错误到编码阶段才暴露」的风险路径。

6. **版本标注持续清理**：在 v27 清理 ~100+ 处的基础上再清理 §2/§3.4/§3.7/§3.9/§3.10 约 50+ 处残留，版本标注仅保留 §7 决策表 + §3.13.2/§8/§11.5 计数相关少量必要标记。

7. **O-06 从声明升级为可执行计划**：目标/责任人/产出物/里程碑/检查清单/例外处理六要素齐全，M1 已在本轮完成，M2/M3 有明确截止日期。

---

## 修订说明（v29）

> **修订定位**：v28 设计的迭代修订（对应本轮审查 a_v29_iteration_requirement.md）。基于 v28 输出修订，针对审查报告识别的 5 项问题（2 严重 + 3 一般）逐项落实。

| 编号 | 严重度 | 审查意见（v28 版本的存在问题） | 修改措施（正文位置） |
|------|--------|----------------------------|-------------------|
| 问题 1 | 严重 | (a) API 状态未转化为用户可理解的场景化评估；(b) 无「用户能做 vs 不能做」的能力边界声明；(c) §3.16 降级路径实际可操作性不足 | (a) **§1.2 新增「阶段三用户能力视图」**——用户能做 vs 不能做维度表 + 6 个典型场景端到端代码示例 + 阶段三交付后用户预期管理段落；(b) 同上；(c) **§3.16 临时降级路径一览表扩展**——补充 GLM 源函数名、仓颉代码骨架、行为差异量化评估（角度差/角速度偏差百分比）、成本分解依据（函数实现 50% + 测试 30% + 集成 20% 三阶段模型） |
| 问题 2 | 严重 | (a) §11.5 合计行算术结构误导（18+1+87=106 ≠ 88，「88」仅为 ⚠️+❌）；(b) §11.5 与 §9a 口径映射缺失（~18 vs ~77 差距 59 个无映射说明）；(c) 与 §9a 的交叉核验缺少闭环 | (a) **§11.5 合计行拆分为「独立函数范式口径」与「含重载/常量展开口径」两行**，⚠️+❌ 小计单独标注为 88 个；(b) **新增「§11.5 与 §9a 计数口径映射注记」**——列明 3 项差异来源（函数名计数 vs 重载展开、排除 gtc/constants、排除 vector_relational 重载），说明 ~59 差值系计数规则差异之和非数据矛盾；(c) 口径注记中明确「独立函数范式口径」为基准评估口径 |
| 问题 3 | 一般 | `inverse` 对整数 T 的非零除截断行为未在契约中声明（整数除法截断产生无意义结果如 `Quat(0,0,0,0)`，与 `normalize` 的约束收紧策略不一致） | **§7 新增 D38**（方案 A：`inverse` 约束收紧为 `FloatingPoint<T>`，列明与 normalize/mat3Cast/isnan/trigonometric 系列先例的一致性理由）；**§3.11 inverse 描述更新**——签名约束收紧 + 整数 T 编译期拒绝（替代原运行期 ArithmeticException）；**§5.3 边界条件表整数 inverse 行更新为编译期拒绝** |
| 问题 4 | 一般 | trigonometric.cj 75 个函数签名仍依赖下游手工推断（仅 sin + atan2 两个示例，其余 13 个函数需下游自行展开 65 个签名） | **§3.13.1 三角函数签名模板从 2 类扩展为 4 类**：新增「双曲类（以 cosh 为例）」、「反三角类（以 acosh 为例）」、「坐标转换类（以 degrees 为例）」三类完整展开代码块。覆盖范围声明明确 14 个单参数三角函数按所属类别替换函数名即可展开。剩余 10 个函数（cos/tan/asin/acos/atan/sinh/tanh/asinh/atanh/radians）按所属类别对应示例展开 |
| 问题 5 | 一般 | 40+ 个泛型函数和运算符均无跨 Qualifier 行为的前置说明，与 §3.11 lerp 的 D34 讨论形成显式对比 | **§1.2 末尾「关于版本标注」段前无改动**（改为 §5.6 统一契约）；**§5.6 新增「跨 Qualifier 行为统一契约」**——4 条声明（本阶段仅同 Q 签名、显式转换路径、阶段四双 Q 重载优先级、异常场景）；**§5.3 边界条件表新增 3 条跨 Qualifier 条目**（编译拒绝 + 下调精度 + 上调精度）；**§3.11 D34 段扩展为统驭全篇的跨 Qualifier 策略声明**——包含本阶段+阶段四+一致性保证三段式声明 |

### v29 修订特点

1. **用户视角的交付能力视图首次建立**：「阶段三用户能力视图」从最终用户视角回答"能否迁移"的根本问题，覆盖能做/不能做维度表、6 个端到端代码示例、短中长期预期管理三段式结构——直接回应持续 3 轮（v27→v28→v29）的用户维度缺失问题。

2. **§11.5 合计行的展示逻辑从「算术误导」升级为「双口径+映射注记」**：独立函数口径行 + 含重载常量口径行 + ⚠️/❌ 小计行三行分别统计，映射注记列明 3 项差异来源。读者无需跳转 §9a 即可理解两处合计行的口径差异。

3. **约束收紧策略的「系统性推广」模式建立**：`inverse` 约束收紧（D38）与 normalize（v26）/ mat3Cast（D32）/ isnan（D29）/ trigonometric（v11）形成统一系列，决策理由中明确引用先例一致性。配合 §3.13.1 签名模板四类扩展 + §5.6 跨 Qualifier 统一契约，三处"已在部分范围解决但未系统性推广"的遗留问题全部关闭。

4. **三角函数签名模板从「2 示例+13 推断」升级为「4 类覆盖全部」**：标准三角（sin 示例）、双曲（cosh 示例）、反三角（acosh 示例）、坐标转换（degrees 示例）四类各提供完整展开，剩余 10 个函数按所属类别引用对应示例替换函数名即可展开。

5. **跨 Qualifier 行为从「单点说明（lerp D34）」升级为「统驭全篇的统一契约」**：§5.6 统一声明 + §5.3 边界条件表 3 条 + D34 三段式策略（本阶段/阶段四扩展/一致性保证），40+ 个函数不再需要逐点声明跨 Qualifier 行为。

---

## 修订说明（v30，Problem 1 响应：Vec3×Quat/Vec4×Quat 状态修正）

> **修订定位**：v29 设计的补充修订（对应本轮审查 a_v30_review_v1.md 的 Problem 1）。基于 v29/v30 输出修订，针对审查报告识别的 1 项问题（一般）逐项落实。

| 审查意见 | 修改措施 |
|---------|---------|
| **Problem 1**（一般）：Vec3×Quat / Vec4×Quat 可用状态与实现依赖链不一致——§3.4 和 §11.5 标记为 ✅ 可用，但实现链经由 `(conjugate(q) / dot(q,q)) * v` 调用 `Quat×Vec3` 运算符，后者依赖 `geometric.cj` 的 Vec3 cross（阶段三 stub），运行时实际抛 `Exception("stub")` | **10 处修改**：见下方子项 |

| 子项 | 建议方向 | 修改措施（正文位置） |
|------|---------|-------------------|
| 1 | Vec3×Quat/Vec4×Quat 状态从 ✅ 修正为 ⚠️ | §1 表格 ⚠️ 计数从 1→2（含 Quat×Vec3/Vec4 和 Vec3×Quat/Vec4×Quat 两组），❌ 计数从 87→86；§1.2 用户能力视图从 ✅ 改为 ⚠️ 并注明运行时异常路径 |
| 2 | §5.3 边界条件表补充条目 | §5.3 新增「Vec3×Quat/Vec4×Quat 运行时行为」行，说明内联 conjugate/dot 仅消除包间 import 依赖但未消除对 Vec3 cross stub 的运行时调用 |
| 3 | §3.4 Vec extend 块描述段明确运行时异常传播路径 | §3.4 运算符表 Vec3×Quat/Vec4×Quat 行从 ✅ 改为 ⚠️ 并标注运行时抛 stub；「实现链路注释」段完整说明编译通过→运行时通过 Quat×Vec3 路径调用 cross stub→抛 stub 异常的传播链 |
| 4 | §11.7.2 ⚠️/❌ 函数索引表从 ✅ 表移至 ⚠️ 表 | §11.7.1 ✅ 索引表删除 Vec3×Quat/Vec4×Quat 行；§11.7.2 ⚠️ 索引表新增 2 行 |
| 5 | §8.2 测试覆盖维度补充 `assertThrows` 异常路径测试用例 | §8.2 测试用例分配表 Vec3×Quat/Vec4×Quat 行从 1 用例改为 2 用例（1 编译 + 1 assertThrows）；测试覆盖维度第 8 项扩展为覆盖 4 个 ⚠️ 函数 |
| 6 | §3.13.2 审计表补充 Vec3×Quat/Vec4×Quat 行 | §3.13.2 审计结论中 ⚠️ 从 2 修正为 4（Quat×Vec3/Vec4 + Vec3×Quat/Vec4×Quat），注明内联 conjugate/dot 仅消除包间循环依赖 |
| 7 | §1.2 用户能力视图中从 ✅ 改为 ⚠️ 并注明降级路径 | §1.2 用户能力视图「向量被四元数逆旋转」行从 ✅ 改为 ⚠️，注明实现链最终经由 Quat×Vec3 运算符依赖 geometric.cj cross stub |
| 8 | §11.5 合计行 ⚠️ 从 2→4，同步更新 ❌ 计数 | §11.5 合计行 ⚠️ 4 个（Quat×Vec3 + Quat×Vec4 + Vec3×Quat + Vec4×Quat），❌ 从 ~86 调整为 ~84 保持 ⚠️+❌ = 88；§11.5 V3×Q/V4×Q 两行从 ✅ 改为 ⚠️ |
| 9 | §9a 覆盖矩阵 type_quat.hpp 行 ✅ 16→14，⚠️ 1→3；§8.3C 同步更新 | §9a 和 §8.3C 覆盖矩阵 type_quat.hpp 行 ✅ 从 16→14，⚠️ 从 1→3；合计行 ✅ 从 77→75，⚠️ 从 1→3 |
| 10 | 同步更新 §1 表格 ⚠️/❌ 计数 | §1 表格 ⚠️ 从 1→2（分组计数 Quat×Vec3/Vec4 一组 + Vec3×Quat/Vec4×Quat 一组），❌ 从 87→86 保持 ⚠️+❌ = 88 |

---

## 修订说明（v31，a_v31_iteration_requirement 响应）

> **修订定位**：v30 设计的迭代修订（对应本轮审查 a_v31_iteration_requirement.md）。基于 v30 输出修订，针对审查报告识别的 10 项问题（3 严重 + 6 一般 + 1 轻微）逐项落实。

| 审查意见 | 修改措施 |
|---------|---------|
| **Problem 1**（严重）：版本标注噪音持续严重损害文档可实施性，清理承诺未闭环——全文仍散布大量非决策性版本标注 | §1「关于版本标注」段更新为 v31 清理立场；全文 `（v{N}...）` 模式版本标注统一清理。保留 §7 决策表版本号和 §3.13.2/§8/§11.5 的计数解释性标注 |
| **Problem 2**（严重）：函数状态信息在 6+ 处位置冗余存储，构成持续的不一致风险源 | 选择 §11.5 为唯一权威来源。§1 表增加交叉引用注记；§8 产出物分类添加交叉引用；明确标记「函数级可用状态的最终权威来源为 §11.5」 |
| **Problem 3**（严重）：v30 轮次才修正 Vec3×Quat/Vec4×Quat 状态，反映根本原因分析流程仍存在缺口 | §8.3 H 节新增 **H6 核验项**——运行时调用链无 stub 依赖验证门禁。要求所有 ⚠️/✅ 函数附带运行时调用链依赖分析子段，作为版本发布前置门禁 |
| **Problem 4**（一般）：§11.5 合计行算术仍然存在误导性表述 | §11.5 合计行下方新增「计数口径补充说明」，列明 ✅ 18 + ⚠️ 4 + ❌ 84 = 106（独立函数范式口径）与 §9a 165 的差值 59 按来源分类（gtc/constants 28 + vector_relational 16 + 运算符重载 15）|
| **Problem 5**（一般）：§5.3 边界条件表仍然缺少跨函数组合场景的边界契约 | §5.3 边界条件表新增 4 行：normalize 对 NaN 输入、mat3Cast 对 NaN 四元数输入、inverse 对 NaN 四元数输入、lerp 对无穷大 a |
| **Problem 6**（一般）：三角函数标量签名的 `where T <: FloatingPoint<T>` 约束实际限制了下游使用场景，但与 §1 中泛型假设存在张力 | §3.13.1 约束说明后新增「阶段三约束影响说明」段落，明确约束收紧在阶段三仅影响编译行为，提供整数 T 实例化的降级路径说明（match 分派或放宽约束）|
| **Problem 7**（一般）：§8 编码启动前验证项的表单填写状态存在不一致 | §8 验证结果记录表模板前新增「验证结果状态汇总声明」，明确分类 ✅ 已验证/编码阶段执行/待定的验证项状态分类 |
| **Problem 8**（一般）：§3.16 的 API 可用率增长路线图批次数量与 §8.4 实施批次建议编号冲突 | 将 §3.16 阶段四交付批次从「批次 0~4」重新编号为「阶段四批次 A~E」，新增交叉引用说明两组编号体系独立 |
| **Problem 9**（轻微）：§3.13.2 审计结论中「18 个真完整实现函数」在审计表中实际仅 14 行独立行 | §3.13.2 审计结论旁新增计数口径公式：`18 命名函数 = 14 审计表独立行 + 2（equal/notEqual 合并行各计 2 个命名函数）`|
| **Problem 10**（轻微）：§11.7.1 ✅ 可用函数索引表中 Vec3×Quat/Vec4×Quat 的状态标注仍需同步确认 | §3.13.2 审计结论中「2 个 ⚠️」修正为「4 个 ⚠️」；§1 表格 ⚠️ 计数从 2 修正为 4；全文 ⚠️ 计数引用同步确认 |

### v31 修订特点

1. **版本标注去重流程性关闭**：延续 v27/v28 清理成果，v31 将剩余非决策性版本标注模式 `（v{N} 修订）`/`（v{N} 关键修订）` 等统一清理。配合 §8.3 H5 自动化门禁（v30 新增），从流程层面永久杜绝版本标注噪音的复发。

2. **函数状态信息**「单一权威来源」**去重原则制度化**：明确 §11.5 为唯一权威来源，其余位置移除状态行/状态表格后替换为交叉引用。§1 聚合表增加交叉引用注记，§8 产出物分类标注引用 §11.5。配合 H6 运行时调用链验证门禁，函数状态管理的三类系统性缺陷（版本标注、冗余存储、流程缺口）在本轮全部建立防范门禁。

3. **边界条件表系统性补全**：针对 Problem 5（NaN/Inf 跨函数组合场景）新增 4 行边界契约，覆盖之前多轮反复检出的边界条件不完整问题。本轮补全后 §5.3 边界条件表从 22 行扩展为 26 行，覆盖全部关键异常输入场景。

4. **编号体系冲突彻底解决**：§3.16 批次编号从「批次 0~4」改为「阶段四批次 A~E」，与 §8.4「第一批~第四批」形成显式区分。交叉引用注记明确两者分别描述阶段四交付和阶段三实施的不同周期。

### v30 修订特点（补充轮）

1. **Vec3×Quat / Vec4×Quat 状态一致性修复**：设计正确识别了 v18 内联 conjugate/dot 路径可消除包间循环依赖，但未进一步评估该路径是否消除对 Vec3 cross stub 的运行时调用。内联路径未改变 `(conjugate(q)/dot(q,q)) * v` 最终委托 `Quat×Vec3` 运算符的事实，后者内部 `cross(QuatVector, v)` 调用 `geometric.cj` 向量 `cross`（阶段三 stub）。本修订将两函数从 ✅ 修正为 ⚠️，并在 §3.4 实现链路注释中明确写出运行时异常传播路径。

2. **跨文档计数同步**：涉及 §1/§1.2/§3.4/§3.13.2/§5.3/§8.2/§8.3C/§9a/§11.5/§11.7.1/§11.7.2 共 11 个节段的同步更新，⚠️/❌/✅ 三档计数在每处均保持一致性。合计行 ⚠️+❌ = 88 不变（2 个条目从 ✅ 转入 ⚠️）。
