...
> **修订日期**：2026-06-25
> **修订定位**：v13 设计的迭代修订（v14）。依据本轮审查报告（a_v10_iteration_requirement.md）识别 8 项问题（4 严重 + 4 一般）+ 2 项质询报告补强建议，逐项落实到对应章节，并追加修订说明（v14）章节。本轮重点修复：(1) §3.2 策略段落与 §3.15 对欧拉函数实现状态的直接矛盾（**第9次诊断，本轮彻底解决**——§3.2 策略段修订为与 §3.15 一致的「完整实现（4 个）+ stub 占位（7 个）+ 从 detail 重导出（4 个）」分类）；(2) `normalize` 零保护分支 T(0) 获取与签名不可闭环（采纳方案 B：`T(Float64(0))` 字面量替代，与 `axis` 函数模式一致，§1 系统性约束同步扩展覆盖 T(0)）；(3) ortho 系族 9→10（补充 3D ortho 一般版本，合计行数字确认仍为 64）；(4) `mat3Cast`/`mat4Cast` 初始化路径歧义（明确选择逐元素填充模式，非依赖单参数构造函数，非对角线 T(0) 采用 `T(Float64(0))` 字面量替代）；(5) 比较函数 `Comparable<T>` 约束决策（新增 D33 设计决策条目说明理由）；(6) 审计表 stub 数量 14→78（新增 gtc/matrix_transform 64 个函数行）；(7) 测试用例数修正。
...
| `tests/glm/test_ext_scalar_constants.cj` | 标量常量（v10 修订：同层平铺命名，**v14 修订，Issue 8 响应**：预计用例数 ≥6 修正为 ≥7——3 个泛型函数 × 2 种浮点类型 = 6 正常路径 + 1 整数类型异常路径 = 至少 7 个；按「完整实现函数：每函数 ≥2 个用例」原则应为 ≥12） | ≥7 |
...
| **合计** | — | **≥185**（v12 修订，Issue 8 响应：v11 累加 179 + `test_ext_quaternion_aliases.cj` 从 ≥4 提升至 ≥9 增量 5；**v14 修订，Issue 8 响应**：`test_ext_scalar_constants.cj` 从 ≥6 提升至 ≥7 增量 1；按上述每行 ≥值求和：40+8+8+12+16+8+4+2+16+7+9+28+27=185） |
...
| 测试用例总数 | ≥185 | 40+8+8+12+16+8+4+2+16+7+9+28+27 = 185（v14 重算：scalar_constants ≥6→≥7 增量 1 + quaternion_aliases ≥4→≥9 增量 5） |
