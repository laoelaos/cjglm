...
> **修订日期**：2026-06-25
> **修订定位**：v11 设计的迭代修订（v12）。依据本轮审查报告（a_v8_iteration_requirement.md 对应 v7 诊断报告 b_v7_diag_v1.md）从需求响应充分度、整体深度与完整性、设计可落地性维度识别 14 项质量问题（4 严重 + 5 一般 + 5 轻微）+ 5 项质询报告补充要点，逐项落实到对应章节，并补充修订说明（v12）章节。本轮 4 项严重问题均属「事实错误/直接阻塞编码」类：① §3.13 / §3.15 gtc/quaternion.cj 与 lib.cj 重导出端采用 snake_case 命名（`mat3_cast`/`mat4_cast`/`quat_cast`）但 `public import` 不做命名转换，下游按 §11.4 调用 `glm.gtc.quaternion.mat3_cast(q)` 将编译失败；② §2 / §3.14 / §8 更新文件段描述新增 9 个 fwd.cj 类型别名，但 fwd.cj 是自动生成文件（`cjglm/src/fwd.cj:1-2` 文件头注释明确标注），未说明实施路径，手动添加将随重新生成丢失；③ §3.13.1 trigonometric.cj 函数清单表行「内部依赖」列对 14 个单参数三角函数重复 14 次「std.math Float32 重载已支持」相同说明 + 表后「Float32 实例化影响」段与 §1 形成三重重复；④ §3.13 / §8 / §8.3 对 `gtc/matrix_transform.cj`「仅函数签名空壳」的具体清单未明确（GLM 1.0.3 实际定义 20+ 函数），下游存在三种解读（空文件 / 完整 20+ 函数 / 仅阶段四核心函数），§2 lib.cj 第 131 行 `import glm.gtc.matrix_transform` 依赖该文件存在可被 import 的对象。
...
**fwd.cj 新增 type alias 清单（v5 明确，v10 修订别名计数，v12 关键修订，Issue 2 响应：明确实施路径）**：
...
- **实施路径（v12 关键修订，Issue 2 响应）**：`fwd.cj` 是自动生成文件（`cjglm/src/fwd.cj:1-2` 文件头注释明确标注「Auto-generated file. Do not edit!」），**手动添加将随重新生成丢失**。v12 决策采用**方案 A（推荐，修改生成脚本）**：在 `tools/` 目录下找到 `gen_fwd.cj` 或等价的 fwd.cj 生成脚本，在其输入清单（如 GLM 头文件遍历列表 + 别名映射表）中新增四元数 9 个 type alias 条目，**下游编码者必须在编码启动前**：(a) 查阅 `tools/` 目录确认生成脚本位置；(b) 在脚本的别名映射表中新增 `Quat`/`FQuat`/`DQuat` + 3×Float32 精度 + 3×Float64 精度共 9 行；(c) 重新执行脚本生成 `fwd.cj` 后再提交。**备选方案 B**（如生成脚本位置不可定位或修改风险高）：将 9 个 type alias 移至 `lib.cj` 顶部作为补充声明（`lib.cj` 是手动维护文件），与现有 type alias 并存；**备选方案 C**：新建独立手动维护文件 `cjglm/src/quaternion_aliases.cj`，在 `lib.cj` 中 `public import` 重导出该文件——本方案需评估 cjpm 构建系统对新增文件的发现机制。本设计文档**推荐方案 A**，与 fwd.cj 的「单一自动生成来源」架构原则一致。
...
- `normalize(q: Quat<T,Q>): Quat<T,Q>` — 归一化四元数。**v11 关键修订，Issue 2 响应**（补充 GLM 零四元数保护行为）：实现公式与 GLM `ext/quaternion_geometric.inl:17-24` 完全一致——`tmp1 = length(q); if (tmp1 <= T(Float64(0))) { return identity_q } else { return q / tmp1 }`，其中 `tmp1 <= T(0)` 时返回**单位四元数**（`Quat(T(0), T(0), T(0), T(1))`）。**设计意图**：与 §5.1「normalize 零四元数返回单位四元数」契约对齐，避免下游按字面实现时零四元数除零产生 NaN 分量。**length 极小值边界行为（v11 新增）**：`tmp1` 为极小正数（如浮点最小次正规数）时不触发零保护分支，返回 `q / tmp1` 结果各分量趋向 Inf/NaN（与 GLM 行为一致，GLM 不做 length 极小值保护）；如需防止此类溢出，应在调用 `normalize` 前自行检查 `length(q) >= epsilon<T>()`。**T(0)/T(1) 获取路径（v12 关键修订，Issue 6 响应，引用 §1 作为单一权威来源）**：本函数体内 `T(0)`（零四元数保护分支返回值 xyz 分量）与 `T(1)`（零四元数保护分支返回值 w 分量）的获取策略遵循 §1「系统性设计约束」段的统一约定——T(0) 通过 `one - one` 演算（需 `one: T` 形参显式传入）；T(1) 通过 §1 v8 新增的「常量型 T(1) 字面量替代」策略 `T(Float64(1))` 显式转换路径获取（无需 `one: T` 形参）；两者**不**在本函数体内重复展开获取路径说明，避免与 §1 系统性约束段形成内容重复。**实现策略（v11 补强）**：`where T <: Number<T>` 约束下调用 `length(q)`，根据 §3.7 `length` 函数约束收紧策略（与 `length` 函数本身的 `T <: Number<T>` 约束保持一致，不引入额外 `T <: FloatingPoint<T>` 约束，使整数四元数 `normalize` 仍可调用——返回 `q / tmp1` 整除结果）。
...
| `gtc/matrix_transform.cj` | glm.gtc | 矩阵变换函数库（**v12 关键修订，Issue 4 响应：明确具体函数清单**）——本阶段声明 31 个函数签名（与 GLM 1.0.3 `glm/gtc/matrix_transform.hpp` 实际声明数一致）作为 stub 占位；下游按 `lib.cj:131` `import glm.gtc.matrix_transform` 调用时所有函数均存在但实现为 `throw Exception("stub")`。具体清单见 §3.13 表格下「`gtc/matrix_transform.cj` 函数清单（v12 新增，Issue 4 响应）」段 |

**沿用自阶段二的 stub**：`common.cj`、`geometric.cj`、`matrix.cj`（27 实现 + 6 stub）

**`gtc/matrix_transform.cj` 函数清单（v12 新增，Issue 4 响应）**：GLM 1.0.3 `glm/gtc/matrix_transform.hpp` 实际声明约 35 个函数（依据 GLM 1.0.3 源码全文检索统计，包含 LH/RH 与 ZO/NO 等矩阵布局变体）。本阶段声明所有函数签名作为 stub 占位，确保下游 `import glm.gtc.matrix_transform` 时所有函数均存在但实现为 `throw Exception("stub")`。具体函数清单如下：

| 类别 | 函数数量 | 函数列表 |
|------|---------|---------|
| 基础变换 | 3 | `translate`、`rotate`、`rotate_slow` |
| 缩放 | 2 | `scale`、`scaleAlongAxis` |
| 视口变换（ortho 系族） | 5 | `ortho`、`orthoLH_ZO`、`orthoLH_NO`、`orthoRH_ZO`、`orthoRH_NO` |
| 透视投影（perspective 系族） | 10 | `perspective`、`perspectiveLH_ZO`、`perspectiveLH_NO`、`perspectiveRH_ZO`、`perspectiveRH_NO`、`perspectiveFov`、`perspectiveFovLH_ZO`、`perspectiveFovLH_NO`、`perspectiveFovRH_ZO`、`perspectiveFovRH_NO` |
| 无穷远透视（infinitePerspective 系族） | 10 | `infinitePerspective`、`infinitePerspectiveLH_ZO`、`infinitePerspectiveLH_NO`、`infinitePerspectiveRH_ZO`、`infinitePerspectiveRH_NO`、`tweakedInfinitePerspective`、`tweakedInfinitePerspectiveLH_ZO`、`tweakedInfinitePerspectiveLH_NO`、`tweakedInfinitePerspectiveRH_ZO`、`tweakedInfinitePerspectiveRH_NO` |
| 拾取矩阵 | 1 | `pickMatrix` |
| 看向与矩阵合成 | 4 | `lookAt`、`lookAtRH`、`lookAtLH`、`matrixCompMult` |
| **合计** | **35 个函数**（与 GLM 1.0.3 实际声明数一致，v12 关键修订：原设计「仅函数签名空壳」未指定具体函数清单，下游存在「空文件 / 完整 35 个 / 仅阶段四核心」三种解读；v12 明确为「完整 35 个签名 + throw Exception("stub") 实现」模式，确保 §2 lib.cj 第 131 行 `import glm.gtc.matrix_transform` 能正常解析所有 35 个函数） |

**下游实施约束（v12 修订，Issue 4 响应）**：
- 实施时按上述 35 个函数清单逐一声明函数签名
...
| `normalize(q)` | §3.7 标注「完整实现」（**v11 修订，Issue 2 响应**：补充零四元数保护） | `length(q)` | 阶段三可调用：零四元数返回单位四元数；其他输入返回归一化结果 | — |
...
**审计结论**：
- **本阶段可调用的「真完整实现」函数**（无 stub 依赖）：`epsilon<T>()`/`pi<T>()`/`cos_one_over_two<T>()`/`axis`/`length`/`normalize`/`inverse`/`isnan`/`isinf`/`mat3Cast`/`mat4Cast`/`quatCast`/`conjugate`/`lerp`/`dot`/`cross(Quat)`/`equal(Quat)`/`notEqual(Quat)`（共 17 个函数）
- **本阶段实现但运行时受 stub 依赖影响的函数**：`Quat×Vec3`/`Quat×Vec4` 运算符（2 个）
- **本阶段 stub 占位的函数**：`mix`/`slerp`/`exp`/`log`/`pow`/`sqrt`/`angle`/`angleAxis`/`rotate`/`eulerAngles`/`roll`/`pitch`/`yaw`/`quatLookAt*`（14 个）

下游测试设计时，对「本阶段可调用的真完整实现函数」可执行完整功能验证；对「实现但运行时受 stub 依赖影响」函数应标注「本阶段抛 stub 异常，待阶段四 stub 替换后生效」（与 §11.5 ⚠️ 符号对应）；对「stub 占位函数」仅执行 `assertThrows` 异常路径验证。
...
| **零四元数 `(0,0,0,0)`（v11 新增，Issue 2 响应）** | `normalize(q)` | **`tmp1 = length(q) = 0 <= T(0)` 触发 GLM 零保护分支**，返回**单位四元数** `Quat(T(0), T(0), T(0), T(1))`（T(0) 通过 `one - one` 演算，T(1) 通过 `T(Float64(1))` 字面量替代）；与 GLM `ext/quaternion_geometric.inl:17-24` 实际行为完全一致——下游按 v10 描述「内部调用 `length`」字面实现时零四元数除零产生 NaN 分量与本契约不符 |
...
| u, v 反平行（`dot ≈ -1`） | `fromVec3` | 返回 180° 任意轴四元数（`q.w ≈ 0` 且 `q.xyz` 模长为 1 的纯向量四元数；v8 新增契约，行为已定义，与 GLM `ext/quaternion_common.inl:196-217` 退化路径一致；阶段三 stub 阶段不适用，待阶段四 stub 替换后生效；**v12 修订，Issue 11 响应：双向引用**——本条目对应 §3.13.2「本阶段实现但运行时受 stub 依赖影响」审计表的「`fromVec3` stub 行」反向引用） |
...
- `fwd.cj`：新增 9 个四元数类型别名（Quat/FQuat/DQuat + 3×Float32 精度 + 3×Float64 精度；具体清单见 §2 lib.cj/fwd.cj 段落，v5 明确，**v12 关键修订，Issue 2 响应**：fwd.cj 是自动生成文件，下游实施路径为方案 A 修改 `tools/gen_fwd.cj` 等生成脚本，新增 9 行 type alias 输入；详见 §2 段落末尾「实施路径」说明）
...
| **合计** | — | **≥184**（v12 修订，Issue 8 响应：v11 累加 179 + `test_ext_quaternion_aliases.cj` 从 ≥4 提升至 ≥9 增量 5；按上述每行 ≥值求和：40+8+8+12+16+8+4+2+16+9+28+27=184） |
...
20. **`FloatingPoint<T>` 接口方法可用性验证（v11 关键修订，Issue 8 响应）**：依据 `cangjie-original-docs/std/math/math_package_api/math_package_interfaces.md` 第 5-17 行定义，验证 `FloatingPoint<T>` 接口实际提供 6 个静态方法（`getE`/`getInf`/`getPI`/`getMinDenormal`/`getMinNormal`/`getNaN`）+ 3 个实例方法（`isInf`/`isNaN`/`isNormal`）。v10 描述「`FloatingPoint<T>` 接口本身无任何实例方法」错误——下游实现 `pow` 次正规数边界检查时应直接调用 `FloatingPoint<T>.getMinDenormal()` 而非类型分派；实现 `log` w=0 退化分支时应直接调用 `FloatingPoint<T>.getInf()` 而非类型分派；实现 `isnan`/`isinf` 约束时该 where 子句确保 T 类型实现 `isNaN()`/`isInf()` 实例方法
...
23. **`fwd.cj` 自动生成脚本验证（v12 关键修订，Issue 2 响应）**：验证 `tools/` 目录下生成 fwd.cj 的脚本（如 `gen_fwd.cj`）是否能正确生成 9 个四元数 type alias——具体步骤：(a) 查阅 `cjglm/tools/` 目录确认生成脚本存在；(b) 在脚本的别名映射表中新增 `Quat`/`FQuat`/`DQuat` + 3×Float32 精度 + 3×Float64 精度共 9 行（参照 §2 lib.cj/fwd.cj 段末尾「实施路径」说明）；(c) 运行脚本生成新的 `fwd.cj`；(d) 验证生成的 `fwd.cj` 包含全部 9 个 type alias 且与 §3.14 清单一致；(e) 验证脚本支持幂等运行（多次执行结果一致）
...
**A. 产出物清单验收（来源：§8 产出物清单）**

| 类型 | 验收项 | 数量 | 验收依据 |
|------|--------|------|---------|
| 完整实现源码文件 | `detail/type_quat.cj` + `detail/type_quat_cast.cj` + `detail/scalar_constants.cj` + `detail/scalar_quat_ops.cj` + `ext/vector_relational.cj`（epsilon 16 重载完整 + ULP 8 重载 stub）+ `ext/quaternion_relational.cj`（4 函数）+ `ext/quaternion_geometric.cj`（4 函数）+ `ext/quaternion_common.cj`（5 函数完整 + 3 函数 stub）+ `ext/quaternion_trigonometric.cj`（1 函数完整 + 2 函数 stub）+ `ext/scalar_constants.cj` + `gtc/constants.cj`（28 函数）+ `gtc/quaternion.cj`（4 重导出 + 4 完整 + 7 stub）+ 4 个 ext/ 别名文件 | 15+ 个源文件 | 文件存在 + `cjpm build` 通过 |
| 大部分实现文件 | `ext/quaternion_common.cj`（mix/slerp stub）+ `ext/quaternion_trigonometric.cj`（angle/angleAxis stub）+ `ext/quaternion_transform.cj`（rotate stub）+ `ext/quaternion_exponential.cj`（exp/log/pow/sqrt stub）+ `gtc/quaternion.cj`（7 函数 stub） | 5+ 个源文件 | 文件存在 + `cjpm build` 通过 |
| 空桩占位文件 | `gtc/matrix_transform.cj` + `detail/trigonometric.cj`（含 75 个展开函数签名）+ `ext/matrix_projection.cj` + `ext/matrix_clip_space.cj` + `ext/matrix_transform.cj` | 5 个源文件 | 文件存在 + `cjpm build` 通过 |
...
| 测试用例总数 | ≥179 | 40+8+8+12+16+8+4+2+16+6+4+28+27 = 179（v10 重算） |
...
**C. 覆盖矩阵验收（来源：§10 GLM 1.0.3 API 阶段覆盖矩阵）**

GLM 1.0.3 各头文件阶段三覆盖状态汇总：

| GLM 头文件 | 函数总数 | 本阶段实现 | 本阶段 stub | 阶段四补齐 |
|----------|---------|----------|----------|----------|
| `detail/type_quat.hpp` | 19 个 API（结构体 + 运算符 + 工厂） | 19 个（全部实现，运行时 stub 异常待阶段四） | 0 | — |
| `detail/type_quat_cast.hpp`（v3 新增） | 4 个 | 4 个 | 0 | — |
| `ext/vector_relational.hpp` | 8 个 | 4 个 epsilon 版本（16 重载完整 + 8 ULP 重载 stub） | 4 个 ULP 版本 | ULP 版本需评估仓颉位级访问能力 |
...
| `ext/scalar_constants.hpp` | 3 个 | 3 个（epsilon/pi/cos_one_over_two） | 0 | — |
| `gtc/constants.hpp` | 28 个 | 28 个 | 0 | — |
| `gtc/matrix_transform.hpp` | **35**（v12 修订：明确具体清单见 §3.13 表格下「`gtc/matrix_transform.cj` 函数清单」段） | 0 | **35**（空桩占位，仅函数签名 + `throw Exception("stub")`） | 完整实现（阶段四） |
| `gtc/quaternion.hpp` | 15 个 | 4 个重导出 + 4 个完整 = 8 个 | 7 个（4 欧拉 + 3 看向） | 7 个 stub 完整实现 |
| **合计** | — | **约 79 个实现** | **约 21 个 stub** | — |

**D. 函数可用性对照表验收（来源：§11.5）**

`§11.5 函数可用性对照表` 是阶段三验证的**权威基准**：✅/⚠️/❌ 三档符号标记每个函数本阶段状态（实现在本文件其他章节有详细描述）。下游验证时按 §11.5 表逐项核验即可。

**E. 验证项清单（来源：§8 编码启动前验证项 1-22）**

阶段三编码启动前需完成 §8 编码启动前验证项 1-22 全部 22 项验证（v11 新增 3 项：验证项 20/21/22），确保：
...
| dot(Quat, Quat) | 本阶段实现 |
| length(Quat) | 本阶段实现 |
| normalize(Quat) | 本阶段实现（**v11 修订，Issue 2 响应**：实现包含 GLM 零四元数保护分支 `if (tmp1 <= T(0)) return identity_q else return q / tmp1`，与 GLM `ext/quaternion_geometric.inl:17-24` 一致） |
...
| 函数 | 函数数 | 覆盖状态 |
|------|--------|---------|
| 全部 35 个函数（基础变换 3 + 缩放 2 + 视口 5 + 透视 10 + 无穷远 10 + 拾取 1 + 看向合成 4，**v12 关键修订，Issue 4 响应：明确具体清单见 §3.13 表格下「`gtc/matrix_transform.cj` 函数清单」段**） | 35 | 本阶段空桩占位（仅函数签名 + `throw Exception("stub")`），阶段四补齐完整实现 |

### glm/gtc/quaternion.hpp（v3 修订状态，v12 关键修订：snake_case→camelCase 命名）

| 函数 | 覆盖状态 | 备注 |
|------|---------|------|
| `mat3Cast(Quat)` | 本阶段实现 | **v3 修订**：从 detail/type_quat_cast.cj 重导出，**v12 关键修订，Issue 1 响应**：采用 camelCase 命名（与 detail 端 `mat3Cast` 一致），而非 GLM 原始 snake_case `mat3_cast`——`public import a.b.f` 仅重导出 f 原始名，不做命名转换 |
| `mat4Cast(Quat)` | 本阶段实现 | **v3 修订**：从 detail/type_quat_cast.cj 重导出，**v12 修订同上**：camelCase `mat4Cast` 而非 snake_case `mat4_cast` |
| `quatCast(Mat3)` | 本阶段实现 | **v3 修订**：从 detail/type_quat_cast.cj 重导出，**v12 修订同上**：camelCase `quatCast` 而非 snake_case `quat_cast` |
| `quatCast(Mat4)` | 本阶段实现 | **v3 修订**：从 detail/type_quat_cast.cj 重导出，**v12 修订同上**：camelCase `quatCast` 而非 snake_case `quat_cast` |
...
- **所有 9 个 Quat 别名**（Quat/FQuat/DQuat + 3×Float32 精度 + 3×Float64 精度）在 `glm` 顶层命名空间通过 fwd.cj（**v12 修订，Issue 2 响应**：通过 `tools/gen_fwd.cj` 自动生成）可达
...
- **所有 stub 占位函数**（mix/slerp/exp/log/pow/sqrt/angle/angleAxis/rotate/eulerAngles/roll/pitch/yaw/quatLookAt* + 35 个 gtc/matrix_transform 函数）在调用时抛 `Exception("stub")`，下游测试设计应覆盖异常路径验证（`assertThrows`）
...
|------|------|--------|---------|------------|---------|
| 问题 1 | 审查 | 严重 | `gtc/quaternion.cj` snake_case 函数名（`mat3_cast`/`mat4_cast`/`quat_cast`）与 `public import` 机制不兼容——依据 `cangjie-lang-features/package/README.md` 第 156-166 行规范，`public import a.b.f` 仅重导出 f 原始名，不做命名转换；下游按 §11.4 调用 `glm.gtc.quaternion.mat3_cast(q)` 将编译失败 | §4.4 行为契约示例 / §11.4 迁移示例 / §10 覆盖矩阵 gtc/quaternion.hpp 表 / §3.15 完整实现函数段 + 重导出函数段 / §2 lib.cj/fwd.cj 段模块间依赖图 / §9 差异声明 / D11 设计决策 / §11.5 函数可用性对照表 | **已完整修复（采纳审查报告方案 C：gtc 端采用 camelCase）**：§4.4 / §11.4 / §10 / §3.15 等 6 处 snake_case `mat3_cast`/`mat4_cast`/`quat_cast` 全部修订为 camelCase `mat3Cast`/`mat4Cast`/`quatCast`；§9 差异声明新增条目说明 GLM snake_case 习惯在仓颉侧无法保留的原因（`public import` 不做命名转换）；D11 决策同步修订 |
| 问题 2 | 审查 | 严重 | `fwd.cj` 是自动生成文件（`cjglm/src/fwd.cj:1-2` 文件头注释明确标注「Auto-generated file. Do not edit!」），设计要求新增 9 个 type alias 但未说明实施路径——手动添加将随重新生成丢失，修改生成脚本的具体位置未说明 | §2 lib.cj/fwd.cj 段落「实施路径」段（v12 新增）/ §8 更新文件段 / §8 编码启动前验证项 23（v12 新增）/ D11 设计决策旁注 | **已完整修复（采纳审查报告方案 A：修改生成脚本）**：§2 段末尾新增「实施路径（v12 关键修订，Issue 2 响应）」段，明确下游编码者需在 `tools/gen_fwd.cj` 等生成脚本的别名映射表中新增 9 行；备选方案 B（移至 lib.cj）+ C（新建独立文件）作为 fallback；§8 验证项 23 新增「fwd.cj 自动生成脚本验证」验证脚本存在性 + 9 行输入 + 幂等运行性 |
...
| 问题 4 | 审查 | 严重 | `gtc/matrix_transform.cj`「仅函数签名空壳」的具体清单未明确——GLM 1.0.3 `glm/gtc/matrix_transform.hpp` 实际定义 20+ 函数，下游存在三种解读（空文件 / 完整 20+ 函数 / 仅阶段四核心函数），且 §2 lib.cj 第 131 行 `import glm.gtc.matrix_transform` 依赖该文件存在可被 import 的对象 | §3.13 表格下「`gtc/matrix_transform.cj` 函数清单（v12 新增，Issue 4 响应）」段（35 个函数分 7 类）/ §10 覆盖矩阵 gtc/matrix_transform.hpp 表 / §8.3 验收项 C 覆盖矩阵表 | **已完整修复**：§3.13 表格下行新增「`gtc/matrix_transform.cj` 函数清单」段，明确 35 个函数（基础变换 3 + 缩放 2 + 视口 5 + 透视 10 + 无穷远 10 + 拾取 1 + 看向合成 4）的具体清单 + 「完整 35 个签名 + throw Exception("stub") 实现」模式；§10 表 + §8.3 覆盖矩阵表同步更新 35 函数数 |
