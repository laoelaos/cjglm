...
> **修订日期**：2026-06-25
> **修订定位**：v18 设计的迭代修订（对应本轮审查 a_v18_iteration_requirement.md）。基于 v17 输出修订，针对审查报告识别的 4 项问题（2 严重 + 2 一般）逐项落实。本轮重点修复：(1) §2 模块间依赖图——删除 `type_quat.cj → ext/quaternion_geometric` 依赖声明以消除包间循环依赖风险，同步将 Vec3×Quat/Vec4×Quat 改为内联 conjugate/dot 计算路径，确保 `glm.detail` 不依赖 `glm.ext`；(2) §8.3 验收项 B——测试用例数从「≥199」修正为「≥192」；(3) 修订说明排序——按版本号升序重排为 v12→v13→v15→v16→v17→v18；(4) §8 新增验证失败后受影响章节清单映射表。
...
- **临时实施建议**：在收到项目管理方确认前，建议按 **路径 C** 推进（保持 `slerp` 为 stub）。该选择对阶段三整体交付范围影响最小——`slerp` 的 stub 状态已在 §11.5 明确标注，不阻塞其他可验证函数的实现与测试。若后续确认需阶段三有 `slerp`，可降级为 **路径 A**（`lerp` 近似），该切换仅影响 `quaternion_common.cj` 中 `slerp` 函数体的实现策略，不改变函数签名或外部依赖关系。
- **工期影响评估**：按路径 C 实施为零额外工期成本；若后续切换至路径 A，估计工期增加 1-2 人日（实现与测试 `slerp` 的 `lerp` 近似版本 + 边界用例覆盖）；若切换至路径 B（有限精度三角函数内联），工期增加约 3-5 人日（需实现泰勒展开、跨类型精度验证、回归测试）。**建议决策**：维持路径 C，阶段三 `slerp` 标注 `[待 Stage 4]`，该决策不改变阶段三交付范围，且具备向下兼容的降级路径。
...
- **临时实施建议**：在收到确认前，建议按 **本设计约定**（使用 `quatLookAt`/`quatLookAtRH`/`quatLookAtLH`）推进。理由：三个函数本身均为 stub（§3.15），函数名变更不影响阶段三实施——无论以哪个名称声明，函数体均为 `throw Exception("stub")`，外部消费者无法调用 stub 函数。阶段四实现前完成命名对齐即可。若后续确认路线图保留 `lookRotate`，可在阶段四新增 `lookRotate` 作为 `quatLookAtRH` 的委托别名（一行代码），不影响已有 API。
- **工期影响评估**：按本设计约定实施工期成本为零（函数均为 stub，命名不影响 stub 声明）。若后续需新增 `lookRotate` 别名，工期成本约 0.5 人日（新增函数签名 + 委托调用 + 1 个可达性测试用例）。**建议决策**：维持本设计命名 `quatLookAt`/`quatLookAtRH`/`quatLookAtLH`，路线图由项目管理方同步修订。
...
- **临时实施建议**：在收到确认前，建议将路线图该行修订为 **`ext/quaternion_common.cj`：[部分可验证]——5 函数 ✅（conjugate/inverse/lerp/isnan/isinf）+ 3 函数 ❌（mix/slerp×2）**。理由：§3.13.2 审计表已明确列示 5 个可调用函数与 3 个 stub 函数的完整清单，§11.5 函数可用性对照表提供每函数级别的状态标注。路线图的粗粒度 `[可验证]` 标注与实际函数级状态不匹配，建议改为模块级标注 + 函数级明细（引用 §11.5）。
- **工期影响评估**：路线图标注修订本身零工期成本（仅为文档修改）。功能实施不受影响——5 个 ✅ 函数已在 §8 第二批实施中完整实现，3 个 ❌ 函数按第四批 stub 方案占位。**建议决策**：将路线图 `ext/quaternion_common.cj` 标注从 `[可验证]` 修改为 `[部分可验证]`，函数级明细引用本设计 §11.5。
...
| 27（Mat4x4 列字段名） | 列字段名为 `c0`/`c1`/`c2`/`c3` | §3.3 item 7 fromMat4 实现 |
