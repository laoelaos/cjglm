...
> **修订日期**：2026-06-27
> **修订定位**：v29 设计的迭代修订（对应本轮审查 a_v30_iteration_requirement.md）。基于 v29 输出修订，针对审查报告识别的 10 项问题（3 严重 + 6 一般 + 1 建议）逐项落实。本轮重点修复：(1) §1 设计目标段重构——将风险声明后移至设计目标首句后立即插入，11%（18/165）作为首要基准口径醒目突出，明确声明阶段四后方可完成完整 GLM 四元数功能；(2) §3.16 降级路径补充——quatLookAt* 给出内联 crossVec3 实现模板及成本估算，slerp→lerp 补充角度阈值适配成本（2-3 人日），nlerp 作为中间方案加入；(3) §11.5 新增独立 Quat×Vec4 行，合计行同步更新为 ⚠️ 2 个；(4) §3.16 末尾新增「阶段四 stub 函数实施优先级排序」（P0/P1/P2 三级）和「API 可用率增长路线图」（4 批次阶梯）；(5) §1 T(0) 清单段补充穷举过滤规则；(6) §1 新增泛型上下文 H1 验证说明；(7) §8.3 H 节新增 H5 版本标注自动化门禁；(8) §8 验证项 21 radians/degrees 行附注 PIVAL<T> 共享常量建议。
> **v30 补充修订（Problem 1 响应）**：Vec3×Quat/Vec4×Quat 可用状态从 ✅ 修正为 ⚠️。设计正确识别了 v18 内联 conjugate/dot 路径可消除包间循环依赖，但未进一步评估该路径是否消除对 Vec3 cross stub 的运行时依赖。实质上，Vec3×Quat/Vec4×Quat 的 `(conjugate(q) / dot(q,q)) * v` 最终委托同包 `Quat×Vec3` 运算符，后者依赖 `geometric.cj` 向量 `cross`（阶段三 stub），运行时抛 stub 异常。涉及修复：§1 表格 ⚠️/❌ 计数、§1.2 用户能力视图、§3.4 运算符表与实现链路注释、§3.13.2 审计结论、§8.2 测试覆盖维度、§9a/§8.3C 覆盖矩阵、§11.5 表行与合计行、§11.7.1/§11.7.2 索引表、§5.3 边界条件表。
...
| ⚠️ 编译通过但抛 stub | 2 | Quat×Vec3/Vec4 + Vec3×Quat/Vec4×Quat 运算符（均依赖 geometric.cj 向量 cross stub） |
...
> **计数口径说明（v30 修订，Problem 7 响应）**：本表以「**约 11%**（18/165）」为**首要基准口径**（独立函数 18 个不含运算符级条目；⚠️ 类别中含 Vec3×Quat/Vec4×Quat——后者因实现链依赖 Quat×Vec3 的 geometric.cj cross stub 已归入 ⚠️，详见 §3.4），在表头和分类行中醒目标注。47%（77/165）仅作为签名行数参考统计附注——两者不可并列使用。上游决策者和下游消费者应以独立函数范式口径的 **11%** 作为阶段三实际可用功能的评估依据，而非 47% 的展开口径。
...
### 关于版本标注（v27 新增，Problem 5 响应；v28 扩展清理，Problem 6 响应）

本设计文档采用迭代演进式修订方式，正文中残留了大量跨轮次的版本标注（"**v14 关键修订**"、"**v15 修订，Issue 6 响应**"等）。这些标注虽在历史迭代中起过追踪作用，但对当前设计的首次阅读者构成视觉噪音和考古负担。

**v27 清理原则（v28 扩展执行）**：
1. **§7 设计决策表**保留各决策的版本号（D01~D37 均标注首次引入的版本号），这是决策溯源必需的元数据
2. **非决策性位置的版本标注**（正文段落中的 "**v{N} 关键修订**"、"**v{N} 修订**"、"**v{N} 新增**" 等）——保留标注内包含的设计决策内容（约束说明、策略选择、Issue 引用等），移除版本标记本身
3. **§3.13.2/§8/§11.5 等计数或表格**中保留少量版本标记仅用于计数变更解释，不从正文中移除
4. **v28 补充清理（Problem 6 响应）**：在 v27 清理基础上，重点清理 §2/§3.4/§3.7/§3.9/§3.10 等章节中残留的非决策性版本标注——移除标注外层 **v{N} 包装**，保留标注包含的设计内容文字。具体清理项包括 (a) §2 lib.cj/fwd.cj 段落中的 **v10/v12/v13 修订标记**（保留 Issue 引用和计数信息）；(b) §3.4 运算符表中的 **v9/v11/v12 修订标记**（保留公式和实现细节）；(c) §3.7 normalize/axis 函数描述中的 **v11/v13/v14/v26 修订标记**（保留边界行为和约束说明）；(d) §3.9 axis 函数描述中的 **v8/v10/v13 修订标记**（保留计算公式和触发条件）；(e) §3.10 pow/log/exp 函数描述中的 **v5/v7/v10/v11 修订标记**（保留依赖分析和实现路径）。

读者如需追溯特定版本的完整变更历史，请查阅独立历史文件 `a_v27_revision_history.md`（本版本修订说明见文件末尾「修订说明（v27）」。当前版本 v29 的修订说明见文件末尾「修订说明（v29）」章节）。

### 1.2 阶段三用户能力视图（v29 新增，Problem 1 响应）
...
#### 典型场景端到端代码示例（v29 新增，Problem 1 响应）
...
#### 阶段三交付后用户预期管理（v29 新增，Problem 1 响应）
...
**lib.cj 新增 import 清单（v5 明确，v9 扩展补齐，Issue 1 响应实际行数声明）**：
...
- `inverse(q: Quat<T,Q>): Quat<T,Q>` — `conjugate(q) / dot(q, q)`。**完整实现**（依赖 dot 和 除法运算符）。**约束收紧（v29 修订，D38 响应）**：函数签名从 `where T <: Number<T>` **收紧为 `where T <: FloatingPoint<T>`**，与 `normalize` 的 v26 收紧策略一致。整数 T（`Int8`~`Int64`）实例化时编译期报类型不匹配错误，消除整数除法截断导致的无声错误结果。**边界行为（v3 新增契约，**v10 措辞统一，Issue 13 响应；v29 修订，D38 响应：整数 T 编译期拒绝**）**：
...
**完整函数签名模板（v28 新增，Problem 4 响应）**：以下按 9 个分类逐一列出全部 64 个函数的完整 Cangjie 签名模板。所有函数统一约束 `where T <: FloatingPoint<T>, Q <: Qualifier`，函数体统一为 `{ throw Exception("stub") }`。签名中的参数类型 `Mat4x4<T, Q>` / `Vec3<T, Q>` / `Vec2<T, Q>` / `Vec4<T, Q>` 使用阶段一/二已定义的类型。
...
**展开规则验证**：上述 9 个代码块合计 11 + 10 + 9 + 9 + 9 + 7 + 2 + 6 + 1 = **64 个函数签名**，与 §3.13 表格中声明的 64 个函数一致。下游编码者按此模板逐一复制到 `gtc/matrix_transform.cj` 中，每个函数体统一为 `{ throw Exception("stub") }`，`where T <: FloatingPoint<T>, Q <: Qualifier` 约束统一适用于每个函数签名。**v28 修订要点**：从 v24 的 6 个代表性模板（覆盖 6 个类别，58/64 个函数需下游自行推断）升级为 9 个类别全部 64 个函数的直接可复制签名列表，消除下游推断的不确定性。
...
**审计结论**：
- **本阶段可调用的「真完整实现」函数**（无 stub 依赖）：`epsilon<T>()`/`pi<T>()`/`cos_one_over_two<T>()`/`axis`/`length`/`normalize`/`inverse`/`isnan`/`isinf`/`mat3Cast`/`mat4Cast`/`quatCast`/`conjugate`/`lerp`/`dot`/`cross(Quat)`/`equal(Quat)`/`notEqual(Quat)`（共 **18 个函数**【v30 修订，Problem 1 响应：Vec3×Quat/Vec4×Quat 为运算符级条目（非独立函数）从 ✅ 移入 ⚠️ 不影响此 18 个命名函数计数】）**——按函数名计数（不含重载）；按 §11.5 表行粒度展开为 19 行**（v24 修订，Issue 4 响应，v30 修订：移除 Vec3×Quat/Vec4×Quat 两行 ✅ 后为 19 行）
...
- **本阶段 stub 占位的函数**：`mix`/`slerp`/`exp`/`log`/`pow`/`sqrt`/`angle`/`angleAxis`/`rotate`/`eulerAngles`/`roll`/`pitch`/`yaw`/`quatLookAt*`（14 个）+ `gtc/matrix_transform` 全部 64 个函数 + `fromVec3`/`fromEuler`（2 个）+ `ext/vector_relational` ULP 版本（4 个 ❌ 条目覆盖 8 重载）+ `type_quat.cj` 中 Quat×Vec3/Vec4/Vec3×Quat/Vec4×Quat（2 个 ⚠️） = **88 个**（**v30 修订**：⚠️ 从 2 个增加至 4 个，同步调整 ❌ 计数保证 ⚠️+❌ = 88）
...
### 3.16 需求对齐说明（v19 修订，D35-D37 已决策；v28 修订：量化偏差累积影响、下游风险评估与临时降级路径）
...
**三项不一致累积影响汇总（v28 新增，Problem 2 响应）**：
...
**临时降级路径一览表（v28 新增，Problem 2 响应；v29 扩展，Problem 1(c) 响应：补充 GLM 源函数名、仓颉代码骨架、行为差异量化评估、成本分解依据；v30 修订，Problem 8 响应：补充 cross 实现、nlerp 中间方案、角度阈值适配成本）**：
...
#### 阶段四 stub 函数实施优先级排序（v30 新增，Problem 9 响应）
...
#### API 可用率增长路线图（v30 新增，Problem 10 响应）
...
|----------|---------|-------------|----------------|---------------------|--------------------------|
| **批次 0**（阶段三现状） | — | 0 | 18 | **~11%** | ~47%（~77 个） |
| **批次 1** | trigonometric.cj 标量版核心函数完工（acos/sin/asin/atan/clamp 等 ~10 个标量函数） | +9 | 27（P0 组：slerp/mix/angle/angleAxis/eulerAngles/roll/pitch/yaw + 4 欧拉重导出的独立函数化） | **~16%** | ~52% |
| **批次 2** | trigonometric.cj 其余标量函数 + ULP 比较（如 CFFI 方案可行）+ geometric.cj（cross/dot/normalize 向量版）完工 | +8（P1 组：fromVec3/fromEuler + 3 quatLookAt + 3 实际 geometric 向量函数 = 8） | 35 | **~21%** | ~57% |
| **批次 3** | common.cj + quaternion_exponential.cj（exp/log/pow/sqrt）+ rotate + trigonometric.cj 向量版完工 | +7（P1+P2 组：exp/log/pow/sqrt/rotate + common.cj 新增 2 独立函数） | 42 | **~25%** | ~62% |
| **批次 4** | gtc/matrix_transform 64 函数 + ULP（如批次 2 未完成）+ 剩余细节补齐 | +2（ULP）+ 64 作为独立工作包 | 44 + 64 | **~65%**（独立函数，含 matrix_transform） | ~100% |
...
#### §8.2 阶段四测试增量规划（v27 新增，Problem 8 响应）
...
### 5.6 跨 Qualifier 行为统一契约（v29 新增，Problem 5 响应）
...
| O-06 | 验证前置化流程（核心假设在设计版本固化前完成验证）尚未制度化 | v25 | **v28 已落实**（详见下方 O-06 可执行落地计划） | v28 |
...
### 11.5 函数可用性对照表（v3 新增，v23 关键修订：补齐缺失 API 条目后升级为单一权威来源）

**权威声明与质量状态（v27 新增，Problem 6 响应）**：
...
**v23 修订**：本表目标为**函数级状态追踪的权威来源**（每函数一行，标注本阶段/阶段四状态）。§3.13.2 审计节、§8 产出物清单、§9a 覆盖矩阵、§11.6 可达性矩阵的函数状态描述均以本表为主要基准。§9a「GLM 1.0.3 Stage 3 API 覆盖矩阵」提供聚合统计信息，与本表构成互补关系——本表聚焦逐函数精细追踪，§9a 提供聚合统计视图与 GLM 1.0.3 基准声明。
...
> **§11.5 与 §9a 计数口径映射注记（v29 新增，Problem 2 响应）**：本表合计行采用「独立函数范式」口径（✅ ~18 个），§9a 覆盖矩阵合计行采用「含重载/常量展开」口径（✅ ~77 个）。两者差距约 59 个，来源于：(a) 本表按函数名计数（如 `equal` 无论有多少个重载/epsilon 变体均计为 1 行），而 §9a 按 GLM 头文件原始 API 计数（含重载展开和常量展开）；(b) 本表排除了 `gtc/constants` 的 28 个常量函数（不视为独立函数），§9a 将其计入 ✅ 可用；(c) 本表排除了 `ext/vector_relational` epsilon 版本的 16 个重载（作为 4 个函数的变体），§9a 以重载粒度统计。**读者应优先以本表「独立函数范式口径」作为阶段三实际可用独立功能的基准评估口径**；§9a「含重载/常量展开口径」仅用于签名行数维度的参考统计。两处合计行的实际差值（~18 vs ~77 ≈ 59）为上述三项计数规则差异之和，非实质性数据矛盾。
...
| 问题 5 | 一般 | 正文残留过时修订标注约 100+ 处（"**v14 关键修订**"、"**v15 修订**"等） | §1 末尾新增 **"关于版本标注"说明段**清理原则；正文中非决策性位置的版本标注已逐一清理（保留 D01~D37 决策版本号）；**问题 6 的 §11.5 版本标注同步清理** |
...
**v26 修订措施核验结果（Problem 2 响应）**：以下逐条确认 v26 修订说明中 5 项修改措施已在正文中实际落实：
