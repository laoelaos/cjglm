...
> **修订日期**：2026-06-26
> **修订定位**：v23 设计的迭代修订（对应本轮审查 a_v23_iteration_requirement.md）。基于 v22 输出修订，针对审查报告识别的 7 项问题（3 严重 + 4 一般）逐项落实。本轮重点修复：(1) 删除全部「审查意见修复率累计达 100%」虚假声称，以 §8.3 I 节已知未解决问题列表替代；(2) §11.5 权威声明从「主要参考来源」升级回「单一权威来源」；(3) §8.3 H 节新增 H3 版本号一致性核验项；(4) §3.16 新增签名冻结例外审批流程 + §8.3 E 节新增签名冻结核验项；(5) §11.7 扩展为全函数集中参考索引（11.7.1 ✅ + 11.7.2 ⚠️/❌）；(6) 全部验证项补充「修复责任人」列；(7) H1 关联自动化工具 + 核验执行者职责声明 + §9a/§10 合并计划。
...
`§11.5 函数可用性对照表` 是阶段三验证的**单一权威来源**（v23 修订，Issue 2 响应：v22 已补齐全部 5 个缺失 API 条目，从「主要参考来源」升级回「单一权威来源」）：✅/⚠️/❌ 三档符号标记每个函数本阶段状态（实现在本文件其他章节有详细描述）。下游验证时按 §11.5 表逐项核验即可。
...
- **§9a 与 §10 合并计划（v23 新增，Issue 7 响应）**：当前 §9a「GLM 1.0.3 Stage 3 API 基准声明」与 §10「GLM 1.0.3 API 阶段覆盖矩阵」存在数据冗余——二者均列示 GLM 头文件、函数计数、覆盖状态。建议在 v24 中将 §9a 的基准声明融入 §10 表头（作为「GLM 基准计数」列），消除两份独立表格的数据同步风险。过渡期间，§10 的合计行计数以 §9a 为基准，两表应保持一致。
...
| O-02 | §9a 与 §10 两张覆盖矩阵存在数据冗余，需合并为单一来源 | v23 | 待设计评审 | v24 |
...
**v23 修订，Issue 2 响应**：本表目标为函数阶段状态追踪的**单一权威来源**。§3.13.2 审计节、§8 产出物清单、§10 覆盖矩阵、§11.6 可达性矩阵的函数状态描述均以本表为主要基准。v22 已补齐全部 5 个缺失 API 条目（`wxyz`/`fromQuat`/`[]`/`static func length`/复合赋值运算符），当前版本已具备完整性要求。v20 的「主要参考来源」声明升级回「单一权威来源」。
...
| 问题 7 | 一般 | §8.3 H 节的 H1（累加验证）和 H2（grep 清除旧字样）两项核验机制仅停留在文档层面，无自动化脚本或核验模板，无核验执行者职责定义 | (a) H1 关联自动化工具（Spreadsheet 模板或 Python 脚本），未就绪时提供人工核验模板引用；(b) H 节末尾新增「核验执行者与周期」职责声明；(c) §9a 与 §10 合并计划声明（v24 实施），过渡期间 §10 以 §9a 为基准 |
