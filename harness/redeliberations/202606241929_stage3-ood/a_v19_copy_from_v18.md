...
> **修订日期**：2026-06-25
> **修订定位**：v19 设计的迭代修订（对应本轮审查 a_v19_iteration_requirement.md）。基于 v18 输出修订，针对审查报告识别的 7 项问题（2 严重 + 4 一般 + 1 轻微）逐项落实。本轮重点修复：(1) §10 覆盖矩阵合计行计数——从「约 80 个 ❌/⚠️（含 1 ⚠️ + 79 ❌）」修正为「87 个 ❌ + 1 个 ⚠️ = 88 个 ⚠️/❌」；同步在 §8.3 新增 H 节自动化核验项；(2) §8.2 用例分配表附注——从引用废弃 ≥8 下限修正为 ≥4 下限；(3) §2 `gen_fwd_aliases.py` 修改指南——补充现有 VEC_FAMILIES 代码片段、修改后完整 Python 代码、grep 验证命令和幂等性验证方案；(4) §3.16 三项「⚠️ 待确认」措辞——修订为「✅ D{N} 已决策」并移除过渡性表述；(5) §3.3 item 5 `identity`——补充 T(0) 获取路径为 §1 统一策略刻意例外的说明；(6) §8 验证项 27 及映射表——补充修复责任人标注；(7) §3.13.1 三角函数表 radians/degrees 行——简化内部依赖列、补充 π 值一致性交叉引用。
...
- **验证项 20（P0）**：`FloatingPoint<T>` 接口方法可用性验证——要求在编码阶段前先编写最小仓颉测试文件（~20 行），实例化 `FloatingPoint<Float32>.getMinDenormal()`/`getInf()`/`isInf()`/`isNaN()`/`getNaN()`/`getMinNormal()`，确认编译通过。验证结果反馈到 §3.10/§3.11 的依赖描述中更新。若编译失败，则 §3.10 `log` 函数中 `FloatingPoint<T>.getInf()` 路径和 §3.10 `pow` 函数中 `FloatingPoint<T>.getMinDenormal()` 路径的实现假设被证伪，需回退至类型分派 fallback 路径。具体回退路径参见 §1 回退方案子节（路径 A/B/C 及决策树执行顺序）。
...
### 11.5 函数可用性对照表（v3 新增，**v15 关键修订，Issue 5 响应：统一权威来源，v13 关键修订，Issue 1 响应：补齐缺失条目 + 约束注记，v15 修订，Issue 2 响应：全文档以此表为最终基准**）
