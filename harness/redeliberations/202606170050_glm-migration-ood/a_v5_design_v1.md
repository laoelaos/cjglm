...
`ComputeEqual<T>` 的 `isIec559` 判定不依赖 `Number<T>` 约束——`NumericLimits` 被拆分为两个独立部分（详见 §3.6 和 §7 D18），`isIec559` 标志对所有 `T`（含 `Bool`）可用，非数值类型默认返回 `false`。
...
struct ComputeEqual<T> {
    ...
    const static func callConst(a: T, b: T): Bool {
        const if (isIec559Of<T>()) {    // 无 T <: Number<T> 约束
...
4. `isIec559Of<T>()` 由 `shim_limits.cj` 提供。实现策略：为每类数值类型提供重载函数，非数值类型（含 `Bool`）使用默认泛型版本返回 `false`。
...
- **`shim_limits.cj`**：提供数值极限查询功能。为避免 `NumericLimits<T>` 上的 `where T <: Number<T>` 约束与 D5（宽松泛型约束，允许 `Bool` 作为 `T`）冲突，将 `NumericLimits` 拆分为两个独立部分：
  ...
  - **`isIec559Of<T>()` 函数**（无约束）：提供编译期 `Bool` 常量，判断 `T` 是否为 IEEE 754 浮点类型。对任意 `T`（含 `Bool`）可用，非浮点类型默认返回 `false`。此函数由 `ComputeEqual.callConst` 中的 `const if` 分支使用，无 `Number<T>` 约束要求。
  
  `isIec559Of<T>()` 的实现策略：在 `shim_limits.cj` 中为 `Float32` 和 `Float64` 各定义一个特化版本（返回 `true`），其余类型通过泛型默认定义（返回 `false`）。
...
public init<T2, Q2>(a: Vec1<T2>, b: T2) where Q2 <: Qualifier                                    // 跨分量数组合
public init(v: Vec1<T>)                                                                            // 从 Vec1 显式构造
public init<T2, Q2>(a: T2, b: Vec1<T2>) where Q2 <: Qualifier                                    // 标量+Vec1
public init<T2, Q2>(v: Vec1<T2>, s: T2, t: T2) where Q2 <: Qualifier                             // Vec1 + 标量*2（仅 Vec3/Vec4 需要更丰富组合）
...
**总结**：`@OverflowWrapping` 在二元算术运算符（4 个）、`<<`（1 个）、`increment()`/`decrement()`（2 个）上直接标注，共约 28 处标注；复合赋值通过编译器自动生成继承标注语义。`@OverflowThrowing` 默认策略对 `>>`、`&`、`|`、`^` 保持原样（这些运算不产生溢出）。无内部矛盾的唯一决策。
...
| 4 | `shim_limits.cj` | 无 | `NumericLimits<T>`（含 `isIec559Of<T>()` 无约束函数，`max/min/epsilon` 受 `Number<T>` 约束） |
...
`isIec559Of<T>()` 需在 `shim_limits.cj` 中以编译期常量形式为每个数值类型定义。各类型取值如下：
...
| `Bool` | `false` | 布尔类型，非 IEC 559。`isIec559Of<Bool>()` 通过默认泛型版本返回 `false` |

**注**：C++ `is_iec559` 判断类型是否完全符合 IEEE 754 标准。仓颉的 `Float32`/`Float64` 预期符合 IEC 60559（IEEE 754），但此符合性需在目标平台上验证。若平台浮点实现有偏差，`isIec559Of<Float32>()`/`isIec559Of<Float64>()` 应设为 `false` 以回退到精确比较路径。
...
| 审查意见 | 修改措施 |
|---------|---------|
| **问题 1（严重）**：`++`/`--` 自增自减运算符完全缺失 | §3.2 Vec 结构体系新增 `++`/`--` 替代方案说明。§4.3 算术运算新增 `increment()`/`decrement()` 具名函数替代方案，标注 `@OverflowWrapping`。§4.6 标注策略增加 `increment()`/`decrement()` 的 `@OverflowWrapping` 标注（共 8 处）。§11 迁移成本评估新增 11.2 节 `++`/`--` 迁移章节，包含表达式模式表格和迁移模式说明。 |
| **问题 2（严重）**：`NumericLimits<T>` 泛型约束与 D5 宽松约束冲突 | 采用方案 A（推荐）。§3.5 `ComputeEqual` 重写为使用 `isIec559Of<T>()` 替代 `NumericLimits<T>.isIec559`。§3.6 `shim_limits.cj` 描述重写，将 `NumericLimits` 拆分为两个独立部分：`NumericLimits<T> where T <: Number<T>`（保留），无约束的 `isIec559Of<T>()` 函数（新增）。§7 新增 D18 记录拆分决策和理由。§10 表格新增 `Bool` 行。依赖图中 `compute_vector_relational.cj` 依赖说明同步更新。 |
| **问题 3（高）**：`bitwiseNot()` 对整数类型的实现路径不明确 | §4.4 位运算重写 `bitwiseNot()` 实现描述：明确 `!` 在仓颉中既服务于 `Bool`（逻辑非）又服务于整数类型（按位取反），因此 `bitwiseNot()` 统一对每个分量调用 `!` 即可。移除了原描述中模糊的"或按位取反逻辑"措辞。 |
| **问题 4（高）**：构造函数签名清单缺少类型参数声明 | §4.1 构造函数签名清单更新：每个跨类型构造函数的签名补充了完整的类型参数声明（`<T2, Q2>`）和 `where Q2 <: Qualifier` 约束。将"完整构造函数签名清单"标题改为"示例性构造函数签名"，并在描述中明确标注"上述签名清单为示例性"。 |
| **问题 5（高）**：事实错误——"仓颉位运算无溢出异常"不适用于 `<<` 运算符 | §4.4 位运算修正：删除"不触发溢出检查（仓颉位运算无溢出异常）"描述，明确 `<<` 默认使用 `@OverflowThrowing` 策略，需标注 `@OverflowWrapping` 以匹配 C++ GLM 行为。§4.6 标注策略将 `<<` 纳入 `@OverflowWrapping` 范围（新增 4 处标注），补充 `<<` 的溢出行为说明。§5.1 错误处理表格新增"位运算溢出"行。 |
| **问题 6（一般）**：`scalar op vec` 具名函数替代缺少 `mod` | §4.3 算术运算的"标量在左的运算替代方案"列表补上 `mod(v: VecN<T,Q>, s: T): VecN<T,Q>` 和 `mod(s: T, v: VecN<T,Q>): VecN<T,Q>` 双向重载。 |
| **问题 7（一般）**：跨类型赋值语义未定义 | §4 末尾（原 §4.6 之前）无合适位置，改为在 §7 新增 D17 设计决策，明确三种候选路径的分析和选择理由（采用显式转换后赋值，不提供跨类型 `=`）。§11 迁移成本评估新增 11.3 节"跨类型赋值差异"，包含表达式映射表格和替换说明。 |
