...
- **相等比较通过 `ComputeEqual<T>` 策略处理**：简化后的 `ComputeEqual` 不再携带 `IsFloat` 参数，使用 `const if (NumericLimits<T>.isIec559)` 在编译期分支（详见 §3.5）。
...
struct ComputeEqual<T> {
    ...
    const static func callConst(a: T, b: T): Bool {
        const if (NumericLimits<T>.isIec559) {
            // 浮点路径：使用 Epsilon 容差比较
            abs(a - b) <= NumericLimits<T>.epsilon()
        } else {
            // 整数路径：精确比较
            a == b
        }
...
`IsFloat` 参数不再需要——`const if (NumericLimits<T>.isIec559)` 在编译期直接查询 `T` 是否为 IEEE 754 浮点类型，零运行时开销地选择比较路径。`ComputeEqual` 的类型参数简化为单一 `T`。

**编译期判定路径**：

1. Vec 结构体定义 `const init` 构造函数。
2. Vec 的 `==` 运算符声明为 `const` 函数。
3. `const` 函数体内通过 `const if (NumericLimits<T>.isIec559)` 在编译期选择比较路径——浮点类型使用容差比较，整数类型使用 `==` 直接比较。
4. `NumericLimits<T>.isIec559` 由 `shim_limits.cj` 提供，为各数值类型（`Int8`、`Int16`、`Int32`、`Int64`、`UInt8`、`UInt16`、`UInt32`、`UInt64`、`Float32`、`Float64`）的编译期常量。
...
- **`shim_limits.cj`**：提供泛型 `NumericLimits<T>` 结构体（或独立函数），查询 `T` 类型的最大值、最小值、是否为 IEEE 754 浮点等特性。实现为各数值类型静态属性的泛型封装：`where T <: Number<T>` 约束。关键接口：
  - `static isIec559: Bool` — 编译期常量，判断 T 是否为 IEEE 754 浮点类型。各类型取值表见附录（§10）。
  - `static max(): T` — 类型最大值。
...
仅整数分量 Vec 有意义。包括 `&`、`|`、`^`、`<<`、`>>`。逐分量运算，不触发溢出检查（仓颉位运算无溢出异常）。二元位运算符通过 `extend` 块定义为扩展成员函数。

`~`（单目按位取反）不在仓颉可重载运算符列表内，不可实现为运算符重载。改为提供具名函数 `bitwiseNot(): VecN<T,Q>`，通过 `extend` 块定义为扩展成员函数，内部逐分量对每个分量调用 `!`（逻辑非）或按位取反逻辑。

### 4.5 比较运算

- **`==`**：逐分量调用 `ComputeEqual<T>.callConst` 比较，全分量相等返回 `true`。声明为 `const` 函数，利用 Vec 的 `const init` 构造函数支持，在函数体内通过 `const if (NumericLimits<T>.isIec559)` 在编译期选择比较路径——浮点类型使用 Epsilon 容差比较，整数类型使用 `==` 直接比较。
...
- **二元算术运算符**（`+`、`-`、`*`、`/`）：在 Vec 结构体内部或 extend 块中定义为 `operator func`（非 `mut`），**直接标注 `@OverflowWrapping`**。共 4 个运算符 × 4 个 Vec 类型 = 16 处标注。运算符内部实现逐分量直接运算——不再使用"copy + op="委托模式（因复合赋值不可显式定义）。
- **`%` 运算符**：同样定义（见 §4.3 整数约束），受整数/浮点类型约束影响。仅在整数类型 Vec 实例上可正常编译，标注 `@OverflowWrapping`。
...
**总结**：`@OverflowWrapping` 在二元算术运算符上直接标注，共 16~20 处标注；复合赋值通过编译器自动生成继承标注语义。无内部矛盾的唯一决策。
...
| 整数溢出 | 通过在二元算术运算符上标注 `@OverflowWrapping` 将溢出行为从"抛异常"改为"wrapping"，与 C++ UB 行为在数值结果上等价。 |
...
| 9~12 | `type_vec1.cj`~`type_vec4.cj` | `qualifier.cj` + `compute_vector_relational.cj` | Vec 结构体定义（含 `const init`、二元算术运算符 `@OverflowWrapping`）+ `extend` 块（位运算符 + 具名函数） |
...
（与 v3 一致，略去具体内容以节省篇幅——保留以下小节标题和核心约定。）
...
（以上四节内容与 v3 方案一致，未受本轮审查影响。）

---

## 10. `NumericLimits<T>.isIec559` 取值对照表

仓颉中 `isIec559` 需在 `shim_limits.cj` 中以编译期常量形式为每个数值类型手动定义。各类型取值如下：
...
**注**：C++ `is_iec559` 判断类型是否完全符合 IEEE 754 标准。仓颉的 `Float32`/`Float64` 预期符合 IEC 60559（IEEE 754），但此符合性需在目标平台上验证。若平台浮点实现有偏差，`isIec559` 应设为 `false` 以回退到精确比较路径。
...
| 审查意见 | 修改措施 |
|---------|---------|
| **P1/N3（严重）**：`mut operator func +=` 不可显式定义；"copy + op="委托模式形成自递归 | §4.6 完全重写：移除所有显式复合赋值运算符定义。二元算术运算符（`+`、`-`、`*`、`/`）标注 `@OverflowWrapping`、直接实现逐分量运算；复合赋值由编译器自动生成。§3.2 和 §7 D3 同步更新。删除 §4.6 中关于"显式定义优先级"和"自动生成机制"的错误描述。 |
| **P3（严重）**：§4.5 引用了未定义的类型 `Vec<Bool, Q>` | 将 §3.2、§4.5 中所有 `Vec<Bool, Q>` 引用替换为"各 VecN 类型分别定义对应版本"的描述方式，修正为 `VecN<Bool, Q>` 措辞。 |
| **N1（高）**：默认构造函数 `public init()` 无法编译 | §3.2 移除"所有成员有初始值时自动生成"的断言。明确数据成员声明为 `var x: T`（无初始值），编译器不生成无参默认构造函数。§4.1 构造函数清单中删除 `public init()` 条目。调用方须使用标量填充构造 `Vec2(T(0))` 作为默认值替代。§7 D8 更新理由说明。 |
| **P5（高）**：`&&`/`||` 改为具名函数的迁移成本未评估 | 新增 §11 迁移成本评估章节，包含受影响表达式模式表格、迁移模式描述（机械可执行的语法替换）、短路语义丢失分析、自动迁移工具建议。 |
| **P4（中）**：跨类型复合赋值未明确承认 | §4.3 新增"跨类型操作"子段，明确说明二元运算符仅支持同类型操作数，不支持跨类型。§7 新增 D15 记录决策理由。 |
| **P6（中）**：Functor/ComputeVec 未被运算符实现实际使用 | §3.3 和 §3.4 标题及描述中明确标注"为后续轮次预留"。§2 模块依赖中标注这些文件定义类型但不被 Vec 运算符消费。§7 新增 D16 记录消费链路和后续轮次计划。§8 文件清单同步更新说明列。 |
| **P7（中）**：`operator[]` 签名未完整指定，缺少赋值形式 | §4.2 补充 `operator[]` 的完整签名：取值形式 `operator func [](i: Int64): T`，赋值形式 `operator func [](i: Int64, value!: T): Unit`（`value` 为命名参数）。 |
| **P8（中）**：§3.6 与 §5.1 关于 Release 模式断言行为矛盾 | 统一为不变立场：§3.6 和 §5.1 均表述为"首轮均保留断言，Release 模式消除标记为后续轮次任务"。删除了 §5.1 中"首轮建议通过 GLM_CONFIG_* 配置 + const if 在 Release 模式下跳过"的错误描述。 |
| **P9（中）**：`%` 运算符无条件定义导致浮点类型编译错误 | 采用策略 B（编译期错误于浮点实例化处）：§3.2 和 §4.3 描述 `%` 在整数 T 时编译通过，在浮点 T 时因原生类型不支持 `%` 产生编译错误。§4.6 `%` 标注段落同步更新。§7 D13 更新理由。 |
| **P10（中）**：`ComputeEqual` 的 `IsFloat` 参数缺少可行的编译期映射 | 移除 `IsFloat` 参数：`ComputeEqual<T>` 简化为单一类型参数。§3.5 重写 `ComputeEqual` 定义为 `struct ComputeEqual<T>`，使用 `const if (NumericLimits<T>.isIec559)` 进行编译期分支选择。 |
| **P11（低）**：`isIec559` 语义不等价于 C++ `is_iec559`，缺少各类型取值表 | 新增 §10 取值对照表：列出 10 个数值类型的 `isIec559` 值（整数均为 `false`，浮点均为 `true`），并注明仓颉浮点类型的 IEC 559 符合性需验证。 |
| **P12（低）**：`setup.cj` 说明列仍包含 `assert` | §8 文件清单中 `setup.cj` 说明列从"配置常量、`assert`"改为"18 个 `GLM_CONFIG_*` 编译期常量"。 |
| **N2（低）**：`countof` 签名缺少 `$` 编译期值参数前缀 | §3.6 `countof` 签名修正为 `func countof<T, $N>(arr: VArray<T, $N>): Int64`。§8 文件说明列同步修正。 |
