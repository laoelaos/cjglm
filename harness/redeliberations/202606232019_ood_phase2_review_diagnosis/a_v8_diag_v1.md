...
**风险**：非方阵的标量运算展开公式与方阵结构相同（逐元素操作），但行/列参数错误（如 `div(s, m)` 中 `s / m.c_j[i]` 的列/行索引写反）在当前测试套件中无法被检出。**注**：OOD §3.5 定义的 36 个全局函数不仅在 `test_scalar_mat_ops.cj` 中缺失单元测试，在 `test_integration_matrix.cj` 中也没有非方阵标量运算的集成测试覆盖。集成测试层面是否存在同类缺口需另案审视，建议 T6 完成后检查 `test_integration_matrix.cj` 是否包含非方阵标量运算场景。
