# 诊断报告：OOD Phase2 Review 问题定位（修订版 v2）

> 报告日期：2026-06-23
> 诊断范围：requirement.md 中列出的全部 T1~T19 问题
> 修订：基于审查质询修正事实偏差，补充遗漏分析
...
**可量化最小覆盖标准**：建议为每个矩阵类型（9 个）至少选择以下场景以 Float32 和 Float64 分别实例化：
- 1 个构造测试（选择代表性构造器，如 `diagonal(scalar)` 或 `init(scalar: T)`）
- 1 个列访问测试（`col(i)` 返回值与预期对照）
- 1 个算术运算测试（矩阵-矩阵乘或标量-矩阵运算，选择其中一个）
此标准与 OOD §4.1 阶段 2"矩阵创建、行列访问、基本算术运算"验证要求对齐。共计约 9 类型 × 3 场景 × 2 精度 = 54 个新增测试。
...
**结论与建议**：建议**保留** ZeroVec 测试（边界有效性验证具有独立语义价值，确认零向量×任意矩阵得到零结果这一数学恒等式成立），但可将 9 个 ZeroVec 测试合并为 1~2 个代表性场景（如选取 Mat2x2 和 Mat4x4 各一个）以减少重复；或选择保持现状（18 个测试的维护成本可接受）。
...
以下问题操作不同的文件，无合并冲突风险，可并行实施：
- T6、T7、T3、T7-cs、T8、T2、T9、T10、T14、T18、T13
...
| 质询意见 | 回应 |
|---------|------|
| Problem 1（T13 未分析已有 test_ext.cj 差异） | 已修正：在 T13 中补充说明 `tests/glm/test_ext.cj` 存在的事实，区分"测试域内 alias 验证就绪"与"外部导入验证缺位"两个层面，并重新评估 T13 的实际缺口。 |
| Problem 2（T15 缺乏 GLM 参考数据获取指导） | 已补充：在 T15 中简要说明 GLM 参考代码位于 `references/glm-1.0.3/glm/glm/detail/func_matrix.inl` 及 `type_mat4x4.inl`，可直接逐行对照。 |
| Problem 3（T13 事实表述偏差） | 已修正文件存在性描述。将"24 个 matrix_*.cj"修正为"18 个 matrix_*.cj + 8 个 vector_*.cj = 26 个总计"。 |
| Problem 4（T18 评估了冗余但未给出结论） | 已追加明确结论：建议保留（边界验证有独立语义价值），但可将 9 个 ZeroVec 合并为 1~2 个代表性场景，或保持现状。 |
| Problem 5（T8 编译器警告声明依赖推测） | 已在 T8 中核实 `cjpm.toml` 指定 cjc-version = "1.1.0"，标注结论为"未验证"——建议修复者在 cjc 1.1.0 下实际编译确认。 |
| Problem 6（T16 可操作性严重不足） | 已给出可量化最小覆盖标准：每个矩阵类型至少选择 1 个构造测试、1 个列访问测试、1 个算术运算测试，分别以 Float32/Float64 实例化，与 OOD §4.1 验证要求对齐。 |
| Problem 7（T10 缺乏仓颉测试框架能力适配分析） | 已补充：`std.unittest` 的 `@ExpectThrows` 仅验证异常类型，不支持消息内容断言。需使用手动 `try-catch` + `e.message.contains("stub")` 模式，并给出具体示例代码。 |
| Problem 8（缺失跨问题依赖分析） | 已新增 §6 跨问题依赖分析，包含依赖图、串行约束（T5↔T17 共享 `test_integration_matrix.cj`、T1↔T19 共享 `deviations.md`、T15↔T16 范围重叠），以及可并行处理的问题列表。 |
