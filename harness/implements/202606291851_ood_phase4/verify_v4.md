...
## 统计
- 通过：0
- 失败：0（编译失败，共2个编译错误，测试未运行）

## 测试执行日志
error: '()' is not a static member of exposed generic parameter 'T'
  ==> C:\Develop\Software\cjglm_wp\cjglm\src\detail\trigonometric.cj:56:14:
   | 
56 |     if (x < -T(1) || x > T(1)) { return T.getNaN() }
   |              ^ 
   | 

error: '()' is not a static member of exposed generic parameter 'T'
  ==> C:\Develop\Software\cjglm_wp\cjglm\src\detail\trigonometric.cj:75:14:
   | 
75 |     if (x < -T(1) || x > T(1)) { return T.getNaN() }
   |              ^ 
   | 
