...
### T1: `init(scalar: T)` 全填充构造函数未在 OOD 登记
...
4. 此问题本质上是 **OOD API 清单缺项 + 偏差登记缺项**：若接受当前实现，则应先修订 `04_ood_phase2.md` 将 `init(scalar: T)` 纳入正式设计，再按需在 `deviations.md` 记录此次设计回补
...
**影响范围**：OOD 文档与实现之间存在主 API 不一致——当前实现和测试均已将 `init(scalar: T)` 作为正式入口使用，因此不能仅停留在 `deviations.md` 的补登记；应优先修订 `04_ood_phase2.md`，随后视需要在 `deviations.md` 追加记录。
...
**修复动作**：优先修改 `docs/04_ood_phase2.md`，将矩阵标量入口拆分为两套明确语义：`init(scalar: T)` 对应 GLM `mat(T s)` 的全填充语义；`diagonal(scalar)` 保留为仓颉侧新增的对角矩阵工厂函数。若项目维护者仍希望保留设计演进轨迹，再在 `docs/deviations.md` 中追加新条目说明该回补过程。
...
| T1 ↔ T19 | 两个问题均以 `docs/04_ood_phase2.md` 为主修复对象：T1 需把 `init(scalar: T)` 纳入正式设计，T19 需修正 `diagonal(scalar)` 与 GLM `mat(T s)` 的错误等价描述。两者共享同一组章节（§3.3 / §9），建议合并为一次 OOD 文档提交；若需保留设计演进轨迹，再补充 `deviations.md` 新条目。 |
...
| **Phase 3：OOD 文档修订** | T1（补入 `init(scalar: T)` 正式设计）、T19（修正 `diagonal` 等价描述） | **必须串行** | 两者均操作 `docs/04_ood_phase2.md` 的同一组章节，建议合并为一次提交。若需额外记录设计演进，再在同阶段补充 `deviations.md` 条目 |
