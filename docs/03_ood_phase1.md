...
| `ComputeEqual<T>` / `ComputeEqualNumeric<T>` | 分量相等比较策略 | 结构体 |
...
> |------|---------|--------------------|--------------------|-------------|
> | **X — 可直接编码** | 26 项验证项全部通过 | ✓ 按 §8.3 迁移顺序直接编码，无需设计修订 | — | — |
...
> 
> **当前评估**：本设计在 26 项验证全部通过时可直接编码；1~3 项部分回退时可基于 §2.1 回退方案和 §11.13 叠加效应分析指导编码；仅当 3 项以上核心假设同时失败或高影响项（①③④⑫）中 2+ 项同时失败时才需停工重评估。编码阶段建议优先执行第①③④⑫项（高影响项）以尽早暴露最高风险项。
...
glm.detail（内部无包引用，同包直接可见）
  ...
  scalar_vec_ops → qualifier（首轮仅提供整数 `mod`，不依赖 `std.math`）
...
2. **完整 cjpm.toml 配置**（可直接拷贝使用，含测试目录设置）：
   ...
   [test]
     src-dir = "tests"
   ```
    此完整配置确保 `cjpm test` 可正确发现位于 `src/` 目录下以 `_test.cj` 后缀命名的测试文件中的 `@Test` 标注测试用例。`[test] src-dir = "tests"` 指定测试源文件根目录，`cjpm test` 在该目录下按 `*_test.cj` 后缀规则发现测试文件（参见 cjpm 文档 §2.1：`lib_test.cj — 测试文件（_test.cj 后缀）`）。**版本要求说明**：`cjc-version = "1.0.5"` 为本设计所依赖的编译器版本号。低于此版本时，以下关键功能可能不可用：① `const` 函数体内的编译期 `if` 分支抑制（§7 D29/D34，依赖 `const` 特性在 1.0.5 中的完整支持）；② `@OverflowWrapping` 标注与自动复合赋值继承性（§7 D30）；③ `@Derive[Hashable]` 自动派生（§3.2）；④ `extend` 块的完整泛型语法支持（§4.3/§4.4）。编码前验证检查清单（§2.1）中的各项原型测试默认在 `cjc 1.0.5` 下执行。若目标编译器版本低于 1.0.5，需逐项重新评估清单中各验证项的通过性。
3. **初始目录结构**：
   ```
    glm-cangjie/
    ├── cjpm.toml
...
**标准库依赖声明**：`scalar_vec_ops.cj` 首轮不再承担浮点 `mod` 路径，`mod(s, v)` 仅对 `Integer<T>` 生效，因此不依赖 `std.math.trunc` 或 `std.math.fmod`。浮点取模保留为后续能力补全项，当前首轮 OOD 仅记录整数路径。

### §2.1 编码前验证检查清单

> **§2.1 维护原则**：本清单是验证任务的**唯一权威出处**（当前共 26 项）。§10 仅保留设计特有行为描述和回退方案依赖关系（如编译期 `if` 分支抑制各依赖点的回退方案、`equalEpsilon` 双路径触发条件等），所有原型测试方法和验证过程描述均以 §2.1 为准。§12.3 仅保留验证→设计路径选择映射关系。凡在此三章节间发现内容重叠，**以 §2.1 为裁决依据**，其余两处应同步消重。维护者在后续轮次中新增验证项时，应优先追加至本清单。

以下清单集中列出所有必须在编码前完成的设计阶段验证任务，确保编译器对关键语言特性的支持符合设计假设。每项验证通过后方可进入正式编码。

> **验证时序建议**：建议按以下顺序执行验证：（1）**第⑮项（编译器版本确认）**——确认 cjc 版本 ≥ 1.0.5，为后续所有验证提供编译器基础；（2）**先执行独立原型验证（①③④⑤⑧）**——每项聚焦单个编译器假设边界，暴露最基础的编译器行为问题。选择①③④⑤⑧为首批执行组的依据与 §1 快速预筛步骤的价值分析逻辑一致——①③④覆盖最高风险编译器行为假设（const 表达式链、编译期 `if` 分支抑制、`@OverflowWrapping` 继承性），⑤是⑫集成验证的前置依赖项，⑧（Bool→Numeric 路径 B）的结果影响构造函数设计方向。第①③④⑤⑧项通过后，各核心编译器假设的独立回退方案已明确；（3）**再执行第⑫项（统一集成验证）**——在独立验证全部通过后执行，覆盖编译期 `if` 分支抑制、`@OverflowWrapping` 继承性和 `const`+`@OverflowWrapping` 语法共存性三项的集成验证，确认多项编译器行为在组合场景下的一致性。第⑫项通过后，再按编号顺序（②→⑥→⑦→⑨→⑩→⑪→⑬→⑭→⑯→⑰→⑱→⑲→⑳→㉑→㉒→㉓→㉔→㉕→㉖）依次验证其余各项。
...
| ㉖ `@OverflowWrapping` 浮点标注编译器行为影响验证 | §4.6 | 编写原型测试，对比标注与不标注 `@OverflowWrapping` 时含 NaN/Infinity 分量的浮点向量运算结果差异——分别构造含 `Float32.NaN`、`Float32.Infinity` 分量的 Vec2，执行 `+`/`-`/`*`/`/` 运算后对比两组结果的 NaN/Infinity 传播路径是否一致 | 标注与不标注 `@OverflowWrapping` 的浮点运算结果一致（NaN/Infinity 传播路径无差异） | 采用条件性标注策略——浮点类型 T（`Float32`/`Float64`）的 Vec 二元算术运算符移除 `@OverflowWrapping` 标注，仅保留整数类型 T 的标注；此策略需要对 §4.6 标注分布进行修订，从"统一标注"改为"仅整数 T 标注"，涉及 Vec1~Vec4 结构体中共 20 个算术运算符的标注调整 | **低** — 仅影响 §4.6 `@OverflowWrapping` 标注策略 | 独立原型验证 |

#### 验证项影响范围分类

以上 26 项验证项按其失败时影响的设计章节数量分为三类：
...
| **低** — 影响 ≤1 个章节，独立局部 | 12 项 | ⑦⑨⑩⑪⑬⑭⑮⑱㉒㉓㉔㉖ | 0-1 章（或无影响，仅验证确认） |

> **量化说明**：26 项验证全部通过时可直接编码；1-4 项部分回退时（以中/低影响项目为主）可基于 §2.1 回退方案局部修订后继续编码，无需停工；仅当高影响项（①③④⑫）中 2+ 项同时失败时才需触发停工评估流程。当前设计确定性声明见 §1 "设计确定性声明"框。
...
**`const init` 与非 `const init` 共存性分析**：Vec 的构造函数体系包含两类 `init`：(A) **`const init`**——逐分量初始化，参数列表为 `const init(x: T, y: T, ...)`（同类型 T 参数），无需类型转换表达式，函数体内仅使用 `=` 赋值初始化 `var` 成员，在 `const` 上下文中合法；(B) **非 `const init`**——跨类型转换，参数列表为 `init<T2, Q2>(v: VecN<T2, Q2>) where Q2 <: Qualifier`，函数体内包含 `T(v.x)` 类型构造表达式（源类型 T2 到目标类型 T 的显式类型转换）。两类构造函数的参数列表**类型结构显著不同**：(A) 接收与分量数相同数量的同类型 `T` 参数，(B) 接收单个 Vec 类型参数 `VecN<T2, Q2>`。因此仓颉函数重载规则中二者不构成歧义——参数个数[^vec1-param-count]与参数类型均不同，编译器可正确区分。**关键区分点**：`const init(x: T, y: T)` 永远不会被误匹配为 `init<T2, Q2>(v: VecN<T2, Q2>)`，因为前者接收多个 `T` 类型的独立标量参数，后者接收单个 `VecN<T2, Q2>` 类型的聚合参数；反之亦然。**关于 `T(v.x)` 在 `const init` 上下文中的合法性**：(B) 类构造函数不声明为 `const`，因此 `T(v.x)` 类型转换表达式不受 `const` 函数体内对表达式必须是 const 表达式的限制（const README §3.2 规则 3 仅要求 `const` 函数体内表达式为 const 表达式，`const init` 例外——规则 3 注明"`const init` 函数除外"）。若将 (B) 类构造函数误标注为 `const init`，则 `T(v.x)` 类型转换表达式需满足 const 表达式要求，对多数 T2→T 组合（如 `Int32(Float32(1.5))`）不可用。因此 (B) 类构造函数**必须**声明为非 `const init`，与 (A) 类 `const init` 的参数列表差异确保二者安全共存。编码前验证项⑯须编写原型确认此共存安全性。此分析结论已在 §4.1 构造函数清单中以 `// const 构造函数` 和 `// 跨类型转换（非 const）` 注释分类标注。

[^vec1-param-count]：Vec1 的 `const init(x: T)` 仅接收一个 `T` 参数，与跨类型构造函数 `init<T2, Q2>(v: Vec1<T2, Q2>)`（同样接收单个 `Vec1<T2, Q2>` 参数）在参数个数上相同。但二者参数类型不同（`T` vs `Vec1<T2, Q2>`），因此不构成重载歧义。Vec2~Vec4 的 `const init` 接收 2~4 个 `T` 参数，与跨类型构造函数参数个数不同，不受此特殊情况影响。
...
- **比较运算符** — `==` 通过 `ComputeEqual<T>` 策略执行精确比较，定义在带约束的 `extend` 块中；`equalEpsilon` 通过 `ComputeEqualNumeric<T>` 提供浮点容差比较。`!=` 定义为 `!(a == b)`。`<`/`<=`/`>`/`>=` 运算符的覆盖见 §4.5。
...
由于仓颉 `extend` 块不支持 `const` 修饰符，且 `scalar-op-vec` 方向包级函数也不能声明为 `const`，首轮设计中以下 API 均不能在 const 上下文调用：

| 不可用的 const API | 所属运算类别 | 运行时替代方案 |
|-------------------|-------------|--------------|
| `v.add(s)` / `add(s, v)` | 算术：具名加法 | 运行时调用；若必须出现在 const 上下文，改写为显式构造表达式 |
| `v.sub(s)` / `sub(s, v)` | 算术：具名减法 | 同上 |
| `v.mul(s)` / `mul(s, v)` | 算术：具名乘法 | 同上 |
| `v.div(s)` / `div(s, v)` | 算术：具名除法 | 同上 |
| `v.mod(s)` / `mod(s, v)` | 算术：具名取模 | 仅运行时可用，且 `mod` 首轮仅支持整数类型 |
| `v.equalExact(other)` | 比较：精确相等比较 | 运行时调用 |
| `v.equalEpsilon(other)` | 比较：浮点容差比较 | 运行时调用 |
| `v.logicalAnd(other)` / `v.logicalOr(other)` | 布尔逻辑 | 运行时调用 |
| `v.increment()` / `v.decrement()` | 算术：`++`/`--` 替代 | 运行时调用 |
| `v.bitwiseNot()` | 位运算：`~` 替代 | 运行时调用 |
| Vec1 广播运算符（`v + vec1`、`v & vec1` 等） | 算术/位运算：Vec1 广播 | 运行时调用 |

以上限制不再区分“扩展成员函数不可 const”与“包级独立函数可 const”两类路径。首轮实现统一按运行时 API 处理；若调用方需要编译期常量结果，应直接内联分量构造表达式，而非依赖这些具名 API。
...
**角色**：提供分量级别的相等比较策略。首轮设计不再尝试将“精确比较”和“浮点容差比较”塞入同一个无约束泛型结构体，而是拆分为两个职责明确的结构体。

```
struct ComputeEqual<T> where T <: Equatable<T> {
    static func call(a: T, b: T): Bool { a == b }
}

struct ComputeEqualNumeric<T> where T <: Number<T> & Equatable<T> & Comparable<T> {
    static func callConst(a: T, b: T): Bool {
        abs(a - b) <= epsilonOf(a)
    }
}
```
...
**设计说明**：
1. `ComputeEqual<T>` 仅承担精确比较职责，服务于 `==` 和 `equalExact`。
2. `ComputeEqualNumeric<T>` 仅承担数值容差比较职责，服务于 `equalEpsilon`。
3. 首轮不再将 `==` 设计为浮点自动容差比较。这样与 C++ GLM 1.0.3 的实际行为保持一致，也规避了仓颉中“单一泛型结构体内同时容纳数值路径和非数值路径”带来的约束冲突。
4. `ComputeEqualNumeric<T>.callConst` 虽保留 `callConst` 命名以强调其内部逻辑简单稳定，但其调用场景为运行时扩展成员函数 `equalEpsilon`，不再承担 `==` 的 const 语义支撑。

**行为约定**：
- `==` / `!=`：对所有类型统一执行精确比较。
- `equalExact`：与 `==` 语义一致，作为显式命名的精确比较 API 保留。
- `equalEpsilon`：仅在数值类型 Vec 上提供，采用 `abs(a - b) <= epsilonOf(a)` 语义。

**值域局限性说明**：`equalEpsilon` 采用机器 epsilon 的绝对容差语义，仅适合值域接近 1.0 的浮点比较场景。对大值或极小值的比较，调用方应自行选择更合适的相对或绝对容差策略。
...
- **`shim_limits.cj`**：提供数值极限查询功能。首轮设计采用“带值参数的类型提示”方案，而非无参 `const` 泛型函数：
  - **`NumericLimits<T>` 结构体**：提供 `static max(hint: T): T`、`static min(hint: T): T`、`static epsilon(hint: T): T` 等接口。`hint` 仅用于类型推断，不参与实际数值语义。
  - **`isIec559Of<T>(hint: T): Bool` 函数**：通过值参数 `hint` 辅助推断 `T`，内部用运行时 `match` 判定是否为 IEEE 754 浮点类型。
  - **`epsilonOf<T>(hint: T): T` 函数**：委托给 `NumericLimits<T>.epsilon(hint)`，统一维持带 `hint` 的调用风格。
  ...
  此设计避免了 `T(0)`/`T(1e-6)` 这类对无约束泛型不成立的构造假设，也不再依赖编译期 `if` 分支抑制行为。
...
- **跨类型转换替代方案**：若仓颉编译器无法稳定支持 OOD 中规划的跨类型 `init<T2, Q2>(...)` 构造函数，允许退化为包级 `castVec1`~`castVec4` 自由函数。该方案按目标分量数拆分函数名，通过 `conv: (T2) -> T` 闭包完成分量级转换，并覆盖 Vec1~Vec4 到 Vec1~Vec4 的 16 种源/目标组合。低维到高维扩展时重复最后一个可用分量，高维到低维时截取前 N 个分量。此方案属于语言限制下的正式替代接口，而非临时实现细节。
...
- **标量在左的运算替代方案**：`scalar + vec`、`scalar - vec`、`scalar / vec` 等左标量右向量的运算符重载不可实现。提供具名函数替代，包括双向重载覆盖两种操作数顺序：
  - `add(v: VecN<T,Q>, s: T): VecN<T,Q>`（扩展成员函数，非 `const`）和 `add(s: T, v: VecN<T,Q>): VecN<T,Q>`（包级独立函数，非 `const`）
  - `sub(v: VecN<T,Q>, s: T): VecN<T,Q>`（扩展成员函数，非 `const`）和 `sub(s: T, v: VecN<T,Q>): VecN<T,Q>`（包级独立函数，非 `const`）
  - `mul(v: VecN<T,Q>, s: T): VecN<T,Q>`（扩展成员函数，非 `const`）和 `mul(s: T, v: VecN<T,Q>): VecN<T,Q>`（包级独立函数，非 `const`）
  - `div(v: VecN<T,Q>, s: T): VecN<T,Q>`（扩展成员函数，非 `const`）和 `div(s: T, v: VecN<T,Q>): VecN<T,Q>`（包级独立函数，非 `const`）
  - `mod(v: VecN<T,Q>, s: T): VecN<T,Q>`（扩展成员函数，非 `const`）和 `mod(s: T, v: VecN<T,Q>): VecN<T,Q>`（包级独立函数，非 `const`）
- **`const` 修饰符差异修订**：首轮不再将 `scalar-op-vec` 方向包级独立函数声明为 `const`。原因是这些函数体同样消费运行时参数 `s` 与 `v`，不满足仓颉 `const func` 对表达式的要求。故两种方向的具名函数均按运行时 API 设计。
- **具名函数的定义位置与形态**：
...
> **⚠ D30 回退场景反转警告**：当前委托模式（扩展成员函数 `{ self + s }` 委托给二元 `+` 运算符）在 D30 回退场景（§7 D30）中**必须反转**——D30 回退要求二元运算符委托给扩展成员函数，而扩展成员函数仍使用 `{ self + s }` 将形成循环委托。回退时扩展成员函数须从"委托给二元运算符"改为"直接逐分量运算"（如 `Vec2(this.x + s, this.y + s)`），二元运算符再委托给改造后的扩展成员函数。详见 §7 D30、§11.6。

**scalar-op-vec 方向包级独立函数**（`add(s, v)` 等，共 20 个，定义于 `scalar_vec_ops.cj`）因不通过 Vec 二元运算符委托（直接使用 `T` 类型原生算术运算符），须在函数定义上**直接标注 `@OverflowWrapping`**，以确保 `add(s, v)` 与 `v.add(s)` 的溢出行为一致。详见 §4.6。
...
- **`%` / `mod` 的整数约束**：首轮 OOD 将 `%` 运算符和两种方向的 `mod` 具名函数统一约束为 `Integer<T>`。当 `T` 为整数类型时正常编译；当 `T` 为浮点类型时，因仓颉原生浮点类型不支持 `%`，在实例化时产生编译错误。浮点 `mod` 的独立重载补全不再作为首轮 OOD 的既定能力，而是记录为后续补强项。
> **API 影响标注**：`v.mod(s)` 与 `mod(s, v)` 在首轮均仅对整数 Vec 可用；浮点 Vec 的取模当前不属于首轮已交付语义。
...
**相等比较**：
- **`==`**：逐分量调用 `ComputeEqual<T>.call` 比较，全分量相等返回 `true`。该运算符定义在 `extend<T, Q> VecN<T, Q> where T <: Equatable<T>, Q <: Qualifier` 块中，统一采用精确比较语义。
- **`!=`**：定义为对 `==` 结果取反——`!(a == b)`。任一分量不等返回 `true`。仓颉中无 `!==` 运算符概念，因此采用取反形式。
- **`equalExact`**：与 `==` 共享精确比较语义，作为显式命名的精确比较 API 保留，定义于同一 `extend` 块中。

  > **与 C++ GLM 一致性说明**：C++ GLM 1.0.3 的 `==` 对浮点与整数 Vec 均执行精确比较。首轮仓颉 OOD 现与该行为保持一致，不再将浮点容差比较设计为 `==` 的默认语义。
- **`equalExact` 的 `const` 声明说明**：仓颉 `extend` 块不支持 `const` 修饰符，因此 `==`、`!=`、`equalExact`、`equalEpsilon` 均按非 `const` 扩展成员函数设计。
...
- **浮点容差比较函数（`equalEpsilon`）**：作为与 `==` 并行的显式 API 提供，不再承担“`==` 回退路径”角色。各 VecN 类型分别在对应 `type_vecN.cj` 的 `extend` 块中定义：
  ```cangjie
  // 以 Vec2<T, Q> 为例
  // 注意：abs() 调用依赖 import std.math.{ abs }（§8.1 已标注依赖），
  //       此处 abs 为 std.math 包级函数（非 Float32.abs()/Float64.abs() 静态方法）
  extend<T, Q> Vec2<T, Q> where Q <: Qualifier {
      public func equalEpsilon(other: Vec2<T, Q>): Bool where T <: Number<T> & Equatable<T> & Comparable<T> {
          ComputeEqualNumeric<T>.callConst(this.x, other.x)
          && ComputeEqualNumeric<T>.callConst(this.y, other.y)
      }
  }
  ```
  **重要限制**：`equalEpsilon` 定义在 `extend` 块中，因此它与 `==` 一样都不是 `const` 函数。它只面向运行时比较场景。

  **Bool Vec 迁移指南**：`equalEpsilon` 不面向 `VecN<Bool, Q>`；Bool Vec 的相等比较使用 `==` 或 `equalExact`。

  **浮点 `Inf`/`NaN` 边界行为**：`equalEpsilon` 基于 `abs(a - b) <= epsilonOf(a)`，因此 `Inf`/`NaN` 参与比较时不保证与精确 `==` 一致。若需要 IEEE 754 精确语义，使用 `==` 或 `equalExact`。

  **定义位置**：`equalEpsilon` 在各 `type_vecN.cj` 的数值约束 `extend` 块中定义，对 Vec1~Vec4 各提供一个版本。
...
- **`<<` 运算符**：在 extend 块中定义为位运算符，**标注 `@OverflowWrapping`**。`<<` 的默认溢出策略属于编译器实现定义行为，当前设计按 `@OverflowThrowing` 假设标注（C++ GLM 的 `vec << scalar` 高位丢弃为 wrapping 语义），编码前通过验证项㉔确认实际编译器行为后决定保留或移除标注。共 1 个运算符 × 4 个 Vec 类型 = 4 处标注（Vec1~Vec4 各自的 `<<` 非广播版本）。此外，Vec1 广播方向位运算符（Vec2~Vec4 `<<` Vec1 方向 3 个 + Vec1 `<<` Vec2~Vec4 方向 3 个 = 6 个）同样在 extend 块中标注 `@OverflowWrapping`。合计 `<<` 及相关广播标注共 10 处。
- **`>>` 运算符**：右移不会产生溢出异常，不需要标注。
- **`&`、`|`、`^` 运算符**：按位运算不会产生溢出，不需要标注。
- **`increment()`/`decrement()` 具名函数**：作为 `++`/`--` 的替代，标注 `@OverflowWrapping`。共 2 个函数 × 4 个 Vec 类型 = 8 处标注。
- **vec-op-scalar 方向扩展成员函数**（`add(v,s)`/`sub(v,s)`/`mul(v,s)`/`div(v,s)`/`mod(v,s)`，共 5 运算 × 4 Vec = 20 个函数）：**无需直接标注**。这些函数的内部实现**委托给对应的二元算术运算符**（后者已标注 `@OverflowWrapping`），因此通过委托**隐式继承 wrapping 语义**。委托实现模式：`func add(self, s) { self + s }`（调用已标注的 `+`）。此策略避免重复标注，且确保扩展成员函数与二元运算符的溢出行为完全一致。</br></br>**⚠ D30 回退场景反转指示**：当前委托模式（扩展成员函数 `{ self + s }` 委托给二元运算符）在 D30 回退场景中须反转——扩展成员函数改为直接逐分量运算（如 `Vec2(this.x + s, this.y + s)`），不再委托；二元运算符改为委托给改造后的扩展成员函数。D30 回退激活后，本条"无需直接标注"失效，扩展成员函数须直接标注 `@OverflowWrapping`。详见 §7 D30、§11.6。
...
public func fromBoolVec<T, Q>(v: Vec2<Bool, Q>, zero: T, one: T): Vec2<T, Q> where Q <: Qualifier {
    Vec2(if (v.x) { one } else { zero }, if (v.y) { one } else { zero })
}
...
public func fromBoolVec<T, Q>(v: Vec3<Bool, Q>, zero: T, one: T): Vec3<T, Q> where Q <: Qualifier {
    Vec3(if (v.x) { one } else { zero }, if (v.y) { one } else { zero }, if (v.z) { one } else { zero })
}
...
public func fromBoolVec<T, Q>(v: Vec4<Bool, Q>, zero: T, one: T): Vec4<T, Q> where Q <: Qualifier {
    Vec4(if (v.x) { one } else { zero }, if (v.y) { one } else { zero }, if (v.z) { one } else { zero }, if (v.w) { one } else { zero })
}
...
**`T` 参数类型范围**：`T` 可为任意需要由调用方显式提供 `zero` / `one` 值的目标类型。首轮主要面向数值类型与 `Bool`。该设计不再假设 `T(1)`/`T(0)` 对泛型 `T` 一定可构造，而是把“零值/壹值如何获得”的责任交给调用方。

**行为约定**：
- 对每个布尔分量，若为 `true` 则生成调用方提供的 `one`，若为 `false` 则生成调用方提供的 `zero`。
...
- 函数体不依赖 `const` 修饰符，所有版本均声明为非 `const` 包级独立函数。
- 调用方使用 `fromBoolVec<Int32, PackedHighp>(boolVec, Int32(0), Int32(1))` 或 `fromBoolVecQ2<Int32, PackedLowp, PackedHighp>(boolVec, Int32(0), Int32(1))` 语法。
- 若编译器对返回值推断不足，可在函数体内部显式写出 `VecN<T, Q>(...)`；这属于实现细节，不影响本节 API 设计。
...
| `GLM_CONFIG_CLIP_CONTROL` | `const GLM_CONFIG_CLIP_CONTROL: Int64` | `0x0A`（十进制 10） | 裁剪空间约定（LH/RH, NO/ZO 位组合）。对应 C++ `RH_BIT \| NO_BIT` = 0x0A，首轮使用默认 RH_NO。**默认值来源已验证（参考实现确认，详见 §8.4 前置验证项①）**：GLM 1.0.3 `setup.hpp` 中的 `GLM_CONFIG_CLIP_CONTROL` 默认值由两层条件编译决定——首先判断 `GLM_FORCE_DEPTH_ZERO_TO_ONE` 设置 `ZO_BIT`/`NO_BIT`，再判断 `GLM_FORCE_LEFT_HANDED` 设置 `LH_BIT`/`RH_BIT`。在无额外定义时，两层条件编译均走 `#else` 分支，最终等效于 `RH_NO = 0x0A`。**位定义值说明**：参考实现中 `RH_BIT = 1 << 3 = 8`（第 3 位，位值为 8），`NO_BIT = 1 << 1 = 2`（第 1 位，位值为 2），两者按位或得 `8 \| 2 = 0x0A`。**已验证结论**（验证方法详见 §8.4 前置验证项①）：通过读取 `setup.hpp:563-584` 的原始条件编译块，逐行确认分支走向与 `0x0A` 推导一致，三层次条件编译最终值完全吻合。**一致性确认**：经与 roadmap §3A.1 核实，默认值已统一为 `0x0A`，跨文件一致，无需协调 |
...
### D11. 引入 `const init` 支持 struct 内 const 成员能力

**决策**：为每个 Vec 结构体定义 `const init` 构造函数，移除与 `const init` 参数列表相同的普通 `init`。所有 `const init` 构造函数均显式声明为 `public`。

**理由**：仓颉语言要求仅当 struct 定义了 `const init` 时，才能定义 `const` 实例成员函数。虽然首轮 Vec 的比较运算符最终定义在 `extend` 块中而非 struct 体内，但 `const init` 仍保留其语言层价值，并继续作为逐分量构造的统一形式。`const init` 在非 const 上下文中同样可用于运行时构造（const README §3.2 规则 5），因此同参数列表的普通 `init` 不再需要。仓颉函数重载规则中 `const` 修饰符不构成区分依据（const README §3.2 规则 7），若同时保留二者将导致重复定义编译错误。移除冗余额外的普通 `init`，仅保留 `const init` 即可覆盖所有构造场景。
...
**决策**：保留 `NumericLimits<T>` 作为数值极限查询入口，同时将类型判定与 epsilon 获取统一改为“带 `hint` 参数”的普通函数形式：`isIec559Of<T>(hint: T)` 与 `epsilonOf<T>(hint: T)`。

**理由**：无参 `isIec559Of<T>()` / `epsilonOf<T>()` 依赖 `T(0)` 和编译期分支等假设，已被仓颉实现约束否定。改为 `hint` 形式后，`Bool` 和各数值类型都可通过值参数完成类型推断，`NumericLimits<T>` 与运行时类型判定解耦，避免无约束泛型构造假设。

### D19. `isIec559Of` 的实现方案选择

**决策**：采用带 `hint` 参数的运行时 `match` 类型检测方案。

**理由**：仓颉泛型函数的重载规则禁止按类型参数约束进行重载，而无参 `const` 泛型函数又依赖 `T(0)` 构造与编译期 `if`。带 `hint` 参数的普通函数是当前已验证实现中最稳定、最直接的路径。三种可行方案的分析：

| 方案 | 描述 | 评估 |
|------|------|------|
| ① `match` + `hint` 类型提示（采用） | `match (hint) { case _: Float32 => true ... }` | 不依赖 `T(0)` 构造，不依赖 `const` 分支抑制，直接可实现。 |
| ② 无参 `const` 泛型函数 | `if (T(0) is Float64 \|\| T(0) is Float32)` | 依赖已被实现偏差否定，不采用。 |
| ③ 反射或注解方式 | 运行时通过 `TypeInfo` 或自定义注解实现 | 成本更高，无明显收益。 |

采用方案①，理由：实现最直接，且与当前 `shim_limits.cj` 的已确认实现方向一致。
...
### D28. `operator[]` 取值形式与赋值形式

**决策**：`operator[]` 的取值形式不标注 `const`，另提供具名函数 `componentAt(i: Int64): T` 作为显式索引访问入口；赋值形式声明为 `mut operator func`。
...
- **`componentAt` 替代方案**：提供 `public func componentAt(i: Int64): T` 具名函数，与非 const 的 `operator[]` 取值形式共享同一实现体（含边界断言）。首轮不再要求其必须可用于 const 上下文。
...
### D29. `==` 精确比较与 `equalEpsilon` 分离策略

**决策**：首轮直接采用精确 `==` + 显式 `equalEpsilon` 的分离策略，不再为 `==` 设计容差路径或 const 表达式链回退树。

**理由**：`==` 默认精确比较已与 C++ GLM 一致，也与当前仓颉实现约束一致。容差比较保留给 `equalEpsilon`，由 `ComputeEqualNumeric<T>` 承担。

**实现要求**：
1. `==` / `!=` 定义在 `T <: Equatable<T>` 的 `extend` 块中。
2. `equalEpsilon` 定义在 `T <: Number<T> & Equatable<T> & Comparable<T>` 的 `extend` 块中。
3. `ComputeEqual<T>` 与 `ComputeEqualNumeric<T>` 分离，避免单一结构体内混入互斥约束路径。
...
**循环委托防治改造步骤**（D30 回退方案的必备前置）：
1. **扩展成员函数实现改造**：将 20 个 vec-op-scalar 方向扩展成员函数的内部实现从"委托给二元算术运算符"（`{ self + s }`）改为"直接逐分量运算"（如 `Vec2(this.x + s, this.y + s)`），同步补充 `@OverflowWrapping` 标注。此改造打破循环——扩展成员函数不再依赖二元运算符，二元运算符再反向委托给改造后的扩展成员函数。
2. **二元运算符委托目标修改**：将 Vec 结构体内的 5 个二元算术运算符（`+`/`-`/`*`/`/`/`%`）的委托目标从"直接逐分量运算"改为"调用改造后的扩展成员函数"（如 `this.add(rhs)`）。
3. **改造范围**：共 20 个扩展成员函数（5 运算 × 4 Vec）需实现改造，预计追加 0.3-0.5 人天（已在 §11.6 成本评估中补充）。

**⚠ 逻辑关系说明**：D30 回退的完整动作序列为：① 扩展成员函数拆弃委托模式，改为直接逐分量运算 + 补充标注 → ② 二元运算符委托给改造后的扩展成员函数 → ③ 复合赋值由编译器从二元运算符自动生成并继承标注语义。当前 §4.3 中"委托模式示例：`func add(self, s) { self + s }`"仅在 D30 未触发（标注继承性成立）时有效——D30 触发后此模式不可用，须按上述改造步骤反转。§4.3 相关段落已增加 ⚠ 警示脚注。
...
### D32. 具名函数（add/sub/mul/div/mod）的声明策略

**决策**：`add`、`sub`、`mul`、`div`、`mod` 的 `scalar-op-vec` 与 `vec-op-scalar` 两个方向均声明为非 `const`。

**理由**：仓颉 `extend` 块不支持 `const` 修饰符，而包级独立函数即使语法上可写 `const`，其函数体同样使用运行时参数，首轮实现中也不满足 `const func` 要求。`mod` 首轮同时取消浮点双路径既定设计，因此不再需要包级 `const` 分支承载能力。

**验证要求**：
1. 验证两种方向的具名函数在运行时可用。
2. 验证 `mod` 在整数类型上可用、在浮点类型上不可用。
3. 不再为这些函数建立 const 兼容性验证分支。
...
| 12c | `type_cast.cj`（新增） | `qualifier.cj` + `type_vec1.cj`~`type_vec4.cj` | `castVec1`~`castVec4` 包级独立跨类型转换函数（4 个目标 Vec 家族 × 4 个源 Vec 形态 = 16 个重载），作为 §4.1 跨类型构造在仓颉语言限制下的替代实现 | 4~7（跨类型转换能力） | 新增 |
...
| T12b | `test_type_fromBoolVec.cj` | `type_fromBoolVec.cj` | 验证 `fromBoolVec`/`fromBoolVecQ2` 对 Vec1~Vec4 各分量数、不同目标标量类型与不同 Bool Vec Qualifier 组合的 Bool→Numeric 转换正确性 |
| T12c | `test_type_cast.cj` | `type_cast.cj` | 验证 `castVec1`~`castVec4` 的跨类型、跨 Qualifier、截断与扩展行为 |
...
| 阶段 | 步骤 | 文件 | 增量验证 | 预期代码量 |
|------|------|------|---------|-----------|
| 0 | 非 Vec 文件 API 快速扫描 + 扩展成员函数可见性验证（编码启动前置） | ① 对 7 个非 Vec 核心文件（setup.hpp/qualifier.hpp/compute_vector_relational.hpp/compute_vector_decl.hpp/_vectorize.hpp/fwd.hpp/shim 文件）按 §1 下方「7 文件快速扫描依赖项核查清单」逐项确认无遗漏移植依赖项；② 按 §2.1 项⑱(b) 验证扩展成员函数在 `public import` 重导出后的对外可见性 | ① 扫描执行人：设计审议方提供核查清单，编码实施者按清单逐项确认并签字；通过标准：7 项核查清单全部确认通过，每项核查要点逐条应答；逐 API 级对照延迟至第 2 轮编码阶段以附录 A 为模板补充；超时升级路径：发现未覆盖依赖项且 24 小时内无法确认处理方案 → 上报主 Agent + 设计审议；发现≠解决闭环原则：扫描标识的差异须以编码阶段实际编译验证为最终闭环依据。② 扩展成员函数可见性验证通过标准：`public import` 重导出后扩展成员函数对下游可见可调用（§2.1 项⑱(b)）。此验证为阶段 0 退出标准的一部分——若验证失败，须在阶段 0 内采用回退方案（改为 `public type` 类型别名 + 包级独立函数形式）后方可进入阶段 1 | ~0 行（扫描审查 + 原型验证，无代码编写） |
...
#### 扩展成员函数 `public import` 可见性风险记录

| 风险项 | 描述 | 影响评估 | 缓解措施 | 风险等级 |
|-------|------|---------|---------|---------|
| 扩展成员函数在 `public import` 重导出后对外不可见 | D37 A20 标注为待验证——`extend` 块中定义的扩展成员函数可能不随 `public import` 重导出到目标包。若此行为不成立，约 30+ 扩展成员函数（vec-op-scalar 方向 20 个 + `logicalAnd`/`logicalOr`/`equalEpsilon`/`equalExact`/`increment`/`decrement`/`bitwiseNot`/Vec1 广播等）将在 `import glm.*` 后对下游不可见 | **中**——影响 Vec 扩展成员函数 API 的对外可访问性。公共 API 面设计（§2 公共 API 面）依赖 `public import` 重导出机制。若不可见，下游代码须改用包级独立函数或 `public type` 别名+包级独立函数模式 | ① 编码前通过 §2.1 项⑱(b) 验证实际编译器行为；② 若验证失败，采用回退方案——在 `package glm` 中通过 `public type` 类型别名暴露 Vec 类型，扩展成员函数改为包级独立函数形式（已在 §2.1 项⑱ 回退方案中定义）；③ 此风险已在 §7 D37 A20 和 §12.2 验收中记录 | 待验证 |
...
| 变更项 | 影响范围 | 工作量评估 |
|-------|---------|-----------|
| 扩展成员函数实现改造：从"委托给二元运算符"（`{ self + s }`）改为"直接逐分量运算"（如 `Vec2(this.x + s, this.y + s)`），同步补充 `@OverflowWrapping` 标注 | 各 Vec 的 extend 块，共 5 函数 × 4 Vec = 20 处改造 + 20 处标注 | **中**（20 处函数体改写，需逐分量展开，约 0.3-0.5 人天）。注意：此改造**必须优先执行**——扩展成员函数若不拆弃委托模式，将与修改后的二元运算符形成循环委托（§4.3 ⚠ 警示） |
| 为改造后的扩展成员函数补充 `@OverflowWrapping` 标注 | 已在上一项中覆盖，此处仅保留标注计数 | 已计入上项 |
...
- **若标注继承性不成立**：额外成本约 1.0-1.5 人天（20 处扩展成员函数改造 0.3-0.5 人天 + 20 处标注补充 0.2 人天 + 5 处运算符委托目标修改 0.5 人天）。此评估已修正此前 v9 中遗漏的"扩展成员函数改造"工作项——改造是打破循环委托（§4.3 ⚠ 警示）的**必备前置步骤**，不可省略。
...
| §11.13（新增）编译器依赖验证回退成本 | 0（验证通过） | 1.0 | 3.0 | **低→中**：依赖 §2.1 26 项验证结果——验证通过后置信度骤升，触发回退则成本大增 |
...
| **算术溢出策略** | 二元运算符标注 `@OverflowWrapping`，复合赋值自动继承 | ① 将 20 个 vec-op-scalar 方向扩展成员函数从"委托给二元运算符"（`{ self + s }`）改造为"直接逐分量运算"（如 `Vec2(this.x + s, this.y + s)`），同步补充 `@OverflowWrapping` 标注；② 二元运算符委托给改造后的扩展成员函数 | §4.3 具名函数、§4.6 `@OverflowWrapping` 策略、§7 D30、§11.6 |
...
2. **扩展成员函数改造**（优先执行，打破循环委托）：将 20 个 vec-op-scalar 方向扩展成员函数从"委托给二元运算符"（`{ self + s }`）改造为"直接逐分量运算"（如 `Vec2(this.x + s, this.y + s)`），同步补充 `@OverflowWrapping` 标注
3. 修改 5 个二元运算符委托目标——从"直接逐分量运算"改为"调用改造后的扩展成员函数"
4. 实现 `modInt`/`modFloat` 替代原 `mod` 双路径方案（包级独立函数降级为非 `const`）
5. 根据 §3.5 路径选择调整 `epsilonOf<T>()` 约束策略
6. 在文档中标注 const 上下文中 `equalEpsilon` 不可用
...
| 扩展成员函数 `public import` 可见性验证（非阻塞验收前置项） | `extend` 块中定义的扩展成员函数（如 `add(v,s)`/`sub(v,s)` 等）在 `public import` 重导出后对下游 `import glm.*` 保持可见和可调用。即从独立测试包中通过 `import glm.*` 后，`Vec2<Float32, PackedHighp>(1.0f, 2.0f).add(3.0f)` 可正常编译和运行 | 在独立测试包中编写测试：通过 `import glm.*` 导入 Vec 类型后调用扩展成员函数（算术方向、比较方向各选一个），验证编译通过且结果正确。**非阻塞说明**：此项为验证前置项，验证结果不影响编码启动（阻塞/非阻塞取决于 D37 A20 验证结果——若 A20 通过，此验收项自动满足；若 A20 失败且回退方案无效，此项标记为"不适用"）。验收状态与 §2.1 项⑱(b) 的验证结果联动 |
